...
A `state` parameter is used to verify that the response from grant flow has not been tampered with along the way. It is recommended that you set this to a value that is verifiable once the user gives permission and the web browser is sent to your `redirect_uri`. A second use for this parameter is to encode basic state information like an internal user ID or the URL of the last page they were on before entering the grant flow.

Include a `state` parameter in the `authorizationString`.
