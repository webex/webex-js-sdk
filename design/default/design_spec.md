...
| G-1 | Addressed | Change: Consumer sequencing and response semantics |
| G-2 | Addressed | Change: Consumer sequencing and response semantics |
| G-3 | Addressed | Component: Realtime coordination, correlation, and receiver delivery |
| G-4 | Addressed | [Component: Public contracts and task API](#component-public-contracts-and-task-api) and [Component: Realtime coordination, correlation, and receiver delivery](#component-realtime-coordination-correlation-and-receiver-delivery) |
| G-5 | Addressed | [Component: Public contracts and task API](#component-public-contracts-and-task-api) and [Change: Cross-cutting safeguards and verification](#change-cross-cutting-safeguards-and-verification) |
| REQ-004 | Addressed | Change: Cross-cutting safeguards and verification |
| REQ-005 | Addressed | Component: Public contracts and task API |
| REQ-006 | Addressed | Component: Realtime coordination, correlation, and receiver delivery |
| REQ-007 | Addressed | Component: Feature enablement and SDK lifecycle |
| REQ-008 | Addressed | Component: Feature enablement and SDK lifecycle |
| REQ-009 | Addressed | Component: Realtime coordination, correlation, and receiver delivery |
| REQ-010 | Addressed | [Component: Public contracts and task API](#component-public-contracts-and-task-api) and [Component: AI Assistant transport and outbound serialization](#component-ai-assistant-transport-and-outbound-serialization) and [Component: Realtime coordination, correlation, and receiver delivery](#component-realtime-coordination-correlation-and-receiver-delivery) and [Change: Cross-cutting safeguards and verification](#change-cross-cutting-safeguards-and-verification) |
| REQ-011 | Addressed | Component: Public contracts and task API |
| REQ-012 | Addressed | Component: Realtime coordination, correlation, and receiver delivery |
| REQ-013 | Addressed | Component: Public contracts and task API |
...
| REQ-021 | Addressed | Component: Public contracts and task API |
| REQ-022 | Addressed | Component: Public contracts and task API |
| REQ-023 | Addressed | Component: Public contracts and task API |
| REQ-024 | Addressed | Component: Public contracts and task API |
| REQ-025 | Addressed | Component: Public contracts and task API |
| REQ-026 | Addressed | Component: Feature enablement and SDK lifecycle |
| REQ-027 | Addressed | Component: Feature enablement and SDK lifecycle |
| REQ-028 | Addressed | Component: Realtime coordination, correlation, and receiver delivery |
| REQ-029 | Addressed | Component: Realtime coordination, correlation, and receiver delivery |
| REQ-030 | Addressed | Component: AI Assistant transport and outbound serialization |
| REQ-031 | Addressed | Component: AI Assistant transport and outbound serialization |
| REQ-032 | Addressed | Component: AI Assistant transport and outbound serialization |
| REQ-033 | Addressed | Component: AI Assistant transport and outbound serialization |
| REQ-034 | Addressed | Component: AI Assistant transport and outbound serialization |
| REQ-035 | Addressed | Component: AI Assistant transport and outbound serialization |
| REQ-036 | Addressed | Component: Feature enablement and SDK lifecycle |
| REQ-037 | Addressed | Component: Realtime coordination, correlation, and receiver delivery |
| REQ-038 | Addressed | Component: Realtime coordination, correlation, and receiver delivery |
| REQ-039 | Addressed | Component: Realtime coordination, correlation, and receiver delivery |
| FR-1 | Addressed | [Component: Public contracts and task API](#component-public-contracts-and-task-api) and [Component: Realtime coordination, correlation, and receiver delivery](#component-realtime-coordination-correlation-and-receiver-delivery) and [Component: Feature enablement and SDK lifecycle](#component-feature-enablement-and-sdk-lifecycle) and [Change: Consumer sequencing and response semantics](#change-consumer-sequencing-and-response-semantics) |
| FR-2 | Addressed | [Component: Public contracts and task API](#component-public-contracts-and-task-api) and [Component: AI Assistant transport and outbound serialization](#component-ai-assistant-transport-and-outbound-serialization) and [Component: Realtime coordination, correlation, and receiver delivery](#component-realtime-coordination-correlation-and-receiver-delivery) |
| FR-3 | Addressed | [Component: Public contracts and task API](#component-public-contracts-and-task-api) and [Component: AI Assistant transport and outbound serialization](#component-ai-assistant-transport-and-outbound-serialization) and [Change: Consumer sequencing and response semantics](#change-consumer-sequencing-and-response-semantics) |
| FR-4 | Addressed | [Component: Public contracts and task API](#component-public-contracts-and-task-api) and [Component: AI Assistant transport and outbound serialization](#component-ai-assistant-transport-and-outbound-serialization) and [Component: Realtime coordination, correlation, and receiver delivery](#component-realtime-coordination-correlation-and-receiver-delivery) |
| FR-5 | Addressed | [Component: Public contracts and task API](#component-public-contracts-and-task-api) and [Component: AI Assistant transport and outbound serialization](#component-ai-assistant-transport-and-outbound-serialization) |
| FR-6 | Addressed | [Component: Public contracts and task API](#component-public-contracts-and-task-api) and [Component: AI Assistant transport and outbound serialization](#component-ai-assistant-transport-and-outbound-serialization) and [Change: Consumer sequencing and response semantics](#change-consumer-sequencing-and-response-semantics) |
| FR-7 | Addressed | [Component: Public contracts and task API](#component-public-contracts-and-task-api) and [Change: Consumer sequencing and response semantics](#change-consumer-sequencing-and-response-semantics) |
| FR-8 | Addressed | [Component: Realtime coordination, correlation, and receiver delivery](#component-realtime-coordination-correlation-and-receiver-delivery) and [Component: Feature enablement and SDK lifecycle](#component-feature-enablement-and-sdk-lifecycle) |
| FR-9 | Addressed | [Component: Realtime coordination, correlation, and receiver delivery](#component-realtime-coordination-correlation-and-receiver-delivery) and [Component: Feature enablement and SDK lifecycle](#component-feature-enablement-and-sdk-lifecycle) |
| FR-10 | Addressed | [Component: Realtime coordination, correlation, and receiver delivery](#component-realtime-coordination-correlation-and-receiver-delivery) and [Component: Feature enablement and SDK lifecycle](#component-feature-enablement-and-sdk-lifecycle) |
| FR-11 | Addressed | [Component: AI Assistant transport and outbound serialization](#component-ai-assistant-transport-and-outbound-serialization) and [Component: Realtime coordination, correlation, and receiver delivery](#component-realtime-coordination-correlation-and-receiver-delivery) |
| FR-12 | Addressed | Component: Realtime coordination, correlation, and receiver delivery |
| DR-1 | Addressed | [Component: Public contracts and task API](#component-public-contracts-and-task-api) and [Component: AI Assistant transport and outbound serialization](#component-ai-assistant-transport-and-outbound-serialization) |
| DR-2 | Addressed | Component: Public contracts and task API |
| DR-3 | Addressed | Component: AI Assistant transport and outbound serialization |
| DR-4 | Addressed | Component: Public contracts and task API |
| DR-5 | Addressed | Component: Realtime coordination, correlation, and receiver delivery |
| REQ-040 | Addressed | Component: Public contracts and task API |
| REQ-041 | Addressed | Component: Public contracts and task API |
| REQ-042 | Addressed | Component: AI Assistant transport and outbound serialization |
| REQ-043 | Addressed | Component: AI Assistant transport and outbound serialization |
| REQ-044 | Addressed | Component: Realtime coordination, correlation, and receiver delivery |
| REQ-045 | Addressed | Component: Realtime coordination, correlation, and receiver delivery |
| REQ-046 | Addressed | Component: Realtime coordination, correlation, and receiver delivery |
| REQ-047 | Addressed | Component: Realtime coordination, correlation, and receiver delivery |
| REQ-048 | Addressed | Component: Realtime coordination, correlation, and receiver delivery |
| REQ-049 | Addressed | Component: Public contracts and task API |
| PR-1 | Addressed | [Component: Public contracts and task API](#component-public-contracts-and-task-api) and [Component: AI Assistant transport and outbound serialization](#component-ai-assistant-transport-and-outbound-serialization) and [Component: Realtime coordination, correlation, and receiver delivery](#component-realtime-coordination-correlation-and-receiver-delivery) and [Component: Feature enablement and SDK lifecycle](#component-feature-enablement-and-sdk-lifecycle) and [Change: Cross-cutting safeguards and verification](#change-cross-cutting-safeguards-and-verification) |
| PR-2 | Addressed | [Component: Public contracts and task API](#component-public-contracts-and-task-api) and [Component: AI Assistant transport and outbound serialization](#component-ai-assistant-transport-and-outbound-serialization) and [Component: Realtime coordination, correlation, and receiver delivery](#component-realtime-coordination-correlation-and-receiver-delivery) and [Change: Cross-cutting safeguards and verification](#change-cross-cutting-safeguards-and-verification) |
| PR-3 | Addressed | [Component: AI Assistant transport and outbound serialization](#component-ai-assistant-transport-and-outbound-serialization) and [Component: Realtime coordination, correlation, and receiver delivery](#component-realtime-coordination-correlation-and-receiver-delivery) and [Change: Consumer sequencing and response semantics](#change-consumer-sequencing-and-response-semantics) and [Change: Cross-cutting safeguards and verification](#change-cross-cutting-safeguards-and-verification) |
| REQ-050 | Addressed | [Component: Public contracts and task API](#component-public-contracts-and-task-api) and [Component: AI Assistant transport and outbound serialization](#component-ai-assistant-transport-and-outbound-serialization) and [Change: Cross-cutting safeguards and verification](#change-cross-cutting-safeguards-and-verification) |
| REQ-051 | Addressed | Change: Cross-cutting safeguards and verification |
| REQ-052 | Addressed | Change: Cross-cutting safeguards and verification |
| REQ-053 | Addressed | Change: Cross-cutting safeguards and verification |
| REQ-054 | Addressed | Component: Feature enablement and SDK lifecycle |
| REQ-055 | Addressed | Component: Feature enablement and SDK lifecycle |
| REQ-056 | Addressed | [Component: Public contracts and task API](#component-public-contracts-and-task-api) and [Component: AI Assistant transport and outbound serialization](#component-ai-assistant-transport-and-outbound-serialization) and [Component: Realtime coordination, correlation, and receiver delivery](#component-realtime-coordination-correlation-and-receiver-delivery) |
| REQ-057 | Addressed | Change: Cross-cutting safeguards and verification |
| AC-1 | Addressed | Change: Cross-cutting safeguards and verification |
| AC-2 | Addressed | Change: Cross-cutting safeguards and verification |
| AC-3 | Addressed | Change: Cross-cutting safeguards and verification |
| AC-4 | Addressed | Change: Cross-cutting safeguards and verification |
| AC-5 | Addressed | Change: Cross-cutting safeguards and verification |
| AC-6 | Addressed | Change: Cross-cutting safeguards and verification |
| AC-7 | Addressed | Change: Cross-cutting safeguards and verification |
| AC-8 | Addressed | Change: Cross-cutting safeguards and verification |
| AC-9 | Addressed | Change: Cross-cutting safeguards and verification |
| AC-10 | Addressed | Change: Cross-cutting safeguards and verification |
| AC-11 | Addressed | Change: Cross-cutting safeguards and verification |
...
Requirements covered: G-1, G-2, G-4, G-5, REQ-005, REQ-010, REQ-011, REQ-013, REQ-021, REQ-022, REQ-023, REQ-024, REQ-025, FR-1, FR-2, FR-3, FR-4, FR-5, FR-7, DR-1, DR-2, DR-4, REQ-040, REQ-041, REQ-049, REQ-050, REQ-056, PR-1, PR-2, PR-3, AC-1, AC-2, AC-3, AC-4, AC-6, AC-7, AC-8, AC-10, and AC-11. Corresponding DAG tasks: `define-ai-summary-contracts` and `expose-task-summary-apis`. For shared G-4/FR-2/FR-4/REQ-010/AC rows, this component owns the public types, Task method selection, gating, Promise composition/propagation, validation, response invocation, and the exact-public-Promise rejection observer; it does not claim inbound parser/settlement, adapter serialization, or consumer-owned sequencing.
...
`requestPostCallSummary()` is a non-`async` public wrapper around one private async operation. The wrapper passes that operation through the single shared helper `private observeAISummaryRequest<T>(operation: Promise<T>): Promise<T>`, which executes `void operation.catch(() => undefined)` and returns `operation` itself. This is deliberately not `return operation.catch(...)`: the application receives the original Promise and therefore still observes its exact rejection, while Task has already installed the rejection observer needed when an internal timeout or lifecycle callback rejects after the application has chosen not to await. The steps below describe the private async operation:
...
Requirements covered: REQ-007, REQ-008, REQ-026, REQ-027, REQ-036, FR-1, FR-8 (full-session reset handoff only), FR-9 (full-session correlation-state teardown only), FR-10, PR-1, PR-2, PR-3, REQ-049, REQ-054, REQ-055, AC-6, AC-10, and AC-11. Corresponding DAG tasks: `define-ai-summary-contracts`, `coordinate-summary-realtime-state`, and `wire-contact-center-summary-lifecycle`. This section does not claim FR-8 matching, buffering, uniqueness re-flush, delivery, or timeout behavior, and it claims no FR-9 matching, receiver-selection, or correlation-key obligation beyond clearing full-session correlation state; those remain solely in Component: Realtime coordination, correlation, and receiver delivery as routed by the Feature Disposition Matrix.
...
REQ-050, REQ-051, REQ-052, REQ-053, and REQ-055 preserve existing runtime/package/configuration behavior. Full-session register/re-establishment/deregister cleanup rejects every live result with lifecycle-only `AI_SUMMARY_REQUEST_CANCELLED` and clears owned state; it never silently drops a resolver. Because both aggregate branches have handlers and the exact returned request Promise has Task's no-op rejection observer, a late HTTP settlement is consumed and an application-unhandled lifecycle rejection emits no process-level event; the old token finds no matching entry and cannot clear a new request, change the lifecycle outcome, recreate state, or emit another metric. Ordinary collection cleanup still preserves an application-held Task's post-call response context; full deregistration ends that guarantee.
...
| `AI_SUMMARY_REQUEST_TIMEOUT_MS` request timeout | rejects `POST_CALL_SUMMARY_TIMEOUT` or `MID_CALL_SUMMARY_TIMEOUT`; awaiting Task emits its single request-failure metric with that bounded code | timer cleared, live result rejected while the key remains occupied rather than silently dropped, then key deleted before the Task Promise reaction; Task's observer prevents a process event when no application handler is attached; TaskManager emits no duplicate operation metric and an immediate explicit retry is admitted |
...
| Owner-task or full-session cleanup while pending | live public Task Promise rejects lifecycle-only `AI_SUMMARY_REQUEST_CANCELLED` | timer clears; every live result rejects rather than being dropped while the owner-matched/all-SDK key remains present; key deletes before reactions; Task's observer prevents a process event when no application handler is attached; this code is never used for base-URL/HTTP failure |
...
- `registerPendingAISummaryRequest(...)` arms the FR-11 timer only on accepted insertion. Zero-candidate `routeReceivingSummary(...)` alone arms/rearms the receiver timer; unique delivery and irreducible ambiguity do not. Feature orphan timing remains keyed to registered canonical interaction presence. The common helper settles pending entries only for resolution, inbound timeout, and lifecycle cleanup; timeout and both lifecycle paths reject every live resolver, while token-checked base-URL/HTTP clear and receiver/feature removal omit the settlement callback.
...
| FR-5 | `index.ts`: compiler-API fixtures accept valid action/response branches and make invalid action/state/feedback combinations plus mid-call `wrapUpCode` fail through `@ts-expect-error`; `Task.ts`: response action selection, branches, counters, feedback/state, agent name, and timestamp validation; `ApiAiAssistant.ts`: exact consult/transfer response bodies, numeric fields, and absent `wrapUpCode`. |
...
| FR-7 | `index.ts`: compiler-API fixtures enforce the `summaryReceived` and response-state discriminants; `Task.ts`: discriminated cancel/exclude/ignored/not-received validation and counter branches plus the test-local `MID_CALL_CANCELLED` no-handoff case; `ApiAiAssistant.ts`: exact cancellation response serialization with no `wrapUpCode`. Other application sequencing remains documented consumer behavior. |
...
| DR-2 | `index.ts`: valid structured/text/no-summary compiler fixtures and invalid discriminator combinations; `Task.ts`: structured/text/no-summary discriminated branches and scoped empty/zero sentinels; `ApiAiAssistant.ts`: representation preserved without collapsing identifiers. |
...
| DR-4 | `Task.ts`: exact feedback allowlist rejection/acceptance; `index.ts`: public payload/type contract remains additive and its `@ts-expect-error` fixture makes an invalid feedback literal fail while valid feedback compiles. |
...
`test/unit/spec/index.ts` is the additive package-root, retained-legacy, and public-type fixture gate. It preserves the frozen pre-feature plus approved export set, excludes internals, verifies exact action-specific `@deprecated` JSDoc on `GET_MID_CALL_SUMMARY` and `MID_CALL_SUMMARY_RESPONSE`, and rejects any production reference resolving to either declaration symbol. Its existing TypeScript compiler-API program is extended with literal in-memory fixture sources and compiler options parsed from `packages/@webex/contact-center/tsconfig.json`, overriding inherited declaration emit with `emitDeclarationOnly: false` and `noEmit: true`. Valid structured, text, no-summary/empty, action, state, feedback, and optional timestamp assignments compile with zero diagnostics. Invalid action/state/feedback/discriminator combinations and a mid-call object literal carrying `wrapUpCode` use reasoned `@ts-expect-error` directives; the test asserts `ts.getPreEmitDiagnostics(...)` is empty, making both a real unexpected error and an unused directive (`TS2578`, such as after accidental union widening) fail `yarn workspace @webex/contact-center test:unit --targets index.ts`. Runtime-only finite/non-negative numeric rules remain in `Task.ts`/`ApiAiAssistant.ts` Jest cases. `yarn workspace @webex/contact-center build:src` is scoped only to source-side compilation of `ITask`, response/coordinator types and barrel exports plus generated `dist/types/index.d.ts`; it is not evidence that `test/unit/spec/**` fixtures compile. No compiler, bundler, manifest, lockfile, or dependency change is expected.

### Concurrency and boundary tests

Named boundary cases cover fast RTD push, HTTP bound, exact-token clear without coordinator settlement, late old-token HTTP rejection after lifecycle cleanup and after a newer same-key registration, inbound timeout races, CONSULT/TRANSFER same-slot overlap, independent post/mid slots, zero-match receiver buffering, unique parent/child and chained-leaf delivery, irreducible sibling/missing/cyclic-lineage ambiguity, distinct buffer expiry, invalid task correlation, feature replacement/expiry, and full cleanup. The four process-level rejection cases are explicitly `HTTP status/network rejection without a consumer handler`, `AI_SUMMARY_REQUEST_TIMEOUT_MS rejection without a consumer handler`, `owner-task cleanup rejection without a consumer handler`, and `SDK deregistration rejection without a consumer handler`. Each leaves the returned Promise application-unhandled until after one post-rejection event-loop turn under an `unhandledRejection` listener, asserts zero events, then attaches the exact rejection assertion to prove the observer did not convert the original Promise to fulfillment; listener cleanup always runs in `finally`. Assertions also require one public terminal outcome/delivery, no registry-order routing, no stale timer/map, and exact separate ambiguity/expiry metric reasons.

### Browser, component, accessibility, responsive, and visual tests

Not applicable - no UI component, DOM, style, browser asset, or supplied visual reference is changed. The typed/error journeys that a consumer UI needs are covered at the SDK boundary. Existing contact-center unit task suites remain regression coverage. Browser suites are unaffected and explicitly outside this change's acceptance gate, so this design makes no browser-regression coverage claim and adds no screenshot/snapshot-only test.
...
The integration-verification node owns final acceptance as soon as all five implementation tasks are complete:
...
The full unit run is mandatory for compatibility with task lifecycle, wrap-up, consult, transfer, events, and transcripts. Browser suites are not part of this SDK-only acceptance gate. Documentation synchronization is a separate successor task with a documentation-only review and `git diff --check` acceptance, so prose findings cannot mask an integration regression or delay this gate.
...
|---|---|---|---|
| `define-ai-summary-contracts` | none | Establish exact public/internal event, type, method, metric, error, and client-type boundaries, plus the frozen pre-feature root-export compatibility baseline, before producers and consumers compile against them. | G-4, G-5, REQ-005, REQ-006, REQ-007, REQ-008, REQ-011, REQ-012, REQ-013, REQ-021 through REQ-039, FR-3, FR-5, FR-10, REQ-049, REQ-050, REQ-056, DR-2, DR-3, DR-4, PR-2, AC-11 |
| `add-ai-summary-transport` | contracts | The adapter needs exact discriminants and internal wire types; it then owns field-by-field serialization, Promise-only input rejection, sanitized HTTP failures, and the shared bounded HTTP outcome. | FR-3, FR-5, DR-1, DR-3, REQ-042, REQ-043, PR-1, PR-2 |
...
| `expose-task-summary-apis` | contracts, transport, coordination | Task methods compose the established adapter and coordinator contracts, retain the post-call response correlation/adapter across collection cleanup, enforce the response/counter branches, observe the exact public request Promise without transforming it, verify cleanup-before-public-rejection retry behavior, and test exact CONSULT/TRANSFER request- and response-event selection. Application-owned response-before-handoff sequencing remains documentation-only in the synchronization task. | G-1, G-2, G-4, REQ-010, FR-1 through FR-7, DR-1 through DR-4, REQ-049, REQ-056, PR-1 through PR-3, AC-1 through AC-4, AC-6 through AC-8, AC-10, AC-11 |
| `wire-contact-center-summary-lifecycle` | contracts, coordination, task APIs | Client event/lifecycle wiring is safe after TaskManager and Task behavior are defined; its focused lifecycle cases also close the ContactCenter-observed privacy and process-level rejection boundaries for cleanup and late branches. | REQ-007, REQ-008, REQ-026, REQ-027, REQ-036, FR-1, FR-8 (full-session reset handoff only), FR-9 (full-session correlation-state teardown only), FR-10, PR-1, PR-3, REQ-049, REQ-054, REQ-055, AC-6, AC-10, AC-11 |
| `verify-summary-integration` | all five implementation tasks | Run the complete contact-center unit, style, and source-build gate against the integrated implementation before any documentation rewrite can obscure whether a failure is executable or editorial. This node changes no source or documentation files. | AC-1 through AC-11 |
| `synchronize-summary-documentation` | `verify-summary-integration` | After the executable gate passes, replace the enumerated non-normative conflicting subjects in the four root `ai-summary*.md` references and the six exact task/agent/metrics `ai-docs` paths listed in Design authority, users, and externally visible boundary. Until this task lands, this design—not the conflicting reference passages—governs implementation; documentation is not the implementation owner for REQ-056 or any AC. | REQ-057 |

The DAG has two parallelizable roots after contracts: transport and coordination. Task APIs join them; client lifecycle wiring follows the task APIs; `verify-summary-integration` joins all five implementation tasks, and documentation synchronization follows that executable gate. There are no database, build-system-configuration, dependency, UI, migration, removal, or standalone packaging nodes because those surfaces do not otherwise change. The public package export surface does change additively in `packages/@webex/contact-center/src/index.ts`; `define-ai-summary-contracts` owns those exports, and its `build:src` acceptance verifies that they are reachable from the built entry point without changing the package manifest, export map, dependencies, or lockfile.
