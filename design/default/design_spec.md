...
| FR-8 | Addressed | requirement.md:L238-L248 -> Component: Realtime coordination, correlation, and receiver delivery; the receiver-buffer retention alias shares the single required 30-second value, and every delivery/replacement/expiry/task/SDK removal uses the coordinator's common timed-entry cleanup. |
...
| FR-11 | Addressed | requirement.md:L266-L275 -> Component: Realtime coordination, correlation, and receiver delivery; the request-timeout alias shares the single required 30-second value and uses the same timed-entry cleanup as receiver retention. |
...
| AC-6 | Addressed | requirement.md:L426-L428 -> `expose-task-summary-apis` (`test/unit/spec/services/task/Task.ts`) owns two-level disabled rejection/no-HTTP assertions against the root-exported disabled codes, `coordinate-summary-realtime-state` (`test/unit/spec/services/task/AISummaryCoordinator.ts` and `test/unit/spec/services/task/TaskManager.ts`) owns latest interaction-flag state, and `wire-contact-center-summary-lifecycle` (`test/unit/spec/cc.ts`) owns independent organization-flag behavior; `synchronize-summary-documentation-and-verify` runs the final regression gate. |
| AC-7 | Addressed | requirement.md:L430-L432 -> `coordinate-summary-realtime-state` (`test/unit/spec/services/task/AISummaryCoordinator.ts` and `test/unit/spec/services/task/TaskManager.ts`) owns exact root-exported timeout codes, shared-duration fake-timer behavior, common cleanup, and late-event assertions, while `expose-task-summary-apis` (`test/unit/spec/services/task/Task.ts`) owns propagation through the public Promise; `synchronize-summary-documentation-and-verify` runs the final regression gate. |
...
| `src/services/task/Task.ts` and `src/services/task/types.ts` | `Task` is the shared base for voice, WebRTC, and digital tasks; `ITask` is the public contract. Existing wrap-up/consult/transfer methods are independent Promises. | Extend | Declare the package-internal `AISummaryRequestCoordinator` interface in `src/services/task/types.ts` and add the four public APIs once on `Task`/`ITask`. `Task` receives the adapter, feature flags, and narrow coordinator contract only through package-internal `configureAISummary(...)` immediately after factory creation; it never imports `TaskManager` or the concrete coordinator. Every existing subclass inherits the APIs without constructor or behavior duplication, and existing call-control methods remain untouched. |
| `src/services/task/TaskManager.ts` | Owns the task registry, parses RTD frames, maps `data.data.conversationId` to tasks for transcripts/suggestions, and controls task cleanup. The current source is already 978 lines and its unit spec is 2,481 lines. | Extend | Preserve registry, RTD parsing, task matching, and transcript/suggestion dispatch. Compose one `AISummaryCoordinator` for the TaskManager lifetime and configure every factory-created Task with that same instance before listener setup or registry insertion. Validate and classify summary frames, then delegate resolver, feature-state, buffer, timer, and summary-cleanup transitions to the coordinator; task removal and the SDK-deregistration facade both converge on its shared timed-entry cleanup path. |
...
| `src/services/config/types.ts` and `src/index.ts` | `AIFeatureFlags.generatedSummaries` already exposes optional `wrapUpSummariesEnabled` and `consultTransferSummariesEnabled`. Raw `POST_CALL_SUMMARY` and `MID_CALL_SUMMARY` names already exist in `CC_TASK_EVENTS`, which is root-exported and spread into the public `CC_EVENTS` object despite its internal JSDoc. | Preserve and extend | Choose the public-compatibility disposition: retain `CC_TASK_EVENTS.POST_CALL_SUMMARY` and `.MID_CALL_SUMMARY` as deprecated aliases so existing property access does not break, but document and test that they are inbound RTD discriminator aliases and are never emitted as Task or ContactCenter public events. Make the new `CC_AI_SUMMARY_EVENTS` group the single source of those two string literals plus feature/subsequent-agent names, and have the legacy properties reference it rather than duplicate literals. Only `cc:featureEnablement` and `task:midCallSummaryForReceivingAgent` are new public emissions. No new config key. |
...
- `AISummaryRequestCoordinator` is declared in `src/services/task/types.ts`; `Task` stores only that contract, never a `TaskManager` reference. TaskManager owns one concrete `AISummaryCoordinator` for its lifetime and injects the shared instance through `Task.configureAISummary(...)` at every task-creation path before the Task becomes observable. The coordinator owns only validated summary state transitions: feature snapshots, pending resolvers, receiver buffering, both timer lifecycles, and scoped/full cleanup.
...
flowchart LR
  ...
  TM -->|configureAISummary at task creation| Task
  CC --> API
...
TaskManager constructs one `AISummaryCoordinator` for its own lifetime. Immediately after every `TaskFactory.createTask(...)` call and before `setupTaskListeners(...)` or `taskCollection` insertion, it invokes `configureAISummary(...)` with the shared coordinator through the narrow interface. `Task` retains the injected adapter, flags, and interface reference for that Task's lifetime, while TaskManager owns and clears the concrete coordinator. This avoids changing `Voice`, `WebRTC`, and `Digital` constructors or exposing either TaskManager or the concrete coordinator publicly. A defensive call on an unconfigured Task rejects `AI_SUMMARY_NOT_INITIALIZED` without touching the backend.
...
- A missing/unknown base URL uses the exact augmented `AI_SUMMARY_ERROR_CODES.AI_ASSISTANT_BASE_URL_NOT_AVAILABLE` contract; remote HTTP failures retain `ApiAIAssistant`/`getErrorDetails()` translation. There is no retry.
...
Both timer policies read their semantic aliases from `src/services/task/constants.ts`: request timers use `AI_SUMMARY_REQUEST_TIMEOUT_MS`, receiver buffers use `AI_SUMMARY_RECEIVER_BUFFER_RETENTION_MS`, and both aliases resolve to the one `AI_SUMMARY_DURATION_MS = 30_000` literal. This preserves meaningful names at each call site without maintaining two independent 30-second values; intentionally changing only one policy later requires an explicit design/requirement update.
...
- `AISummaryCoordinator` owns every summary resolver, feature snapshot, receiver buffer, and request/expiry timer; TaskManager owns none of those collections, duration literals, or timer-clear branches.
...
- `test/unit/spec/services/ApiAiAssistant.ts`: exact wire bodies for all six outbound names, distinct conversation/interaction fields, numeric fields, field omission, one request attempt, exact exported base-URL code, HTTP errors, and privacy spies.
- `test/unit/spec/cc.ts`: summary-controlled RTD connection, feature-event forwarding, proof that initiator aliases are never re-triggered, named-listener cleanup, deregistration through the coordinator's full timed-entry cleanup, and existing event/register behavior.
