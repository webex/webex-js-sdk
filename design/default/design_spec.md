...
| FR-1 | Addressed | requirement.md:L143-L153 -> Component: Feature enablement and SDK lifecycle owns organization-flag propagation and public re-emission; Component: Realtime coordination, correlation, and receiver delivery owns `FEATURE_ENABLEMENT` parsing plus the latest per-interaction setter/getter state; Component: Public contracts and task API owns the two-level request-time gate and no-backend disabled outcome. `wire-contact-center-summary-lifecycle`, `coordinate-summary-realtime-state`, and `expose-task-summary-apis` implement those respective slices. |
...
- TaskManager classifies and metrics every `FEATURE_ENABLEMENT` frame, emits one bounded inbound-drop metric for each malformed, unknown, late, or uncorrelated summary frame, then calls `AISummaryCoordinator.setFeatureEnablement(...)` for each valid feature payload before emitting `AGENT_EVENTS.FEATURE_ENABLEMENT` through its EventEmitter; Task reads the same latest snapshot through `getFeatureEnablement(...)`, and the named `cc.ts` listener re-triggers every valid payload as `cc:featureEnablement` without deduplication.
...
The event literal controls both the timeout code and resolved payload type. A post-call registration therefore returns `Promise<PostCallSummaryEventPayload>`, while a mid-call registration returns `Promise<MidCallSummaryEventPayload>`; Task supplies no generic type argument and performs no assertion. The two package-internal lookup types are source-module exports only so `Task` and `AISummaryCoordinator` share this relationship without duplicating it; neither is a package-root contract.
...
3. Initialize a call-local `registered = false`, then call the non-`async` `registerPendingAISummaryRequest(taskId, conversationId, 'POST_CALL_SUMMARY', 'POST_CALL_SUMMARY_TIMEOUT')` before HTTP. The literal arguments infer `Promise<PostCallSummaryEventPayload>` from `AISummaryPayloadByInboundType`; Task supplies no type argument or cast. The logical key is `(conversationId, 'POST_CALL_SUMMARY')`; `taskId` is stored as the entry owner but is not part of the key. If that key is occupied, registration synchronously throws augmented `AI_SUMMARY_REQUEST_ALREADY_PENDING` before constructing/returning a Promise, inserting an entry, or arming a timer. Only after registration returns its pending Promise does this call set `registered = true`; the synchronous overlap path therefore cannot reach the HTTP method.
...
These methods accept typed, already-validated payloads rather than raw frames. `setFeatureEnablement(...)` replaces the value keyed by the payload's `interactionId`, while `getFeatureEnablement(...)` returns that latest value to Task through `AISummaryRequestCoordinator`; missing entries remain disabled. The type-indexed pending maps let `resolvePendingAISummaryRequest(...)` pass its payload directly to the matching resolver with no assertion, and Task call sites receive the corresponding concrete payload Promise without a cast. That boundary makes feature gating, overlap, resolver settlement, timeout, latest-only replacement, buffer expiry/delivery, ambiguity, and cleanup directly testable with fake timers without entering TaskManager's RTD parser. `routeReceivingSummary` emits only when `matchingTasks` has exactly one member; zero or multiple candidates retain the latest bounded payload. `flushReceivingSummary` retains the entry while the set is empty or ambiguous and delivers only for exactly one candidate.
...
2. For a classified `FEATURE_ENABLEMENT` frame, validate the inner object, its non-empty `interactionId`, and any present enablement fields. On success, call `aiSummaryCoordinator.setFeatureEnablement(innerPayload)` before emitting the same validated payload through TaskManager's EventEmitter as `AGENT_EVENTS.FEATURE_ENABLEMENT`, then return. This TaskManager parser is the only writer path into `interactionFeatureEnablement`; `Task.requestPostCallSummary()` and `Task.requestMidCallSummary()` are the readers through the injected coordinator's `getFeatureEnablement(interactionId)`. Invalid feature input neither changes the map nor emits the event.
