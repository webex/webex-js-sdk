...
- Receiver payloads are cleared after delivery, replacement, expiry, or full SDK deregistration. Individual task removal does not blindly clear a buffer: it re-flushes against the post-removal candidate set, which either delivers to one remaining task or retains for zero/multiple matches. Unscoped full-SDK cleanup clears each pending timer before rejecting every pending post-call and mid-call result Promise with `AI_SUMMARY_REQUEST_CANCELLED`; it also clears the receiver buffer, feature snapshots, and their timers. The inactive-lifecycle guard makes any subsequently classified AI-summary frame a metadata-only drop until the next registration's `setConfigFlags(...)` call. Feature snapshots use the top-level interaction key throughout: unmatched snapshots expire through `AI_SUMMARY_FEATURE_ORPHAN_RETENTION_MS`, matching task registration removes the orphan timer, final matching-task removal clears lifecycle state, and full deregistration clears both forms.
