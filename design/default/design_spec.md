...
- `test/unit/spec/services/task/TaskManager.ts`: parser/delegation and initiator isolation; conversation match followed by deterministic parent/child leaf selection; parent+child and three-leg chains deliver the unique leaf independent of registry order; sibling leaves and missing/cyclic lineage emit no task event and exactly one `ambiguous-receiver` metric; zero-match buffering re-flushes on insertion/update/removal; expiry emits the separate `receiver-buffer-expired` metric. Invalid task correlation remains exception-safe. Existing feature, inactive-lifecycle, composition, response-retention, privacy—including distinct human-authored section-key and section-value sentinels—transcript, and suggestion regressions remain.
