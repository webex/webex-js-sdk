...
- A feature snapshot's canonical interaction key is the event's non-empty `interactionId`, which must represent the same top-level identifier returned as `getAISummaryCorrelation(task.data).interactionId`; the stable `conversationId`/`mainInteractionId` is never substituted. The writer, Task gating reader, task-registration retention, and task cleanup all use that one derivation.
- No dependency, package, lockfile, schema, persistence, worker, stream, or new SDK configuration key is introduced. The receiver buffer is bounded, process-local memory rather than persistence and is cleared on delivery, replacement, expiry, task cleanup, or SDK deregistration. Feature snapshots associated with a registered task are cleared when the final task for their canonical interaction key leaves; a snapshot received before any matching task is bounded by the same 30-second duration and is cleared unless a matching task claims it, and full SDK cleanup clears both forms.
...
| REQ-010 | Addressed | requirement.md:L82-L82 -> Shared failure ownership: Component: Public contracts and task API owns disabled outcomes and propagation through the public Promise; Component: AI Assistant transport and outbound serialization owns missing-base-URL, HTTP-failure, and bounded-acknowledgement outcomes; Component: Realtime coordination, correlation, and receiver delivery owns the 30-second inbound timeout plus malformed, unknown-task, uncorrelated, and late-event isolation; Change: Cross-cutting safeguards and verification owns the integrated regression gate. |
...
| FR-1 | Addressed | requirement.md:L143-L153 -> Component: Feature enablement and SDK lifecycle owns organization-flag propagation and public re-emission; Component: Realtime coordination, correlation, and receiver delivery owns `FEATURE_ENABLEMENT` parsing plus latest-state storage under the canonical top-level interaction key, active-task lifecycle eviction, and 30-second orphan retention; Component: Public contracts and task API owns the two-level request-time gate and no-backend disabled outcome. `wire-contact-center-summary-lifecycle`, `coordinate-summary-realtime-state`, and `expose-task-summary-apis` implement those respective slices. |
...
- TaskManager classifies and metrics every `FEATURE_ENABLEMENT` frame, emits one bounded inbound-drop metric for each malformed, unknown, late, or uncorrelated summary frame, then calls `AISummaryCoordinator.setFeatureEnablement(payload, hasRegisteredTask)` for each valid feature payload before emitting `AGENT_EVENTS.FEATURE_ENABLEMENT` through its EventEmitter. Writer presence checks, Task gating through `getFeatureEnablement(...)`, registration retention, and final-task cleanup all use `getAISummaryCorrelation(task.data).interactionId`, exactly the task's top-level interaction ID; an event value is never compared with the conversation key. A snapshot with no registered match expires after 30 seconds unless task registration calls `retainFeatureEnablementForTask(interactionId)` first. The named `cc.ts` listener still re-triggers every valid payload without deduplication.
...
- After TaskManager inserts or updates a task, it first calls `retainFeatureEnablementForTask(getAISummaryCorrelation(task.data).interactionId)` so any orphan timer is cleared before the task becomes observable. After it synchronously emits the incoming/hydrate lifecycle event, it recomputes candidates for that conversation and calls `flushReceivingSummary(...)`. A sole match makes the coordinator remove the buffer and clear its timer before emitting the payload exactly once; zero or multiple matches retain it until a later registration/update or expiry.
- Task removal derives both keys once with `getAISummaryCorrelation(task.data)`, invokes scoped request/receiver cleanup with `taskId` and `conversationId`, and, after registry removal, calls `clearFeatureEnablement(interactionId)` only when no remaining task has that same canonical interaction key. `ContactCenter.deregister()` invokes `TaskManager.clearAISummaryState()` before RTD listener/socket shutdown; feature orphan expiry, receiver delivery/expiry, scoped cleanup, and this full-SDK handoff all clear their entries and any timers through the coordinator's common timed-entry removal path.
...
The coordinator has one private generic `removeTimedEntry<T extends {timeoutId?: ReturnType<typeof setTimeout>}>(entries: Map<string, T>, key: string): T | undefined` helper for all three maps. It deletes the entry, calls `clearTimeout(entry.timeoutId)` exactly once when a handle is present, and returns the removed entry when the caller must resolve, reject, deliver, or reinsert it. Request resolution, HTTP-error cancellation, timeout rejection, owner-task cleanup, and SDK cleanup clear the FR-11 timer through this helper before settlement. Receiver replacement, delivery from `routeReceivingSummary(...)` or `flushReceivingSummary(...)`, expiry, conversation/task cleanup, and SDK cleanup clear the FR-8 timer through the same helper before replacement, emission, or drop. Feature replacement, orphan promotion/expiry, final-task cleanup, and SDK cleanup use it for the optional orphan timer. No other path directly pairs map deletion with `clearTimeout`.

`clearAISummaryState` iterates both type-indexed pending maps, the receiver map, and every feature entry through that helper, then rejects removed pending entries with an augmented cancellation error. `clearTaskAISummaryState` selects each pending map by its inbound type and uses the same helper to reject/delete an entry only when `entry.taskId === taskId`; it must not delete solely because another task derives the same `conversationId` as the map key. It also performs scoped receiver-buffer cleanup by conversation. Thus cleanup of a sibling task sharing the conversation cannot strand the requesting task's Promise, while cleanup of the owning task leaves no Promise or timer pending. Feature cleanup is intentionally separate because its key is `interactionId`: `removeTaskFromCollection` calls `clearFeatureEnablement(interactionId)` only after registry removal proves no remaining task has that same canonical interaction key. `cc.deregister()` invokes TaskManager's unscoped facade, so task cleanup, orphan expiry, and SDK deregistration all converge on the common removal helper.

Modify `packages/@webex/contact-center/src/services/task/TaskManager.ts` and its test to compose one private `AISummaryCoordinator`. TaskManager remains the raw RTD parser and task-registry owner, holds the current profile-derived `ConfigFlags` set by `ContactCenter`, derives exact receiving-task candidates and canonical interaction-key presence, and delegates only validated state transitions. It exposes `clearAISummaryState(reason?)` as the existing lifecycle facade for `cc.ts`. `removeTaskFromCollection` derives `{conversationId, interactionId}` once, delegates owner/scoped request-receiver cleanup, removes the task, and clears the feature entry only when the remaining registry has no task whose `getAISummaryCorrelation(data).interactionId` equals that `interactionId`. TaskManager makes its constructor-required `apiAIAssistant` field non-optional and adds `private createManagedTask(taskData: TaskData): Task`, a wrapper around the existing `TaskFactory.createTask(...)`; it then calls `task.configureAISummary(this.apiAIAssistant, this.aiSummaryCoordinator, this.getGeneratedSummaryFlags)`. All existing TaskFactory call sites in TaskManager use this wrapper. `TaskFactory` and concrete subclass constructors remain unchanged.
...
2. For a classified `FEATURE_ENABLEMENT` frame, validate the inner object, its non-empty `interactionId`, and any present enablement fields. Treat that field only as the canonical top-level interaction key. Determine `hasRegisteredTask` by comparing it with `getAISummaryCorrelation(task.data).interactionId` for the current registry, then call `aiSummaryCoordinator.setFeatureEnablement(innerPayload, hasRegisteredTask)` before emitting the same validated payload through TaskManager's EventEmitter as `AGENT_EVENTS.FEATURE_ENABLEMENT`. This TaskManager parser is the only writer path into `interactionFeatureEnablement`; `Task.requestPostCallSummary()` and `Task.requestMidCallSummary()` derive the identical key and read through `getFeatureEnablement(interactionId)`. Invalid feature input neither changes the map nor emits the event.
...
6. For `MID_CALL_SUMMARY_RESPONSE_SUBSEQUENT_AGENT`, search registered tasks for all tasks whose `getAISummaryCorrelation(task.data).conversationId` equals the inbound `conversationId`. Do not consult any inbound `interactionId`.
7. Pass the validated payload and complete candidate set to `aiSummaryCoordinator.routeReceivingSummary(...)`. The coordinator emits to the sole match or replaces the bounded 30-second buffer when there are zero or multiple matches.
8. After each normal task insertion/update, derive its correlation once and call `aiSummaryCoordinator.retainFeatureEnablementForTask(interactionId)` before the task's incoming/hydrate event becomes externally observable; this promotes a matching orphan snapshot to task-lifecycle state and clears its timer. After synchronously emitting that lifecycle event, recompute all candidates for `conversationId` and call `aiSummaryCoordinator.flushReceivingSummary(...)`. This latter ordering lets the application attach a task listener before a buffered receiver payload is emitted. Delivery deletes/clears the buffer first, then emits once.
...
- Receiver payloads are cleared after delivery, expiry, task cleanup, or deregistration. Pending timers receive owner-checked task/request cleanup and unscoped full-SDK cleanup. Feature snapshots use the top-level interaction key throughout: unmatched snapshots expire after 30 seconds, matching task registration removes the orphan timer, final matching-task removal clears lifecycle state, and deregistration clears both forms.
...
2. is delegated to `AISummaryCoordinator.setFeatureEnablement(payload, hasRegisteredTask)`, where TaskManager computes presence only by comparing the payload key with `getAISummaryCorrelation(task.data).interactionId`; the latest value replaces the prior entry and timer for that key, and an unmatched entry receives the 30-second orphan timer;
...
Interaction flags are not persisted between registrations. Repeated feature events are forwarded and overwrite only the gating snapshot under their canonical top-level interaction key. Orphan snapshots expire after 30 seconds; task-bound snapshots are removed with the final matching task; SDK deregistration clears all remaining entries and timers. Task response APIs remain usable for cancellation/`NOT_RECEIVED` reporting even if a later feature event disables a new request; gating applies only to `request*Summary()`.
...
`AISummaryCoordinator.ts`: latest feature values replace prior values under the top-level interaction key, missing values remain disabled, interaction isolation, 30-second unmatched-entry expiry/replacement, matching-task promotion without payload loss, final-task scoped cleanup, full cleanup, and no stale feature timer.

`TaskManager.ts`: `getGeneratedSummaryFlags()` returns the latest `setConfigFlags(...)` view; every valid/repeated feature frame is delegated with canonical-key task presence and emitted; writer, Task gating reader, task-registration promotion, and final-task cleanup use the same `getAISummaryCorrelation(data).interactionId` derivation and never the conversation key; every classified invalid payload metrics once without delegation/forwarding; unparseable/unclassifiable frames do not increment the feature metric but do produce their applicable bounded inbound-drop metric; and malformed input is isolated. Together with `Task.ts` and `cc.ts`, these cover FR-1, PR-2, REQ-055, AC-6, AC-10, and AC-11.
...
- `removeTaskFromCollection` derives both correlation keys through `getAISummaryCorrelation(...)`, delegates scoped request/receiver cleanup—which clears a pending entry only when its stored task owner matches—and clears the feature entry by top-level interaction ID only after no remaining task shares that key; it never uses the conversation key for feature cleanup or removes a sibling owner's pending entry;
