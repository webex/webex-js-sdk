...
addCommand(
  'waitForSpecificText',
  function waitForSpecificText(selector, value, timeout = 5000, reverse = false) {
    let condition;
    ...
    if (reverse) {
      this.waitUntil(
        () => !condition(),
        timeout,
        `$('${selector}') did not change from ${value} within the expected timeout`
      );
    } else {
      this.waitUntil(
        () => condition(),
        timeout,
        `$('${selector}') did not equal ${value} within the expected timeout`
      );
    }
  }
);
