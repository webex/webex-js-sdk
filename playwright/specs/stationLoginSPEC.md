...
assumptions:
  ...
  - Incoming tasks are triggered externally (caller optional)
clarifications:
  - Desktop mode: Agent accepts/declines calls via widget buttons
  - Extension mode: Agent accepts/declines calls via widget buttons in the sample app
  - RONA occurs when agent doesn't respond within timeout
  - RONA state is shown when agent explicitly declines or times out
  - Login mode can temporarily appear as "Choose Agent Login …" during reload/reconnect recovery
unresolved_items: []
```
...
```typescript
// Common setup for all suites
await testManager.basicSetup(browser);

// Station Login Tests (examples)
await dialLogin(page, process.env[`${projectName}_ENTRY_POINT`]);
await extensionLogin(page, process.env[`${projectName}_AGENT1_EXTENSION_NUMBER`]);
await desktopLogin(page);
await verifyLoginMode(page, LOGIN_MODE.DIAL_NUMBER);

// Ensure authenticated state before test actions
await ensureUserStateVisible(page, LOGIN_MODE.DIAL_NUMBER, process.env[`${projectName}_ENTRY_POINT`]);
...
```typescript
USER_STATES: { MEETING, AVAILABLE, LUNCH, RONA, ENGAGED, WRAPUP }
...
| 6 | Click on Dial Number option | Dial Number mode selected |
...
| 10 | Wait for station-login area to switch to logged-in state | Login successful |
| 11 | Verify user state is visible | State widget displayed |
| 12 | Perform station logout | Agent logged out |
| 13 | Verify login controls visible again | Logout successful |

```typescript
await expect(page.locator(getSelector('agentLoginSelect'))).toBeVisible({timeout: 2000});
await expect(page.locator(getSelector('dialNumberInput'))).toBeVisible({timeout: 2000});
await expect(page.locator(getSelector('teamsDropdown'))).toBeVisible({timeout: 2000});
await expect(page.locator(getSelector('loginButton'))).toBeVisible({timeout: 2000});
...
| 8 | Verify mode/state after reconnect | State restored |
...
| 4 | Check team dropdown | Dropdown visible |
| 5 | Check login button | Button visible |
| 6 | Click Extension option | Extension mode selected |
...
await extensionLogin(page, process.env[`${projectName}_AGENT1_EXTENSION_NUMBER`]!);
await ensureUserStateVisible(page, LOGIN_MODE.EXTENSION, process.env[`${projectName}_AGENT1_EXTENSION_NUMBER`]!);
await stationLogout(page);
```
...
| Step | Action | Expected Result |
|------|--------|-----------------|
| 1 | Incoming task created externally | Call initiated |
...
const taskId = await waitForIncomingTask(agent1Page, 40000);
await acceptIncomingTask(agent1Page, taskId);
await waitForState(agent1Page, USER_STATES.ENGAGED);
await agent1Page.locator(getSelector('endButton')).click();
...
| 6 | Verify RONA state | State changed |
...
```typescript
const taskId = await waitForIncomingTask(agent1Page, 40000);
await declineIncomingTask(agent1Page, taskId);
await agent1Page.locator('#agentStatePopup').waitFor({state: 'visible', timeout: 15000});
await verifyCurrentState(agent1Page, USER_STATES.RONA);
...
await testManager.agent1Page.locator('#agentStatePopup').waitFor({state: 'visible', timeout: 15000});
...
const incomingTaskList = testManager.agent1Page.locator('#taskList .task-item');
await expect(incomingTaskList.first()).toBeHidden();
...
| `selectLoginMode(page, mode)` | Select login mode in station widget |
| `stationLogout(page)` | Logout from station |
| `verifyLoginMode(page, mode)` | Verify current login mode |
| `ensureUserStateVisible(page, mode, number?)` | Ensure logged-in state and visible user-state widget |
...
| `waitForState(page, expected)` | Poll until expected state is selected |
| `verifyCurrentState(page, expected)` | Assert current state |
| `getStateElapsedTime(page)` | Get timer value |
| `timerToSeconds(timer)` | Normalize timer text to seconds |
| `parseDualTimer(timer)` | Parse dual timer text (`current / previous`) |
| `checkCallbackSequence(logs, state)` | Verify callback order for a state |

### 7.3 Incoming Task Utilities (incomingTaskUtils.ts)

| Utility | Purpose |
|---------|---------|
| `waitForIncomingTask(page, timeout)` | Wait for task ID in task list |
| `acceptIncomingTask(page, taskId)` | Accept via widget |
| `declineIncomingTask(page, taskId)` | Decline via widget |
| `endCallTask(page)` | End call via widget |
...
| Utility | Purpose |
|---------|---------|
| `waitForConsolePattern(getLogs, pattern, timeout?)` | Wait for expected console pattern |
| `setOffline(context)` | Simulate network disconnect |
| `setOnline(context)` | Restore network |
| `navigateToContactCenter(page, baseUrl?)` | Navigate from sample root to Contact Center sample |
...
## 12. Guardrails For Playwright Failure Prevention (Required)

### GL-1: Idempotent Login Submission

- `submitLogin()` must not fail if the agent is already logged in.
- If `#logoutAgent` is visible and `#loginAgent` is disabled, treat as valid logged-in state and skip login click.
- Prevents false failure: `expect(locator('#loginAgent')).toBeEnabled()` when session is restored.

### GL-2: Robust Login Mode Verification

- `verifyLoginMode()` must accept either:
  1. UI mode selected in `#AgentLogin`, or
  2. SDK state from `webex.cc.agentConfig.deviceType`.
- Prevents false mismatch when UI shows placeholder `Choose Agent Login …` after reload/network recovery.

### GL-3: Reconnect-Only Mode Assertion Rule

- During network tests, do **not** assert login mode while offline.
- Required sequence:
  1. assert disconnect signal,
  2. restore online,
  3. assert reconnect signal,
  4. then assert login mode and user state.

### GL-4: Strong Session Check in `ensureUserStateVisible()`

- `ensureUserStateVisible()` must verify both:
  - user state widget visibility (`#idleCodesDropdown`), and
  - logged-in state (`#logoutAgent` visible or `#loginAgent` hidden).
- If either check fails, perform login flow before continuing.

### GL-5: Selector Consistency Rule

- Sample-app spec snippets must use actual selectors present in `docs/samples/contact-center/index.html`.
- Replace non-app selectors/test-ids in examples with real ids:
  - RONA popup: `#agentStatePopup`
  - RONA state select: `#agentStateSelect`
  - RONA confirm: `#setAgentState`
  - Task list container: `#taskList`

### GL-6: Spec/Test Sync Rule

- Any change to station login network/reload behavior must update:
  - this spec,
  - `tests/station-login-test.spec.ts`,
  - `Utils/stationLoginUtils.ts`,
  - `specs/stationLoginSPEC.md`.
