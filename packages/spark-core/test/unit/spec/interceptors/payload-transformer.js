...
import {PayloadTransformerInterceptor} from '../../..';
  ...
  describe(`Interceptors`, () => {
    describe(`PayloadTransformerInterceptor`, () => {
      ...
      beforeEach(() => {
        ...
        spark.config.payloadTransformer = {
          predicates: [{
        ...
        interceptor = Reflect.apply(PayloadTransformerInterceptor.create, spark, []);
        assert.isDefined(interceptor.spark);
        assert.isDefined(interceptor.spark.config);
      });

      describe(`#transform()`, () => {
        it(`transforms request objects`, () => assert.becomes(interceptor.transform(`outbound`, {
          body: {
            actor: {
