...
async function fetchGuestAccessTokenLambda() {
  const response = await fetch(
    'https://pbw56237i55l2vkcpc5dhskhra0bplhr.lambda-url.us-east-2.on.aws'
  );
...
async function initCalling(e) {
  ...
  calling.on('ready', () => {
    ...
    calling.register().then(async () => {
      ...
      console.log('Shreyas: callingClient', calling);
      ...
      if (window.callSettings === undefined) {
        console.log('Shreyas: callSettings', calling.callSettingsClient);
...
function renderContacts(contacts, groupIdDisplayNameMap) {
  contactsTable.innerHTML = contacts.reduce((acc, contact, i) => {
    ...
    const phoneNumbers = contact.phoneNumbers?.reduce(
      (acc, currValue) => `${acc}<p>${currValue.type}:${currValue.value}</p>`,
      ''
    );
    ...
    return `${acc}
      <tr>
        ...
        <td>${parentGroups}</td>
        <td><button onclick="deleteContact('${
          contact.contactId
        }')" class="btn--red">Delete</button></td>
      </tr>
    `;
  }, '');
}
  ...
  contactGroupsTable.innerHTML = groups.reduce((acc, group, i) => {
    return `${acc}
      <tr>
        ...
        <td>${group.groupId}</td>
        <td>${group.members}</td>
        <td><button onclick="deleteContactGroup('${
          group.groupId
        }')" class="btn--red">Delete</button></td>
      </tr>
    `;
...
function definedTable(callHistoryResponse) {
  ...
  for (let i = 0; i < callHistoryResponse.data.userSessions.length; i += 1) {
    ...
    <td>${
      callHistoryResponse.data.userSessions[i].callingSpecifics?.redirectionDetails?.reason ===
      undefined
        ? 'NA'
        : callHistoryResponse.data.userSessions[i].callingSpecifics.redirectionDetails.reason
    }</td>
    <td>${
      callHistoryResponse.data.userSessions[i].callingSpecifics?.redirectionDetails?.name ===
      undefined
        ? 'NA'
        : callHistoryResponse.data.userSessions[i].callingSpecifics.redirectionDetails.name
    }</td>
...
async function createVoiceMail() {
  ...
  if (backendConnector === 'NATIVE_SIP_CALL_TO_UCM') {
    ...
    try {
      ...
      const {voicemailList} = getVoicemailListResponse.data;
...
async function createContactGroup() {
  ...
  const res = await contacts.createContactGroup(
    contactGroupsElem.elements.displayName.value,
    undefined,
    groupType
  );
...
async function fetchVoicemailSetting() {
  ...
  console.log('Voicemail response: ', response);
  if (response.statusCode === 200) {
...
document.getElementById('Voice-mail').addEventListener('click', createVoiceMail);
...
window.getMediaStreams = getMediaStreams;
window.toggleNoiseReductionEffect = toggleNoiseReductionEffect;
window.endCall = endCall;
window.answer = answer;
window.muteUnmute = muteUnmute;
window.holdResume = holdResume;
window.sendDigit = sendDigit;
window.initiateTransfer = initiateTransfer;
window.endSecondCall = endSecondCall;
window.getCallQuality = getCallQuality;
window.clearMediaDeviceList = clearMediaDeviceList;
window.getMediaDevices = getMediaDevices;
window.changeStream = changeStream;
window.cleanUpMedia = cleanUpMedia;
window.createCallHistory = createCallHistory;
window.createVoiceMail = createVoiceMail;
window.fetchVoicemailList = fetchVoicemailList;
window.resolveContactInfo = resolveContactInfo;
window.fetchTranscript = fetchTranscript;
window.fetchVoicemailSummary = fetchVoicemailSummary;
window.getSettings = getSettings;
window.toggleDNDSetting = toggleDNDSetting;
window.updateCallForwardSetting = updateCallForwardSetting;
window.getCallForwardAlwaysSetting = getCallForwardAlwaysSetting;
window.updateVoicemailSetting = updateVoicemailSetting;
window.getContacts = getContacts;
window.createCustomContact = createCustomContact;
window.createCloudContact = createCloudContact;
