...
let taskId;
let wrapupCodes = [];
let isTransferOptionsShown = false;
let entryPointId = '';
let stateTimer;
...
async function onConsultTypeSelectionChanged() {
  ...
  if (destinationTypeDropdown.value === 'agent') {
    ...
    async function refreshBuddyAgentsForConsult() {
      ...
      agentNodeList.forEach((n) => {
        consultDestinationInput.appendChild(n);
      });
    }
...
async function onTransferTypeSelectionChanged() {
  ...
  if (document.querySelector('#transfer-destination-type').value === 'agent') {
    ...
    const agentNodeList = await fetchBuddyAgentsNodeList();
    agentNodeList.forEach((n) => {
      transferDestinationInput.appendChild(n);
    });
...
function registerTaskListeners(task) {
  ...
  task.on('task:consultQueueCancelled', (task) => {
    if (currentTask.data.interactionId === task.data.interactionId) {
      ...
      console.log(
        `Received task:consultQueueCancelled for task: ${currentTask.data.interactionId}`
      );
      currentConsultQueueId = null;
      hideEndConsultButton();
      showConsultButton();
  ...
  if (isNew) {
    ...
    if (callProcessingDetails) {
      ...
      pauseResumeRecordingElm.innerText =
        isPaused === 'true' ? 'Resume Recording' : 'Pause Recording';
    }
...
function startStateTimer(lastStateChangeTimestamp, lastIdleCodeChangeTimestamp) {
  ...
  stateTimer = setInterval(() => {
    ...
    const idleCodeChangeTimeDifference = lastIdleCodeChangeTimestamp
      ? currentTime - new Date(lastIdleCodeChangeTimestamp).getTime()
      : null;

    const stateHours = String(Math.floor(stateTimeDifference / (1000 * 60 * 60))).padStart(2, '0');
    const stateMinutes = String(
      Math.floor((stateTimeDifference % (1000 * 60 * 60)) / (1000 * 60))
    ).padStart(2, '0');
    const stateSeconds = String(Math.floor((stateTimeDifference % (1000 * 60)) / 1000)).padStart(
      2,
      '0'
    );
    ...
    if (
      idleCodeChangeTimeDifference !== null &&
      lastStateChangeTimestamp !== lastIdleCodeChangeTimestamp
    ) {
      console.log(
        'Idle code change time difference: ',
        lastStateChangeTimestamp,
        ' ',
        lastIdleCodeChangeTimestamp
      );
      const idleCodeChangeHours = String(
        Math.floor(idleCodeChangeTimeDifference / (1000 * 60 * 60))
      ).padStart(2, '0');
      const idleCodeChangeMinutes = String(
        Math.floor((idleCodeChangeTimeDifference % (1000 * 60 * 60)) / (1000 * 60))
      ).padStart(2, '0');
      const idleCodeChangeSeconds = String(
        Math.floor((idleCodeChangeTimeDifference % (1000 * 60)) / 1000)
      ).padStart(2, '0');

      timerDisplay =
        `${idleCodeChangeHours}:${idleCodeChangeMinutes}:${idleCodeChangeSeconds}` +
        ` / ${timerDisplay}`;
    }
...
function updateUnregisterButtonState() {
  const isLoggedIn =
    webex?.cc?.agentConfig?.isAgentLoggedIn || !logoutAgentElm.classList.contains('hidden');
...
function register() {
  webex.cc
    ...
    .then((agentProfile) => {
      ...
      const {loginVoiceOptions} = agentProfile;
      ...
      idleCodesList.forEach((idleCodes) => {
        if (idleCodes.isSystem === false) {
          ...
          if (agentProfile.lastStateAuxCodeId && agentProfile.lastStateAuxCodeId === idleCodes.id) {
            ...
            startStateTimer(
              agentProfile.lastStateChangeTimestamp,
              agentProfile.lastIdleCodeChangeTimestamp
            );
          }
...
function doAgentLogin() {
  ...
  webex.cc
    ...
    .then((response) => {
      ...
      const DEFAULT_CODE = '0'; // Default code when no aux code is present
      const auxCodeId =
        response.data.auxCodeId?.trim() !== '' ? response.data.auxCodeId : DEFAULT_CODE;
      const {lastStateChangeTimestamp} = response.data;
      const {lastIdleCodeChangeTimestamp} = response.data;
      const index = [...idleCodesDropdown.options].findIndex(
        (option) => option.value === auxCodeId
      );
...
function setAgentStatus() {
  ...
  webex.cc
    .setAgentState({state, auxCodeId, lastStateChangeReason: agentStatus, agentId})
    .then((response) => {
...
async function renderBuddyAgents() {
  ...
  buddyAgentsDropdownNodes.forEach((n) => {
    buddyAgentsDropdownElm.appendChild(n);
  });
}
...
function togglePauseResumeRecording() {
  ...
  if (pauseResumeRecordingElm.innerText === 'Pause Recording') {
    ...
    pauseResumeRecordingElm.disabled = true;
    const resumeParams = autoResumed ? {autoResumed} : undefined;
    currentTask
      .resumeRecording(resumeParams)
      .then(() => {
...
function wrapupCall() {
  ...
  currentTask
    .wrapup({wrapUpReason: wrapupReason, auxCodeId})
    .then(() => {
...
function handleTaskSelect(task) {
  ...
  currentTask = task;
  if (
    chatAndSocial.includes(task.data.interaction.mediaType) &&
    isBundleLoaded &&
...
window.doAgentLogin = doAgentLogin;
window.logoutAgent = logoutAgent;
window.showupdateAgentProfileUI = showupdateAgentProfileUI;
window.applyupdateAgentProfile = applyupdateAgentProfile;
window.setAgentStatus = setAgentStatus;
window.renderBuddyAgents = renderBuddyAgents;
window.answer = answer;
window.decline = decline;
window.holdResumeCall = holdResumeCall;
window.muteUnmute = muteUnmute;
window.showInitiateConsultDialog = showInitiateConsultDialog;
window.toggleTransferOptions = toggleTransferOptions;
window.endConsult = endConsult;
window.initiateConsultTransfer = initiateConsultTransfer;
window.endCall = endCall;
window.initiateConsult = initiateConsult;
window.closeConsultDialog = closeConsultDialog;
window.initiateTransfer = initiateTransfer;
window.wrapupCall = wrapupCall;
window.togglePauseResumeRecording = togglePauseResumeRecording;
window.pressKey = pressKey;
window.startOutdial = startOutdial;
