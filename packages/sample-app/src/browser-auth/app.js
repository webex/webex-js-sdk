...
// Make handleAuthTypeChange globally accessible for the HTML onchange handlers
window.handleAuthTypeChange = handleAuthTypeChange;

// Initialize DOM elements using the same pattern as other working routes
function initializeBrowserAuth() {
  // Initialize the auth type selection
  handleAuthTypeChange();

  // Add event listeners to radio buttons
  const implicitRadio = document.getElementById('implicit-radio');
  const jwtRadio = document.getElementById('jwt-radio');

  if (implicitRadio) {
    implicitRadio.addEventListener('change', handleAuthTypeChange);
  }
  if (jwtRadio) {
    jwtRadio.addEventListener('change', handleAuthTypeChange);
  }
}

// Use the same initialization pattern as other routes
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', initializeBrowserAuth);
} else {
