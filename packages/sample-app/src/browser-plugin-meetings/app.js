...
let localMediaMicMuted;
let localMediaCameraMuted;

// Declare DOM elements as variables first
let authTypeElm;
let credentialsFormElm;
let tokenElm;
let saveElm;
let authStatusElm;
let oauthFormElm;
...
let registerElm;
let unregisterElm;
let registrationStatusElm;
let integrationEnv;
let eventsList;
let multistreamLayoutElm;
let preferLiveVideoElm;
let breakoutsList;
let breakoutTable;
let breakoutHostOperation;
let getStatsButton;
let tcpReachabilityConfigElm;
let tlsReachabilityConfigElm;
let guestName;
let getGuestToken;
let voiceaSpokenLanguage;
let voiceaCaptionLanguage;
let voiceaSpokenLanguageBtn;
let voiceaCaptionLanguageBtn;
let voiceaTranscriptionTemplate;
let voiceaTranscriptionFormattedDisplay;
let toggleUnifiedMeetings;
let currentMeetingInfoStatus;
let enableLLM;
let enableTranscript;
let spokenLangNote;
let fedRampInput;

// Initialize DOM elements when DOM is ready
function initializeDOMElements() {
  authTypeElm = document.querySelector('#auth-type');
  credentialsFormElm = document.querySelector('#credentials');
  tokenElm = document.querySelector('#access-token');
  saveElm = document.querySelector('#access-token-save');
  authStatusElm = document.querySelector('#access-token-status');
  oauthFormElm = document.querySelector('#oauth');
  oauthLoginElm = document.querySelector('#oauth-login-btn');
  oauthStatusElm = document.querySelector('#oauth-status');
  registerElm = document.querySelector('#registration-register');
  unregisterElm = document.querySelector('#registration-unregister');
  registrationStatusElm = document.querySelector('#registration-status');
  integrationEnv = document.getElementById('integration-env');
  eventsList = document.getElementById('events-list');
  multistreamLayoutElm = document.querySelector('#multistream-layout');
  preferLiveVideoElm = document.querySelector('#prefer-live-video');
  breakoutsList = document.getElementById('breakouts-list');
  breakoutTable = document.getElementById('breakout-table');
  breakoutHostOperation = document.getElementById('breakout-host-operation');
  getStatsButton = document.getElementById('get-stats');
  tcpReachabilityConfigElm = document.getElementById('enable-tcp-reachability');
  tlsReachabilityConfigElm = document.getElementById('enable-tls-reachability');
  guestName = document.querySelector('#guest-name');
  getGuestToken = document.querySelector('#get-guest-token');
  voiceaSpokenLanguage = document.querySelector('#voicea-spoken-language');
  voiceaCaptionLanguage = document.querySelector('#voicea-caption-language');
  voiceaSpokenLanguageBtn = document.querySelector('#voicea-spoken-language-btn');
  voiceaCaptionLanguageBtn = document.querySelector('#voicea-caption-language-btn');
  voiceaTranscriptionTemplate = document.querySelector('#voicea-transcription-template');
  voiceaTranscriptionFormattedDisplay = document.querySelector(
    '#voicea-transcription-formatted-display'
  );
  toggleUnifiedMeetings = document.getElementById('toggle-unified-meeting');
  currentMeetingInfoStatus = document.getElementById('current-meeting-info-status');
  enableLLM = document.getElementById('meetings-enable-llm');
  enableTranscript = document.getElementById('meetings-enable-transcription');
  spokenLangNote = document.getElementById('only-host-spoken-language');
  fedRampInput = document.querySelector('#enable-fedramp');

  // Initialize after elements are available
  if (fedRampInput) {
    fedRampInput.checked = false;
  }

  // Store and Grab `access-token` from localstorage
  if (tokenElm && localStorage.getItem('date') > new Date().getTime()) {
...
function initWebex(e) {
  ...
  webex.once('ready', () => {
    ...
    currentMeetingInfoStatus.innerText = webex.meetings.config.experimental.enableUnifiedMeetings
      ? 'V2'
      : 'V1';
...
function register() {
  ...
  webex.meetings
    ...
    .finally(() => {
      registrationStatusElm.innerText = webex.meetings.registered ? 'Registered' : 'Not Registered';
    });
...
function unregister() {
  ...
  webex.meetings
    ...
    .finally(() => {
      registrationStatusElm.innerText = webex.meetings.regisered ? 'Registered' : 'Not Registered';
    });
}
...
const spaceIDError =
  'Using the space ID as a destination is no longer supported. Please refer to the <a href="https://github.com/webex/webex-js-sdk/wiki/Migration-to-Unified-Space-Meetings" target="_blank">migration guide</a> to migrate to use the meeting ID or SIP address.';
const BNR = 'BNR';
const VBG = 'VBG';
...
createMeetingSelectElm.addEventListener('change', (event) => {
  if (event.target.value === 'CONVERSATION_URL') {
    createMeetingActionElm.innerText =
      'Create Adhoc Meeting using conversation URL (INTERNAL-USE ONLY)';
  } else {
...
meetingsListElm.onclick = (e) => {
  ...
  const selectedMeetingType =
    createMeetingSelectElm.options[createMeetingSelectElm.selectedIndex].innerText;
...
function doPreMediaSetup(meeting) {
  if (isMultistream) {
    ...
    console.log(
      'MeetingStreams#doPreMediaSetup() :: registering for media:ready and media:stopped events'
    );
...
async function joinMeeting({withMedia, withDevice} = {withMedia: false, withDevice: false}) {
  ...
  try {
    ...
    console.error(
      `failed to join a meeting (withMedia=${withMedia} withDevice=${withDevice}): `,
      err
    );
...
function updateMeetingInfoSection(meeting) {
  ...
  titleElm.innerText = meeting.destination.info
    ? meeting.destination?.info?.webExMeetingName
    : meeting.destination;
...
const toggleAdditionalMediaDirection = document.querySelectorAll(
  '[name=ts-additional-media-direction]'
);
...
function revertMediaSettings(changedMediaSettings) {
  ...
  for (const setting in changedMediaSettings) {
    const element = document.querySelector(`[value=${setting}]`);
...
function cleanUpMedia() {
  ...
  [meetingStreamsRemoteVideo, meetingStreamsRemoteShare, meetingStreamsRemoteAudio].forEach(
    (elem) => {
      if (elem.srcObject) {
...
async function toggleTranscription(enable = false) {
  ...
  if (isEnabled && !enable) {
    ...
    const {firsttime} = generalToggleTranscription.dataset;
...
function setTranscriptEvents() {
  ...
  if (meeting) {
    ...
    const transcriptTemplate = Handlebars.compile(voiceaTranscriptionTemplate.innerHTML);
    ...
    meeting.on('meeting:caption-received', (payload) => {
      ...
      voiceaTranscriptionFormattedDisplay.scrollTop =
        voiceaTranscriptionFormattedDisplay.scrollHeight;
    });
...
async function getUserMedia(constraints = {audio: true, video: true}) {
  ...
  console.log(
    'MeetingControls#getUserMedia() :: following local streams are ready to use:',
...
async function loadCamera(constraints) {
  try {
    ...
    const videoConstraints = {
      ...localMedia.videoConstraints[localVideoQuality[localResolutionInp.value]],
      ...constraints,
    };

    console.log(
      'MeetingControls#loadCamera() :: getting camera stream with constraints: ',
      videoConstraints
    );
    localMedia.cameraStream = await webex.meetings.mediaHelpers.createCameraStream(
      videoConstraints
    );
    ...
    localMedia.cameraStream.on('user-mute-state-change', (muted) => {
      console.log(
        'MeetingControls#loadCamera() :: local camera stream user mute state changed to',
        muted
      );
    });
    ...
    localMedia.cameraStream.on('system-mute-state-change', (muted) => {
      console.log(
        'MeetingControls#loadCamera() :: local camera stream system mute state changed to',
        muted
      );
    ...
    console.log(
      'MeetingControls#loadCamera() :: Successfully got camera stream:',
      localMedia.cameraStream
    );
  } catch (e) {
...
async function handleVbg() {
  ...
  try {
    ...
    if (!effect?.isEnabled) {
      console.log(
        'MeetingControls#handleVbg() :: applying virtual background to local camera stream'
      );
      ...
      handleEffectsButton(toggleVbgBtn, VBG, effect);
      console.log(
        'MeetingControls#handleVbg() :: successfully applied virtual background to local camera stream'
      );
    } else {
      console.log(
        'MeetingControls#handleVbg() :: disabling virtual background from local camera stream'
      );

      await effect.disable();
      handleEffectsButton(toggleVbgBtn, VBG, effect);
      console.log(
        'MeetingControls#handleVbg() :: successfully disabled virtual background from local camera stream'
      );
    }
  } catch (e) {
...
async function loadMicrophone(constraints) {
  try {
    ...
    console.log(
      'MeetingControls#loadMicrophone() :: getting microphone stream with constraints: ',
      audioConstraints
    );
    localMedia.microphoneStream = await webex.meetings.mediaHelpers.createMicrophoneStream(
      audioConstraints
    );
    ...
    localMedia.microphoneStream.on('user-mute-state-change', (muted) => {
      console.log(
        'MeetingControls#loadMicrophone() :: local microphone stream user mute state changed to',
        muted
      );
      handleAudioButton();
    });
    ...
    localMedia.microphoneStream.on('system-mute-state-change', (muted) => {
      console.log(
        'MeetingControls#loadMicrophone() :: local microphone stream system mute state changed to',
        muted
      );
    ...
    console.log(
      'MeetingControls#loadMicrophone() :: Successfully got microphone stream:',
      localMedia.microphoneStream
    );
  } catch (e) {
...
async function handleBNR() {
  ...
  try {
    ...
    if (!effect?.isEnabled) {
      ...
      handleEffectsButton(toggleBNRBtn, BNR, effect);
      console.log(
        'MeetingControls#handleBNR() :: successfully applied BNR to local microphone stream'
      );
    } else {
      ...
      handleEffectsButton(toggleBNRBtn, BNR, effect);
      console.log(
        'MeetingControls#handleBNR() :: successfully disabled BNR from local microphone stream'
      );
    }
  } catch (e) {
...
function getMediaDevices() {
  ...
  if (meeting) {
    ...
    webex.meetings.mediaHelpers.getDevices().then((devices) => {
...
async function updateMedia() {
  ...
  console.log(
    `MeetingStreams#updateMedia() :: calling updateMedia(${JSON.stringify(changedMediaSettings)}`
  );

  return meeting
    .updateMedia(changedMediaSettings)
    .then(() => {
      currentMediaSettings = getMediaSettings();
    })
    .catch((error) => {
...
function getAudioVideoInput() {
  const deviceId = (id) => {
    if (id === 'default') return {deviceId: id};
...
function setVideoInputDevice() {
  ...
  if (meeting) {
    ...
    return getUserMedia({video}).then(() => {
...
function setAudioInputDevice() {
  ...
  if (meeting) {
    ...
    return getUserMedia({audio}).then(() => {
...
function handleMuteAudioMessage() {
  muteAudioMessage.innerHTML = localMedia.microphoneStream.systemMuted
    ? 'Warning: microphone may be muted by the system'
    : '';
}
...
function toggleSendAudio() {
  ...
  if (localMedia.microphoneStream) {
    ...
    console.log(
      `MeetingControls#toggleSendAudio() :: Successfully ${
        newMuteValue ? 'muted' : 'unmuted'
      } audio!`
    );
...
function handleMuteVideoMessage() {
  muteVideoMessage.innerHTML = localMedia.cameraStream.systemMuted
    ? 'Warning: camera may be muted by the system'
    : '';
}
...
function toggleSendVideo() {
  ...
  if (localMedia.cameraStream) {
    ...
    console.log(
      `MeetingControls#toggleSendVideo() :: Successfully ${
        newMuteValue ? 'muted' : 'unmuted'
      } video!`
    );
...
function setLocalMeetingQuality() {
  ...
  const videoConstraints = {
    ...localMedia.videoConstraints[localVideoQuality[localResolutionInp.value]],
    ...audioVideoInputDevices.video,
  };

  console.log(
    'MeetingControls#setLocalMeetingQuality() :: applying new constraints to camera stream: ',
    videoConstraints
  );
...
function updateRemoteSourcesInfo() {
  document.getElementById(
    'remote-sources-info'
  ).innerText = `Remote live sources: audio=${remoteSourcesCount.audio.live}/${remoteSourcesCount.audio.total} video=${remoteSourcesCount.video.live}/${remoteSourcesCount.video.total}`;
}
...
function addVideoPane(parent, className) {
  ...
  const videoEl = addElement(containerEl, 'video', {
    className: 'multistream-remote-video',
    autoplay: true,
    playsInline: true,
  });
  ...
  const overlayTextEl = addElement(overlayEl, 'p', {
...
function createContainer(parent, id) {
  return addElement(parent || document.getElementById('multistream-videos-container'), 'div', {
  ...
  videoPane.onclick = () => {
    ...
    if (videoPane.isActive) {
      ...
      updateVideoPane(
        ...
        'no source',
        undefined,
        `stage.${paneId} ${videoPane.remoteMedia.id}`,
        'removed from stage'
      );
...
function createStageLayoutVideos() {
  ...
  const legendEl = addElement(stageFieldset, 'legend', {
    innerText: `Stage (for maximum of ${STAGE_SIZE} participants)`,
  });
  ...
  const thumbnailsLegendEl = addElement(thumbnailsFieldset, 'legend', {
    innerText: 'Active speakers',
  });
  ...
  for (let idx = 0; idx < columns * rows; idx += 1) {
    ...
    videoPane.onclick = () => {
      ...
      if (meeting.remoteMediaManager.isPinned(videoPane.remoteMedia)) {
        ...
        console.log(
          `pinning remoteMedia ${videoPane.remoteMedia.id} to CSI=${videoPane.remoteMedia.csi}`
        );
...
function updateVideoGridPaneSizes() {
  ...
  if (currentLayoutId === 'Stage') {
    const numOfActiveVideoPanes = Object.values(allVideoPanes.stage).filter(
      (pane) => pane.isActive
    ).length;
...
function updateVideoPane(videoPane, meeting, sourceState, memberId, title, debugString) {
  ...
  if (sourceState === 'no source') {
    ...
    console.log(
      `MeetingStreams#updateVideoPane() :: ${debugString} ${sourceState} "${newName}" ${title} `
    );
  }
...
function updateMultistreamVideoLayout() {
  ...
  console.log(
    `MeetingStreams#updateMultistreamVideoLayout() :: changing layout from ${oldLayoutId} to ${newLayoutId}`
  );
...
function setPreferLiveVideo() {
  ...
  const {value} = preferLiveVideoElm;
...
function setSizeHint() {
  ...
  const [width, height] = sizeHintValue.split(',');
...
function processNewVideoPane(meeting, paneGroupId, paneId, remoteMedia) {
  ...
  updateVideoPane(
    ...
    `${paneGroupId}.${paneId} ${remoteMedia.id}`,
    'initialization'
  );
...
function stopForcedScreenShareViewLayout(meeting) {
  if (meeting.remoteMediaManager) {
    if (meeting.remoteMediaManager.getLayoutId() === 'ScreenShareView') {
      ...
      const newLayout =
        layoutSelectedBeforeRemoteShareStarted || selectLayoutElm.firstElementChild.value;
...
function setupMultistreamEventListeners(meeting) {
  ...
  meeting.on('media:remoteAudio:created', (audioMediaGroup) => {
    console.log(
      'MeetingStreams#setupMultistreamEventListeners :: got AUDIO remote media group created'
    );
  ...
  meeting.on('media:remoteScreenShareAudio:created', (screenShareAudioMediaGroup) => {
    document.getElementById('multistream-remote-share-audio').srcObject =
      screenShareAudioMediaGroup.getRemoteMedia()[0].stream;
  });
  ...
  meeting.on(
    ...
    ({layoutId, activeSpeakerVideoPanes, memberVideoPanes, screenShareVideo}) => {
      console.log(
        `MeetingStreams#VideoLayoutChanged :: got video layout changed: layoutId=${layoutId}`
      );
...
function addMediaOptionsLocal(elementId) {
  ...
  const optionElements = mediaOptions.reduce((acc, resolution) => {
    acc += `<option value="${resolution}" ${
      resolution === '720p' && 'selected'
    }>${resolution}</option>`;
...
function changeLayout() {
  ...
  currentMeeting
    .changeVideoLayout(layoutVal, {main: {height, width}})
    .then(() => {
  ...
  webex.devicemanager
    ...
    .then(() => {
      ...
      document.querySelector(
        '#p-paired-device-id'
      ).innerText = `Paired Device: ${currentDevice.deviceInfo.description} [${currentDevice.id}]`;
    })
    .catch((error) => {
...
function refreshDevicesList() {
  ...
  webex.devicemanager.refresh().then((devices) => {
    if (devices.length === 0) {
      devicesListItemsElm.innerText = 'There are currently no meetings devices to display';
  ...
  webex.devicemanager
    ...
    .then((results) => {
      results.forEach((device) =>
        findDevicesListElm.appendChild(generateMeetingsDevicesSearchItem(device))
      );
...
function moveToDevice() {
  const deviceId = webex.devicemanager._pairedDevice
    ? webex.devicemanager._pairedDevice.identity.id
    : undefined;
...
function claimPersonalMeetingRoom() {
  ...
  webex.meetings.personalMeetingRoom
    ...
    .then((pmr) => {
      ...
      pmrDetailsElm.innerText = JSON.stringify(webex.meetings.personalMeetingRoom, null, 2);
    })
    .catch((error) => {
    ...
    meeting
      ...
      .then((res) => {
        ...
        participantsArr.forEach((obj) => {
          if (obj.person.email === emailVal || obj.person.primaryDisplayString === emailVal) {
            mostRecentParticipantId = obj.id;
            mostRecentparticipantName = obj.person.name || obj.person.primaryDisplayString;
...
function muteMember(muteButton) {
  ...
  const newMuteStatus = !selectedMember.isAudioMuted;

  if (meeting) {
    meeting
      .mute(participantID, newMuteStatus)
      .then((res) => {
    ...
    const createGroup = async (newGroup) => {
      await meeting.breakouts.getBreakout().then((res) => {
        ...
        if (existedGroup && existedGroup.status !== 'CLOSED') {
          ...
          sessionList = [
            {name: 'session1', anyoneCanJoin: true},
            {name: 'session2', anyoneCanJoin: false},
          ];
          meeting.breakouts
            .create(
              newGroup || {
...
function viewBreakouts(event) {
  ...
  const createBroadcastDiv = (breakoutSession) => {
    ...
    button.onclick = () => {
      ...
      if (targetBo === 'all') {
        ...
        const boInstance =
          meeting.breakouts.sessionId === targetBo
            ? meeting.breakouts.currentBreakoutSession
            : meeting.breakouts.breakouts.get(targetBo);
  ...
  currentBreakoutSessionName.innerText = meeting.breakouts.isInMainSession
    ? 'Main Session'
    : meeting.breakouts.name;
  ...
  !meeting.breakouts.isInMainSession &&
    !selfIsHost &&
    currentBreakoutInformationEl.appendChild(
      createAskForHelpButton(meeting.breakouts.currentBreakoutSession)
    );
  selfIsHost &&
    currentBreakoutInformationEl.appendChild(
      createAskAllReturnButton(meeting.breakouts.currentBreakoutSession)
    );
  currentBreakoutInformationEl.appendChild(
    createLeaveSessionButton(meeting.breakouts.currentBreakoutSession)
  );

  selfIsHost &&
    hasBreakoutSessions &&
    currentBreakoutInformationEl.appendChild(
      createBroadcastDiv(meeting.breakouts.currentBreakoutSession)
    );
  breakoutTable.innerHTML = '';
...
function createMembersTable(members) {
  ...
  function createRow(member) {
    ...
    if (isMemberSelf(member) && member.isInMeeting) {
      td6.appendChild(
        createButton(member.isBrb ? 'Back to meeting' : 'Step away', toggleBrb, {id: 'brb-btn'})
      );
    } else {
    ...
    .then(() => {
      ...
      if (meeting) {
        ...
        getUserMedia({
          ...
          meeting.joinWithMedia({joinOptions, mediaOptions}).then(() => {
            doPostMediaSetup(meeting);
            meeting.acknowledge('INCOMING').then(() => {
...
window.getGuestAccessToken = getGuestAccessToken;
window.register = register;
window.unregister = unregister;
window.toggleMeetingInfo = toggleMeetingInfo;
window.loadCamera = loadCamera;
window.handleVbg = handleVbg;
window.toggleSendVideo = toggleSendVideo;
window.stopStartVideo = stopStartVideo;
window.loadMicrophone = loadMicrophone;
window.handleBNR = handleBNR;
window.toggleSendAudio = toggleSendAudio;
window.stopStartAudio = stopStartAudio;
window.changeLayout = changeLayout;
window.addToStage = addToStage;
window.getStats = getStats;
window.setSizeHint = setSizeHint;
window.collectMeetings = collectMeetings;
window.refreshCaptcha = refreshCaptcha;
window.verifyPassword = verifyPassword;
window.updateMultistreamUI = updateMultistreamUI;
window.joinMeeting = joinMeeting;
window.getUserMedia = getUserMedia;
window.addMedia = addMedia;
window.updateMedia = updateMedia;
window.answerMeeting = answerMeeting;
window.rejectMeeting = rejectMeeting;
window.claimPersonalMeetingRoom = claimPersonalMeetingRoom;
window.clearMediaDeviceList = clearMediaDeviceList;
window.getMediaDevices = getMediaDevices;
window.setAudioInputDevice = setAudioInputDevice;
window.setVideoInputDevice = setVideoInputDevice;
window.setAudioOutputDevice = setAudioOutputDevice;
window.toggleWebexDeviceSection = toggleWebexDeviceSection;
window.refreshDevicesList = refreshDevicesList;
window.searchForDevices = searchForDevices;
window.deviceRequestPair = deviceRequestPair;
window.deviceRequestUnpair = deviceRequestUnpair;
window.deviceGetAudioState = deviceGetAudioState;
window.setLocalMeetingQuality = setLocalMeetingQuality;
window.setRemoteMeetingQuality = setRemoteMeetingQuality;
window.toggleBreakout = toggleBreakout;
window.startScreenShare = startScreenShare;
window.publishScreenShare = publishScreenShare;
window.unpublishScreenShare = unpublishScreenShare;
window.stopScreenShare = stopScreenShare;
window.moveToDevice = moveToDevice;
window.moveFromDevice = moveFromDevice;
window.startRecording = startRecording;
window.pauseRecording = pauseRecording;
window.resumeRecording = resumeRecording;
window.stopRecording = stopRecording;
window.lockMeeting = lockMeeting;
window.unlockMeeting = unlockMeeting;
window.toggleTranscription = toggleTranscription;
window.setCaptionLanguage = setCaptionLanguage;
window.setSpokenLanguage = setSpokenLanguage;
window.sendDtmfTones = sendDtmfTones;
