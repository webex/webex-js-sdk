...
describe(`plugin-mashups`, () => {
  ...
  describe(`#create()`, () => {
    ...
    it(`create request sent`, () => {
      return spark.mashups.create({type: `TYPE`, roomId: `ROOM-ID`})
        .then(() => {
  ...
  describe(`#remove()`, () => {
    it(`throws an error when a type is not provided`, () => {
      return assert.isRejected(spark.mashups.remove({
        displayName: `mashupName`
      }), /options.type is required/);
    });

    it(`throws an error when a mashup id is not provided`, () => {
      return assert.isRejected(spark.mashups.remove({
        displayName: `mashupName`,
        type: `test`
    ...
    it(`remove request sent`, () => {
      return spark.mashups.remove({id: `ID`, type: `TYPE`})
        .then(() => {
          assert.calledOnce(spark.request);
          const req = spark.request.args[0][0];
  ...
  describe(`#get()`, () => {
    ...
    it(`get request sent`, () => {
      return spark.mashups.get({id: `ID`})
        .then(() => {
          assert.calledOnce(spark.request);
          const req = spark.request.args[0][0];
  ...
  describe(`#list()`, () => {
    it(`list request sent`, () => {
      return spark.mashups.list()
        .then(() => {
          assert.calledOnce(spark.request);
          const req = spark.request.args[0][0];
