...
- **Agent Wellness Break**: Effective enablement, agent/org-scoped RTD offers, active-session action reporting, system idle-code lookup, and legacy state transitions.
...
- **UC-6 Wellness lifecycle:** consume an agent/org-scoped offer regardless of its notification session metadata, report a user action using the active local session, resolve the system idle code, and demonstrate legacy Idle/Available transitions. Evidence: `src/cc.ts`, `src/services/ApiAiAssistant.ts`, `src/services/config/index.ts`, `test/unit/spec/cc.ts`, `docs/samples/contact-center/`.
...
| CONTACT_CENTER-R-007 | `test/unit/spec/cc.ts`, `test/unit/spec/services/ApiAiAssistant.ts`, `test/unit/spec/services/core/websocket/WebSocketManager.ts` | Wellness normalization on primary and RTD sockets, notification-session mismatch acceptance, invalid agent/org/generation filtering, active-session outbound validation, RTD status, stale-socket isolation, reconnect cleanup, and unchanged non-wellness forwarding. |
| CONTACT_CENTER-R-008 | `test/unit/spec/wellnessSampleUtils.ts`, sample build | Safe-work classification, legacy external/RONA decisions, minimal refresh marker, recovery decisions, and standalone browser constant exposure. |
