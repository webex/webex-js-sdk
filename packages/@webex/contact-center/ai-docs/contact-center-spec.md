...
- **Agent Wellness Break**: Effective enablement, agent/org-scoped RTD offers, active-session action reporting, system idle-code lookup, and legacy state transitions, with State Control V2 compatibility retained internally for first-party consumers.
...
| `contact-center.agent-wellness-break` | SDK API / events | `Profile.isWellnessBreakEnabled`, `requestWellnessBreak`, `respondToWellnessBreak`, `getWellbeingBreakIdleCode`, and the wellness/RTD event contracts. | Let hosts implement the public wellness-break lifecycle without reproducing backend payloads, routing rules, or session validation. | Additive public API; the effective gate is false unless all supported configuration inputs are affirmative. State Control V2 methods, types, and events are internal and excluded from declarations. | `src/cc.ts`, `src/types.ts`, `src/services/config/types.ts` | `CONTRACTS.md` |
...
| CONTACT_CENTER-R-006 | Expose Agent Wellness Break methods, action-value objects, and types only through the package façade. Derive the public action unions from those `as const` value objects. Action calls accept no identity or context-mutating API; ApiAIAssistant reads effective enablement and organization/agent/session identity from a live internal ContactCenter-owned provider and rejects when that context is incomplete. Logout, deregistration, a current-session multi-login close, and relogin update the provider's source state. | Wellness actions are session-scoped and must not leak across agents, relogins, multi-login closures, or registrations; one public value/type source prevents wire-value drift. | `src/cc.ts`, `src/types.ts`, `src/services/ApiAiAssistant.ts`, `src/index.ts` | `test/unit/spec/cc.ts`, `test/unit/spec/services/ApiAiAssistant.ts`, `docs/samples/contact-center/` | The backend-delivered `agentWellbeing.enable` value is treated as the server rollout decision because no distinct rollout field exists in the supported SDK response contracts. | PRESENT |
| CONTACT_CENTER-R-007 | When wellness is effectively enabled, connect the RTD socket without blocking registration and route valid `Wellness_Break_Handler` messages from either the primary data-notification socket or RTD socket to `CC_AGENT_EVENTS.WELLNESS_BREAK`. Notification eligibility is scoped to the current agent and organization and deliberately does not compare the notification-provided `agentSessionId` with the active session; outbound wellness APIs remain active-session validated. Report RTD status transitions, ignore malformed/foreign-identity/old-generation messages, reconnect after an unexpected RTD close, and invalidate handlers on reconnect/deregister. Registration or relogin failure must safely cancel an in-flight RTD subscription even before a browser socket exists, and a socket close before Welcome must reject the pending initialization so a later generation can replace it. Non-wellness RTD messages continue to TaskManager unchanged. Internal State Control V2 relogin and channel-state notifications bypass the generic nested-event pass-through and are normalized exactly once for first-party compatibility. | A long-lived host needs parity with Desktop notification routing without dropping legitimate nudges whose backend session metadata differs, while actions must still be sent only for the active local session and failed initialization must not leave an orphaned RTD connection. | `src/cc.ts`, `src/constants.ts`, `src/services/config/types.ts`, `src/services/ApiAiAssistant.ts`, `src/services/core/websocket/WebSocketManager.ts` | `test/unit/spec/cc.ts`, `test/unit/spec/services/ApiAiAssistant.ts`, `test/unit/spec/services/core/websocket/WebSocketManager.ts`, `docs/samples/contact-center/` | Notification delivery remains best-effort; the backend owns route selection, offer generation, and ordering. State Control V2 is not an application-facing SDK contract. | PRESENT |
| CONTACT_CENTER-R-008 | The standalone SDK sample must keep `WellbeingBreak` out of the ordinary idle-code selector, wait for legacy state confirmation and all tasks to become safe, restore the legacy pre-break state with bounded recovery, synchronize authoritative external state changes, use a session-scoped refresh marker without replaying an offer, action, or break, and revoke manual-request eligibility on `WELLNESS_BREAK_NOT_ALLOWED`. It must consume the exported wellness action constants and must not demonstrate State Control V2 methods, types, or events. | The sample is the executable public-contract reference and must track backend eligibility, avoid duplicated wire literals, avoid exposing the separate State Control V2 feature, and avoid state stranding. | `docs/samples/contact-center/app.js`, `docs/samples/contact-center/wellness-utils.js`, `docs/samples/contact-center/index.html` | `test/unit/spec/wellnessSampleUtils.ts`, sample build | Live QA still supplies authoritative backend/session integration evidence. | PRESENT |
...
After station login or relogin, ContactCenter retains the current agent-session id. ApiAIAssistant reads the current effective gate and organization/agent/session identity through a live internal ContactCenter-owned provider; it exposes no context-mutating method. Logout, deregistration, and a current-session multi-login close invalidate the provider's source state. `requestWellnessBreak()` and `respondToWellnessBreak()` do not accept identity fields; they POST a direct custom event with a fresh numeric timestamp and reject disabled or incomplete context before transport. Public action types are derived from exported `as const` action-value objects so applications and samples can use the same wire values without an enum. `getWellbeingBreakIdleCode()` resolves the exact active system code named `WellbeingBreak` and caches it for the registration lifetime. The implementation retains a separate State Control V2 route for first-party widget compatibility, but its method, inputs, results, and events are marked internal and stripped from published declarations.

Wellness payloads are handled by ContactCenter on both the primary data-notification socket and the RTD socket before ordinary routing. Only known actions with matching agent and organization are emitted; the notification session is retained as diagnostic metadata, and RTD payloads additionally require the active connection generation. The canonical optional `actionText` field is normalized to public `actionText`. Each RTD reconnect receives a new generation so late callbacks from an older socket are ignored. Registration/relogin failure and deregistration can safely cancel RTD while its subscribe request is still pending; a close before Welcome rejects that initialization, preventing a stale promise or late socket from surviving into a newer generation. Deregistration also removes the message/close handlers, cancels reconnect, and clears the session/code cache. Raw State Control V2 relogin and channel-state notifications are excluded from the generic nested-event pass-through so the internal first-party bridge receives exactly one normalized notification.

The standalone SDK browser sample keeps `WellbeingBreak` out of the ordinary status selector. It creates a session-scoped recovery marker before changing legacy state, confirms the transition, sends `ACCEPTED` only after state success, and waits for every task to leave incoming, active, consult, conference, campaign, and wrap-up work before starting its 60-second demonstration timer. Restoration targets `Available / 0`, retries bounded failures, and retains reconnect recovery ownership after exhaustion. Authoritative external Available changes end the local lifecycle, while refresh recovery restores a matching current session without replaying the offer, response action, or break. The sample intentionally contains no State Control V2 API, type, or event usage.
...
- **UC-6 Wellness lifecycle:** consume an agent/org-scoped offer regardless of its notification session metadata, report a user action using the active local session, resolve the system idle code, and demonstrate legacy Idle/Available transitions without exposing State Control V2. First-party widgets may use the retained internal V2 bridge. Evidence: `src/cc.ts`, `src/services/ApiAiAssistant.ts`, `src/services/config/index.ts`, `src/services/agent/index.ts`, `test/unit/spec/cc.ts`, `docs/samples/contact-center/`.
...
- Wellness actions take no public identity or context-mutating input and require the effective profile flag plus non-empty SDK-owned organization, registered-agent, and current-session context read from the internal live provider; all checks fail closed after current-session logout, multi-login close, or deregistration.
- Wellness action unions derive from exported `as const` value objects; backend message-type and enablement sentinels use internal constants rather than duplicated literals.
- Malformed, unknown-action, stale-agent, stale-org, and old-generation wellness RTD messages are ignored and measured without logging the full payload. A different notification `agentSessionId` is retained as diagnostic metadata and does not make an otherwise valid current-agent/current-org wellness event stale.
- RTD failure must not reject an otherwise successful Contact Center registration; a failed registration/relogin must still cancel any in-flight RTD initialization.
...
| CONTACT_CENTER-R-006 | `test/unit/spec/cc.ts`, `test/unit/spec/services/ApiAiAssistant.ts` | Effective-gate and internal organization/agent/session validation, current-session multi-login invalidation, identity-free public input, exact request body, action validation, and direct-HTTP failure paths. |
| CONTACT_CENTER-R-007 | `test/unit/spec/cc.ts`, `test/unit/spec/services/ApiAiAssistant.ts` | Wellness normalization on primary and RTD sockets, notification-session mismatch acceptance, invalid agent/org/generation filtering, active-session outbound validation, exactly-once internal State Control V2 normalization, RTD status, reconnect cleanup, and unchanged non-wellness forwarding. |
| CONTACT_CENTER-R-008 | `test/unit/spec/wellnessSampleUtils.ts`, sample build | Safe-work classification, legacy external/RONA decisions, minimal refresh marker, recovery decisions, and absence of State Control V2 usage from the public sample. |
