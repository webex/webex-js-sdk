...
| `contact-center.consult-transfer-lists` | Contact Center/Services | Existing `getQueues`, `getEntryPoints`, `ContactServiceQueueSearchParams`, `EntryPointSearchParams`, `ContactServiceQueuesResponse`, `EntryPointListResponse`, `ContactServiceQueue`, and `EntryPointRecord` | Consult/transfer telephony eligibility, profile views, and `name,ASC` are defaults on the existing list methods; existing filter/sort/profile parameters override defaults and full records are returned in backend order | behavioral default correction on existing public methods; no specialized methods, projected response types, or record-type additions | `ai-docs/features/consult-transfer-list-policy/spec/feature-spec.md` | `src/cc.ts`, `src/services/Queue.ts`, `src/services/EntryPoint.ts`, `src/types.ts` |
