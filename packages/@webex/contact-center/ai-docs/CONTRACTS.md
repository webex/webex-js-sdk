...
| `contact-center.consult-transfer-lists` | Contact Center/Services | `getConsultTransferQueues`, `getConsultTransferEntryPoints`, `ConsultTransferListOptions`, `ConsultTransferMediaType`, `ConsultTransferDestination`, `ConsultTransferListResponse` | Minimal page/page-size/search/media input and paginated `{id, name, dbId?}` destinations; the SDK owns filters, projections, views, ordering, and cache policy | additive semver public; unsupported runtime media rejects before request construction | `ai-docs/features/agent-desktop-consult-transfer-list-policy/spec/feature-spec.md` | `src/cc.ts`, `src/types.ts`, `src/index.ts` |
