...
| `contact-center.consult-transfer-lists` | Contact Center/Services | Existing `getQueues`, `getEntryPoints`, `ContactServiceQueueSearchParams`, `EntryPointSearchParams`, `ContactServiceQueuesResponse`, `EntryPointListResponse`, `ContactServiceQueue`, and `EntryPointRecord` | Agent Desktop telephony eligibility, profile views, and `name,ASC` are defaults on the existing list methods; existing filter/sort/profile parameters override defaults, full records are returned in backend order, and both entity types declare optional `dbId` | behavioral default correction on existing public methods plus additive optional record fields; no specialized methods or projected response types | `ai-docs/features/agent-desktop-consult-transfer-list-policy/spec/feature-spec.md` | `src/cc.ts`, `src/services/Queue.ts`, `src/services/EntryPoint.ts`, `src/types.ts` |
