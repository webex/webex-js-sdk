...
export default class ContactCenter extends WebexPlugin implements IContactCenter {
   ...
   *
   * When an agent decides not to call the previewed contact, they can skip it.
   * The system will offer the next contact from the campaign if available.
   *
   * @param {PreviewContactPayload} payload - The preview contact payload containing interactionId and campaignId (campaign name, not UUID).
   * @returns {Promise<TaskResponse>} Promise resolving when skip is successful.
   * @throws {Error} If the operation fails.
   *
   * @example
   * ```typescript
   * // campaignId is the campaign name (e.g. "MyCampaign"), not a UUID
   * await webex.cc.skipPreviewContact({ interactionId, campaignId });
   * ```
   */
   ...
   *
   * When an agent decides the previewed contact should not be called again,
   * they can remove it from the campaign. The system will offer the next
   * contact from the campaign if available.
   *
   * @param {PreviewContactPayload} payload - The preview contact payload containing interactionId and campaignId (campaign name, not UUID).
   * @returns {Promise<TaskResponse>} Promise resolving when removal is successful.
   * @throws {Error} If the operation fails.
   *
   * @example
   * ```typescript
   * // campaignId is the campaign name (e.g. "MyCampaign"), not a UUID
   * await webex.cc.removePreviewContact({ interactionId, campaignId });
   * ```
   */
