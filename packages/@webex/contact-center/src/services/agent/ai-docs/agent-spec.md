...
State Control V2 is a separate feature retained as an internal first-party compatibility path. `createInternalRoutingAgent`, `stateChangeV2`, `setAgentChannelState`, the channel-state payload types, and the agent-channel event constants are marked `@internal` and stripped from published declarations.
...
| AGENT-R-006 | BROWSER station login must send `dialNumber` and `deviceId` as `webrtc-{agentId}` on `POST /v1/agents/login`. EXTENSION and AGENT_DN continue to send the caller-supplied dial number for both fields. | QRM stores `agent.dn` from `dialNumber`. Legacy VPOP RTMS only treats a DN as WebRTC when it starts with `webrtc-`; a bare UUID gets a steering prefix and the agent invite fails with SIP 484. | `src/cc.ts` | `test/unit/spec/cc.ts` | Public `stationLogin({loginOption: 'BROWSER'})` input is unchanged; only the backend login payload is prefixed. | PRESENT |
| AGENT-R-007 | The internal State Control V2 path must use PUT `/v2/agents/session/state` with `channelType` as a non-empty string array and nested `AgentChannelStateChanged` success/failure binds. Recovery reload must correlate the State Control V2 response using its outer `AgentRequestEvent` envelope and nested `AgentChannelReloginSuccess` type. ContactCenter validates Idle/Available input and normalizes each successful channel-state or relogin notification exactly once for first-party consumers. None of these methods, types, or events may appear in the public declaration surface. | Channel-specific first-party compatibility requires the v2 wire contract and correct envelope correlation, while State Control V2 remains independently releasable from Wellness Break. | `src/services/agent/index.ts`, `src/services/agent/types.ts`, `src/cc.ts`, `src/types.ts`, `src/services/config/types.ts` | `test/unit/spec/services/agent/index.ts`, `test/unit/spec/cc.ts`, declaration-surface check | The internal first-party widget integration is version-aligned with this SDK implementation. | PRESENT |

## Design Overview
`routingAgent` is the public pure AQM factory. It receives an initialized AqmReqs instance and returns five request functions: `reload`, `logout`, `stationLogin`, `stateChange`, and `buddyAgents`. Each request config declares the endpoint/payload plus exact notification binds. The internal `createInternalRoutingAgent` factory also returns `stateChangeV2` for the first-party ContactCenter implementation.

The internal `stateChangeV2` route targets agent channel sessions. ContactCenter trims and validates the internal channel list, requires an auxiliary code for Idle, omits it for Available, adds the registered agent id, and returns normalized nested channel-state details. Reload accepts the legacy `AgentReloginSuccess` envelope and the State Control V2 outer `AgentRequestEvent` envelope with nested `AgentChannelReloginSuccess`; ContactCenter refreshes the session id and restores disconnected State Control V2 channels to Available when the backend reports `agent-wss-disconnect`. State Control V2 relogin and state notifications bypass the generic nested-event re-emission path and are delivered once through the internal bridge.
...
- **UC-6 Internal State Control V2 compatibility:** first-party code changes selected channel sessions through v2 and refreshes the current session/profile from channel relogin notifications without publishing that surface to SDK consumers. Evidence: `src/services/agent/index.ts`, `src/cc.ts`, `test/unit/spec/services/agent/index.ts`, `test/unit/spec/cc.ts`.
...
The `AgentState` type (`'Available' | 'Idle' | 'RONA' | string`) is extensible -- the `string` union member allows backend-defined states beyond the known values listed below. The internal State Control V2 path accepts only `Available` or `Idle`, requires at least one channel, and requires `auxCodeId` for Idle.
...
- Public `routingAgent` has exactly five request functions and no `deviceUpdate`; the internal factory additionally exposes `stateChangeV2`. Profile/device changes belong to ContactCenter.
- Internal State Control V2 uses a distinct payload and nested `AgentChannelStateChanged` completion contract; it must not be flattened into legacy state change or exposed through the public factory.
...
| `AGENT-R-006` | `test/unit/spec/cc.ts` | None. |
| `AGENT-R-007` | `test/unit/spec/services/agent/index.ts`, `test/unit/spec/cc.ts` | None. |
