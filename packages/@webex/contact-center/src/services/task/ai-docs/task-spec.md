...
**wxApp consumer contract (WXCC-6026):** Hosts enable `enableAnswerOnWebex` / `setManageWebexCallingInWxcc`, bind UI to `task.uiControls`, and call **`task.accept()`**, **`task.decline()`**, **`task.toggleMute({ muted? })`**, **`task.transmitDtmf({ dtmf })`**. SDK routes wxApp telephony internally on `Voice`.
