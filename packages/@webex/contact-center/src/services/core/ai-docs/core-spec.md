...
| CORE-R-002 | `initWebSocket` requires `{body: SubscribeRequest, resource: string}` and resolves only after WebSocket Welcome. It rejects on register/connect failure or socket closure before Welcome. `close()` is safe before a browser socket exists and cancels the active initialization generation so a late register response cannot connect. Every socket callback must match both the active socket identity and initialization generation, so a delayed close from a replaced socket cannot reject the replacement initialization. | Subscription resource selection and an explicit readiness/ownership boundary are required for a valid realtime session without orphaned sockets or permanently pending initialization promises. | `src/services/core/websocket/WebSocketManager.ts` | `test/unit/spec/services/core/websocket/WebSocketManager.ts` | None. | PRESENT |
...
2. `WebSocketManager` registers a subscription using both `body` and `resource`, connects the socket, owns the keepalive worker, emits raw messages/socket lifecycle events, and uses socket identity plus initialization generation to isolate replaced connections.
...
- `WebSocketManager.close()` must invalidate an initialization when subscription registration is still pending or no browser socket exists; any pending Welcome promise must reject exactly once. Once a socket exists, callbacks may mutate lifecycle state only while both that socket and its captured generation remain current.
...
`WebSocketManager.initWebSocket` accepts `{body: SubscribeRequest, resource: string}`. Subscription uses the host Webex request API. Initialization generations prevent a late subscription response from creating a socket after cancellation, closure before Welcome rejects the pending initialization, and socket-identity checks discard delayed callbacks from a replaced connection. AqmReqs operational HTTP uses the WebexRequest wrapper. AQM request configs carry `host`, `url`, optional `method`/`data`, `notifSuccess.bind`, optional `notifFail.bind`, optional cancel bind, and optional timeout. HTTP acknowledgement never substitutes for the matching WebSocket operation result.
