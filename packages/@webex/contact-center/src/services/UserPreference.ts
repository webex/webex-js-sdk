...
export class UserPreference {
   ...
   * @returns {Promise<UserPreferenceResponse>} Promise resolving to created user preferences
   * @throws {Error} If the API call fails
   * @public
   * @example
   * ```typescript
   * const newPreferences = await userPreferenceAPI.createUserPreference({
   *   userId: 'user123',
   *   desktopPreference: '{"theme": "dark", "notifications": true}'
   * });
   * ```
   */
   ...
   * @returns {Promise<UserPreferenceResponse>} Promise resolving to updated user preferences
   * @throws {Error} If the API call fails
   * @public
   * @example
   * ```typescript
