...
| SERVICES-R-008 | ApiAIAssistant wellness requests must accept no public identity input, validate effective enablement and non-empty SDK-owned organization/agent/session context, then POST the exact `WellnessBreakAction` custom-event body directly to `/event`; only HTTP 202 resolves successfully. | SDK-owned organization and session context prevents callers from supplying stale or different login identities, while direct HTTP matches the AI event endpoint rather than AQM completion semantics. | `src/services/ApiAiAssistant.ts`, `src/types.ts` | `test/unit/spec/services/ApiAiAssistant.ts` | None. | PRESENT |
