...
describe('plugin-webhooks', function () {
  ...
  describe('#webhooks', () => {
    ...
    beforeEach(() => webex.webhooks.create({
      resource: 'messages',
      event: 'created',
    ...
    after(() => Promise.all(webhooks.map((webhook) => webex.webhooks.remove(webhook)
      .catch((reason) => {
        console.warn('failed to remove webhook', reason);
      }))));

    after(() => webex.rooms.remove(room)
      .catch((reason) => {
        console.warn('failed to remove room', reason);
      }));

    describe('#create()', () => {
      it('creates a webhook that listens for new messages', () => webex.webhooks.create({
        resource: 'messages',
        event: 'created',
        filter: `roomId=${room.id}`,
        targetUrl: 'https://example.com',
    ...
    describe('#get()', () => {
      it('retrieves a specific webhook', () => webex.webhooks.get(webhook.id)
        .then((w) => {
    ...
    describe('#list()', () => {
      it('retrieves all the webhooks to which I have access', () => webex.webhooks.list()
        .then((webhooks) => {
      ...
      it('retrieves a bounded set of webhooks', () => {
        ...
        return webex.webhooks.list({max: 1})
          .then((webhooks) => {
    ...
    describe('#remove()', () => {
      it('removes the specified webhook', () => webex.webhooks.remove(webhook)
        .then((body) => {
          assert.notOk(body);
