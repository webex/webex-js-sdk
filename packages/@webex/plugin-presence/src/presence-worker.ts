...
export default class PresenceWorker {
  private presences: IGenericKeyValue; // current presence objects; updated time
  private watchers: IGenericKeyValue; // counter of visible presence-required objects
  private fetchers: IGenericKeyValue; // waiting to get presence object
  private flights: IGenericKeyValue; // in flight to get presence object
  private campers: IGenericKeyValue; // waiting to subscribe; updated time
  private subscribers: IGenericKeyValue; // current subscriptions; expiration time
  private webex: IWebex;
  ...
  groundskeeper(): void {
    ...
    if (ids.length) {
      this.webex.presence.subscribe(ids).then((body) => {
        const now = new Date().getTime();
