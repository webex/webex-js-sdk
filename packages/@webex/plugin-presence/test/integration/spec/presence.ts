...
describe.skip('plugin-presence', function () {
  ...
  describe('Presence', () => {
    ...
    describe('#enable()', () => {
      it('enables presence', () =>
        spock.webex.presence
          .enable()
          .then((isEnabled) => {
            assert.equal(isEnabled, true);
          })
          .then(() => spock.webex.presence.isEnabled())
          .then((isEnabled) => {
            assert.equal(isEnabled, true);
          }));
    });
    ...
    describe('#get()', () => {
      it("gets a person's status by id", () =>
        spock.webex.presence.get(mccoy.id).then((presenceResponse) => {
    ...
    describe('#list()', () => {
      it('returns an object with the status of all requested people', () =>
        spock.webex.presence.list([mccoy.id, spock.id]).then((presenceResponse) => {
      ...
      describe('batching of presence requests', () => {
        ...
        it('executes network requests for max limit', () => {
          spock.webex.presence.config.batcherMaxCalls = 2;
          sinon.spy(spock.webex.presence.batcher, 'submitHttpRequest');

          return spock.webex.presence
            .list(batchTestUsers.map((user) => user.id))
            .then((presenceResponse) => {
              assert.equal(presenceResponse.statusList.length, 6);
              assert.calledThrice(spock.webex.presence.batcher.submitHttpRequest);
              spock.webex.presence.batcher.submitHttpRequest.restore();
            });
        });
      });
    });
    ...
    describe('#subscribe', () => {
      afterEach(
        () => spock && spock.webex && spock.webex.presence.unsubscribe([mccoy.id, spock.id])
      );

      it('subscribes to a person id and returns subscription status', () =>
        spock.webex.presence.subscribe(mccoy.id).then((presenceResponse) => {
      ...
      it('subscribes and returns subscription status of a list of people ids', () =>
        spock.webex.presence.subscribe([mccoy.id, spock.id]).then((presenceResponse) => {
          assert.property(presenceResponse, 'responses');
        ...
        spock.webex.presence
          ...
          .then(() =>
            Promise.all([
              ...
              mccoy.webex.presence.setStatus('active', 1500),
            ])
          )
          .then(([event]) =>
            assert.equal(
          ...
          .then(() =>
            Promise.all([
              ...
              mccoy.webex.presence.setStatus('dnd', 1500),
            ])
          )
          .then(([event]) =>
    ...
    describe('#unsubscribe', () => {
      it('unsubscribes to presence updates of a single person id', () =>
        spock.webex.presence.unsubscribe(mccoy.id).then((res) => assert.statusCode(res, 200)));
