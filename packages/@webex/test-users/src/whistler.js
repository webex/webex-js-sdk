...
export default function createTestUser(options = {}) {
  console.log('### TEST - using whistler');
  ...
  const machineAccount = options.machineAccount || process.env.WHISTLER_MACHINE_ACCOUNT || randomName();
  const machinePassword = options.machinePassword || process.env.WHISTLER_MACHINE_PASSWORD || `${uuid.v4()}zAY1*`;
