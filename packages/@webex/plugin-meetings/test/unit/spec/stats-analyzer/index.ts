...
const createMockNetworkQualityMonitor = () => ({
  config: {},
  networkQualityScore: 1,
  networkQualityStatus: {},
  mediaType: null,
  determineUplinkNetworkQuality: jest.fn(),
});
...
describe('plugin-meetings', () => {
  describe('StatsAnalyzer', () => {
    describe('parseStatsResult', () => {
        ...
        beforeEach(() => {
          const networkQualityMonitor = createMockNetworkQualityMonitor();
    ...
    describe('startAnalyzer', () => {
      let pc: {
        ...
        getTransceiverStats: { mockResolvedValue: (stats: Record<string, { senders: unknown[]; receivers: unknown[]; }>) => void };
      };

      let statsAnalyzer: StatsAnalyzer;
      let mqeData: Record<string, any>
      let loggerSpy: jest.SpyInstance;
      let receiveSlot: Partial<ReceiveSlot>;

      let receivedEventsData: Record<string, unknown> = {
        local: {},
        remote: {},
      };
      ...
      beforeEach(() => {
        ...
        statsAnalyzer = new StatsAnalyzer(      {
          config: initialConfig,
          receiveSlotCallback: () => receiveSlot as ReceiveSlot,
          networkQualityMonitor,
        });
      ...
      const mergeProperties = (
        ...
        for (const key in target) {
          if (typeof target[key] === 'object' && target[key] !== null) {
            mergeProperties(target[key] as Record<string, unknown>, properties, keyValue, matchKey, matchValue);
          }
      ...
      describe('FEC packet reporting in stats analyzer', () => {
        ...
        it('should correctly adjust the FEC packet count when packets are discarded', async () => {
          ...
          expect(mqeData.audioReceive[0].common.fecPackets).toBe(15);
        });
      });
      ...
      describe('remote loss rate reporting in stats analyzer', () => {
        ...
        it('should report a zero remote loss rate for both audio and video at the start', async () => {
          expect(mqeData.audioTransmit[0].common.maxRemoteLossRate).toBe(0);
          expect(mqeData.videoTransmit[0].common.maxRemoteLossRate).toBe(0);
        });
        ...
        it('should accurately calculate the remote loss rate for both audio and video after packet loss is detected', async () => {
          ...
          expect(mqeData.audioTransmit[0].common.maxRemoteLossRate).toBe(5);
          expect(mqeData.videoTransmit[0].common.maxRemoteLossRate).toBe(5);
        });
      });
