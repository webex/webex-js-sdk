...
describe('RemoteMediaManager', () => {
  ...
  describe('setLayout', () => {
    ...
    it('switches from AllEqual to AllEqual25 and allocates additional slots', async () => {
      const config = cloneDeep(DefaultTestConfiguration);
      config.video.layouts['AllEqual25'] = {
        activeSpeakerVideoPaneGroups: [
          {id: 'main', numPanes: 25, size: 'best', priority: 255},
      ...
      assert.strictEqual(remoteMediaManager.getLayoutId(), 'AllEqual');
      assert.strictEqual(remoteMediaManager.slots.video.activeSpeaker.length, 9);

      await remoteMediaManager.setLayout('AllEqual25');

      assert.strictEqual(remoteMediaManager.getLayoutId(), 'AllEqual25');
      assert.strictEqual(remoteMediaManager.slots.video.activeSpeaker.length, 25);
    });

    it('switches from AllEqual25 to AllEqual and releases extra slots', async () => {
      const config = cloneDeep(DefaultTestConfiguration);
      config.video.layouts['AllEqual25'] = {
        activeSpeakerVideoPaneGroups: [
          {id: 'main', numPanes: 25, size: 'best', priority: 255},
      ...
      assert.strictEqual(remoteMediaManager.getLayoutId(), 'AllEqual');
      assert.strictEqual(remoteMediaManager.slots.video.activeSpeaker.length, 9);
      assert.called(fakeReceiveSlotManager.releaseSlot);
    });
