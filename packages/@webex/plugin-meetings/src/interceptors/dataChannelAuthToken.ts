...
export default class DataChannelAuthTokenInterceptor extends Interceptor {
  ...
  onResponseError(options, reason) {
    ...
    //   (key) => key.toLowerCase() === DATA_CHANNEL_AUTH_HEADER.toLowerCase()
    // );
    //
    // if (!hasHeader) {
    ...
    const headerToken = this.getHeader(options.headers, DATA_CHANNEL_AUTH_HEADER.toLowerCase());

    if (headerToken && (reason.statusCode === 401 || reason.statusCode === 403)) {
      return Promise.reject(reason);
    }
