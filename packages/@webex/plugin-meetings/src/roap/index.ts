...
export default class Roap extends StatelessWebexPlugin {
   ...
   * @memberof Roap
   */
  private sendRoapOK(options: SeqOptions) {
   ...
   * @memberof Roap
   */
  private sendRoapAnswer(options: SeqOptions) {
    const meeting = this.webex.meetings.meetingCollection.getByKey('correlationId', options.correlationId);
    const roapMessage = {
   ...
   * @memberof Roap
   */
  private sendRoapError(session: object, locus: object, errorType: string) {
    const msg = {
   ...
   * @memberof Roap
   */
  sendRoapCallRequest = (options: RoapOptions) => {
