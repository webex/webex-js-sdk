...
export default class RoapHandler extends StatelessWebexPlugin {
   ...
   * @returns {null}
   */
  perform(session: object, meeting: Meeting, action: object) {
   ...
   * @returns {null}
   */
  execute(signal: string, session: object, action: object, meeting: Meeting, prefix: string) {
   ...
   * @returns {Boolean}
   */
  handleAction(session: object, action: object, meeting: Meeting, correlationId: string) {
    let signal;
