...
export class StatsAnalyzer extends EventsScope {
   ...
   */
  constructor(config: object, networkQualityMonitor: object = {}, statsResults: object = defaultStats) {
    super();
   ...
   * @returns {void}
   */
  private updatePeerconnection(peerConnection: PeerConnection) {
   ...
   * @returns {Promise}
   */
  public startAnalyzer(peerConnection: PeerConnection) {
   ...
   * @returns {void}
   */
  parseAudioSource(result: string, type: boolean) {
    if (!result) {
      return;
    }
