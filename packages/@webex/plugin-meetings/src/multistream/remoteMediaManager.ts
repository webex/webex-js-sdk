...
export class RemoteMediaManager extends EventsScope {
  ...
  public async setReceiveNamedMediaGroup(mediaType: MediaType, languageId: number) {
    ...
    await this.createAudioNamedMedia(false);
   ...
   */
  private async createAudioNamedMedia(commitRequest: boolean) {
    // create slot for interpretation language audio
    const siGroup = this.config.namedMediaGroup;
    if (siGroup?.type === 1 && siGroup?.value) {
      ...
      this.emit(
        {file: 'multistream/remoteMediaManager', function: 'createAudioNamedMedia'},
