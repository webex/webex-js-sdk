...
export default class Meeting extends StatelessWebexPlugin {
  ...
  closeRemoteTracks() {
     ...
     * @param {string} type Type of media that was stopped
     * @returns {void}
     * @inner
     */
    const triggerMediaStoppedEvent = (type) => {
      Trigger.trigger(
        this,
        {
          file: 'meeting/index',
  ...
  setMeetingQuality(level) {
    ...
    return (sendVideo ? this.setLocalVideoQuality(level) : Promise.resolve())
      .then(() => {
        if (receiveAudio || receiveVideo) return this.setRemoteQualityLevel(level);
  ...
  endMeetingForAll() {
    Metrics.postEvent({
      ...
      meeting: this,
      data: {
        trigger: trigger.USER_INTERACTION,
        canProceed: false,
      },
    });
