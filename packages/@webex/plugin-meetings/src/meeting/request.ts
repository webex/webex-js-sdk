...
export default class MeetingRequest extends StatelessWebexPlugin {
   ...
   */
  touchBreakout({breakoutUrl, locusUrl}: {breakoutUrl: string; locusUrl: string}) {
    // @ts-ignore
    return this.request({
      method: HTTP_VERBS.POST,
      uri: breakoutUrl,
      ...
      LoggerProxy.logger.error(`Meeting:request#touchBreakout --> Error provisioning error ${err}`);
      throw err;
    });
  }
