...
export default class Meeting extends StatelessWebexPlugin {
  ...
  constructor(attrs: any, options: object, callback: (meeting: Meeting) => void) {
    ...
    // Set up sync metrics callback so that HashTreeParser can report latency metrics
  ...
  async updateLLMConnection({isInitialJoinPhase = false}: {isInitialJoinPhase?: boolean} = {}) {
    ...
    return this.webex.internal.llm
      ...
      .then((registerAndConnectResult) => {
        ...
        // Emit join-phase LLM connect metric only during initial join.
        if (registerAndConnectResult) {
          if (isInitialJoinPhase) {
            this.sendLLMConnectMetric(registerAndConnectResult);
          }

          // If this reconnect happened after a breakout move/join, emit the
          // latency on the breakout join response event as well.
  ...
  private sendLLMConnectMetric(timings: RegisterAndConnectTiming, error?: any) {
    ...
    const payload: any = {
      llmLatency: {
        clientLLMDatachannelResponseTime: timings.clientLLMDatachannelResponseTime,
    ...
    console.info('[metrics] sending client.llm.connect.response', {
      meetingId: this.id,
      hasError: Boolean(error),
      llmLatency: payload.llmLatency,
  ...
  /**
   * Sends the client.locus.sync.complete diagnostic event with sync latency metrics.
   * @param {Object} metrics - The sync latency metrics from HashTreeParser
   * @param {string} metrics.dataSet - The dataset name that was synced
   * @param {SyncLatencyMetrics} metrics.syncLatency - The latency measurements
   * @returns {void}
   * @private
   */
  private sendSyncCompleteMetric(metrics: {dataSet: string; syncLatency: SyncLatencyMetrics}) {
    ...
    // eslint-disable-next-line no-console
    console.info('[metrics] sending client.locus.sync.complete', {
      meetingId: this.id,
      dataSet: metrics.dataSet,
      syncLatency: metrics.syncLatency,
      identifiers: payload.identifiers,
