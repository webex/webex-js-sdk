    ...
    describe('shutdown protocol support', () => {
      describe('#onmessage() with sequence number tolerance', () => {
        it('should process valid sequence numbers normally', () => {
          ...
          const messageWithValidSequence = {
            data: JSON.stringify({
              type: 'regular',
              sequenceNumber: 5,
          ...
          mockWebSocket.emit('message', messageWithValidSequence);

          assert.calledOnce(messageSpy);
          assert.notCalled(sequenceMismatchSpy);
          assert.equal(socket.expectedSequenceNumber, 6);
        });

        it('should detect sequence mismatches for valid sequence numbers', () => {
          const messageSpy = sinon.spy();
          const sequenceMismatchSpy = sinon.spy();
          ...
          const messageWithMismatchedSequence = {
            data: JSON.stringify({
              type: 'regular',
              sequenceNumber: 7, // Expected 5, got 7
              id: 'regular-message-id',
            }),
          };

          mockWebSocket.emit('message', messageWithMismatchedSequence);

          assert.calledOnce(messageSpy);
          assert.calledWith(sequenceMismatchSpy, 7, 5);
          assert.calledOnce(sequenceMismatchSpy);
          assert.equal(socket.expectedSequenceNumber, 8);
        });

        it('should handle string sequence numbers correctly', () => {
          const messageSpy = sinon.spy();
          const sequenceMismatchSpy = sinon.spy();
          ...
          const messageWithStringSequence = {
            data: JSON.stringify({
              type: 'regular',
              sequenceNumber: '5', // String representation
              id: 'regular-message-id',
            }),
          };

          mockWebSocket.emit('message', messageWithStringSequence);
        ...
        it('should handle invalid sequence number strings gracefully', () => {
          const messageSpy = sinon.spy();
          const sequenceMismatchSpy = sinon.spy();
          ...
          const messageWithInvalidSequence = {
            data: JSON.stringify({
              type: 'regular',
          ...
          mockWebSocket.emit('message', messageWithInvalidSequence);

          assert.calledOnce(messageSpy);
          // Should detect mismatch because parseInt('not-a-number') = NaN, which !== 5
          assert.calledOnce(sequenceMismatchSpy);
      ...
      describe('#onmessage() error handling', () => {
        it('should handle malformed JSON gracefully', () => {
          const loggerSpy = sinon.spy(socket.logger, 'warn');
          const messageSpy = sinon.spy();
          ...
          assert.calledWith(loggerSpy, sinon.match(/error while receiving WebSocket message/));
          assert.notCalled(messageSpy);

          loggerSpy.restore();
        });

        it('should continue processing after JSON parse error', () => {
          const messageSpy = sinon.spy();
