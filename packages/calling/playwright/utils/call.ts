...
export const waitForIncomingCall = async (page: Page): Promise<void> => {
  await expect(page.locator(CALLING_SELECTORS.ANSWER_BTN).first()).toBeEnabled({timeout: 30000});
};

export const answerCall = async (page: Page): Promise<void> => {
  await page.locator(CALLING_SELECTORS.ANSWER_BTN).first().click({timeout: AWAIT_TIMEOUT});
  await expect(page.locator(CALLING_SELECTORS.ANSWER_BTN).first()).toBeDisabled({
