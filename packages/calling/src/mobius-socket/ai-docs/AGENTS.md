...
| `makeRequest(request)` | `MobiusSocket#sendWssRequest` when WSS is enabled, otherwise `webex.request` | All Mobius REST traffic (register, deregister, call setup/state/media/status, supplementary services, keepalive 404 recovery, etc.). |
