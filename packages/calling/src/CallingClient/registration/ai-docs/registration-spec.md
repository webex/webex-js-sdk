...
| ID | WHAT | WHY | Source Evidence | Test / Example Evidence | Assumptions / Gaps | Confidence |
|---|---|---|---|---|---|---|
| REGISTRATION-R-011 | A keepalive `KEEPALIVE_FAILURE` that is final or has reached the retry-count threshold mutates registration state (`failoverImmediately`, status, keepalive/failback timers, `UNREGISTERED` emission, log upload) under the shared `this.mutex`, acquired exactly once and released before the non-final `reconnectOnFailure`/`404` follow-up runs, so the follow-up never nests a second `runExclusive` on top of the still-held one. | `async-mutex` is non-reentrant; wrapping the whole terminal branch (mutation plus follow-up) in one `runExclusive` risks deadlock if the follow-up itself needs the mutex (CAI-8594, R1), and leaving the mutation unserialized risks racing another registration flow's state changes. | `src/CallingClient/registration/register.ts` | `src/CallingClient/registration/register.test.ts` (`Keep-alive Tests` → `keepalive` → `keepalive terminal failure is serialized under the mutex`, `keepalive terminal failure does not deadlock`) | none identified | PRESENT |

**Provenance:** `AGENTS.md` rule 7 (typed error/event contracts, never silently swallow) and rule 8 (positive+negative tests, keep specs current in the same merge); Triage risk R1.
...
- **Terminal failure serialization:** when a `KEEPALIVE_FAILURE` is final (`abort`) or has reached the retry-count threshold, the state mutation (`failoverImmediately`, `setStatus(INACTIVE)`, `clearKeepaliveTimer()`, `clearFailbackTimer()`, `lineEmitter(UNREGISTERED)`, `uploadLogs()`) runs inside a single `this.mutex.runExclusive(...)` call so it cannot race another registration flow's state changes. The mutex is released before the non-final follow-up (`reconnectOnFailure`/`handle404KeepaliveFailure` for a `404`) runs, so that follow-up — which itself funnels back through the mutex-guarded registration paths — is never nested inside the still-held mutex (async-mutex is non-reentrant).
