...
| REGISTRATION-R-002 | A dedicated Web Worker schedules keepalives, prevents overlapping requests, reports success after recovery, and reports each failure with its retry count. An in-flight keepalive is guarded by a bounded timeout of `interval * 2` seconds: if no `KEEPALIVE_RESULT` arrives within that window, the worker clears its in-flight flag so the loop resumes forward progress instead of wedging permanently. The main thread performs the Mobius request and handles recovery or terminal failure. | Moving scheduling off the main thread protects heartbeat timing while retaining authenticated network access and lifecycle decisions on the SDK thread; the bounded in-flight timeout guarantees a lost or dropped result cannot permanently stall the heartbeat loop. | `src/CallingClient/registration/register.ts`; `src/CallingClient/registration/webWorkerStr.ts`; `src/CallingClient/registration/webWorker.ts` | `src/CallingClient/registration/register.test.ts`; `src/CallingClient/registration/webWorker.test.ts` | none identified | PRESENT |
...
- A **lost or dropped `KEEPALIVE_RESULT`** cannot wedge the loop: the in-flight flag set when a tick dispatches is automatically cleared after a bounded `interval * 2` second timeout, allowing the next tick to send again.
...
- **In-flight guard:** Each time a `SEND_KEEPALIVE` is dispatched the worker marks itself in-flight and arms a bounded timeout of `interval * 2` seconds; if no `KEEPALIVE_RESULT` arrives before it elapses, the worker clears the in-flight flag so the next tick can dispatch again. `START_KEEPALIVE`, `KEEPALIVE_RESULT`, and `CLEAR_KEEPALIVE` all cancel this pending timeout.
