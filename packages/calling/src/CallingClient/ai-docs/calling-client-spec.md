...
## Evidence Rules
Claims cite current source/tests; routed legacy docs supply detailed initialization, registration, recovery, transport, and cleanup context.

## Source Material Register
| Source doc | Scope | Decision | Disposition |
|---|---|---|---|
| `src/CallingClient/ai-docs/AGENTS.md` | API/config/examples | reconciled | Public Surface, requirements, use cases, dependencies |
| `src/CallingClient/ai-docs/ARCHITECTURE.md` | init/registration/recovery/transport/cleanup | reconciled | design, sequences, state, failures, pitfalls |

## Overview
CallingClient is the coordinated entry point for calling. It initializes SDK/device/backend context, creates and exposes Lines, routes session/network events, coordinates registration and call cleanup, supports HTTP/Mobius transport selection, and exposes diagnostic operations.

## Purpose / Responsibility
Own the top-level calling client lifecycle and coordination boundary; delegate call, registration, transport, metrics, and media details to their owning modules.

## Stack
TypeScript, Eventing, Webex SDK plugins, XState-backed child modules, Jest/Playwright.

## Folder / Package Structure
```text
CallingClient/{CallingClient.ts,types.ts,constants.ts,calling/,line/,registration/,utils/,*.test.ts,ai-docs/}
```

## Key Files (source of truth)
| File | Holds |
|---|---|
| `src/CallingClient/CallingClient.ts` | lifecycle/coordination/factory |
| `src/CallingClient/types.ts` | `ICallingClient`, config, client state types |
| `src/CallingClient/constants.ts` | timers/endpoints/feature and file constants |
| `src/Events/types.ts` | public events |

## Public Surface
| ID | Type | Surface | Purpose | Compatibility | Detail | Root index |
|---|---|---|---|---|---|---|
| calling.client.create | SDK | `createClient` → `ICallingClient` | initialize/use calling | semver public | `src/index.ts`, `types.ts` | `ai-docs/CONTRACTS.md` |
| calling.client.events | event | `CALLING_CLIENT_EVENT_KEYS` | lifecycle/network/call notifications | semver public | `src/Events/types.ts` | `ai-docs/CONTRACTS.md` |

## Requires (dependencies)
Initialized Webex SDK/device/features, SDKConnector, Line/Registration/Calling, Mobius socket or HTTP request path, Logger, Metrics, media helpers, browser network events.

## Requirements
| ID | WHAT | WHY | Source Evidence | Test Evidence | Gaps | Confidence |
|---|---|---|---|---|---|---|
| CC-R-001 | Initialize SDK context/config/backend and expose ready calling lines. | Consumers need one supported setup path. | `src/CallingClient/CallingClient.ts`, `src/CallingClient/types.ts` | `src/CallingClient/CallingClient.test.ts` | none | PRESENT |
| CC-R-002 | Coordinate registration, network disruption/restoration, and cleanup without orphaning calls/listeners. | Calling must recover or terminate deterministically. | `src/CallingClient/CallingClient.ts`, `src/CallingClient/constants.ts` | unit and Playwright registration/network tests | none | PRESENT |
| CC-R-003 | Select configured HTTP or Mobius WSS request/event paths while preserving caller-visible contracts. | Transport rollout must not change public semantics. | `src/CallingClient/utils/`, `src/mobius-socket/` | utility/client tests | none | PRESENT |

## Design Overview
CallingClient composes smaller owners rather than implementing call control itself. Initialization wires SDKConnector, backend/device/config, metrics, listeners, and Line instances. Network recovery coordinates line registration and active calls. The Mobius feature path separates request mapping/socket events from higher-level call objects.

## Data Flow
```mermaid
flowchart LR
 Consumer --> CallingClient --> Line --> Registration
 Line --> Calling[CallManager/Call]
 CallingClient --> SDK[Webex SDK plugins]
 Registration --> Mobius
 Calling --> Mobius
 Calling --> Media
 CallingClient --> Metrics
```

## Sequence Diagram(s)
| Operation group | Diagram | Failure/recovery coverage |
|---|---|---|
| Initialize | Client initialization | device/config/backend failure |
| Register/recover | Line registration/network recovery | retry/failover/deregister |
| Cleanup | Deregister/destroy | late events and active calls |
```mermaid
sequenceDiagram
 ...
 C->>CC: createClient + init
 CC->>CC: configure SDK/backend/listeners
 CC->>L: create line
 C->>L: register()
 L->>R: trigger registration
 alt success
  R-->>L: registered
  L-->>C: line event
 else failure/network flap
  R->>R: retry/failover or final error
  R-->>C: typed event/error
 end
```
...
```mermaid
classDiagram
 Eventing <|-- CallingClient
 ICallingClient <|.. CallingClient
 CallingClient --> Line
 Line --> Registration
 Line --> CallManager
...
## Use Cases
- Initialize client and line, then register.
- Make/receive calls through Line and typed events.
- Recover from network disruption and rehome registration.
- Upload diagnostics and query active calls/devices. Evidence: `src/CallingClient/CallingClient.test.ts`, `playwright/`.

## State Model
Client state includes configuration/device/backend, Lines, listeners, connection state, and recovery timers; child modules own registration/call/socket state.

## Business Rules & Invariants
- A Line belongs to the initialized CallingClient context.
- Network recovery must not duplicate listeners, registrations, or calls.
- Consumer-facing transport behavior remains stable across HTTP/WSS selection.

## Concurrency & Reactive Flow
Network, Mercury, Mobius, registration, call, and media events are asynchronous. Cleanup and restoration must tolerate late events and serialize ownership transitions.

## State Machine
```mermaid
stateDiagram-v2
 [*] --> Created
 Created --> Initialized: init succeeds
 Initialized --> Registered: line registration succeeds
 Registered --> Recovering: network disruption
 Recovering --> Registered: restoration/re-registration
 Recovering --> Failed: final failure
 Registered --> Deregistered: deregister/cleanup
 Deregistered --> [*]
```
...
|---|---|---|
| init/device/backend failure | CallingClient error/event | correct SDK/config and recreate/retry |
| registration/network failure | line/client error and recovery flow | automatic bounded retry or caller action |
| transport mapping/socket failure | typed error/fallback where configured | reconnect or use supported HTTP path |

## Pitfalls
- Listener/timer cleanup is part of correctness.
- Feature-flag/local override resolution must not expose unsupported origins.
- Network recovery and active-call cleanup are coupled but owned by distinct modules.

## Module Do's / Don'ts
- DO delegate stateful details and keep top-level orchestration explicit.
- DON'T bypass SDKConnector, typed events, or owning child-module cleanup.

## Test-Case Strategy (module)
Unit tests cover init/config/device/backend, lines, listeners, transport feature flag, network transitions, cleanup, diagnostics, and errors; Playwright covers integrated registration/call journeys.
| Requirement | Tests | Gap |
|---|---|---|
| CC-R-001..003 | `src/CallingClient/CallingClient.test.ts`, `src/CallingClient/utils/*.test.ts`, `playwright/` | independent validation pending |
...
### Source snapshot: `src/CallingClient/ai-docs/AGENTS.md`
...
### Source snapshot: `src/CallingClient/ai-docs/ARCHITECTURE.md`
