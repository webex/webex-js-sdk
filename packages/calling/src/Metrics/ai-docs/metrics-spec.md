...
## Evidence Rules
Metric names, actions, fields, and submission conditions cite Metrics source/tests.

## Source Material Register
| Source | Scope | Decision | Disposition |
|---|---|---|---|
| `src/Metrics/ai-docs/AGENTS.md` | interface/enums/examples/conditions | reconciled | requirements, public/internal surface, rules |
| `src/Metrics/ai-docs/ARCHITECTURE.md` | singleton/submission sequences/troubleshooting | reconciled | design, flow, relationships, failures |

## Overview
Metrics provides the package-internal MetricManager singleton and typed helpers for operational and behavioral registration, call, media, connection, voicemail, discovery, and log-upload telemetry.

## Purpose / Responsibility
Normalize and submit calling telemetry without allowing metrics failures to control call behavior.

## Stack
TypeScript, Webex metrics plugin, singleton factory, Jest.

## Folder / Package Structure
```text
Metrics/{index.ts,types.ts,index.test.ts,ai-docs/}
```

## Key Files (source of truth)
| File | Holds |
|---|---|
| `src/Metrics/index.ts` | singleton and submission helpers |
| `src/Metrics/types.ts` | metric/action enums and interfaces |
| `src/Metrics/index.test.ts` | payload and conditional submission behavior |

## Public Surface
| ID | Type | Surface | Purpose | Compatibility | Detail | Root index |
|---|---|---|---|---|---|---|
| calling.metrics | internal SDK | `getMetricManager`, `IMetricManager` | typed telemetry | internal contract | `Metrics/index.ts`, `Metrics/types.ts` | `ai-docs/CONTRACTS.md` |

## Requires (dependencies)
Initialized Webex metrics plugin, service indicator, optional device/call/error context, Logger.

## Requirements
| ID | WHAT | WHY | Source Evidence | Test Evidence | Gaps | Confidence |
|---|---|---|---|---|---|---|
| MET-R-001 | Return a stable MetricManager singleton for the configured SDK/service indicator. | Modules need one consistent telemetry context. | `Metrics/index.ts` | `Metrics/index.test.ts` | none | PRESENT |
| MET-R-002 | Build typed metric payloads from action/event/error/device context and submit only valid names. | Dashboards require consistent fields and invalid telemetry must not destabilize calling. | `Metrics/index.ts`, `Metrics/types.ts` | `Metrics/index.test.ts` | none | PRESENT |

## Design Overview
Factory access returns the singleton. Domain-specific methods construct common and action-specific payload fields, normalize error access patterns, validate metric names, and call the SDK metrics plugin conditionally.

## Data Flow
```mermaid
flowchart LR
 Modules --> MetricManager --> Normalize[typed payload] --> MetricsPlugin --> TelemetryService
```

## Sequence Diagram(s)
| Operation group | Diagram | Failure/recovery coverage |
|---|---|---|
| Initialize/submit | Metric submission | missing plugin/invalid name/submission failure |
```mermaid
sequenceDiagram
 participant M as Calling module
 participant MM as MetricManager
 participant P as Metrics plugin
 M->>MM: submit domain metric(context)
 MM->>MM: validate + build payload
 alt valid/configured
  MM->>P: submitOperational/BehavioralMetric
 else invalid/unavailable
  MM->>MM: log/skip safely
 end
```
...
classDiagram
 ...
 MetricManager --> WebexMetricsPlugin
 MetricManager --> Logger
```

## Use Cases
- Set device information.
- Submit registration/call/media/connection/voicemail/discovery/log-upload success or failure metrics. Evidence: `Metrics/index.test.ts`.

## State Model
The singleton retains SDK/service/device context used to enrich later metrics; it owns no durable telemetry data.

## Business Rules & Invariants
- Metric event/action names come from `Metrics/types.ts`.
- Telemetry failure must not throw into call-control success paths unless explicitly contracted.

## Concurrency & Reactive Flow
Multiple modules may submit concurrently; methods build per-call payloads and must not leak mutable context between events.

## Pitfalls
- Error shapes differ by layer; preserve guarded field extraction.
- Conditional metrics and common fields differ by method/event.

## Module Do's / Don'ts
- DO add enums/types/tests with any new metric.
- DON'T include tokens or sensitive payloads or turn telemetry into control flow.

## Test-Case Strategy (module)
Tests cover singleton behavior, payload fields, action/event variants, error extraction, validation, conditional submission, and missing context.
| Requirement | Tests | Gap |
|---|---|---|
| MET-R-001..002 | `Metrics/index.test.ts` | independent validation pending |

## Traceability
- `ai-docs/ARCHITECTURE.md` · `ai-docs/SERVICE_STATE.md` · `.sdd/manifest.json`
...
### Source snapshot: `src/Metrics/ai-docs/AGENTS.md`
...
### Source snapshot: `src/Metrics/ai-docs/ARCHITECTURE.md`
