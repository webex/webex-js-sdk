...
| CONTACTS-R-010 | `ContactsClient` serializes check-then-act mutations of `groups`/`contacts`/`defaultGroupId` under a private `Mutex` (mirroring `register.ts`): `getContacts`'s `this.groups`/`this.contacts` assignment, and `createContactGroup`'s uniqueness-scan-then-push, each run inside a single `mutex.runExclusive` call scoped to that mutation only. `fetchDefaultGroup` does not take its own lock (avoiding nesting with `createContactGroup`'s); instead it treats `createContactGroup`'s "already exists" rejection as a signal that a concurrent caller committed the default group and re-reads `this.groups` for that group's id, so concurrent callers converge on the same `defaultGroupId` instead of creating duplicates. | Concurrent flows (e.g. two near-simultaneous `createContact` calls needing a default group) previously raced the check-then-act on shared state, risking duplicate groups or corrupted assignments (CAI-8594, R2). | `src/Contacts/ContactsClient.ts` | `src/Contacts/ContactsClient.test.ts` (`ContactsClient concurrency` → `createContactGroup uniqueness is race-safe`, `getContacts does not overwrite concurrent updates`, `fetchDefaultGroup does not create duplicate default groups`) | Each lock is scoped to a single mutation and never held across a nested call that itself acquires the mutex (R2): `createContactGroup` calls `fetchEncryptionKeyUrl`/`getContacts` before acquiring, not while holding. | PRESENT |

**Provenance:** `AGENTS.md` rule 8 (positive+negative tests, keep specs current in the same merge); `SECURITY.md` Contact-encryption row; Triage boundary impact (ContactsClient encryption-key trust, `external-validation-required`) and risk R3 (CONTACTS-R-009); Triage risk R2 and sealed invariant on serializing `groups`/`contacts`/`defaultGroupId` mutations (CONTACTS-R-010).
