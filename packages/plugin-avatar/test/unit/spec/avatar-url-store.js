    ...
    describe(`AvatarUrlStore`, () => {
      ...
      it(`add item failures`, () => {
        return Promise.all[
          ...
          assert.isRejected(store.add({uuid: `id1@example.com`, size: 80}), `\`url\` is required`),
          assert.isRejected(store.add({uuid: `id1@example.com`, size: 80, url: `http://www.example.com`}), `\`cacheControl\` is required`)
        ];
      });
      ...
      it(`set / get / remove`, () => {
        return Promise.all[
          ...
          store.remove({uuid: `id1@example.com`}),
