...
document.getElementById('getStatsRaw').addEventListener('click', () => {
  if (meeting) {
    const k = meeting.getRawMediaStatsStream();
    k.on('data', (raw) => {
      // do something with the raw getStats stream data
    });
    // OR
    // k.pipe(...)
    // etc
...
document.getElementById('getStatsFilter').addEventListener('click', () => {
  if (meeting) {
    const k = meeting.getStatsMediaStream();
    const a = meeting.getStatsStreamAggregator('media');
    const h = meeting.getStatsStreamHistory('media');
    const i = meeting.getStatsStreamInterval('media');
    k.on('data', (filtered) => {
      console.log(filtered);
      a.aggregate(filtered);
      h.addHistory(filtered);
      i.updateInterval(filtered);
    });
  }
});
...
document.getElementById('getAggregation').addEventListener('click', () => {
  if (meeting) {
    console.log(meeting.getStatsStreamAggregator('media').getSummary());
  }
});

document.getElementById('getHistory').addEventListener('click', () => {
  if (meeting) {
    console.log(meeting.getStatsStreamHistory('media').getHistory());
  }
});

document.getElementById('getInterval').addEventListener('click', () => {
  if (meeting) {
    console.log(meeting.getStatsStreamInterval('media').getInterval());
  }
});
