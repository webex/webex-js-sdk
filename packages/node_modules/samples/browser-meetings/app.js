...
// And finally, let's wire up dialing
document.getElementById('type-dialer').addEventListener('submit', (event) => {
  // again, we don't want to reload when we try to dial
  event.preventDefault();
  return spark.meetings.createWithType(document.getElementById('type').value, document.getElementById('value')).then((m) => {
    console.log(m);
    startMeeting(m);
  });
});
