...
function bindCallEvents(call) {
  ...
  call.getStatsStream()
    .on('data', (data) => {
      ...
      document.getElementById('incoming-audio-stats').innerHTML = JSON.stringify(data.incomingAudio, null, 2);
      document.getElementById('incoming-video-stats').innerHTML = JSON.stringify(data.outgoingVideo, null, 2);
      document.getElementById('raw-video-stats').innerHTML = JSON.stringify(data.report, null, 2);
    });
  ...
  call.on('change:sendingAudio', () => {
    document.getElementById('microphone-state').innerHTML = call.sendingAudio ? 'on' : 'off';
  });

  call.on('change:sendingVideo', () => {
    document.getElementById('camera-state').innerHTML = call.sendingVideo ? 'on' : 'off';
  });

  call.on('change:remoteAudioMuted', () => {
    document.getElementById('microphone-state-remote').innerHTML = call.remoteAudioMuted ? 'off' : 'on';
  });

  call.on('change:remoteVideoMuted', () => {
    document.getElementById('camera-state-remote').innerHTML = call.remoteVideoMuted ? 'off' : 'on';
  });
  ...
  call.on('all', (e, details) => {
    console.info(`Call event: ${e}`);
...
document.getElementById('start-receiving-audio').addEventListener('click', () => {
  if (activeCall) {
    activeCall.startReceivingAudio();
  }
});

document.getElementById('stop-receiving-audio').addEventListener('click', () => {
  if (activeCall) {
    activeCall.stopReceivingAudio();
  }
});

document.getElementById('start-receiving-video').addEventListener('click', () => {
  if (activeCall) {
    activeCall.startReceivingVideo();
  }
});

document.getElementById('stop-receiving-video').addEventListener('click', () => {
  if (activeCall) {
    activeCall.stopReceivingVideo();
  }
});

document.getElementById('toggle-sending-audio').addEventListener('click', () => {
  if (activeCall) {
    activeCall.toggleSendingAudio();
  }
});

document.getElementById('toggle-receiving-audio').addEventListener('click', () => {
  if (activeCall) {
    activeCall.toggleReceivingAudio();
  }
});

document.getElementById('toggle-sending-video').addEventListener('click', () => {
  if (activeCall) {
    activeCall.toggleSendingVideo();
  }
});

document.getElementById('toggle-receiving-video').addEventListener('click', () => {
  if (activeCall) {
    activeCall.toggleReceivingVideo();
  }
});
