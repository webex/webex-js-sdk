...
describe('Single Party Calling with Mute - Normal Dialing', () => {
  ...
  it('places call from spock to mccoy', () => {
    ...
    browserFirefox.waitUntil(() => (browserFirefox.$('#call-status-local').getText() === 'IN_MEETING'), 10000, 'Timed-out waiting for local user to connect to meeting');
  });
  ...
  it('joins the call on mccoy', () => {
    ...
    browserFirefox.waitUntil(() => (browserFirefox.$('#call-status-remote').getText() === 'IN_MEETING'), 10000, 'Timed-out waiting for remote user to connect to meeting');
  });
  ...
  it('turns the local camera off', () => {
    ...
    browserFirefox.waitUntil(() => (browserFirefox.$('#camera-state').getText() === 'off'), 10000, 'Timed-out waiting for camera state to chage');
  }, 4);

  it('turns the local camera back on', () => {
    browserFirefox.$('[title="start sending video"]').click();
    // Test runs too quickly sometimes and tries to click before call is fully established
    browserFirefox.waitUntil(() => (browserFirefox.$('#camera-state').getText() === 'on'), 10000, 'Timed-out waiting for camera state to change');
  }, 4);

  it('turns sending audio off', () => {
    browserFirefox.$('[title="stop sending audio"]').click();
    // Test runs too quickly sometimes and tries to click before call is fully established
    browserFirefox.waitUntil(() => (browserFirefox.$('#microphone-state').getText() === 'off'), 10000, 'Timed-out waiting for microphone state to chage');
  }, 4);

  it('turns sending audio on', () => {
    browserFirefox.$('[title="start sending audio"]').click();
    // Test runs too quickly sometimes and tries to click before call is fully established
    browserFirefox.waitUntil(() => (browserFirefox.$('#microphone-state').getText() === 'on'), 10000, 'Timed-out waiting for microphone state to chage');
  }, 4);

  it('ends the call', () => {
    browserFirefox.$('[title="hangup"]').click();
    browserFirefox.waitUntil(() => (browserFirefox.$('#call-status-local').getText() === 'NOT_IN_MEETING'), 10000, 'Timed-out waiting for local user to disconnect from meeting');
  });
});
