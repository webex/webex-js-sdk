...
function handleRadio() {
  // checks which radio is enabled and saves the result in localstorage
  isPaste = document.getElementById('paste').checked;
  isOAuth = document.getElementById('oauth').checked;
  localStorage.setItem('paste', isPaste);
  localStorage.setItem('oauth', isOAuth);

  const div = document.createElement('div');

  // removes the child node of the form
  while (authForm.firstChild) {
  ...
  if (isOAuth) {
    div.innerHTML = `
        <legend>Authentication with OAuth </legend>
              ...
              <input id="logout" title="logout" type="submit" value="logout">
            </div>
    `;

    document.getElementById('authForm').appendChild(div);

    // Now, let's set up oauth
  ...
  if (isPaste) {
    div.innerHTML = `
          <legend>Authentication with Access Token </legend>
              ...
              type="text">
              <input id="webexInit" title="webex.init()" type="submit" value="webex.init()">
              <input id="register" title="register" type="submit" value="register">
              <p id="register-status">not registered</p>
            </div>
    `;
...
document.getElementById('paste').checked = localStorage.getItem('paste');
document.getElementById('oauth').checked = localStorage.getItem('oauth');
handleRadio();
