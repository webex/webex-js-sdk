...
describe('Multi Party Calling - Normal Dialing', () => {
  ...
  it('places call from spock to shared room', () => {
    ...
    browserFirefox.waitUntil(() => (browserFirefox.$(`#member-status-${spock.id}`).getText() === 'IN_MEETING'), 10000, `Timed-out waiting for local user (${spock.id}) to connect to meeting`);
  });
  ...
  it('ends the call', () => {
    browserFirefox.waitUntil(() => (browserFirefox.$(`#member-status-${mccoy.id}`).getText() === 'IN_MEETING'), 10000, 'Timed-out waiting for remote user to connect to meeting');
    browserFirefox.$('[title="hangup"]').click();
    browserFirefox.waitUntil(() => (browserFirefox.$(`#member-status-${spock.id}`).getText() === 'NOT_IN_MEETING'), 10000, 'Timed-out waiting for local user to disconnect from meeting');
  });
});
