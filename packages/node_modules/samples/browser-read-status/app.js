...
function updateStatus(authorized, me = {}) {
  ...
  if (authorized) {
    ...
    const msg = ('displayName' in me) ?
      `Looking up initial space status for ${me.displayName}.  This can take some time....` :
      'Looking up initial space status.  This can take some time....';
...
function updateSpaceTable(user) {
  ...
  document.getElementById('read-count').innerHTML = user.readSpacesCount;
...
async function processInitialRoomStatus(roomStates) {
  ...
  try {
    ...
    if ('id' in userInfo.lastUnread) {
      lastUnreadPromise = initMemberDetails(userInfo.lastUnread);
    }
    else {
      lastUnreadPromise = Promise.resolve(emptySpaceList);
    }
    if ('id' in userInfo.lastRead) {
      lastReadPromise = initMemberDetails(userInfo.lastRead);
    }
    else {
    ...
    userInfo.roomStates.sort((a, b) => (a.lastActivityDate < b.lastActivityDate ? 1 : -1));
...
async function getRoomIndex(id) {
  try {
    ...
    //   // rooms.listWithReadStatus is "lossy" or
    ...
    //     roomState.isUnreadByMe = false;
    //   }  
    //   userInfo.roomStates.unshift(room);
    //   roomIdx = 0;
    // }
