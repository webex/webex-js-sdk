...
const localVideoResolution = document.querySelector('#local-video-resolution');
const localVideoFps = document.querySelector('#local-video-fps');
const localVideoLoss = document.querySelector('#local-video-loss');

const remoteVideoResolution = document.querySelector('#remote-video-resolution');
const remoteVideoFps = document.querySelector('#remote-video-fps');
const remoteVideoLoss = document.querySelector('#remote-video-loss');

const shareVideoResolution = document.querySelector('#share-video-resolution');
const shareVideoFps = document.querySelector('#share-video-fps');
const shareVideoLoss = document.querySelector('#share-video-loss');
...
function addMedia() {
  ...
  meeting.on('media:stats', (result) => {
    localVideoResolution.innerHTML = `${result.video.send.width} x ${result.video.send.height}`;
    remoteVideoResolution.innerHTML = `${result.video.recv.width} x ${result.video.recv.height}`;
    shareVideoResolution.innerHTML = `${result.share.recv.width} x ${result.share.recv.height}`;

    localVideoFps.innerHTML = `${result.video.send.fps || 0}fps`;
    remoteVideoFps.innerHTML = `${result.video.recv.fps || 0}fps`;
    shareVideoFps.innerHTML = `${result.share.recv.fps || 0}fps`;

    localVideoLoss.innerHTML = result.video.send.loss || '';
    remoteVideoLoss.innerHTML = result.video.recv.loss || '';
    shareVideoLoss.innerHTML = result.share.recv.loss || '';
  });
}
