...
function playVideoIfPaused(videoElem) {
...
function startMeeting(m) {
  ...
  const remoteShare = document.getElementById('sharevideo');
  ...
  meeting.on('media:ready', (media) => {
    ...
    if (media.type === 'local') {
      ...
      playVideoIfPaused(localVideo);
    }
    if (media.type === 'remoteVideo') {
      remoteVideo.srcObject = media.stream;
      playVideoIfPaused(remoteVideo);
    }
    if (media.type === 'remoteAudio') {
      document.getElementById('remoteaudio').srcObject = media.stream;
    }
    if (media.type === 'remoteShare') {
      remoteShare.srcObject = media.stream;
      playVideoIfPaused(remoteShare);
    }
    if (media.type === 'localShare') {
      localShare.srcObject = media.stream;
      playVideoIfPaused(localShare);
    }
  });
...
document.getElementById('upload-logs').addEventListener('click', () => {
  webex.meetings.uploadLogs();
});
...
document.getElementById('PMR-edit').addEventListener('submit', (event) => {
  ...
  const pin = document.getElementById('wbx-pmr-pin').value;

  if (!pmrLink && !pin) {
    return;
  }
  webex.meetings.personalMeetingRoom.claim(pmrLink, pin).then((claimed) => {
    console.log(claimed);
...
document.getElementById('join').addEventListener('click', () => {
  ...
  const pin = document.getElementById('pin-join').value;
  const moveToResource = false;
  const moderator = document.getElementById('moderator-join').checked;

  const meetingQuality = document.getElementById('setMeetingQualityLevel').value;
  ...
  meeting.join({
    ...
    moderator,
    moveToResource,
    // Check if the default value was not provided and then add it as a param
    // Do not include this optional param check unless you will be overriding the quality
    // This is for sample use only
    ...(meetingQuality !== 'HIGH' && {
      // meetingQuality can be either a STRING or an OBJECT
      // Passing a STRING (i.e 'HIGH', 'MEDIUM', 'LOW') will set both local and remote
      // Passing a OBJECT (like below) will allow you to set them individually
      meetingQuality: {
        local: meetingQuality,
        remote: meetingQuality
      }
    })
  })
    .then((res) => {
...
document.getElementById('updateMeetingQuality')
  .addEventListener('click', () => {
    if (meeting) {
      const level = document.getElementById('updateMeetingQualityLevel').value;

      meeting.setMeetingQuality(level);
    }
  });

document.getElementById('updateLocalVideoQuality')
  .addEventListener('click', () => {
    if (meeting) {
      const level = document.getElementById('updateLocalVideoQualityLevel').value;

      meeting.setLocalVideoQuality(level);
    }
  });

document.getElementById('updateRemoteMeetingQuality')
  .addEventListener('click', () => {
    if (meeting) {
      const level =
        document.getElementById('updateRemoteMeetingQualityLevel').value;

      meeting.setRemoteQualityLevel(level);
    }
  });

function handleJoinIntent(err, meetingToJoin, deviceId) {
  if (err.joinIntentRequired) {
    ...
    document.getElementById('joinAsHost').addEventListener('click', () => {
      ...
      meetingToJoin.join({resourceId: deviceId, pin, moderator})
        .then((res) => {
          console.log(res);
        })
        .catch((err) => {
    ...
    document.getElementById('joinAsGuest').addEventListener('click', () => {
      ...
      meetingToJoin.join({resourceId: deviceId, moderator})
        .then((res) => {
          console.log(res);
        })
        .catch((err) => {
