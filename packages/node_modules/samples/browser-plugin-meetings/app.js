...
async function detectAudioSilence() {
  ...
  if (meeting) {
    const audioEl = document.getElementById('local-video');

    const isSilent = await webex.meetings.media.detectAudioSilence(audioEl);

    document.getElementById('audio-silence-status').innerHTML = `AUDIO SILENCE: ${isSilent.toString().toUpperCase()}`;
  }

  setTimeout(() => {
    document.getElementById('audio-silence-status').innerHTML = 'AUDIO SILENCE: ----';
  }, 5000);
}

async function detectVideoSilence() {
  const meeting = getCurrentMeeting();

  if (meeting) {
    const videoEl = document.getElementById('local-video');

    const isSilent = await webex.meetings.media.detectVideoSilence(videoEl);

    document.getElementById('video-silence-status').innerHTML = `VIDEO SILENCE: ${isSilent.toString().toUpperCase()}`;
  }

  setTimeout(() => {
    document.getElementById('video-silence-status').innerHTML = 'VIDEO SILENCE: ----';
  }, 5000);
}
