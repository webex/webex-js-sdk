...
function getMediaStreams(mediaSettings = getMediaSettings(), audioVideoInputDevices = {}) {
  ...
  return meeting.getMediaStreams(mediaSettings, audioVideoInputDevices)
    ...
    .then(async ([localStream]) => {
      window.WASM_babbleLocalStreamRef = localStream;
      const asm = createWasm(blabs);
...
function removeNoise() {
  ...
  if (meeting) {
    meeting.updateAudio({
      sendAudio,
      receiveAudio,
      stream: window.newProcessedBlabStream.stream
    });
  }
}
...
window.onload = () => {
  ...
  window.blabs = async function blabs() {
    const localStream = window.WASM_babbleLocalStreamRef;
    ...
    try {
      ...
      srcNode = audioContext.createMediaStreamSource(localStream);
    }
    catch (_) {
      audioContext = new ac();
      srcNode = audioContext.createMediaStreamSource(localStream);
    ...
    window.newProcessedBlabStream = dstNode;
