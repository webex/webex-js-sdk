...
const meetingStreamsLocalCanvas = document.querySelector('#local-canvas');
...
const blurBtn = document.getElementById('effect');
const unblurBtn = document.getElementById('uneffect');
const maskBtn = document.getElementById('mask');
const unmaskBtn = document.getElementById('unmask');
const ctx = meetingStreamsLocalCanvas.getContext('2d');

meetingStreamsLocalVideo.onplaying = () => {
  meetingStreamsLocalCanvas.height = meetingStreamsLocalVideo.videoHeight;
  meetingStreamsLocalCanvas.width = meetingStreamsLocalVideo.videoWidth;
};
...
async function perform(net) {
  while (blurBtn.hidden) {
    const segmentation = await net.segmentPerson(meetingStreamsLocalVideo); // eslint-disable-line
    ...
    bodyPix.drawBokehEffect(
      meetingStreamsLocalCanvas, meetingStreamsLocalVideo, segmentation, backgroundBlurAmount,
...
async function performMask(net) {
  while (maskBtn.hidden) {
    const segmentation = await net.segmentPerson(meetingStreamsLocalVideo); // eslint-disable-line
    ...
    bodyPix.drawMask(
      meetingStreamsLocalCanvas, meetingStreamsLocalVideo, backgroundDarkeningMask, opacity, maskBlurAmount, flipHorizontal
    );
  }
}

function loadBodyPixMask() {
  const options = {
    multiplier: 0.75,
    stride: 32,
    quantBytes: 4
...
blurBtn.addEventListener('click', () => {
  ...
  meetingStreamsLocalCanvas.hidden = false;
  meetingStreamsLocalVideo.hidden = true;
  loadBodyPix();
});
...
unblurBtn.addEventListener('click', () => {
  ...
  meetingStreamsLocalCanvas.hidden = true;
  meetingStreamsLocalVideo.hidden = true;
});

maskBtn.addEventListener('click', () => {
  maskBtn.hidden = true;
  unmaskBtn.hidden = false;
  meetingStreamsLocalCanvas.hidden = false;
  meetingStreamsLocalVideo.hidden = true;
  loadBodyPixMask();
});
