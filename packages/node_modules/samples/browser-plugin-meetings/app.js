...
function joinMeeting(meetingId) {
  ...
  meeting.members.on('members:update', (delta) => {
    ...
    console.log('members update!!!!!!!', full);
  });
...
function sendDtmfTones() {
  ...
  if (meeting) {
    console.log('MeetingControls#sendDtmfTones()', dtmfToneParticipant, meeting.members);
    meeting.members.sendDTMF(tones, dtmfToneParticipant)
      .then(() => {
...
function inviteMember(addButton) {
  const meeting = getCurrentMeeting();
  const inputValue = addButton.previousElementSibling.value.trim();

  if (meeting) {
    meeting.invite({phoneNumber: inputValue}).then((res) => {
      ...
      particpantsArr.forEach((obj) => {
        if (obj.person.email === inputValue ||
          obj.person.primaryDisplayString === inputValue) {
