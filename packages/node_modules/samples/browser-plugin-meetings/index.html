...
<html lang="en">
  ...
  <body class="md">
    ...
    <section id="meetingAuth">
        ...
        <div class="md-alert-banner md-alert-banner--info md-mv">
          <div class="md-alert-banner__text">
              <p>You may get test access token from our <a href="https://developer.webex.com/docs/api/getting-started" target="_blank">developer portal</a>.</p>
          </div>
        </div>
        <form id="credentials">
          <fieldset>
              <legend>Step 1: Credentials</legend>
              <input id="access-token" name="accessToken" placeholder="Access Token" type="text" class="md-input" />
              <button id="access-token-save" type="button" onclick="initWebex()" class="md-button">Webex.init()</button>
              <label id="access-token-status"></label>
          </fieldset>
        </form>
        <form id="registration-form">
          <fieldset>
              <legend>Step 2: Registration</legend>
              
              <div id="registrationBanner" class="md-alert-banner md-alert-banner--warning md-mv">
                <div class="md-alert-banner__text">
                    <p id="registration-status">Not registered!</p>
                </div>
              </div>
              <button id="registration-register" type="button" onclick="register()" class="md-button" disabled>webex.meetings.register()</button>
              <button id="registration-unregister" type="button" onclick="unregister()" class="md-button" disabled>webex.meetings.unregister()</button>
          </fieldset>
        </form>
    </section>


    <!-- Meetings Management / Create Meeting / Active Meeting / Current Meeting -->
    <section id="meetingsManagement">
        <h2>Meetings Management</h2>
        <form id="meetings-create">
          <fieldset>
              <legend>Create a Meeting</legend>
              <input id="create-meeting-destination" name="createMeetingDestination" placeholder="Destination" value="" type="text" class="md-input" />
              <p class="md-input__help-text">Common supported destination types: user email, person ID, room ID, SIP URI</p>
              <button id="create-meeting-action" type="button" class="md-button" onclick="createMeeting()">webex.meetings.create()</button>
          </fieldset>
        </form>
        <form id="meetings-list-group">
          <fieldset>
              ...
              <div class="row">
                <input id="meetings-join-pin" name="createMeetingHostpin" placeholder="PIN" value="" type="text" class="md-input" />
                <div class="md-input-container md-checkbox md-mt">
                    <input class="md-input md-checkbox__input" type="checkbox" id="meetings-join-moderator" />
                    <label class="md-checkbox__label" for="meetings-join-moderator">Join as Moderator</label>
                </div>
              </div>
              <p id="meetings-list" class="md-input__help-text">There are currently no meetings to display</p>
              <br />
              <button id="meetings-list-collect" type="button" onclick="collectMeetings()" class="md-button">webex.meetings.syncMeetings()</button>
          </fieldset>
        </form>
        <form id="current-meeting">
          <fieldset>
              <legend>Currently Joined Meeting</legend>
              <button id="meetings-add-media" type="button" onclick="addMedia()" class="md-button">meeting.addMedia()</button>
              <button id="meetings-leave" type="button" onclick="leaveMeeting()" class="md-button">meeting.leave()</button>
        ...
        <fieldset id="incomingsection" style="visibility: hidden;">
          ...
          <button onclick="answerMeeting(this)" id="answer" class="md-button">answer</button>
          <button onclick="rejectMeeting(this)" id="reject" class="md-button">reject</button>
        </fieldset>
    </section>

    <!-- Meeting Controls / General / Recording / Tones / Transcribe -->
      ...
      <form id="general-controls">
         <fieldset>
            ...
            <div class="md-button-group md-button-group--justified" role="group">
               <button id="gc-lock" type="button" onclick="lockMeeting()" class="md-button">lock</button>
               <button id="gc-unlock" type="button" onclick="unlockMeeting()" class="md-button">unlock</button>
         ...
         <fieldset>
            ...
            <div class="md-button-group md-button-group--justified" role="group">
               <button id="gc-start-recording" type="button" onclick="startRecording()" class="md-button">start</button>
               <button id="gc-pause-recording" type="button" onclick="pauseRecording()" class="md-button">pause</button>
               <button id="gc-resume-recording" type="button" onclick="resumeRecording()" class="md-button">resume</button>
               <button id="gc-stop-recording" type="button" onclick="stopRecording()" class="md-button">stop</button>
         ...
         <fieldset>
            <legend>Tones</legend>
            <div>
               <input id="gc-dtmf-tones" name="dtmfTones" placeholder="Enter DTMF tones" value="" type="text" class="md-input" />
               <button id="gc-dtmf-tones-send" type="button" onclick="sendDtmfTones()" class="md-button">sendDTMF()</button>
         ...
         <fieldset>
            ...
            <div class="transcribe">
               <div class="md-button-group md-button-group--justified" role="group">
                  <button id="gc-start-receiving-transcription" disabled type="button" class="md-button" onclick="startReceivingTranscription()">start receiving transcription (click me before joining)</button>
                  <button id="gc-stop-receiving-transcription" disabled type="button" class="md-button" onclick="stopReceivingTranscription()">stop receiving transcription</button>
               </div>
               <textarea id="gc-transcription-content" disabled class="md-input md-mt" rows="10">Transcription Content: Webex Assistant must be enabled, check the console!</textarea>
            </div>
         </fieldset>
      </form>
   </section>

    <section id="meetingDeviceControls">
      ...
      <form id="toggling-sources-controls">
        <fieldset>
          ...
          <div>
            <div class="row">
              <div class="columns medium-4"><label>Sending Qualities</label></div>
              <div class="columns medium-8">
