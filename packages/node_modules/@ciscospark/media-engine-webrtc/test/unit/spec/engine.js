...
browserOnly(describe)('media-engine-webrtc', function () {
  ...
  describe('WebRTCMediaEngine', () => {
        ...
        audioEndStates.forEach((audioEnd) => {
          videoStartStates.forEach((videoStart) => {
            ...
            videoEndStates.forEach((videoEnd) => {
              ...
              const brokenInFirefox = audioStart !== 'inactive' && audioEnd === 'inactive' && videoStart === 'inactive' && videoEnd !== 'inactive' || audioStart === 'inactive' && audioEnd !== 'inactive' && videoStart !== 'inactive' && videoEnd === 'inactive';
              ...
              describe(message, () => {
                ...
                (((browser.chrome && (flakyInChrome || brokenInChrome)) || (browser.firefox && brokenInFirefox)) ? it.skip : step)(`transitions to audio=${audioEnd} and video=${videoEnd}`, () => {
                ...
                (((browser.chrome && (flakyInChrome || brokenInChrome)) || (browser.firefox && brokenInFirefox)) ? it.skip : step)(`returns to audio=${audioStart} and video=${videoStart}`, () => {
                  const sendingAudioSpy = sinon.spy();
                  const sendingVideoSpy = sinon.spy();
                  const receivingAudioSpy = sinon.spy();
                  const receivingVideoSpy = sinon.spy();
    ...
    firefoxOnly(describe)('screensharing', () => {
      describe('staring session with screenshare', () => {
        it('creates a session with only a screenshare stream', () => handleErrorEvent(new WebRTCMediaEngine(), (engine) => {
          engine.setMedia('screen', 'sendonly');
          return engine.createOffer()
            .then((offer) => {
              assert.equal(getMediaDirectionFromSDP('screen', offer), 'sendonly');
              return offer;
            })
            .then(mockAnswer)
            .then((answer) => {
            ...
            .then((answer) => engine.acceptAnswer(answer))
            .then(() => {
              assert.lengthOf(engine.pc.getSenders().filter((s) => s.track.kind === 'video'), 1);
              assert.lengthOf(engine.pc.getSenders().filter((s) => s.track.kind === 'audio'), 0);
            });
        }));
      });

      describe('mid-session screen sharing', () => {
        let engine;
        ...
        step('adds a screenshare stream', () =>
          handleErrorEvent(engine, () => Promise.resolve()
            ...
            .then((answer) => engine.acceptAnswer(answer))
            .then(() => {
              assert.lengthOf(engine.pc.getSenders().filter((s) => s.track.kind === 'video'), 2);
              assert.lengthOf(engine.pc.getSenders().filter((s) => s.track.kind === 'audio'), 1);
            })));
        ...
        step('adds back a screenshare stream', () => handleErrorEvent(engine, () => {
          ...
          return engine.createOffer()
            .then((offer) => {
              assert.lengthOf(engine.pc.getSenders().filter((s) => s.track.kind === 'audio'), 1, 'There should be an audio sender after creating the offer');
              assert.isTrue(engine.pc.getSenders().find((s) => s.track.kind === 'audio').track.enabled, 'The audio sender should be enabled');
              assert.lengthOf(engine.pc.getSenders().filter((s) => s.track.kind === 'video'), 2, 'There should be 2 video senders after creating the offer');
              assert.isTrue(engine.pc.getSenders().filter((s) => s.track.kind === 'video')[0].track.enabled, 'The video sender should be enabled');
              assert.isTrue(engine.pc.getSenders().filter((s) => s.track.kind === 'video')[1].track.enabled, 'The screenshare sender should be enabled');
