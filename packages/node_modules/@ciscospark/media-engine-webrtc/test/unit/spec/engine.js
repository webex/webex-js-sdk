            ...
            videoEndStates.forEach((videoEnd) => {
              ...
              describe(message, () => {
                ...
                step(`transitions to audio=${audioEnd} and video=${videoEnd}`, () => {
                  ...
                  .then(() => {
                    if (expectToRenegotiateOnTransition) {
                      assert.called(negotiationneededSpy);
                    }
                    else {
                      assert.notCalled(negotiationneededSpy);
                    }

                    if (expectNewRemoteTrackOnTransition) {
                      assert.called(trackSpy);
                    }
                    else {
                      assert.notCalled(trackSpy);
                    }
                  });
                });
                ...
                step(`returns to audio=${audioStart} and video=${videoStart}`, () => {
                  ...
                  return Promise.resolve(expectToRenegotiateOnReturn && expectEvent(20000, `negotiationneeded`, engine)
                    ...
                    // .then(() => new Promise((resolve) => setTimeout(resolve, 1000)))
                    .then(() => {
                      if (expectToRenegotiateOnReturn) {
                        assert.called(negotiationneededSpy);
                      }
                      else {
                        assert.notCalled(negotiationneededSpy);
                      ...
                      if (expectNewRemoteTrackOnReturn) {
                        assert.called(trackSpy);
                      }
                      else {
                        assert.notCalled(trackSpy);
