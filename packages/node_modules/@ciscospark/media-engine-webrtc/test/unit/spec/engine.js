...
import {setupRemotePeerConnection, simulateAnswer} from '../lib/offer-answer';
...
function assertLocalKind(kind, direction, engine) {
  ...
  senders.forEach((s) => console.log(`assterSenders ${s.track.kind} ${s.track.id} ${s.track.enabled}`));
  if (direction.includes('send')) {
...
function assertRemoteKind(kind, direction, engine) {
  ...
  receivers.forEach((s) => console.log(`assertReceivers ${s.track.kind} ${s.track.id} ${s.track.enabled}`));
