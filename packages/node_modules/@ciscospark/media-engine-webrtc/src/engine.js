...
export default class WebRTCMediaEngine {
   ...
   */
  constructor(attrs = {}) {
    // localMediaStream and remoteMediaStream don't get changed anymore, so
    // we'll simulate previous behavior by firing their change events once on
    // nextTick
    process.nextTick(() => {
      this.emit(`change:remoteMediaStream`);
    });

    process.nextTick(() => {
      this.emit(`change:localMediaStream`);
    });
