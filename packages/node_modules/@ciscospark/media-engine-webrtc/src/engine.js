...
export default class WebRTCMediaEngine {
  ...
  acceptAnswer(sdp) {
    ...
    return this.pc.setRemoteDescription(new RTCSessionDescription({
      ...
      .then(() => {
        ...
        // Update media enabled flags in case we get extra media streams
        this.updateLocalMediaToTargetDirection();
    ...
    return new Promise((resolve) => process.nextTick(resolve))
      ...
      .then(([userStream, screenStream]) => {
        if (userStream) {
          ...
          this.logger.info('ENGINE getUserMedia\n---------------------------------------------');
          this.logSendersAndReceivers();
        }
  ...
  /**
   * Logs senders and receivers from PC
   * @returns {undefined}
   * @private
   */
  logSendersAndReceivers() {
    this.logger.info('senders:');
    this.pc.getSenders().forEach((s) => s.track && this.logger.info(`${s.track.kind} - ${s.track.id} - ${s.track.enabled}`));
    this.logger.info('receivers:');
    this.pc.getReceivers().forEach((r) => r.track && this.logger.info(`${r.track.kind} - ${r.track.id} - ${r.track.enabled}`));
    this.logger.info('---------------------------------------------');
  }

  /**
   * Logs sdp media values
   * @param {String} sdp
   * @returns {undefined}
   * @private
   */
  logSdpMedia(sdp) {
    parse(sdp).media.forEach((m) => {
      this.logger.info(`${m.type} - ${m.id} - ${m.direction}`);
    });
    this.logger.info('---------------------------------------------');
  }
