  ...
  describe(`ClientMetricsBatcher`, () => {
    ...
    describe(`#request()`, () => {
      describe(`when the request completes successfully`, () => {
        it(`clears the queue`, () => {
          clock.uninstall();
          return spark.metrics.clientMetricsBatcher.request({
            key: `testMetric`
          })
            .then(() => {
              assert.calledOnce(spark.request);
              assert.lengthOf(spark.metrics.clientMetricsBatcher.queue, 0);
            });
        });
      });
      ...
      describe(`when the request fails due to network disconnect`, () => {
        it(`reenqueues the payload`, () => {
          ...
          const promise = spark.metrics.clientMetricsBatcher.request({
            key: `testMetric`
          });

          return promiseTick(50)
            .then(() => assert.lengthOf(spark.metrics.clientMetricsBatcher.queue, 1))
            ...
            .then(() => promiseTick(50))
            .then(() => assert.lengthOf(spark.metrics.clientMetricsBatcher.queue, 0))
