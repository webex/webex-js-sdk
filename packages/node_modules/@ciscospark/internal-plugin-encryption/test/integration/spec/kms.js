    ...
    describe('#fetchKey(onBehalfOf)', () => {
      ...
      before('create compliance officer test user', () => testUsers.create({
        ...
        config: {
          ...
          roles: [{name: 'spark.synckms'}, {name: 'spark.kms_orgagent'}]
        }
      })
        .then((users) => {
          jim = users[0];
      ...
      let key;
      it('retrieves a specific key on behalf of another user', () => spock.spark.internal.encryption.kms.createUnboundKeys({count: 1})
        .then(([k]) => {
          key = k;
          // Compliance Officer Jim retrieve a key on behalf of Spock
          return jim.spark.internal.encryption.kms.fetchKey({uri: key.uri, onBehalfOfUserId: spock.id});
        })
        .then((key2) => {
          assert.property(key2, 'uri');
          assert.property(key2, 'jwk');
