...
describe(`Encryption`, function() {
  ...
  describe(`KMS`, () => {
    ...
    /**
     *Test getAuthorizationList function
     *Setup: Create a resource, then authorize a user to a key, then authorize a resource to a key
     *Test:
     *1)Invoke getAuthorizationList on the resource, verify that the function returns the array that contains two authorized items
     *2)Remove the authorized user from the resource, then invoke getAuthorizationList again, verify that the returned array doesn't have the user.
     *3)Remove the authorized resource from the resource again, then invoke getAuthorizationList again, verify that he returned array doesn't have the resource.
     */
    describe(`#getAuthorizationList()`, () => {
      let boundedKeyUri, kro, otherKro;
      const testUserId = mccoy.spark.internal.device.userId;
      let testResourceId;
      ...
      before(`authorizes a user to a key`, () => spark.internal.encryption.kms.addAuthorization({
        ...
        .then(([auth]) => {
          assert.equal(auth.resourceUri, kro.uri);
          assert.equal(auth.authId, testUserId);
          return assert.isFulfilled(mccoy.spark.internal.encryption.kms.fetchKey({uri: boundedKeyUri}));
        }));
      ...
      before(`authorizes a resource to a key`, () => spark.internal.encryption.kms.createUnboundKeys({count: 1})
        ...
        .then(([auth]) => {
          assert.equal(auth.resourceUri, kro.uri);
          assert.equal(auth.authId, otherKro.uri);
          return spark.internal.encryption.kms.getAuthorizationList({kroUri: boundedKeyUri})
            .then((authIds) => {
              assert.equal(authIds.length, 2);
              assert.notEqual(authIds.indexOf(testUserId), -1);
              assert.notEqual(authIds.indexOf(testResourceId), -1);
            });
        }));
      ...
      it(`remove the user and verify this user is not in the authorization list `, () => spark.internal.encryption.kms.removeAuthorization({
        ...
        .then(([auth]) => {
          ...
          return spark.internal.encryption.kms.getAuthorizationList({kroUri: boundedKeyUri})
            .then((authIds) => {
              assert.equal(authIds.indexOf(testUserId), -1);
            });
        }));
      ...
      it(`remove the resource and verify this resource is not in the authorizaiton list`, () => spark.internal.encryption.kms.removeAuthorization({
        ...
        .then(([auth]) => {
          ...
          return spark.internal.encryption.kms.getAuthorizationList({kroUri: boundedKeyUri})
            .then((authIds) => {
              assert.equal(authIds.indexOf(testResourceId), -1);
            });
        }));
    });
