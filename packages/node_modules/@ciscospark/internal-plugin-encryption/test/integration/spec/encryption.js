...
describe('Encryption', function () {
  ...
  describe('#onBehalfOf', () => {
    ...
    it('decrypt text', () => spark.internal.encryption.encryptText(key, PLAINTEXT)
      .then((ciphertext) => {
        assert.notEqual(ciphertext, PLAINTEXT);
        return complianceUser.spark.internal.encryption.decryptText(key, ciphertext, user.id);
      })
      .then((plaintext) => assert.equal(plaintext, PLAINTEXT)));

    it('encrypt and decrypt text', () => complianceUser.spark.internal.encryption.encryptText(key, PLAINTEXT, user.id)
      .then((ciphertext) => {
        assert.notEqual(ciphertext, PLAINTEXT);
        return complianceUser.spark.internal.encryption.decryptText(key, ciphertext, user.id);
      })
    ...
    it('decrypt scr', () => spark.internal.encryption.encryptBinary(FILE)
      .then(({scr}) => {
        scr.loc = 'file:///file.enc';
        return spark.internal.encryption.encryptScr(key, scr)
          .then((cipherScr) => complianceUser.spark.internal.encryption.decryptScr(key, cipherScr, user.id))
          .then((decryptedScr) => assert.deepEqual(decryptedScr, scr));
      }));
    ...
    it('decrypt scr', () => spark.internal.encryption.encryptBinary(FILE)
      .then(({scr}) => {
        scr.loc = 'file:///file.enc';
        return complianceUser.spark.internal.encryption.encryptScr(key, scr, user.id)
          .then((cipherScr) => complianceUser.spark.internal.encryption.decryptScr(key, cipherScr, user.id))
          .then((decryptedScr) => assert.deepEqual(decryptedScr, scr));
      }));

    it('getKey', () => complianceUser.spark.internal.encryption.getKey(key.uri, user.id)
      .then((key2) => {
    ...
    it('getKey forbidden as user does not have access', () => complianceUser.spark.internal.encryption.getKey(key.uri, '7851fe79-7c87-40cc-ac36-8b77b011b399')
      .then(
        (value) => expect.fail(`Should not be found as 7851fe79-7c87-40cc-ac36-8b77b011b399 does not have access ${value}`),
        (error) => expect(error.body.status).to.equal(403)
      ));

    it('getKey onBehalfOf and then by compliance officer only', () => complianceUser.spark.internal.encryption.getKey(key.uri, user.id)
      .then((key2) => {
        assert.property(key2, 'uri');
        assert.property(key2, 'jwk');
        assert.notEqual(key2, key);
