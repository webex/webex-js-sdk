...
const Encryption = SparkPlugin.extend({
   ...
   * @param {string} [onBehalfOf] - Fetch the KMS key on behalf of another user (using the user's UUID), active user requires the 'spark.kms_orgagent' role
   * @returns {Object} Decrypted SCR
   */
  decryptScr(key, cipherScr, onBehalfOf) {
   ...
   */
  encryptText(key, plaintext, onBehalfOf) {
    return this.getKey(key, onBehalfOf)
      .then((k) => jose.JWE
