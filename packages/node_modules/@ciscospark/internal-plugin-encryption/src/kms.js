...
const KMS = SparkPlugin.extend({
   ...
   * @param {string} options.onBehalfOf The id of a user, upon whose behalf, the key is to be retrieved
   * @returns {Promise<Key>}
   */
  ...
  fetchKey({uri, onBehalfOf}) {
    ...
    // Note: payload.onBehalfOf is not part of the KMS specification.
    return this.request({
      method: 'retrieve',
      uri,
      onBehalfOf
  ...
  prepareRequest(payload) {
    ...
    return Promise.resolve(isECDHRequest ? partialContexts.get(this) : this._getContext(payload.onBehalfOf))
      .then((context) => {
  ...
  /**
   * @private
   * @param {String} onBehalfOf create context on behalf of another user
   * @returns {Promise<Object>}
   */
  _getContext(onBehalfOf) {
    ...
    return Promise.all([
      ...
      .then(([context, authorization]) => {
        ...
        if (onBehalfOf) {
          // KMS 'retrieve' requests can be made on behalf of another user. This is known
          // as KMS RBAC. It's useful for scenarios such as eDiscovery where an authorized
          // compliance officer is entitled to retrieve content generated by any org user.
          // Note: Setting the userId here as _prepareContext() generates a 'create' request
          // where the userId must match the token.
          context.clientInfo.credential.userId = onBehalfOf;
        }
        else {
          // For normal requests reset the userId back to the spark user
          context.clientInfo.credential.userId = this.spark.internal.device.userId;
        }
        return context;
      });
  },
