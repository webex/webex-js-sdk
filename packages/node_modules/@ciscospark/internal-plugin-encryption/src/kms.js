...
const KMS = SparkPlugin.extend({
   ...
   * @param {string} options.onBehalfOfUserId The id of a user, upon whose behalf, the key is to be retrieved
   * @returns {Promise<Key>}
   */
  ...
  prepareRequest(payload) {
    ...
    return Promise.resolve(isECDHRequest ? partialContexts.get(this) : this._getContext(payload.onBehalfOfUserId))
      .then((context) => {
  ...
  /**
   * @private
   * @param {String} onBehalfOfUserId create context on behalf of another user
   * @returns {Promise<Object>}
   */
  _getContext(onBehalfOfUserId) {
    ...
    return Promise.all([
      ...
      .then(([context, authorization]) => {
        ...
        if (onBehalfOfUserId) {
          ...
          context.clientInfo.credential.userId = onBehalfOfUserId;
        }
        return context;
      });
  },
