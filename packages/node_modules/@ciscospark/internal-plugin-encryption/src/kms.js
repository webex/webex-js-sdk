...
const KMS = SparkPlugin.extend({
  ...
  prepareRequest(payload, onBehalfOf) {
    ...
    return Promise.resolve(isECDHRequest ? partialContexts.get(this) : this._getContext(onBehalfOf))
      .then((context) => {
   ...
   * @param {Object} [onBehalfOf] Optionally run the request on behalf of another user, used in compliance scenarios
   * @returns {Promise<Object>}
   */
    ...
    return this.spark.internal.mercury.connect()
      ...
      .catch((reason) => {
        if (process.env.NODE_ENV === 'test' && (reason.status === 403 || reason.statusCode === 403) && reason.message.match(/Failed to resolve authorization token in KmsMessage request for user/)) {
          ...
          return this.request(payload, undefined, onBehalfOf);
        }
  ...
  _getContext(onBehalfOf) {
    ...
    return Promise.all([
      ...
      .then(([context, authorization]) => {
        ...
        // compliance officer is entitled to retrieve content generated by any organisation user.
        // For normal requests reset the userId back to the spark user.
        // Note: Setting the onBehalfOf userId here as _prepareContext() generates a 'create'
