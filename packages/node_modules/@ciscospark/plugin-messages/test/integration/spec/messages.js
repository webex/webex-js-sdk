    ...
    describe('#listen()', () => {
      [
        ...
        it(`emits a "created" event when a "${verb}" activity arrives`, () => {
          const event = makeActivityEvent(verb);
          const spy = sinon.spy();
    ...
    describe('#createMessagesEventData', () => {
      [
        ...
        it(`creates a messages event for a ${input.roomType} room`, () => {
          const activityEvent = makeActivityEvent('post', input.tags);
          const {activity} = activityEvent.data.data;
...
function makeActivityEvent(verb = 'post', tags = ['ONE_ON_ONE'], published = Date.now()) {
  const activity = {
