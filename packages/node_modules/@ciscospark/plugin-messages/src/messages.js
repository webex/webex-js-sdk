...
const MESSAGES_CREATED = 'messages:created';
...
const Messages = SparkPlugin.extend({
  ...
  listen() {
    return this.spark.internal.mercury.connect()
      .then(() => this.listenTo(this.spark.internal.mercury, 'event:conversation.activity',
        (event) => this.onConversationActivityEvent(event)));
  },

  /**
   * Trigger a "messages:created" event.
   * @param {Object} event
   * @returns {undefined}
   */
  async onConversationActivityEvent(event) {
    ...
    switch (activity.verb) {
      ...
      case API_ACTIVITY_VERB.POST:
        const messagesEventData = await this.createMessagesEventData(activity);
        debug(`messages:created payload: ${JSON.stringify(messagesEventData)}`);
        this.trigger(MESSAGES_CREATED, messagesEventData);
        break;
      default: {
        break;
      }
   ...
   * @param {boolean} fromHydra get decrypted text from Hydra by default
   * @returns {Object} constructed event
   */
  async createMessagesEventData(activity, fromHydra = true) {
    const id = constructHydraId(hydraTypes.MESSAGE, activity.id);
    ...
    const messagesEventData = {
      ...
      text: activity.object.displayName, // encrypted
    ...
    return Promise.resolve(messagesEventData);
  },
