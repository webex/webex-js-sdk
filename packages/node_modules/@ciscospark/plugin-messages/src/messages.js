...
const Messages = SparkPlugin.extend({
  ...
  initialize(...args) {
    ...
    // Listen to new activities from Mercury.
    this.listenTo(this.spark.internal.mercury, 'event:conversation.activity',
      (event) => this._onActivityEvent(event));
  },
  ...
  _onActivityEvent(event) {
    ...
    switch (activity.verb) {
      ...
      case API_ACTIVITY_VERB.POST:
        const messagesEventData = this._constructMessagesEventData(activity);
   ...
   */
  _constructMessagesEventData(activity) {
    return {
      ...
      personEmail: activity.actor.entryEmail,
