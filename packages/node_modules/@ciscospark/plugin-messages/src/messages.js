...
// This actually generates all kind of activities related events
// Should we give this  a name that represents that
...
const Messages = SparkPlugin.extend({
  ...
  listen() {
    return this.spark.internal.mercury.connect()
      .then(() => this.listenTo(this.spark.internal.mercury, INCOMING_MESSAGES_CREATED,
  ...
  onConversationActivityEvent(event) {
    ...
    switch (activity.verb) {
      ...
      case API_ACTIVITY_VERB.POST:
        const messageCreatedEventData = this.messageCreatedEventData(activity);
        debug(`messages "created" payload: ${JSON.stringify(messageCreatedEventData)}`);
        this.trigger(MESSAGE_CREATED, messageCreatedEventData);
        break;

      case API_ACTIVITY_VERB.DELETE:
      case API_ACTIVITY_VERB.TOMBSTONE:
        const messageDeletedEventData = this.messageDeletedEventData(activity);
        debug(`messages "deleted" payload: ${JSON.stringify(messageDeletedEventData)}`);
        this.trigger(MESSAGE_DELETED, messageDeletedEventData);
        break;
   ...
   */
  messageCreatedEventData(activity) {
    return {
      ...
      personOrgId: constructHydraId(hydraTypes.ORGANIZATION, activity.actor.orgId),
      created: activity.published
    };
  },

  /**
   * Constructs an event data object for the "deleted" event,
   * adhering to Hydra's Message details.
   * @see https://developer.webex.com/docs/api/v1/messages/get-message-details
   * @param {Object} activity from mercury
   * @returns {Object} constructed event
   */
  messageDeletedEventData(activity) {
    return {
      id: constructHydraId(hydraTypes.MESSAGE, activity.object.id),
      roomId: constructHydraId(hydraTypes.ROOM, activity.target.id),
      roomType: activity.target.tags.includes('ONE_ON_ONE') ? 'direct' : 'group',
      personId: constructHydraId(hydraTypes.PEOPLE, activity.actor.entryUUID),
      personEmail: activity.actor.emailAddress || activity.actor.entryEmail,
      personOrgId: constructHydraId(hydraTypes.ORGANIZATION, activity.actor.orgId),
      created: activity.published
    };
