...
describe(`plugin-mercury`, () => {
  describe(`Mercury`, () => {
    ...
    describe(`#_prepareUrl()`, () => {
      beforeEach(() => {
        spark.device.webSocketUrl = `ws://example.com`;
      });

      it(`requests text-mode WebSockets`, () => assert.isFulfilled(spark.mercury._prepareUrl())
        .then((wsUrl) => assert.match(wsUrl, /outboundWireFormat=text/)));

      it(`requests the buffer state message`, () => assert.isFulfilled(spark.mercury._prepareUrl())
        .then((wsUrl) => assert.match(wsUrl, /bufferStates=true/)));


      it(`does not add conditional properties`, () => assert.isFulfilled(spark.mercury._prepareUrl())
        .then((wsUrl) => {
      ...
      describe(`when 'web-shared-socket' is enabled`, () => {
        ...
        it(`requests shared socket support`, () => assert.isFulfilled(spark.mercury._prepareUrl())
          .then((wsUrl) => assert.match(wsUrl, /isRegistrationRefreshEnabled=true/)));

        it(`requests the registration banner`, () => assert.isFulfilled(spark.mercury._prepareUrl())
          .then((wsUrl) => assert.match(wsUrl, /mercuryRegistrationStatus=true/)));

        it(`does not request the buffer state message`, () => assert.isFulfilled(spark.mercury._prepareUrl())
          .then((wsUrl) => {
      ...
      describe(`when using an ephemeral device`, () => {
        ...
        it(`indicates multiple connections may be coming from this user`, () => assert.isFulfilled(spark.mercury._prepareUrl())
