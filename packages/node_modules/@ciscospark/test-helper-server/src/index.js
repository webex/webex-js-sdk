...
// This is definitely a hack, but it's the best I've got for supporting the
// `--raw` flag. If we're running from the 'src' dir instead of the 'dist' dir,
// hack the node resolver so we run everything from 'src' and babel-transform
// it.
if (process.argv[1].includes('src')) {
  ...
  Module._resolveFilename = wrap(Module._resolveFilename, function resolveToSrcIndex(_resolveFilename, request, parent, isMain) {
    if (request.startsWith('@ciscospark') && request.split('/').length === 2 || request === 'ciscospark') {
      request = path.resolve(__dirname, '../../../../../packages', 'node_modules', request, 'src/index.js');
    }
    // eslint-disable-next-line no-invalid-this
    return Reflect.apply(_resolveFilename, this, [request, parent, isMain]);
  });

  const pattern = path.resolve(__dirname, '../../../../../packages/node_modules/**/*.js');
  ...
  require('babel-register')({
    only: [pattern],
    plugins: [
      path.resolve(__dirname, '../../../../../tooling/babel-plugin-inject-package-version')
    ],
    sourceMaps: true
  });
}
