  ...
  describe(`Metrics`, () => {
    ...
    describe(`#storeMetric()`, () => {
      it(`stores a metric`, () => {
        ...
        assert.equal(metrics.getMetrics().size, 1);
        assert.isNotNull(metrics.storeMetric(`2`, `bar`, `clientMetric`));
        assert.equal(metrics.getMetrics().size, 2);
      });
    });
    ...
    describe(`#deleteMetric`, () => {
      it(`it deletes a metric from map`, () => {
        ...
        assert.equal(metrics.getMetrics().size, 0);
      });
    });

    describe(`#registerMetric()`, () => {
      it(`registers and stores a metric that does not exist`, () => {
        assert.isNotNull(metrics.registerMetric(`Foo`, `1`, testPayload, `clientMetric`));
        assert.equal(metrics.getMetrics().size, 1);
        assert.equal(metrics.getMetric(`1`).type, `clientMetric`);
        assert.isNotNull(metrics.registerMetric(`bar`, `2`, testPayload, `clientMetric`));
        assert.equal(metrics.getMetrics().size, 2);
      ...
      it(`does not register or store a metric that already exists`, () => {
        assert.isNotNull(metrics.registerMetric(`horse`, `1`, testPayload, `clientMetric`));
        assert.equal(metrics.getMetrics().size, 1);
        assert.isFalse(metrics.registerMetric(`horse`, `1`, `BazBar`, `clientMetric`));
        assert.equal(metrics.getMetrics().size, 1);
        assert.equal(testPayload.fields, metrics.getMetric(`1`).metric.fields);
      });
    });

    describe(`#clientRegisterMetric()`, () => {
      it(`client registers and stores a metric that does not exist`, () => {
        assert.isNotNull(metrics.clientRegisterMetric(`Foo`, `1`, `fooBar`));
        assert.equal(metrics.getMetrics().size, 1);
        assert.isNotNull(metrics.clientRegisterMetric(`bar`, `2`, `barBaz`));
        assert.equal(metrics.getMetrics().size, 2);
        assert.equal(metrics.getMetric(`1`).type, `clientMetric`);
    ...
    describe(`#sdkRegisterMetric()`, () => {
      it(`sdk registers and stores a metric that does not exist`, () => {
        assert.isNotNull(metrics.sdkRegisterMetric(`Foo`, `1`, `fooBar`));
        assert.equal(metrics.getMetrics().size, 1);
        assert.isNotNull(metrics.sdkRegisterMetric(`bar`, `2`, `barBaz`));
        assert.equal(metrics.getMetrics().size, 2);
    ...
    describe(`#updateRegisteredMetric`, () => {
      it(`updates metric props`, () => {
        ...
        metrics.registerMetric(`horse`, `1`, testPayload, `clientMetric`);
      ...
      it(`does not update on null args`, () => {
        ...
        metrics.registerMetric(`testNullType`, `3`, testPayload, null);
        assert.isFalse(metrics.updateRegisteredMetric(`3`, `someValue`));
      });
    });

    describe(`#clientSendRegisteredMetric`, () => {
      it(`sends client metric`, () => {
        metrics.registerMetric(`horse`, `1`, testPayload, `clientMetric`);
        metrics.clientSendRegisteredMetric(`1`)
        .then(() => {
          assert.calledOnce(spark.request);
          const req = spark.request.args[0][0];
          const metric = req.body.metrics[0];
      ...
      it(`does not send non-client metric`, () => {
        metrics.registerMetric(`test`, `2`, testPayload, `sdkMetric`);
        assert.isFalse(metrics.clientSendRegisteredMetric(`2`));
      });
      it(`does not send null metric`, () => {
        assert.isFalse(metrics.clientSendRegisteredMetric(`4`));
      });
    });

    describe(`#sdkSendRegisteredMetric`, () => {
      it(`sends sdk metric`, () => {
        metrics.registerMetric(`horse`, `1`, testPayload, `sdkMetric`);
        metrics.sdkSendRegisteredMetric(`1`)
        .then(() => {
          assert.calledOnce(spark.request);
          const req = spark.request.args[0][0];
          const metric = req.body.metrics[0];
      ...
      it(`does not send non-sdj metric`, () => {
        metrics.registerMetric(`test`, `2`, testPayload, `clientMetric`);
        assert.isFalse(metrics.sdkSendRegisteredMetric(`2`));
      });
      it(`does not send null metric`, () => {
        assert.isFalse(metrics.sdkSendRegisteredMetric(`4`));
      });
    });

    describe(`#sendUnstructured`, () => {
      it(`sends an unstructured metric`, () => {
        metrics.sendUnstructured(`foo`, `bar`)
        .then(() => {
          assert.calledOnce(spark.request);
        });
      });
