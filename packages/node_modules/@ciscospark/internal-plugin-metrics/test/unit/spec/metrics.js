  ...
  describe(`Metrics`, () => {
    ...
    describe(`#registerSdkMetric()`, () => {
      it(`sdk registers and stores a metric that does not exist`, () => {
        assert.isNotNull(metrics.registerSdkMetric(`Foo`, `1`, `fooBar`));
        assert.equal(metrics.metricMap.size, 1);
        assert.isNotNull(metrics.registerSdkMetric(`bar`, `2`, `barBaz`));
        assert.equal(metrics.metricMap.size, 2);
    ...
    describe(`#clientSendRegisteredMetric`, () => {
      it(`sends client metric`, () => {
        metrics._registerMetric(`horse`, `1`, testPayload, `clientMetric`);
        return metrics.clientSendRegisteredMetric(`1`)
          .then(() => {
            assert.calledOnce(spark.request);
            const req = spark.request.args[0][0];
            const metric = req.body.metrics[0];
    ...
    describe(`#sdkSendRegisteredMetric`, () => {
      it(`sends sdk metric`, () => {
        ...
        return metrics.sdkSendRegisteredMetric(`1`)
          .then(() => {
            assert.calledOnce(spark.request);
            const req = spark.request.args[0][0];
            const metric = req.body.metrics[0];
    ...
    describe(`#sendUnstructured`, () => {
      it(`sends an unstructured metric`, () => {
        return metrics.sendUnstructured(`foo`, `bar`)
        .then(() => {
          assert.calledOnce(spark.request);
        });
      });
