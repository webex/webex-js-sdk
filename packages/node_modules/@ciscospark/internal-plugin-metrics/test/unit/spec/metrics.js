  ...
  describe(`Metrics`, () => {
    ...
    describe(`#registerMetric()`, () => {
      it(`registers and stores a metric that does not exist`, () => {
        assert.isNotNull(metrics.registerMetric(`1`, `Foo`, `clientMetric`));
        assert.equal(metrics.getMetrics().size, 1);
        assert.isNotNull(metrics.registerMetric(`2`, `bar`, `clientMetric`));
        assert.equal(metrics.getMetrics().size, 2);
      });

      it(`does not register or store a metric that already exists not exist`, () => {
        assert.isNotNull(metrics.registerMetric(`horse`, `1`, `Baz`, `clientMetric`));
        assert.equal(metrics.getMetrics().size, 1);
