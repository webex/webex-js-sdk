...
describe(`plugin-lyra`, function() {
  ...
  describe(`Space`, () => {
    ...
    describe(`#unbindConversation()`, () => {
      ...
      it(`removes the binding between conversation and lyra space`, () => assert.isFulfilled(spock.spark.internal.lyra.space.unbindConversation(lyraMachine.space, conversation))
        .then(() => spock.spark.internal.lyra.space.getCurrentBindings(lyraMachine.space))
    ...
    describe(`#deleteBinding()`, () => {
      ...
      it(`removes the binding between conversation and lyra space`, () => assert.isFulfilled(spock.spark.internal.lyra.space.deleteBinding(lyraMachine.space, {kmsResourceObjectUrl: conversation.kmsResourceObjectUrl, bindingId}))
        .then(() => spock.spark.internal.lyra.space.getCurrentBindings(lyraMachine.space))
        .then(({bindings}) => assert.lengthOf(bindings, 0)));
    });
  });
