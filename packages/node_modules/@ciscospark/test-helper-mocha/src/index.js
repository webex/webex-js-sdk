...
/**
 * Indeicates if we're running in a web browser
 * @returns {boolean}
 * @private
 */
function inBrowser() {
...
/**
 * Indeicates if we're running in firefox
 * @returns {boolean}
 * @private
 */
function inFirefox() {
...
module.exports = {
  ...
  flaky: function flaky(mochaMethod) {
    ...
    // FIXME: These env variables are not available within the browsers.
    return process.env.SKIP_FLAKY_TESTS ? mochaMethod.skip : mochaMethod;
  },

  /**
   * Wrap the desired tests to check in a `describe` block and use this method
   * in its `before` to determine whether or not to skip the block.
   *
   * @param {string} envVar
   * @returns {boolean} true to skip test, false otherwise
   */
  skipFlaky: function skipFlaky(envVar) {
    return envVar && envVar !== 'false' && !!envVar;
  },
