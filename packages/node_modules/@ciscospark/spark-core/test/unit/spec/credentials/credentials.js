  ...
  describe('Credentials', () => {
    ...
    describe('#buildLoginUrl()', () => {
      ...
      it('generates the login url', () => {
        ...
        assert.equal(credentials.buildLoginUrl({state: {page: 'front'}}), `${process.env.IDBROKER_BASE_URL || 'https://idbrokerbts.webex.com'}/idb/oauth2/v1/authorize?state=eyJwYWdlIjoiZnJvbnQifQ&client_id=fake&redirect_uri=http%3A%2F%2Fexample.com&scope=scope%3Aone&response_type=code`);
      });
      ...
      it('generates the login url with empty state param', () => {
        ...
        assert.equal(credentials.buildLoginUrl({state: {}}), `${process.env.IDBROKER_BASE_URL || 'https://idbrokerbts.webex.com'}/idb/oauth2/v1/authorize?client_id=fake&redirect_uri=http%3A%2F%2Fexample.com&scope=scope%3Aone&response_type=code`);
      });
    });
    ...
    describe('#buildLogoutUrl()', () => {
      it('generates the logout url', () => {
        ...
        const credentials = new Credentials(undefined, {parent: spark});
        spark.trigger('change:config');
        assert.equal(credentials.buildLogoutUrl(), `${process.env.IDBROKER_BASE_URL || 'https://idbrokerbts.webex.com'}/idb/oauth2/v1/logout?cisService=spark&goto=ru`);
      });
      ...
      it('includes a token param if passed', () => {
        ...
        const credentials = new Credentials(undefined, {parent: spark});
        spark.trigger('change:config');
        assert.equal(credentials.buildLogoutUrl({token: 't'}), `${process.env.IDBROKER_BASE_URL || 'https://idbrokerbts.webex.com'}/idb/oauth2/v1/logout?cisService=spark&goto=ru&token=t`);
      });
