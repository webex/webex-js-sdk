    ...
    describe(`RateLimitInterceptor`, () => {
      ...
      describe(`#isRateLimited`, () => {
        ...
        it(`returns false when URI is not IDbroker URI`, () => {
          return assert.equal(interceptor.isRateLimited(`https://example.com/testFake`), false);
        });
        it(`returns false if the URI is null`, () => {
          return assert.equal(interceptor.isRateLimited(null), false);
        });
      ...
      describe(`#getApiName`, () => {
        it(`returns null when there is no regex match`, () => {
          return assert.equal(interceptor.getApiName(`https://example.com/testFake`), null);
        });
        it(`returns null when the argument is null`, () => {
          return assert.equal(interceptor.getApiName(null), null);
        });
        it(`returns idbroker API name if there is a match`, () => {
          return assert.equal(interceptor.getApiName(`https://idbroker.webex.com/horse/v1/myID`), `horse`);
        });
        it(`returns identity API name if there is a match`, () => {
          return assert.equal(interceptor.getApiName(`https://identity.webex.com/horse/v1/myID`), `horse`);
        });
      });

      describe(`#getRateLimitStatus`, () => {
        it(`returns false if API name is not in rate limit expiry map`, () => {
          return assert.equal(interceptor.getRateLimitStatus(`https://example.com/testFake`), false);
        });
      ...
      describe(`#setRateLimitExpiry`, () => {
        it(`returns false if URI results in API name that is null`, () => {
          return assert.equal(interceptor.setRateLimitExpiry(`https://example.com/testFake`, 1), false);
        });
      ...
      describe(`#extractRetryAfterTime`, () => {
        ...
        it(`returns 60K milliseconds when retry-after <= 0S`, () => {
          return assert.equal(interceptor.extractRetryAfterTime({
            headers: {'retry-after': -1}}), 60 * milliMultiplier);
        });
        it(`returns 60K milliseconds when retry-after is null`, () => {
          return assert.equal(interceptor.extractRetryAfterTime({
            headers: {'retry-after-missing': 10}}), 60 * milliMultiplier);
        });
        it(`returns 3600K milliseconds when retry-after is > 3600S`, () => {
          return assert.equal(interceptor.extractRetryAfterTime({
            headers: {'retry-after': 7200}}), 3600 * milliMultiplier);
        });
        it(`returns retry-after * 1000 (converts to milliseconds) if 0S < retry-after < 3600S`, () => {
          return assert.equal(interceptor.extractRetryAfterTime({
            headers: {'retry-after': 55}}), 55 * milliMultiplier);
        });
      });
      ...
      describe(`#onResponseError`, () => {
        ...
        it(`Stores API name and retry-after when status code is 429 and URI is idbroker`, () => {
          return interceptor.onResponseError(optionsIdBroker, err429).catch((resp) => {
            assert.equal(interceptor.isRateLimited(`https://idbroker.webex.com/horse/v1/myID`), true);
            assert.equal(resp, err429);
          });
        });
        it(`Stores API name and retry-after when status code is 429 and URI is identity`, () => {
          return interceptor.onResponseError(optionsIdentity, err429identity).catch((resp) => {
        ...
        it(`Does not store API name and retry-after when URI is idbroker and status code is not 429`, () => {
          return interceptor.onResponseError(optionsIdBroker, err404).catch((resp) => {
        ...
        it(`Does not store API name and retry-after when URI is identity and status code is not 429`, () => {
          return interceptor.onResponseError(optionsIdentity, err404).catch((resp) => {
        ...
        it(`Does not store API name and retry-after when status is 429 and URI is not idbroker`, () => {
          return interceptor.onResponseError(optionsNotIdBroker, err429notIdBroker).catch((resp) => {
