    ...
    describe('RateLimitInterceptor', () => {
      ...
      describe('#isRateLimited', () => {
        it('returns false when previously rate limited idbroker API expiry time has been met', () => {
          interceptor.setRateLimitExpiry(`${process.env.IDBROKER_BASE_URL || 'https://idbroker.webex.com'}/horse/v1/myID`, 1);
          clock.tick(2000);
          return assert.equal(interceptor.isRateLimited(`${process.env.IDBROKER_BASE_URL || 'https://idbroker.webex.com'}/horse/v1/myID`), false);
        });
        it('returns false when previously rate limited identity API expiry time has been met', () => {
          interceptor.setRateLimitExpiry(`${process.env.IDENTITY_BASE_URL || 'https://identity.webex.com'}/horse/v1/myID`, 1);
          clock.tick(2000);
          return assert.equal(interceptor.isRateLimited(`${process.env.IDENTITY_BASE_URL || 'https://identity.webex.com'}/horse/v1/myID`), false);
        });
        ...
        it('returns true when idbroker API is rate limited', () => {
          interceptor.setRateLimitExpiry(`${process.env.IDBROKER_BASE_URL || 'https://idbroker.webex.com'}/horse/v1/myID`, new Date().getTime() * 2);
          return assert.equal(interceptor.isRateLimited(`${process.env.IDBROKER_BASE_URL || 'https://idbroker.webex.com'}/horse/v1/myID`), true);
        });
        it('returns true when identity API is rate limited', () => {
          interceptor.setRateLimitExpiry(`${process.env.IDENTITY_BASE_URL || 'https://identity.webex.com'}/horse/v1/myID`, new Date().getTime() * 2);
          return assert.equal(interceptor.isRateLimited(`${process.env.IDENTITY_BASE_URL || 'https://identity.webex.com'}/horse/v1/myID`), true);
        });
      });
      ...
      describe('#getApiName', () => {
        ...
        it('returns idbroker API name if there is a match', () => assert.equal(interceptor.getApiName(`${process.env.IDBROKER_BASE_URL || 'https://idbroker.webex.com'}/horse/v1/myID`), 'horse'));
        it('returns identity API name if there is a match', () => assert.equal(interceptor.getApiName(`${process.env.IDENTITY_BASE_URL || 'https://identity.webex.com'}/horse/v1/myID`), 'horse'));
      });
      ...
      describe('#getRateLimitStatus', () => {
        ...
        it('returns false if idbroker API name is not rate limited', () => {
          interceptor.setRateLimitExpiry(`${process.env.IDBROKER_BASE_URL || 'https://idbroker.webex.com'}/horse/v1/myID`, 1);
          clock.tick(2000);
          return assert.equal(interceptor.getRateLimitStatus(`${process.env.IDBROKER_BASE_URL || 'https://idbroker.webex.com'}/horse/v1/myID`), false);
        });
        it('returns false if identity API name is not rate limited', () => {
          interceptor.setRateLimitExpiry(`${process.env.IDENTITY_BASE_URL || 'https://identity.webex.com'}/horse/v1/myID`, 1);
          clock.tick(2000);
          return assert.equal(interceptor.getRateLimitStatus(`${process.env.IDENTITY_BASE_URL || 'https://identity.webex.com'}/horse/v1/myID`), false);
        });
        it('returns true if idbroker the API name is rate limited', () => {
          const future = (new Date().getTime()) * 2;
          interceptor.setRateLimitExpiry(`${process.env.IDBROKER_BASE_URL || 'https://idbroker.webex.com'}/horse/v1/myID`, future);
          return assert.equal(interceptor.getRateLimitStatus(`${process.env.IDBROKER_BASE_URL || 'https://idbroker.webex.com'}/horse/v1/myID`), true);
        });
        it('returns true if the identity API name is rate limited', () => {
          const future = (new Date().getTime()) * 2;
          interceptor.setRateLimitExpiry(`${process.env.IDENTITY_BASE_URL || 'https://identity.webex.com'}/horse/v1/myID`, future);
          return assert.equal(interceptor.getRateLimitStatus(`${process.env.IDENTITY_BASE_URL || 'https://identity.webex.com'}/horse/v1/myID`), true);
        });
      });
      ...
      describe('#onRequest', () => {
        it('Rejects idbroker request if API is rate limited', () => {
          ...
          interceptor.setRateLimitExpiry(`${process.env.IDBROKER_BASE_URL || 'https://idbroker.webex.com'}/horse/v1/myID`, future);
          return interceptor.onRequest({uri: `${process.env.IDBROKER_BASE_URL || 'https://idbroker.webex.com'}/horse/v1/myID`}).catch((err) => {
        ...
        it('Rejects identity request if API is rate limited', () => {
          const future = (new Date().getTime() / 1000) * 2;
          interceptor.setRateLimitExpiry(`${process.env.IDENTITY_BASE_URL || 'https://identity.webex.com'}/horse/v1/myID`, future);
          return interceptor.onRequest({uri: `${process.env.IDENTITY_BASE_URL || 'https://identity.webex.com'}/horse/v1/myID`}).catch((err) => {
            assert.notCalled(spark.request);
        ...
        it('Does not reject idbroker request if API is not rate limited', () => {
          const options = {uri: `${process.env.IDBROKER_BASE_URL || 'https://idbroker.webex.com'}/horse/v1/myID`};
        ...
        it('Does not reject identity request if API is not rate limited', () => {
          const options = {uri: `${process.env.IDENTITY_BASE_URL || 'https://identity.webex.com'}/horse/v1/myID`};
          return interceptor.onRequest(options).then((result) => {
            assert.equal(result, options);
          });
        });
      ...
      describe('#onResponseError', () => {
        const err429 = new SparkHttpError.TooManyRequests({
          ...
          options: {
            ...
            uri: `${process.env.IDBROKER_BASE_URL || 'https://idbroker.webex.com'}/horse/v1/myID`
          },
          body: {
        ...
        const err429identity = new SparkHttpError.TooManyRequests({
          ...
          options: {
            ...
            uri: `${process.env.IDENTITY_BASE_URL || 'https://identity.webex.com'}/horse/v1/myID`
          },
          body: {
            error: 'Too Many Requests'
          }
        ...
        it('Stores API name and retry-after when status code is 429 and URI is idbroker', () => interceptor.onResponseError(optionsIdBroker, err429).catch((resp) => {
          assert.equal(interceptor.isRateLimited(`${process.env.IDBROKER_BASE_URL || 'https://idbroker.webex.com'}/horse/v1/myID`), true);
        ...
        it('Stores API name and retry-after when status code is 429 and URI is identity', () => interceptor.onResponseError(optionsIdentity, err429identity).catch((resp) => {
          assert.equal(interceptor.isRateLimited(`${process.env.IDENTITY_BASE_URL || 'https://identity.webex.com'}/horse/v1/myID`), true);
        ...
        it('Does not store API name and retry-after when URI is idbroker and status code is not 429', () => interceptor.onResponseError(optionsIdBroker, err404).catch((resp) => {
          assert.equal(interceptor.isRateLimited(`${process.env.IDBROKER_BASE_URL || 'https://idbroker.webex.com'}/horse/v1/myID`), false);
        ...
        it('Does not store API name and retry-after when URI is identity and status code is not 429', () => interceptor.onResponseError(optionsIdentity, err404).catch((resp) => {
          assert.equal(interceptor.isRateLimited(`${process.env.IDENTITY_BASE_URL || 'https://identity.webex.com'}/horse/v1/myID`), false);
          assert.equal(resp, err404);
        }));
