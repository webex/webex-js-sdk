...
describe(`spark-core`, function() {
  ...
  describe(`Spark`, () => {
    ...
    describe(`#logout()`, () => {
      ...
      it(`clears all stores`, () => assert.isFulfilled(spark.boundedStorage.get(`Credentials`, `@`))
        .then((data) => {
      ...
      it(`executes logout actions in the correct order`, () => assert.isFulfilled(spark.boundedStorage.get(`Credentials`, `@`))
        .then((data) => {
          assert.isDefined(data.supertoken);
          assert.equal(data.supertoken.access_token, `AT`);
          return spark.logout();
