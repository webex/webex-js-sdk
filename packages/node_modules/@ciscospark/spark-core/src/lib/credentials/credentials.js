...
import url from 'url';
import {base64, oneFlight, tap, whileInFlight} from '@ciscospark/common';
...
const Credentials = SparkPlugin.extend({
  ...
  buildLoginUrl(options = {}) {
    ...
    if (!options.response_type) {
      // eslint-disable-next-line camelcase
      options.response_type = this.config.clientType === `public` ? `token` : `code`;
    }

    options.state = base64.toBase64Url(querystring.stringify(options.state));
    return `${this.config.authorizationUrl}${querystring.stringify(options)}`;
  },
  ...
  initialize() {
    this.listenToOnce(this.parent, `change:config`, () => {
      if (this.config.authorizationString) {
        ...
        this.config.authorizationUrl = parsed.href.substr(0, parsed.href.indexOf(`?`));
   ...
   */
  invalidate() {
    this.logger.info(`credentials: invlidating tokens`);
    ...
    if (supertoken) {
      promises.push(supertoken.revoke()
        .catch((reason) => this.logger.warn(`credentials: failed to reokve supertokn`, reason)));
    }

    // TODO SparkCore needs to clear all stores and fire an event
