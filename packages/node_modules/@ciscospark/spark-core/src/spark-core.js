...
const SparkCore = AmpState.extend({
  ...
  logout(options, ...rest) {
    ...
    // to the access token
    const token = this.credentials.supertoken.refresh_token || this.credentials.supertoken.access_token;
    ...
    return this.config.onBeforeLogout.reverse().reduce((promise, {plugin, fn}) => promise.then(() => Reflect.apply(fn, this[plugin] || this.internal[plugin], [options, ...rest])), Promise.resolve())
      .then(() => Promise.all([
