...
import {inBrowser, proxyEvents, retry, transferEvents} from '@ciscospark/common';
import {HttpStatusInterceptor, defaults as requestDefaults} from '@ciscospark/http-core';
import {defaults, get, has, isArray, isFunction, isString, last, merge, omit, set, unset} from 'lodash';
...
const SparkCore = AmpState.extend({
  ...
  inspect(depth) {
    return util.inspect(omit(this.serialize({
      ...
      session: true,
      derived: true
    }), `boundedStorage`, `unboundedStorage`, `request`), {depth});
  },
   ...
   * interested in {@link `spark.mercury.disconnect()`| Mercury#disconnect()}
   * and {@link `spark.device.unregister()`|Device#unregister()}
   ...
   */
  logout(...args) {
    return this.config.onBeforeLogout.reduce((promise, {plugin, fn}) => promise.then(() => Reflect.apply(fn, this[plugin], args)), Promise.resolve())
      .then(() => Promise.all([
...
function makeSparkConstructor() {
  const def = {
    ...
    derived: Object.assign({}, derived, {
      /**
       * When true, indicates that all instance values can be trusted
       * @instance
       * @memberof SparkCore
       * @type {boolean}
       */
      ready: {
        deps: Object.keys(children).map((key) => `${key}.ready`).concat(`loaded`),
        fn() {
          return this.loaded && Object.keys(children).reduce((acc, key) => acc && this[key].ready, true);
        }
      }
    })
  };
 ...
 * @returns {Spark}
 */
export default function ProxySpark(attrs = {}, options) {
  if (!Spark) {
  ...
  const spark = new Spark(attrs, options);
  return spark;
}
...
export function registerPlugin(name, constructor, options) {
  ...
  if (!children[name] || options.replace) {
    ...
    if (options.onBeforeLogout) {
      ...
      if (isArray(options.onBeforeLogout)) {
        config.onBeforeLogout = config.onBeforeLogout.concat(options.onBeforeLogout);
      }
      else {
        config.onBeforeLogout.push(options.onBeforeLogout);
      }
    }
