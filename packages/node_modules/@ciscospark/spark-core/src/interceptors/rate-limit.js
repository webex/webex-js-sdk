  ...
  */
  ...
  onResponseError(options, reason) {
    if (reason.statusCode === 429 && (options.uri.includes(`idbroker`) || options.uri.includes(`identity`))) {
      ...
      this.setRateLimitExpiry(options.uri, this.extractRetryAfterTime(options) * 1000);
    }
  ...
  getRateLimitStatus(uri) {
    ...
    const currTimeSec = new Date().getTime();
    const dict = rateLimitExpiryTime.get(this);
    return dict.get(apiName) === null ? false : currTimeSec < dict.get(apiName);
  }
