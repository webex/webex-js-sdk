...
// An object to hold current idbroker and identity base host names
// while keeping around what the default was
const CIConfig = AmpState.extend({
  props: {
    url: 'string',
    defaultUrl: 'string'
  }
...
// The credentials info which includes the CI server info and derived
// URL properties from them for the various CI services.  The URL's
// will be updated when the CI changes.
const CredentialInfo = AmpState.extend({
...
export default {
  ...
  credentials: new CredentialInfo({
    idbroker: {url: IDBROKER_BASE_URL, defaultUrl: IDBROKER_BASE_URL},
    identity: {url: process.env.IDENTITY_BASE_URL, defaultUrl: process.env.IDENTITY_BASE_URL}
  }),
  device: {
