/**!
 *
 * Copyright (c) 2015-2017 Cisco Systems, Inc. See LICENSE file.
 */

import '@ciscospark/internal-plugin-presence';

import {assert} from '@ciscospark/test-helper-chai';
import CiscoSpark from '@ciscospark/spark-core';
import testUsers from '@ciscospark/test-helper-test-users';

describe(`plugin-presence`, function() {
  this.timeout(60000);
  describe(`Presence`, () => {
    let mccoy, spock;

    before(`create users`, () => testUsers.create({count: 2})
      .then((users) => {
        [spock, mccoy] = users;
        spock.spark = new CiscoSpark({
          credentials: {
            authorization: users[0].token
          }
        });
        mccoy.spark = new CiscoSpark({
          credentials: {
            authorization: users[1].token
          }
        });
      })
    );

    before(`register with wdm`, () => Promise.all([
      spock.spark.internal.device.register(),
      mccoy.spark.internal.device.register()
    ]));


    describe(`#get()`, () => {
      it(`gets a person's status by id`, () => {
        return spock.spark.internal.presence.get(mccoy.id)
          .then((presenceResponse) => {
            assert.property(presenceResponse, mccoy.id);
          });
      });
    });

    describe(`#list()`, () => {
      it(`returns an object with the status of all requested people`, () => {
        return spock.spark.internal.presence.list([mccoy.id, spock.id])
          .then((presenceResponse) => {
            assert.isAbove(Object.keys(presenceResponse).length, 0);
            assert.property(presenceResponse, mccoy.id);
            assert.property(presenceResponse, spock.id);
          });
      });
    });

    describe(`#subscribe`, () => {
      it(`subscribes to and returns current status of a person id`, () => spock.spark.internal.presence.subscribe(mccoy.id)
        .then((presenceResponse) => assert.isDefined(presenceResponse[mccoy.id]))
      );
    });

    describe(`#subscribeList`, () => {
      it(`subscribes and returns current status of a list of people ids`, () =>
        spock.spark.internal.presence.subscribeList([mccoy.id, spock.id])
          .then((presenceResponse) => {
            assert.isAbove(Object.keys(presenceResponse).length, 0);
            assert.property(presenceResponse, mccoy.id);
            assert.property(presenceResponse, spock.id);
          })
      );
    });

    describe(`#unsubscribe`, () => {
      it(`unsubscribes to presence updates of a single person id`, () =>
        spock.spark.internal.presence.unsubscribe(mccoy.id)
          .then((res) => assert.statusCode(res, 200))
      );
    });

    describe(`#unsubscribeList`, () => {
      it(`unsubscribes to presence updates of a list of people ids`, () =>
        spock.spark.internal.presence.unsubscribeList([mccoy.id, spock.id])
          .then((res) => assert.statusCode(res, 200))
      );
    });

    describe(`#setStatus`, () => {
      it(`sets the presence status of the current user`, () =>
        spock.spark.internal.presence.setStatus(`dnd`, 1500)
          .then((statusResponse) => {
            assert.property(statusResponse, spock.id);
            assert.equal(statusResponse[spock.id].status, `dnd`);
          })
      );
    });
  });
});
