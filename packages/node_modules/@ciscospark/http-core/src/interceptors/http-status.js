...
export default class HttpStatusInterceptor extends Interceptor {
  ...
  onResponse(options, response) {
    if (response.statusCode && (response.statusCode < 400 || (response.statusCode === 404 && response.options && response.options.qs && response.options.qs.alternateRedirect))) { // to handle locus redirects
