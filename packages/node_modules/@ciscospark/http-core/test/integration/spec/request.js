...
import {setRateLimitExpiry} from '@ciscospark/http-core/src/request/request';
...
describe(`http-core`, function() {
  ...
  describe(`request()`, () => {
    describe(`GET`, () => {
      ...
      it(`fails with 429 response when URI is rate limited`, () => {
        // set the expiry for the API "horse" to one hour in the future
        setRateLimitExpiry(makeLocalUrl(`/idbroker.webex.com/horse/param/one`), (new Date().getTime() / 1000) + 3600);
        // expect that request returns a 429 indicating rate limiting
        return request({uri: makeLocalUrl(`/idbroker.webex.com/horse/param/one`)})
          .catch((res) => {
            assert.statusCode(res, 429);
            assert.isString(res.body);
            assert.equal(res.body, `TooManyRequests`);
          });
      });
