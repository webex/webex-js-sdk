...
describe(`plugin-lyra`, () => {
  describe(`Spaces`, () => {
    ...
    describe(`#list()`, () => {
      it(`returns spaces`, () => assert.isFulfilled(spock.spark.lyra.spaces.list()));
    });
    ...
    describe(`#joinSpace()`, () => {
      it(`adds the current user to lyra space`, () => spock.spark.lyra.spaces.joinSpace(lyraMachine.space)
    ...
    describe(`#leaveSpace()`, () => {
      it(`allows the current user to leave lyra space`, () => spock.spark.lyra.spaces.joinSpace(lyraMachine.space)
          .then(() => spock.spark.lyra.spaces.leaveSpace(lyraMachine.space))
          .then(() => lyraMachine.spark.lyra.spaces.get(lyraMachine.space))
      ...
      describe(`when a user has multiple devices in the space`, () => {
        before(`add another device`, () => {
          ...
          return spock.spark2.device.register()
            .then(() => spock.spark.lyra.spaces.joinSpace(lyraMachine.space))
            .then(() => spock.spark2.lyra.spaces.joinSpace(lyraMachine.space));
        });

        it(`removes all devices from lyra space`, () => spock.spark.lyra.spaces.leaveSpace(lyraMachine.space, {
    ...
    describe(`#getAudioState`, () => {
      ...
      before(`put audio state`, () => {
        ...
        return lyraMachine.spark.lyra.spaces.putAudioState(lyraMachine.space, audioState);
      });

      it(`returns audio state`, () => lyraMachine.spark.lyra.spaces.getAudioState(lyraMachine.space)
          .then((res) => {
    ...
    describe(`#bindConversationToSpace()`, () => {
      before(`ensure participant joined space`, () => spock.spark.lyra.spaces.joinSpace(lyraMachine.space)
        .then(() => lyraMachine.spark.lyra.spaces.verifyOccupant(lyraMachine.space, spock.id))
        .then(() => spock.spark.lyra.spaces.bindConversationToSpace(lyraMachine.space, conversation)));

      after(`remove binding`, () => spock.spark.lyra.spaces.unbindConversationFromSpace(lyraMachine.space, conversation));

      it(`binds conversation and lyra space`, () => spock.spark.lyra.spaces.getSpaceBindings(lyraMachine.space)
        .then(({bindings}) => {
      ...
      describe(`when a call is in progress`, () => {
        ...
        it(`allows muting`, (done) => {
          ...
          return spock.spark.lyra.spaces.mute(lyraMachine.space);
        });
        ...
        it(`allows unmuting`, (done) => {
          ...
          return spock.spark.lyra.spaces.unmute(lyraMachine.space);
        });
        ...
        it(`allows increasing volume`, (done) => {
          ...
          return spock.spark.lyra.spaces.increaseVolume(lyraMachine.space);
        });
        ...
        it(`allows decreasing volume`, (done) => {
          ...
          return spock.spark.lyra.spaces.decreaseVolume(lyraMachine.space);
        });
        ...
        it(`allows setting volumne`, (done) => {
          ...
          return spock.spark.lyra.spaces.setVolume(lyraMachine.space, 2);
        });
    ...
    describe(`#unbindConversationFromSpace()`, () => {
      before(`ensure participant joined space`, () => spock.spark.lyra.spaces.joinSpace(lyraMachine.space)
        .then(() => lyraMachine.spark.lyra.spaces.verifyOccupant(lyraMachine.space, spock.id))
        .then(() => spock.spark.lyra.spaces.bindConversationToSpace(lyraMachine.space, conversation))
        .then(() => spock.spark.lyra.spaces.getSpaceBindings(lyraMachine.space))
        .then(({bindings}) => assert.lengthOf(bindings, 1)));

      it(`removes the binding between conversation and lyra space`, () => assert.isFulfilled(spock.spark.lyra.spaces.unbindConversationFromSpace(lyraMachine.space, conversation))
        .then(() => spock.spark.lyra.spaces.getSpaceBindings(lyraMachine.space))
