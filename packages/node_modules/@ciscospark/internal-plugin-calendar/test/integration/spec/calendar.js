...
function makeMockMeetingPayload(params) {
  ...
  console.info(`PAYLOAD: ${JSON.stringify(payload, null, 4)}`);
...
function postToWhistler(spark, type, payload) {
  return retry(() => spark.request({
    ...
    qs: {
      changeType: type,
      useProduction: typeof process.env.WDM_SERVICE_URL === `undefined` || process.env.WDM_SERVICE_URL.indexOf(`wdm-a.wbx2.com`) >= 0
    }
  })
    .then((res) => {
      // Test response is using our parameters
      console.info(`WHISTLER RESPONSE\n${JSON.stringify(res, null, 4)}`);
          ...
          .then((res) => {
            ...
            return assert.isFulfilled(mercuryPromise)
              .then((calendarMeetingExternal) => {
                console.info(`MERCURY MESSAGE\n${JSON.stringify(calendarMeetingExternal, null, 4)}`);
