...
const RealtimeChannel = Mercury.extend({
  ...
  send(data) {
    ...
    console.log('board realtime: send', this.channelId);
  ...
  _attemptConnection(callback) {
    ...
    console.log('board realtime: connect to', this.socketUrl);
    Promise.all([this._prepareUrl(this.socketUrl), this.spark.credentials.getAuthorization()])
      .then(([webSocketUrl, authorization]) => socket.open(webSocketUrl, {
