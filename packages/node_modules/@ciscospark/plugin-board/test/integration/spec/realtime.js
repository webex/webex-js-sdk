...
describe(`plugin-board`, () => {
  describe(`realtime`, () => {
    ...
    before(`connect to realtime channel`, () => {
      return Promise.all(map(participants, (participant) => {
        return participant.spark.internal.board.realtime.connectByOpenNewMercuryConnection(board);
      }));
    });
    ...
    after(`disconnect realtime`, () => Promise.all(map(participants, (participant) => {
      ...
      if (participant.spark.internal.board.realtime.connected) {
        return participant.spark.internal.board.realtime.disconnect()
          .then(() => {
            participant.spark.internal.board.realtime.set({boardWebSocketUrl: ``});
            participant.spark.internal.board.realtime.set({boardBindings: []});
          });
      }
      return true;
    })));
    ...
    describe(`#config`, () => {
      ...
      it(`shares board values`, () => {
        // board values
        assert.isDefined(participants[0].spark.internal.board.realtime.config.pingInterval);
        assert.isDefined(participants[0].spark.internal.board.realtime.config.pongTimeout);
        assert.isDefined(participants[0].spark.internal.board.realtime.config.forceCloseDelay);

        // mercury values not defined in board
        assert.isUndefined(participants[0].spark.internal.board.realtime.config.backoffTimeReset);
        assert.isUndefined(participants[0].spark.internal.board.realtime.config.backoffTimeMax);
      });
    });
    ...
    describe(`#publish()`, () => {
      describe(`string payload`, () => {
        ...
        it(`posts a message to the specified board`, (done) => {
          ...
          participants[1].spark.internal.board.realtime.once(`event:board.activity`, ({data}) => {
          ...
          assert.isTrue(participants[0].spark.internal.board.realtime.connected, `participant 0 is connected`);
          assert.isTrue(participants[1].spark.internal.board.realtime.connected, `participant 1 is connected`);

          // do not return promise because we want done() to be called on
          // board.activity
          participants[0].spark.internal.board.realtime.publish(board, data);
        });
      });
      ...
      describe(`file payload`, () => {
        ...
        it(`uploads file to spark files which includes loc`, () => {
          return participants[1].spark.internal.board._uploadImage(board, fixture)
            .then((scr) => {
        ...
        it(`posts a file to the specified board`, (done) => {
          ...
          assert.isTrue(participants[0].spark.internal.board.realtime.connected, `participant 0 is connected`);
          assert.isTrue(participants[1].spark.internal.board.realtime.connected, `participant 1 is listening`);
