...
const RealtimeService = SparkPlugin.extend({
  ...
  createRealtimeChannel(channel) {
    ...
    const bindingObj = {requestBindings};
  ...
  connectByOpenNewMercuryConnection(channel) {
    ...
    if (!realtimeChannel) {
      promise.then(() => this.createRealtimeChannel(channel))
        .then((rc) => {
          realtimeChannel = rc;
        });
    }
    ...
    return promise
      .then(() => this.realtimeChannels.get(channel.channelId).connect());
  },
  ...
  disconnectFromSharedMercury(channel) {
    ...
    if (!realtimeChannel.isSharingMercury && realtimeChannel.socket && realtimeChannel.connected) {
      promise = realtimeChannel.disconnect();
    }
    else {
      promise = this.spark.internal.board.unregisterFromSharedMercury(channel, realtimeChannel.binding);
    }
