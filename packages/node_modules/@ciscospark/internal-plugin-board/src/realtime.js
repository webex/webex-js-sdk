 ...
 * @memberof Board
 */
const RealtimeService = SparkPlugin.extend({
  namespace: `Board`,
  ...
  publishEncrypted(channel, encryptedPayloadAndKeyUrl, contentType) {
    ...
    if (!realtimeChannel) {
      Promise.reject(`Realtime Channel not found`);
    }
  ...
  connectByOpenNewMercuryConnection(channel) {
    ...
    return promise
      .then(() => realtimeChannel.connect());
  },

  disconnectMercuryConnection(channel) {
    const realtimeChannel = this.realtimeChannels.get(channel.channelId);
    if (!realtimeChannel) {
      return Promise.reject(`Realtime Channel not found!`);
    }

    return realtimeChannel.disconnect()
      .then(tap(() => {
        this.realtimeChannels.remove(channel.channelId);
      }));
  },
  ...
  connectToSharedMercury(channel) {
    return this.spark.board.registerToShareMercury(channel)
      ...
      .then(tap(({sharedWebSocket}) => {
        const realtimeChannel = this.realtimeChannels.get(channel.channelId);
        if (!sharedWebSocket) {
          console.log('board realtime: socket cannot be shared, open new socket...');
          return realtimeChannel.connect();
        }

        realtimeChannel.isSharingMercury = true;
        realtimeChannel.socket = this.spark.internal.mercury.socket;
        this.spark.internal.mercury.on(`event:board.activity`, this.handleBoardActivityMessages.bind(this));
  ...
  disconnectFromSharedMercury(channel) {
    ...
    return this.spark.internal.board.unregisterFromSharedMercury(channel, realtimeChannel.binding)
      .then(tap(() => {
        this.realtimeChannels.remove(channel.channelId);
      }));
  }
