  ...
  describe('realtime', () => {
    ...
    describe(`#disconnectMercuryConnection`, () => {
      it(`disconnects the mercury connection`, () => {
        ...
        return assert.isFulfilled(spark.internal.board.realtime.disconnectMercuryConnection(channel))
          .then(() => {
    ...
    describe(`on errors`, () => {
      ...
      it(`submits connection error to metric`, () => {
        ...
        return assert.isRejected(spark.internal.board.realtime.connectByOpenNewMercuryConnection(channel));
      });
      ...
      it(`rejects on BadRequest`, () => {
        spark.internal.board.config.maxRetries = 1;
        spark.credentials.getUserToken.returns(Promise.reject(new BadRequest()));
        return assert.isRejected(spark.internal.board.realtime.connectByOpenNewMercuryConnection(channel));
      });

      it(`rejects on Forbidden`, () => {
        spark.internal.board.config.maxRetries = 1;
        spark.credentials.getUserToken.returns(Promise.reject(new Forbidden()));
        return assert.isRejected(spark.internal.board.realtime.connectByOpenNewMercuryConnection(channel));
      });
    });
