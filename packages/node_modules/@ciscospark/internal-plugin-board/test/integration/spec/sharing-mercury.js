...
describe(`plugin-board`, () => {
  describe(`realtime - sharing mercury`, () => {
    ...
    beforeEach(`connect to mercury channel`, () => Promise.all(Array.map(participants, (participant) => {
    ...
    describe(`#publish()`, () => {
      describe(`string payload`, () => {
        ...
        after(() => Promise.all(Array.map(participants, (participant) => {
        ...
        it(`posts a message from shared connection to the specified board`, () => {
          ...
          return maxWaitForEvent(2000, `event:board.activity`, mccoyRealtimeChannel)
            .then(({data}) => {
        ...
        it(`posts a message from separated socket connection to the specified board`, () => {
          ...
          return maxWaitForEvent(2000, `event:board.activity`, spockRealtimeChannel)
            .then(({data}) => {
              assert.equal(data.contentType, `STRING`);
              assert.equal(data.payload.msg, uniqueRealtimeData);
            });
    ...
    describe(`multiple boards sharing a connection`, () => {
      ...
      afterEach(() => Promise.all(Array.map(participants, (participant) => {
        return promiseSeries([
