...
describe('plugin-board', () => {
  describe.skip('realtime - sharing mercury', () => {
    ...
    // disconnect realtime
    afterEach('disconnect mercury', () => Promise.all(map(participants, (participant) => participant.spark.internal.mercury.disconnect())));

    describe('#connectToSharedMercury', () => {
      it('registers to share mercury connection', () => participants[0].spark.internal.board.realtime.connectToSharedMercury(board)
        .then((res) => {
          ...
          assert.deepEqual(res.mercuryConnectionServiceClusterUrl, participants[0].spark.internal.mercury.localClusterServiceUrls.mercuryConnectionServiceClusterUrl);
        }));
    });

    describe('#disconnectFromSharedMercury', () => {
      it('removes board binding from registration', () => participants[0].spark.internal.board.realtime.disconnectFromSharedMercury(board)
        .then((res) => {
          assert.property(res, 'action');
          assert.property(res, 'binding');
          assert.property(res, 'webSocketUrl');
    ...
    describe('#publish()', () => {
      describe('string payload', () => {
        ...
        after(() => Promise.all(map(participants, (participant) => {
          if (participant.spark.internal.board.realtime.isSharingMercury) {
        ...
        it('posts a message from shared connection to the specified board', (done) => {
          ...
          assert.isTrue(participants[0].spark.internal.board.realtime.isSharingMercury, 'participant 0 is sharing mercury connection');
          assert.isTrue(participants[0].spark.internal.mercury.connected, 'participant 0 is connected');
          assert.isFalse(participants[1].spark.internal.board.realtime.isSharingMercury, 'participant 1 does not share mercury connection');
