...
const ReportGenerator = SparkPlugin.extend({
  ...
  initialize(attrs, options) {
    ...
    if (!attrs || !attrs.spark) {
      throw new Error('Missing Required Argument spark');
    }

    this.reportId = attrs.reportId;
    this.spark = attrs.spark;
  },
  ...
  map(activity) {
    ...
    const mime = new MimeBuilder('multipart/mixed')
      .setHeader('From', 'Mr Todo <to@do.com>') // TODO: activity.actorId is now a UUID which cannot be parsed by MimeBuilder
  ...
  decrypt(activities) {
    ...
    for (let i = 0; i < activities.length; i += 1) {
      ...
      if (activity.verb === 'post' && activity.encryptionKeyUrl) {
        const promise = this.spark.internal.encryption.decryptText(activity.encryptionKeyUrl, activity.objectDisplayName, activity.actorId)
  ...
  add(activities) {
    ...
    this.decrypt(activities).then((activities) => {
      if (!this.zip) {
        this.logger.info('Creating Zip');
        this.zip = new JSZip();
