...
const Ediscovery = SparkPlugin.extend({
    ...
    for (let i = 0; i < activities.length; i += 1) {
      ...
      if (activity.verb === 'post' && activity.objectDisplayName && activity.encryptionKeyUrl) {
        ...
        for (let i = 0; i < activity.files.length; i += 1) {
          ...
          if (file && file.scr) {
            const filePromise = this.spark.internal.encryption.decryptText(activity.encryptionKeyUrl, file.displayName, activity.actorId)
              .then((decryptedFileName) => {
                this.logger.info(`Decrypted file name: ${decryptedFileName}`);
                return this._downloadDecryptedFile(activity.encryptionKeyUrl, file.scr, activity.actorId)
                  .then((decryptedFile) => {
                    file.content = decryptedFile;
              ...
              .catch((error) => {
                this.logger.error(`Unable to decrypt file name due to error: ${error}`);
                return activity;
              });
  ...
  _downloadDecryptedFile(key, scr, actorId) {
    return this.spark.internal.encryption.decryptScr(key, scr, actorId)
      ...
      .catch((error) => {
        this.logger.error('Scr decryption error: ' + error);
        return error;
      });
  }
});
