...
const Ediscovery = SparkPlugin.extend({
  ...
  @waitForValue('@')
  getContent(reportId, offset = 0, size = 10) {
    if (!reportId) {
      throw new Error('Invalid parameter(s)');
    }
  ...
  generateReport(reportId) {
    ...
    this.getReport(reportId)
      .then((reportResponse) => {
        ...
        // const activityCount = reportResponse.body.contentCounter.activityCount;
        const size = 1000;
        const zip = new JSZip();
        // Generate a directory within the Zip file structure
        const convFolder = zip.folder(reportId + '/conversation');

        const offset = 0;
        // for (let offset = 0; offset < activityCount; offset += size) {
        this.getContent(reportId, offset, size).then((response) => {
          const blob = new Blob([JSON.stringify(response, null, 4)], {type: 'application/json'});
          convFolder.file(offset + '_activity.json', blob);
