  ...
  generateReport(reportId) {
    ...
    return this.getReport(reportId)
      .then((reportResponse) => {
        ...
        for (let offset = 0; offset < activityCount; offset += numberToRetrieve) {
          const promise = this.getPagedContent(reportId, offset, numberToRetrieve).then((response) => {
            ...
            if (activities && Array.isArray(activities) && activities.length) {
              ...
              // TODO: Temporary usage of the getKey(..) api. Will be removed and replaced by decryption of the activity.
              const activity = activities[0];
              if (activity && activity.encryptionKeyUrl) {
                this.spark.internal.encryption.getKey(activity.encryptionKeyUrl, activity.actorId)
                  .then((result) => {
        ...
        return Promise.all(promises).then(() => reportGenerator.saveAs());
      })
