...
const Ediscovery = SparkPlugin.extend({
        ...
        function decryptNextActivities(plugin) {
          return plugin.request({
            ...
            .then((response) => {
              ...
              return plugin._decryptActivities(activity);
            })
            .then((activities) => {
  ...
  _decryptActivities(activities) {
    ...
    for (let i = 0; i < activities.length; i += 1) {
      ...
      else if (activity.verb === 'share' && activity.files.length && activity.encryptionKeyUrl) {
        for (let i = 0; i < activity.files.length; i += 1) {
          this.logger.info('Files loop');
          const file = activity.files[i];
          if (file && file.scr) {
            const filePromise = this._downloadDecryptedFile(activity.encryptionKeyUrl, file.scr, activity.actorId)
              .then((decryptedFile) => {
                // TODO: Temporarily saving file, will need to attach to the EML for the activity instead
                const fileName = file.displayName + '.' + file.mimeType.split('/')[1];
                const jsonBlob = new Blob([decryptedFile], {type: file.mimeType});
                saveAs(jsonBlob, fileName);
                return activity;
              })
              .catch((error) => {
                this.logger.error(`Unable to decrypt file due to error: ${error}`);
                return activity;
              });
  ...
  _downloadDecryptedFile(key, scr, actorId) {
    return this.spark.internal.encryption.decryptScr(key, scr, actorId)
      .then((decryptedScr) => {
        this.logger.info('Decrypted scr: ' + JSON.stringify(decryptedScr, undefined, 2));
        return this.spark.internal.encryption.download(decryptedScr)
          .then((file) => {
            this.logger.info('Downloaded file: ' + JSON.stringify(file, undefined, 2));
            return file;
          }).catch((error) => {
      ...
      .catch((error) => {
        this.logger.error('decryptScr() error: ' + error);
        return error;
      });
  }
});
