      ...
      return plugin.request({
        ...
        .then((activities) => {
          if (activities && Array.isArray(activities) && activities.length > 0) {
            ...
            offset += size;
            return decryptNextActivities(plugin).then((nextActivities) => activities.concat(nextActivities));
          }
  ...
  _decryptActivities(activities) {
    ...
    for (let i = 0; i < activities.length; i += 1) {
      ...
      if (activity.verb === 'post' && activity.objectDisplayName && activity.encryptionKeyUrl) {
        const textPromise = this.spark.internal.encryption.decryptText(activity.encryptionKeyUrl, activity.objectDisplayName, {onBehalfOf: activity.actorId})
        ...
        for (let i = 0; i < activity.files.length; i += 1) {
          ...
          if (file && file.displayName && file.scr) {
            const filenamePromise = this.spark.internal.encryption.decryptText(activity.encryptionKeyUrl, file.displayName, {onBehalfOf: activity.actorId})
              .catch((reason) => {
                this.logger.error(`Unable to decrypt filename due to error: ${reason}`);
                return Promise.reject(reason);
              });
            const fileContentPromise = this.spark.internal.encryption.decryptScr(activity.encryptionKeyUrl, file.scr, {onBehalfOf: activity.actorId})
