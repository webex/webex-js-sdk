  ...
  generateReport(reportId) {
    ...
    return this.getReport(reportId)
      .then((reportResponse) => {
        ...
        const activityCount = 300000;// reportResponse.body.contentCounter.activityCount;
        const size = 1000;
        const generator = new ReportGenerator();

        return this.getContent(reportId, 0, size).then((response) => {
          const activities = [];
          for (let offset = 0; offset < activityCount; offset += 1) {
            activities.push(response);
          }

          generator.add(reportId, activities);

          return generator.downloadAsync(reportId);
        });
      })
      .catch((reason) => reason);
  }
