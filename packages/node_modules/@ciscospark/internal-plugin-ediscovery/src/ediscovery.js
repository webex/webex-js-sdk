...
const Ediscovery = SparkPlugin.extend({
  ...
  createReport(emails, keywords, spaceNames, spaceIds, range) {
    ...
    const keywordPromise = self._encryptKeywords(keywords);

    return Promise.all([uuidPromise, keywordPromise])
      .then((values) => {
        const body = reportRequest;
        body.userIds = values[0];
        body.keywords = values[1].encryptedKeywords;
        body.keywordEncryptionKeyUrl = values[1].encryptionkeyUrl;
  ...
  _encryptKeywords(keywords) {
    if (!keywords) {
      throw Error('Invalid parameter');
    }

    const encryptedKeywordInfo = {encryptedKeywords: [], encryptionkeyUrl: ''};
    if (keywords.length <= 0) {
      return Promise.resolve(encryptedKeywordInfo);
    }
    ...
    return this.spark.internal.encryption.kms.createUnboundKeys({count: 1})
      .then((keys) => {
        ...
        if (keys && keys.length > 0 && keys[0]) {
          encryptedKeywordInfo.encryptionkeyUrl = keys[0].uri;
          for (let i = 0; i < keywords.length; i += 1) {
            if (keywords[i]) {
              promises.push(this.spark.internal.encryption.encryptText(keys[0], keywords[i])
                .then((encryptedKeyword) => {
                  encryptedKeywordInfo.encryptedKeywords.push(encryptedKeyword);
                }));
            }
          }
        }
        return Promise.all(promises)
          .then(() => encryptedKeywordInfo);
      })
      .catch((reason) => {
        this.logger.error('Error encrypting keywords: ', reason);
        return Promise.reject(reason);
      });
  },

  _decryptKeywords(key, keywords) {
    if (!key || !keywords) {
      throw Error('Invalid parameter');
    }
    ...
    for (let i = 0; i < keywords.length; i += 1) {
      if (keywords[i]) {
        promises.push(this.spark.internal.encryption.decryptText(key, keywords[i])
          .then((encryptedKeyword) => {
            decryptedKeywords.push(encryptedKeyword);
          })
          .catch((reason) => {
            this.logger.error('Error decrypting keywords: ', reason);
            return Promise.reject(reason);
          }));
      }
    }
    ...
    return this.request({
      ...
      .then((res) => {
        ...
        if (res.body.reportRequest && res.body.reportRequest.keywordEncryptionKeyUrl && res.body.reportRequest.keywords) {
          return this._decryptKeywords(res.body.reportRequest.keywordEncryptionKeyUrl, res.body.reportRequest.keywords)
            .then((decryptedKeywords) => {
              res.body.reportRequest.keywords = decryptedKeywords;
              return res;
            });
        }
        return res;
      ...
      .then((res) => {
        ...
        for (let i = 0; i < res.body.length; i += 1) {
          if (res.body[i].reportRequest && res.body[i].reportRequest.keywordEncryptionKeyUrl && res.body[i].reportRequest.keywords) {
            promises.push(this._decryptKeywords(res.body[i].reportRequest.keywordEncryptionKeyUrl, res.body[i].reportRequest.keywords)
              .then((decryptedKeywords) => {
                res.body[i].reportRequest.keywords = decryptedKeywords;
              }));
          }
        }
        return Promise.all(promises)
