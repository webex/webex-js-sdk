...
const Ediscovery = SparkPlugin.extend({
  ...
  _decryptActivities(activities) {
    ...
    for (let i = 0; i < activities.length; i += 1) {
      ...
      else if (activity.verb === 'share' && activity.scr && activity.encryptionKeyUrl) {
        const filePromise = this._downloadDecryptedFile(activity.encryptionKeyUrl, activity.scr, activity.actorId)
          .then((decryptedFile) => {
            activity.file = decryptedFile;
            return activity;
          })
          .catch((error) => {
