...
describe('Ediscovery Content API Tests', () => {
  ...
  describe('Get Contents Tests', () => {
    it('getContentPagedSuccess', async () => {
      const result = spark.internal.ediscovery.getPagedContent(uuid, 0, 10).then((res) => {
        expect(res.statusCode).to.equal(200);
      });
      return result;
    });

    it('getContentWithNoParamFailure', async () => {
      const result = expect(spark.internal.ediscovery.getContent()).to.be.rejectedWith(Error, 'Invalid parameter(s)');
      return result;
    });
  });
