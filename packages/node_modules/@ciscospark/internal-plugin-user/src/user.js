...
const User = SparkPlugin.extend({
  ...
  verify(options) {
    ...
    // For Federation, there is only one Atlas, which we get the uesr's CI from
  ...
  /**
   * If the passed-in condition is true, retrieve the user's
   * CI and update config.
   * Otherwise, do nothing, leave currently configured CI.
   * @param {string} email (required)
   * @param {boolean} condition (required)
   * @returns {Promise<Object>}
   */
  getUserCI(email, condition) {
    let verifyPromise;
    if (condition) {
      ...
      verifyPromise = this.verify(verifyOptions).then((res) => Promise.resolve(res.userEntities));
    }
    else {
