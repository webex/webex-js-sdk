...
const User = SparkPlugin.extend({
  ...
  activate(options = {}) {
    ...
    // and Federation flag is enabled, call verify first to
    // get the user's CI, but suppress sending another email.
    // otherwise, do nothing for verify step.
    let verifyPromise;
    if (this.spark.config.credentials.federation && options.email && options.verificationToken) {
      const verifyOptions = {
        email: options.email,
        suppressEmail: true
      };
      verifyPromise = this.verify(verifyOptions);
    }
    else {
    ...
    return this.spark.credentials.getUserToken()
      ...
      .then((res) => {
        ...
        if (this.spark.config.credentials.federation) {
          if (res.body.userEntities.idBrokerUrl !== this.spark.config.credentials.idbroker.url) {
            this.spark.config.credentials.idbroker.url = res.body.userEntities.idBrokerUrl;
          }
          if (res.body.userEntities.identityUrl !== this.spark.config.credentials.identity.url) {
