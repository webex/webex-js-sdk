...
describe(`plugin-user`, function() {
  ...
  describe(`#setPassword()`, () => {
    it(`sets the user's password`, () => spark.internal.user.setPassword({userId: user1.id, password: `P@ssword123`})
  ...
  describe(`#activate()`, () => {
    ...
    it(`retrieves a valid user token and sets the password`, () => {
      ...
      return unauthSpark.internal.user.verify({email})
        ...
        .then((user) => unauthSpark.internal.user.setPassword({userId: user.id, password: `P@ssword123`}))
  ...
  describe(`#asUUID()`, () => {
    ...
    it(`maps an email address to a uuid`, () => assert.eventually.equal(spark.internal.user.asUUID(user2, {force: true}), user2.id));

    it(`maps an email address for a non-existent user to a fake uuid`, () => assert.eventually.match(spark.internal.user.asUUID(email), patterns.uuid)
    ...
    describe(`with {create: true}`, () => {
      ...
      it(`creates a new user`, () => assert.eventually.match(spark.internal.user.asUUID(email, {create: true}), patterns.uuid)
        .then(() => spark.internal.user.store.getByEmail(email))
        .then((u) => assert.isTrue(u.userExists, `User exists`)));

      it(`does not use a cached value if the previous value was marked as non-existent`, () => assert.eventually.match(spark.internal.user.asUUID(email), patterns.uuid)
          .then(() => spark.internal.user.store.getByEmail(email))
          .then((u) => assert.isFalse(u.userExists, `User does not exist`))
      ...
      it(`does not use a cached value if the previous value's existence is unknown`, () => spark.internal.user.recordUUID({
        ...
        .then(() => assert.eventually.equal(spark.internal.user.asUUID(user3.email, {create: true}), user3.id))
