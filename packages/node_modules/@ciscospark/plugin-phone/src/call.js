...
/**
 * Helper for finding a media share and setting up the appropriate event
 * listeners in case we need to wait for it to arrive
 *
 * @param {Call} call
 * @private
 * @returns {Function}
 */
function findMediaShare(call) {
...
function getMediaDirectionFromSDP(kind, sdp) {
  ...
  let media;
  if (kind === `screen`) {
    media = sdp.media.find((m) => m.type === `video` && m.content);
  }
  else {
    media = sdp.media.find((m) => m.type === kind);
  }
...
const Call = SparkPlugin.extend({
  ...
  _requestFloor() {
    // TODO skip if we already have the floor
    return Promise.resolve()
      .then(findMediaShare(this))
  ...
  stopShare() {
    // I'm told the floor will be release automatically by Calliope
    return new Promise((resolve) => {
      this.media.once(`answeraccepted`, resolve);
  ...
  _join(locusMethodName, target, options = {}) {
    ...
    return this.media.createOffer()
      ...
      .then(() => this.spark.internal.locus[locusMethodName](target, Object.assign({
  ...
  _pollForExpectedLocusAndSdp() {
    return new Promise((resolve, reject) => {
      ...
      const screenShareOfferDirection = getMediaDirectionFromSDP(`screen`, offer);
      ...
      function isExpectedDirection(sdp) {
        ...
        const screenShareAnswerDirection = getMediaDirectionFromSDP(`screen`, answer);
        ...
        const expectedScreenShareDirection = reverseMediaDirection(screenShareOfferDirection);

        self.logger.info(`audio direction: ${audioOfferDirection}, ${audioAnswerDirection}, ${expectedAudioAnswerDirection}`);
        self.logger.info(`video direction: ${videoOfferDirection}, ${videoAnswerDirection}, ${expectedVideoAnswerDirection}`);
        self.logger.info(`screen direction: ${screenShareOfferDirection}, ${screenShareAnswerDirection}, ${expectedScreenShareDirection}`);

        return audioAnswerDirection === expectedAudioAnswerDirection && videoAnswerDirection === expectedVideoAnswerDirection && screenShareAnswerDirection === expectedScreenShareDirection;
      }
    });
  },
