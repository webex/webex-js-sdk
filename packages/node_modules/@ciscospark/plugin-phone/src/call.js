...
const Call = SparkPlugin.extend({
  ...
  _changeReceivingMedia(kind, value) {
    return new Promise((resolve) => {
      ...
      const newDirection = boolToStatus(this[`sending${capitalize[kind]}`], value);
  ...
  _changeSendingMedia(kind, value) {
    return new Promise((resolve) => {
      ...
      if (!section || !section.direction.includes(`send`)) {
        ...
        this.once(`change:sending${capitalize[kind]}`, () => resolve(this._updateSendingMedia()));
      }

      const newDirection = boolToStatus(value, this[`receiving${capitalize[kind]}`]);
      this.media.setMedia(kind, newDirection);
    });
  },
  ...
  _join(locusMethodName, target, options = {}) {
    ...
    if (options.localMediaStream) {
      ...
      if (mediaOptions.offerOptions || options.constraints) {
        this.media.setMedia(`audio`, boolToStatus(mediaOptions.audio, mediaOptions.offerToReceiveAudio), mediaOptions.audio);
        this.media.setMedia(`video`, boolToStatus(mediaOptions.video, mediaOptions.offerToReceiveVideo), mediaOptions.video);
      }
    }
