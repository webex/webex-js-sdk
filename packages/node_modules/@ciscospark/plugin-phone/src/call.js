...
const Call = SparkPlugin.extend({
  ...
  onMediaNegotiationNeeded() {
    ...
    this.media.createOffer()
      ...
      .then(() => {
        let {sdp} = JSON.parse(this.mediaConnection.remoteSdp);

        const parsed = parse(sdp);
        // OnWhen adding a new track, locus returns an answer sdp that does not
        // have the BUNDLE line, causing a DOMException in Chrome. We splice in
        // a patch until there is a server side fix
        if (browser.chrome && !parsed.group && parsed.media && parsed.media.length > 0) {
          const media = parsed.media.map((l) => l.type);
          const insert = sdp.includes('a=ice-lite') ? 'a=ice-lite' : 't=0 0';
          sdp = sdp.replace(
            insert,
            `${insert}\r\na=group:BUNDLE ${media.join(' ')}`
          );
        }
