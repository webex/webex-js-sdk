...
/**
 * Turns an event into a wait-for-promise
 * @param {Emitter} context
 * @param {string} event
 * @returns {Promise}
 */
function when(context, event) {
  // TODO unbind race loser;
  return Promise.race([
    new Promise((resolve) => {
      context.once(event, resolve);
    }),
...
const Call = SparkPlugin.extend({
  ...
  initialize(...args) {
    ...
    this.listenTo(this.roapSession, 'transmit', (msg) => {
      if (!this.locus) {
        ...
        console.warn('cannot transmit roap message without first having a locus');
        return;
      }
  ...
  _join(locusMethodName, target, options = {}) {
    ...
    return this.media.createOffer()
      ...
      .then(() => {
        if (true) {
          const promise = when(this.roapSession, 'transmit');
          this.roapSession.transmitOffer([this.media.offerSdp]);
          return Promise.all([
            promise
              .then((roapMessage) => this.spark.internal.locus[locusMethodName](target, {
                correlationId: this.correlationId,
                roapMessage
              }))
              .then((locus) => this._setLocus(locus)),
            when(this.roapSession, 'receiveAnswer')
              .then((answer) => {
        ...
        return this.spark.internal.locus[locusMethodName](target, {
      ...
      .then((locus) => true || this._setLocus(locus))
  ...
  onLocusEvent(event) {
    if (event.data.eventType === 'locus.message.roap') {
      if (get(event, 'data.message.locusUrl') === this.locus.url || get(event, 'data.correlationId') === this.correlationId) {
        console.log();
