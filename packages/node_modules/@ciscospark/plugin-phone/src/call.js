...
import RoapSession from '@ciscospark/helper-roap';
import {
  base64,
  oneFlight,
  retry,
...
/**
 * Log helper
 * @param {SparkPlugin|Object|undefined} self
 * @param {string} level
 * @param {string} msg
 * @returns {Function}
 */
function logAndReject(self, level, msg) {
  return function wrapped(err) {
    const logger = (self && self.logger) || (self && self.spark && self.spark.logger) || console;
 ...
 * @returns {Function}
 */
function logAndAccept(self, level, msg) {
...
const Call = SparkPlugin.extend({
  ...
  initialize(...args) {
    ...
    this.listenTo(this.roapSession, 'transmit', (msg) => {
      if (!this.locus) {
        this.logger.error((new Error()).stack);
        this.logger.warn('cannot transmit roap message without first having a locus');
        return;
      }
      this.logger.info(`transmitting ROAP message ${msg}`);
      this.spark.internal.locus.transmitRoapMessage(this.locus, msg)
        .then(logAndAccept(this, 'info', 'transmitted ROAP message'))
        .catch(logAndReject(this, 'error', 'failed to transmit ROAP message'));
    });
    ...
    this.listenTo(this.media, 'negotiationneeded', () => this.onMediaNegotiationNeeded());
   ...
   *
   */
  join(methodName, target) {
    return this.media.createOffer()
      .then((sdp) => {
        const offerMessage = this.roapSession.transmitOffer(sdp);

        return Promise.all([
          this.spark.internal.locus[methodName](target, {
            correlationId: this.correlationId,
            roapMessage: offerMessage
          }),
          offerMessage.waitForAnswer()
        ]);
      })
      .then(([locus, answerMessage]) => this.setLocus(locus)
        .then(() => {
          if (this.media.ended) {
            return Promise.resolve();
          }

          return this.media.acceptAnswer(answerMessage.sdps[0])
            .then(logAndAccept(this, 'info', 'answer accepted'))
            .then(() => answerMessage.sendOk())
            .catch(logAndReject(this, 'error', 'failed to accept answer'));
        }));
  },
  ...
  /**
   * Joins a locus. Used by {@link Call#answer()} and {@link Call#call()}
   * @private
  ...
  _join(locusMethodName, target, options = {}) {
    ...
    return this.join(locusMethodName, target);
  },
   ...
   */
  onLocusEvent(event) {
    if (event.data.eventType === 'locus.message.roap') {
      if (get(event, 'data.message.locusUrl') === get(this, 'locus.url') || get(event, 'data.correlationId') === this.correlationId) {
        // console.log();
        // console.log('ROAPMESSAGEROAPMESSAGEROAPMESSAGEROAPMESSAGEROAPMESSAGEROAPMESSAGE');
        // console.log();
        // console.info(require('util').inspect(event.data, {depth: null}));
        // console.log();
        // console.log();
        this.roapSession.receive(event.data.message);
      }
      return;
    }
  ...
  onMediaNegotiationNeeded() {
    this.media.createOffer()
      .then((sdp) => {
        const offerMessage = this.roapSession.transmitOffer(sdp);
        return Promise.all([
          this.spark.internal.locus.transmitRoapMessage(this.locus, offerMessage),
          offerMessage.waitForAnswer()
        ]);
      })
      .then(([locus, answerMessage]) => this.media.acceptAnswer(answerMessage.sdps[0]))
   ...
   */
  updateMuteToggles() {
    return this.spark.internal.locus.updateMedia(this.locus, {
      sdp: this.media.pc.localDescription.sdp,
      mediaId: this.mediaId,
      audioMuted: this.sendingAudio, // didChange('audio', this.media.offerSdp, this.media.answerSdp) ? this.sendingAudio : !this.sendingAudio,
      videoMuted: this.sendingVideo // didChange('video', this.media.offerSdp, this.media.answerSdp) ? this.sendingVideo : !this.sendingVideo
    })
      .then((locus) => this.setLocus(locus))
