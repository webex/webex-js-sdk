...
const Call = SparkPlugin.extend({
  ...
  /**
   * Change the sending audio media state. Should not induce a renegoatiation,
   * unless tracks were inactive -> active
   * @instance
   * @memberof Call
   * @param {boolean} value
   * @private
   * @returns {Promise}
   */
  changeSendingAudio(value) {
    return new Promise((resolve) => {
      this.logger.info(`changeSendingAudio: changing sending audio to "${value}"`);
      if (value) {
        try {
          this.media.unpauseSendingMedia('audio');
        }
        catch (e) {
          // This will throw if there is nothing to unpause, but is expected
          if (e.message !== 'No audio media senders to unpause') {
            throw e;
          }
          // Since we have no audio to unpause, we have create one
          this.once('mediaNegotiationCompleted', () => resolve());
          const newDirection = boolToStatus(value, this.media.receivingAudio);
          this.media.setMedia('audio', newDirection);
          return;
        }
      }
      else {
        this.media.pauseSendingMedia('audio');
      }
      resolve(this.updateMuteToggles('audio', !value));
    });
  },
  ...
  changeSendingMedia(kind, value) {
    ...
    if (kind === 'audio') {
      return this.changeSendingAudio(value);
    }

    if (kind === 'video') {
      return this.changeSendingVideo(value);
    }
  ...
  /**
   * Change the sending video media state. Triggers a renegotiation.
   * @instance
   * @memberof Call
   * @param {boolean} value
   * @private
   * @returns {Promise}
   */
  changeSendingVideo(value) {
    return new Promise((resolve) => {
      this.logger.info(`changeSendingVideo: changing sending video to "${value}"`);
      if (value) {
        try {
          this.media.unpauseSendingMedia('video');
        }
        catch (e) {
          // This will throw if there is nothing to unpause, but is expected
          if (e.message !== 'No video media senders to unpause') {
            throw e;
          }
          // Since we have no video to unpause, we have create one
      this.once('mediaNegotiationCompleted', () => resolve());
      const newDirection = boolToStatus(value, this.media.receivingVideo);
      this.media.setMedia('video', newDirection);
          return;
        }
      }
      else {
        this.media.pauseSendingMedia('video');
      }
      resolve(this.updateMuteToggles('video', !value));
    });
    // return new Promise((resolve) => {
    //   this.logger.info(`changeSendingVideo: changing sending video to "${value}"`);
    //   this.once('mediaNegotiationCompleted', () => resolve());
  ...
  onMediaNegotiationNeeded() {
    ...
    this.media.createOffer()
      .then(() => {
        ...
        this.logger.info(`onMediaNegotiationNeeded: audioOfferDir=${audioOfferDir} videoOfferDir=${videoOfferDir}`);
   ...
   */
  startSendingAudio() {
    return this.changeSendingAudio(true);
  },
   ...
   */
  startSendingVideo() {
    return this.changeSendingVideo(true);
  },
   ...
   */
  stopSendingAudio() {
    return this.changeSendingAudio(false);
  },
   ...
   */
  stopSendingVideo() {
    return this.changeSendingVideo(false);
  },
  ...
  toggleFacingMode() {
    ...
    this.media.setMedia('video', boolToStatus(this.sendingAudio, this.sendingVideo), constraint);
    return new Promise((resolve) => {
   ...
   * @returns {Promise}
   */
  updateMuteToggles(kind, isMuted) {
    this.logger.info(`updating mute toggles: ${kind}=${!isMuted}`);
    const payload = {
      mediaId: this.mediaId
    };
    if (kind === 'audio') {
      payload.audioMuted = isMuted;
    }
    else if (kind === 'video') {
      payload.videoMuted = isMuted;
      payload.sdp = this.media.offerSdp;
    }
