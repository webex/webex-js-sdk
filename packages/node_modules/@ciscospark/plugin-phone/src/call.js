...
const Call = SparkPlugin.extend({
    ...
    state: {
      ...
      fn() {
        if (this.config.enableExperimentalGroupCallingSupport) {
          // Locus stops sending events when we disconnect from the wireless
          // share, so we never get the locus update that tells us the locus has
          // gone inactive.
          if (this.isWirelessShare && this.me.state === 'disconnected') {
            return 'inactive';
          }
  ...
  createOrJoinLocus(target, options = {}) {
    if (options.localMediaStream) {
      ...
      if (get(options, 'constraints.video.mediaSource') === 'screen' || get(options, 'constraints.video.mediaSource') === 'application') {
        this.media.setMedia('screen', 'sendonly', options.constraints.video);
      }
      else {
  ...
  leave() {
    ...
    let options;
    if (this.isWirelessShare) {
      const device = this.locus.self.devices.find((d) => get(d, 'intent.type') === 'OBSERVE');
      const associatedWith = device && get(device, 'intent.associatedWith');
      const usingResource = associatedWith && get(this.locus.participants.find((p) => p.url === associatedWith), 'identity');
      options = usingResource && {usingResource};
    }

    return this.spark.internal.locus.leave(this.locus, options)
      .catch((err) => {
