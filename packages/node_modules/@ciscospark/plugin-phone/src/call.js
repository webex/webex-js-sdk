...
const Call = SparkPlugin.extend({
  ...
  derived: {
    ...
    status: {
      ...
      fn() {
        return getState(this.spark, this.locus, this.previousAttributes().locus);
      }
    },
    /**
  ...
  initialize(...args) {
    ...
    this.memberships.listenToAndRun(this, `change:locus`, () => {
      ...
      if (this.locus && this.locus.participants) {
        ...
        // lot simpler
        this.memberships.set(this.locus.participants
          .filter((participant) => participant.type === `USER`)
          .map((participant) => ({
            personId: this.spark.people.inferPersonIdFromUuid(participant.person.id),
            isInitiator: this.direction === `out` ? participant.url === this.locus.self.url : participant.url !== this.locus.self.url,
        ...
        console.log(require(`util`).inspect(this.memberships.serialize(), {depth: null}));
      }
    });
  ...
  _onLocusEvent(event) {
    ...
    if (this.locus && makeInternalCallId(event.data.locus) === this.internalCallId || this.correlationId && this.correlationId === device.correlationId) {
