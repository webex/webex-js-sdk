...
const Call = SparkPlugin.extend({
  ...
  answer(options) {
    ...
    return this.createOrJoinLocus('join', this.locus, options)
  ...
  dial(invitee, options) {
    ...
    this.spark.internal.mercury.connect()
      .then(() => this.createOrJoinLocus(locusMethodName, invitee, options))
  ...
  initialize(...args) {
    ...
    if (this.config.enableExperimentalGroupCallingSupport) {
      ...
      this.on('disconnected', () => this.finish());
   ...
   */
  internalHangup() {
    this.logger.info('internalHangup: attempting to leave locus');
    ...
    this.logger.info('internalHangup: leaving locus');
    return this.spark.internal.locus.leave(this.locus)
      .catch((err) => {
        this.logger.error('internalHangup: locus leave error: ', err.stack || err.toString());
        return Promise.reject(err);
      })
      .then(tap(() => this.logger.info(`internalHangup ${this.locus.id}: finished leaving via locus`)))
      .then(tap(() => this.logger.info(`internalHangup ${this.locus.id}: setting locus`)))
      .then((locus) => this.setLocus(locus))
      .then(tap(() => this.logger.info(`internalHangup ${this.locus.id}: finished setting locus`)))
  ...
  /**
   * Poorly named. Does the cleanup after a call has ended
   * @instance
   * @memberof Call
   * @private
   * @returns {Promise}
   ...
   * @returns {Promise}
   */
  createOrJoinLocus(locusMethodName, target, options = {}) {
    // TODO remove locusMethodName
