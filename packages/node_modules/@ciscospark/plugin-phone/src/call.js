...
const Call = SparkPlugin.extend({
  ...
  derived: {
    ...
    supportsDtmf: {
      deps: [`locus`],
      fn() {
        return this.locus && this.locus.self && this.locus.self.enableDTMF;
      }
    }
  },
  ...
  _join(locusMethodName, target, options = {}) {
    ...
    return this.media.createOffer()
      ...
      .then((locus) => {
        ...
        // This is here to debug an edge case that I can't reproduce on demand
        if (!this.mediaConnection) {
          this.logger.info(`EDGE CASE`);
          this.logger.info(JSON.stringify(this.locus.self, null, 2));
          throw new Error(`Call#mediaConnection() is missing after calling Locus#${locusMethodName}(). Please check the logs for the string 'EDGE CASE' and supply the following JSON to the support team`);
        }
