        ...
        return Promise.all([
          ...
          Promise.resolve(call.locus || expectChangeLocusEvent(call, `spock expects to receive a locus`))
            .then(() => {
              ...
              const onMembershipConnected = sinon.spy();
              call.on(`membership:connected`, onMembershipConnected);
              return expectMembershipConnectedEvent(call, mccoy.id, `spock expects mccoy to answer`);
            })
            .then(() => {
              ...
              return expectMembershipChangeEvent(call, mccoy.id, `spock expects mccoy to mute audio`);
            })
            .then((membership) => {
            ...
            .then(() => expectMembershipDisconnectedEvent(call, mccoy.id, `spock expects mccoy to hangup`))
            .then(() => {
              assert.equal(call.me.state, `connected`);
