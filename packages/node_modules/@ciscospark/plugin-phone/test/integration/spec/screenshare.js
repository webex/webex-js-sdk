  ...
  describe('Call', () => {
    firefoxOnly(describe)('mid-call screen and application sharing', () => {
      ...
      it('proceeds through a series of events', () => handleErrorEvent(spock.spark.phone.dial(mccoy.id), async (sc) => {
        ...
        assert.equal(sdp.media.find((m) => m.type === 'audio').direction, 'sendrecv');
        assert.equal(sdp.media.find((m) => m.type === 'video' && !m.content).direction, 'sendrecv');
        assert.equal(sdp.media.find((m) => m.type === 'video' && !!m.content).direction, activeScreenDirection);
        ...
        assert.equal(sdp.media.find((m) => m.type === 'audio').direction, 'sendrecv');
        assert.equal(sdp.media.find((m) => m.type === 'video' && !m.content).direction, 'sendrecv');
        assert.equal(sdp.media.find((m) => m.type === 'video' && !!m.content).direction, inactiveScreenDirection);
