  ...
  describe(`Call`, () => {
    ...
    describe(`#status`, () => {
      ...
      afterEach(() => {
        const c = call;
        call = undefined;
        return maxWaitForPromise(20000, c.hangup())
        ...
        it(`is "disconnected"`, () => {
          ...
          return handleErrorEvent(call, () => mccoy.spark.phone.when(`call:incoming`)
            ...
            .then(() => {
              ...
              call.on(`change:status`, () => spy(call.status));
              return Promise.all([
                call.hangup(),
            ...
            .then(() => {
              assert.equal(call.status, `disconnected`);
              assert.neverCalledWith(spy, `initiated`);
            }));
        });
      });
      ...
      describe(`when the remote party has left the call`, () => {
        it(`is "disconnected"`, () => {
          ...
          return handleErrorEvent(call, () => mccoy.spark.phone.when(`call:incoming`)
            .then(([c]) => {
              remote = c;
              c.on(`change:status`, () => spy(c.status));
              return Promise.all([
                c.answer(),
                call.when(`connected`)
              ]);
