...
import {browserOnly, expectEvent, maxWaitForPromise} from '@ciscospark/test-helper-mocha';
  ...
  describe(`Call`, () => {
        ...
        it(`emits all the correct events`, () => {
          ...
          const handler = handleErrorEvent(spock.spark.phone.dial(mccoy.email),
            (localCall) => {
              ...
              return expectEvent(10000, `call:incoming`, mccoy.spark.phone)
                .then((c) => {
                  remoteCall = c;
                  ...
                  return Promise.all([
                    ...
                    expectEvent(10000, `ringing`, localCall)
                  ]);
                })
                .then(() => assert.equal(localCall.status, `ringing`))
                .then(() => Promise.all([
                  remoteCall.answer(),
                  expectEvent(10000, `connected`, localCall),
                  expectEvent(10000, `connected`, remoteCall)
                ]))
                .then(() => {
                ...
                .then(() => Promise.all([
                  expectEvent(10000, `disconnected`, localCall),
                  expectEvent(10000, `disconnected`, remoteCall),
                  localCall.hangup()
                ]))
                .then(() => {
      ...
      describe(`when the receiving party has declined the call`, () => {
        it(`is "disconnected"`, () => {
          ...
          return handleErrorEvent(call, () => Promise.all([
            expectEvent(10000, `call:incoming`, mccoy.spark.phone)
              .then((c) => c.reject()),
    ...
    describe(`#to`, () => {
      ...
      afterEach(() => {
        const c = call;
        call = undefined;
        return maxWaitForPromise(10000, c.hangup())
      ...
      it(`represents the receiving party`, () => {
        ...
        return handleErrorEvent(call, () => Promise.all([
          expectEvent(10000, `call:incoming`, mccoy.spark.phone)
            .then((c) => c.answer()),
          expectEvent(10000, `connected`, call)
            .then(() => {
    ...
    describe(`#direction`, () => {
      it(`indicates the initiating and receiving members of the call`, () => {
        ...
        return handleErrorEvent(call, () => expectEvent(10000, `call:incoming`, mccoy.spark.phone)
          .then((c) => {
            assert.equal(call.direction, `out`);
    ...
    describe(`#answer()`, () => {
      it(`accepts an incoming call`, () => {
        ...
        return handleErrorEvent(call, () => Promise.all([
          expectEvent(10000, `call:incoming`, spock.spark.phone)
            .then((c) => c.answer()),
          expectEvent(10000, `connected`, call)
            .then(() => assert.equal(call.status, `connected`))
        ]));
      ...
      it(`accepts an active outgoing call that is not already joined`, () => {
        ...
        return handleErrorEvent(call, () => Promise.all([
          ...
          .then(([mccoyCall]) => Promise.all([
            expectEvent(10000, `connected`, mccoyCall),
            mccoyCall.answer()
          ])
            .then(() => assert.equal(call.status, `connected`))));
      });
        ...
        const handler = handleErrorEvent(spock.spark.phone.dial(mccoy.email), (call) => {
          ...
          const handler = Promise.all([
            ...
            .then(() => {
              ...
              mccoyCall.hangup();
              return expectEvent(10000, `disconnected`, call)
            ...
            return expectEvent(10000, `call:incoming`, mccoy.spark.phone)
              .then((c) => {
                ...
                return Promise.all([
                  ...
                  expectEvent(30000, `disconnected`, c)
    ...
    describe(`#reject()`, () => {
      ...
      it(`is a noop for outbound calls`, () => handleErrorEvent(spock.spark.phone.dial(mccoy.email), (call) => {
        return Promise.all([
          ...
          expectEvent(10000, `ringing`, call)
        ])
    ...
    describe(`on(error)`, () => {
      it(`gets triggered when something fails in a non-promise-returning method`, () => {
        ...
        return expectEvent(10000, `error`, call)
