  ...
  describe('Call', () => {
    ...
    describe('#answer()', () => {
      ...
      it('is a noop for outbound calls', () => handleErrorEvent(spock.spark.phone.dial(mccoy.id), (call) => {
        sinon.spy(call, 'createOrJoinLocus');
        return call.answer()
          .then(() => {
            // We called createOrJoinLocus to create the call
            assert.calledWith(call.createOrJoinLocus, 'create');
            // But we did not call createOrJoinLocus when we invoked answer()
            assert.neverCalledWith(call.createOrJoinLocus, 'join');
          });
      }));
