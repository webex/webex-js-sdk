  ...
  describe(`Phone`, () => {
    ...
    describe(`#dial()`, () => {
      ...
      it(`calls a room by hydra room id`, () => handleErrorEvent(spock.spark.phone.dial(room.id), (call) => {
        ...
        return Promise.all([
          Promise.resolve()
            .then(() => call.locus || expectEvent(10000, `change:locus`, call))
            .then(() => call.state === `active` || expectEvent(10000, `change:state`, call))
          ...
          .then(([_, mc]) => {
            ...
            return Promise.all([
              mccoyCall.activeParticipantsCount === 2 || expectEvent(10000, `change:activeParticipantsCount`, mccoyCall),
              call.activeParticipantsCount === 2 || expectEvent(10000, `change:activeParticipantsCount`, call),
              mccoyCall.answer()
            ]);
          })
          .then(() => {
