...
browserOnly(describe)(`plugin-phone`, function() {
  ...
  describe(`Phone`, () => {
    ...
    describe(`group callign events model`, () => {
      let room;
      ...
      it(`proceeds through a series of events`, () => handleErrorEvent(spock.spark.phone.dial(room.id), (call) => {
        return Promise.all([
          ...
          expectEvent(5000, `change:locus`, call)
            .then(() => {
              ...
              return Promise.all([
                ...
                expectEvent(5000, `membership:declined`, call)
              ...
              .then(() => {
                ...
                return expectEvent(5000, `membership:change`, call);
              })
              .then((membership) => {
                ...
                return expectEvent(5000, `membership:disconnected`, call);
              })
          ...
          expectEvent(5000, `call:incoming`, mccoy.spark.phone)
            .then((mc) => {
              ...
              .then(() => expectEvent(5000, `membership:disconnected`, mc))
              .then(() => mc.hangup())
              ;
            }),

          // This execution chain represents Chekov's view of the call
          expectEvent(5000, `call:incoming`, chekov.spark.phone)
            .then((cc) => {
          ...
          expectEvent(5000, `call:incoming`, uhura.spark.phone)
            .then((uc) => {
              ...
              return Promise.all([
                ...
                .then(() => expectEvent(5000, `membership:change`, uc))
                .then((membership) => {
                  assert.equal(membership.personUuid, mccoy.id);
                  assert.isTrue(membership.audioMuted);
                  return uc.hangup();
