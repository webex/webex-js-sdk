          ...
          .then((res) => {
            ...
            return handleErrorEvent(spock.spark.phone.dial(room.sipAddress), (call) => Promise.all([
              ...
              expectEvent(5000, `membership:connected`, call)
                .then(() => expectEvent(5000, `membership:disconnected`, call))
                .then(() => Promise.all([
                  call.hangup(),
                  expectEvent(5000, `inactive`, call)
                ])),

              // McCoy's view of the call
              expectEvent(5000, `call:incoming`, mccoy.spark.phone)
                .then((mc) => Promise.all([
    ...
    describe(`group calling events model`, () => {
      ...
      it(`proceeds through a series of events`, () => handleErrorEvent(spock.spark.phone.dial(room.id), (call) => {
        return Promise.all([
          ...
          Promise.resolve(call.locus || expectEvent(5000, `change:locus`, call, `spock expects to receive a locus`))
            .then(() => {
              ...
              return Promise.all([
                ...
                expectEvent(5000, `membership:declined`, call, `spock expects chekov to decline`)
              ...
              .then(() => {
                ...
                return expectEvent(5000, `membership:change`, call, `spock expects mccoy to mute audio`);
              })
              .then((membership) => {
                assert.equal(membership.personUuid, mccoy.id);
                assert.isTrue(membership.audioMuted);
                // wait for uhura to hangup
                return expectEvent(5000, `membership:disconnected`, call, `spock expects uhura to hangup`);
              })
              // wait for mccoy to hangup
              .then(() => expectEvent(5000, `membership:disconnected`, call, `spock expects mccoy to hangup`))
              .then(() => {
                assert.equal(call.me.state, `connected`);
          ...
          expectEvent(5000, `call:incoming`, mccoy.spark.phone, `mccoy expects a group call`)
            .then((mc) => {
              ...
              .then(() => mc.toggleSendingAudio())
                // wait for uhura to hangup
              .then(() => expectEvent(5000, `membership:disconnected`, mc, `mccoy expects uhura to hangup`))
              .then(() => mc.hangup())
              ;
            }),

          // This execution chain represents Chekov's view of the call
          expectEvent(5000, `call:incoming`, chekov.spark.phone, `chekov expects a group call`)
            .then((cc) => {
          ...
          expectEvent(5000, `call:incoming`, uhura.spark.phone, `uhura expects a group call`)
            .then((uc) => {
              ...
              return Promise.all([
                ...
                .then(() => expectEvent(5000, `membership:change`, uc, `uhura expects mccoy to mute`))
                .then((membership) => {
                  if (membership.personUuid !== mccoy.id) {
                    return expectEvent(5000, `membership:change`, uc, `uhura still expects mccoy to mute`);
                  }
