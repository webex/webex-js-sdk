  ...
  describe(`Call`, () => {
    describe(`Media State Controls`, () => {
      ...
      describe(`#toggleSendingAudio()`, () => {
        describe(`when the call is started with audio`, () => {
          it(`stops sending audio then starts sending audio`, () => handleErrorEvent(spock.spark.phone.dial(mccoy.email),
            (call) => {
              const spy = sinon.spy();
              let mccoyCall;
              return mccoy.spark.phone.when(`call:incoming`)
                .then(([c]) => {
                  ...
                  return Promise.all([
                    c.answer(),
                    c.when(`connected`)
                      .then(() => c.on(`remoteAudioMuted:change`, spy))
                  ]);
                })
                .then(() => assertLocusMediaState(call, {
                  sendingAudio: true,
                ...
                .then(() => maxWaitForEvent(2000, `remoteAudioMuted:change`, mccoyCall))
                .then(() => {
                ...
                .then(() => maxWaitForEvent(1000, `remoteAudioMuted:change`, mccoyCall))
                .then(() => {
      ...
      describe(`#toggleSendingVideo()`, () => {
        describe(`when the call is started with video`, () => {
          it(`stops sending video then starts sending video`, () => handleErrorEvent(spock.spark.phone.dial(mccoy.email),
            (call) => {
              const spy = sinon.spy();
              let mccoyCall;
              return mccoy.spark.phone.when(`call:incoming`)
                .then(([c]) => {
                  ...
                  return Promise.all([
                    c.answer(),
                    c.when(`connected`)
                      .then(() => c.on(`remoteVideoMuted:change`, spy))
                  ]);
                })
                .then(() => assertLocusMediaState(call, {
                  sendingAudio: true,
                ...
                .then(() => maxWaitForEvent(2000, `remoteVideoMuted:change`, mccoyCall))
                .then(() => {
                  assert.calledOnce(spy);
                  assert.equal(remoteParticipant(mccoyCall.locus).status.audioStatus.toLowerCase(), boolToStatus(true, true));
                ...
                .then(() => maxWaitForEvent(1000, `remoteVideoMuted:change`, mccoyCall))
                .then(() => {
                  assert.calledTwice(spy);
                  assert.equal(remoteParticipant(mccoyCall.locus).status.audioStatus.toLowerCase(), boolToStatus(true, true));
                  assert.equal(remoteParticipant(mccoyCall.locus).status.videoStatus.toLowerCase(), boolToStatus(true, true));
