...
function setup(users, assign) {
  ...
  beforeEach(() => users.spock.spark.request({
    ...
    .then((room) => {
      return Promise.all(Object.values(users).filter((user) => user !== users.spock).map((user) => users.spock.spark.request({
        method: `POST`,
        service: `hydra`,
  ...
  after(`unregister users`, () => Promise.all(Object.entries(users).map(([name, user]) => {
