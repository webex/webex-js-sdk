  ...
  describe(`Phone`, () => {
    ...
    describe(`#dial()`, () => {
      ...
      it.skip(`calls a PSTN phone number`, () => {
        ...
        return handleErrorEvent(call, () => expectEvent(10000, `connected`, call)
          .then(() => call.hangup()));
      });

      describe(`by hydra user id`, () => {
        let hydraUserId;
        before(`get mccoy's hydra user id`, () => mccoy.spark.request({
          method: `GET`,
          service: `hydra`,
          resource: `people/me`
        })
          .then((res) => {
            hydraUserId = res.body.id;
          }));

        it(`calls a user by hydra user id`, () => handleErrorEvent(spock.spark.phone.dial(hydraUserId),
            (call) => expectEvent(10000, `call:incoming`, mccoy.spark.phone)
              .then(() => call.hangup())));
      });
    ...
    describe(`#register()`, () => {
      ...
      it(`fetches active calls`, () => {
        ...
        // /call completes.
        return handleErrorEvent(call, () => expectEvent(10000, `change:locus`, call)
          .then(() => {
            assert.isFalse(kirk.spark.phone.registered);
            kirk.spark.phone.register();
            return expectEvent(10000, `call:incoming`, kirk.spark.phone)
