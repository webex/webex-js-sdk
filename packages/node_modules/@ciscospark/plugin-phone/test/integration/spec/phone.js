...
browserOnly(describe)('plugin-phone', function () {
  ...
  describe('Phone', () => {
      ...
      .then(({jwt}) => {
        ...
        return guestSpark.authorization.requestAccessTokenFromJwt({jwt}).then(() =>
          ...
          guestSpark.people.get('me').then((p) => {
            ...
            quark.email = email;
            quark.jwt = jwt;
            quark.spark = guestSpark;

            return quark;
          }));
      })
      .then(() => Promise.all([
        quark.spark.phone.register(),
    ...
    after('unregister spock and mccoy', () => Promise.all([
      quark && quark.spark.phone.deregister()
    ...
    describe('guest user calling', () => {
      it('can call a guest user by hydra user id', () => handleErrorEvent(spock.spark.phone.dial(quark.id), (call) => Promise.all([
        expectConnectedEvent(call)
          .then(() => {
            assert.isDefined(call.correlationId);
            assert.equal(call.locus.self.devices[0].correlationId, call.correlationId);
          }),
        expectCallIncomingEvent(quark.spark.phone)
          .then((c) => c.answer()
            .then(() => {
              assert.equal(c.locus.self.devices[0].correlationId, c.correlationId);
              assert.isDefined(c.correlationId);
      ...
      it('can call a guest user by guest email', () => handleErrorEvent(spock.spark.phone.dial(quark.email), (call) => Promise.all([
        expectConnectedEvent(call)
          .then(() => {
            assert.isDefined(call.correlationId);
            assert.equal(call.locus.self.devices[0].correlationId, call.correlationId);
      ...
      it('can place a call to a regular user', () => handleErrorEvent(quark.spark.phone.dial(spock.email), (call) => Promise.all([
        expectConnectedEvent(call)
          .then(() => {
            assert.isDefined(call.correlationId);
            assert.equal(call.locus.self.devices[0].correlationId, call.correlationId);
      ...
      it('calls a regular user by hydra user id', () => handleErrorEvent(quark.spark.phone.dial(mccoy.hydraUserId), (call) => Promise.all([
        expectConnectedEvent(call)
          .then(() => {
            assert.isDefined(call.correlationId);
            assert.equal(call.locus.self.devices[0].correlationId, call.correlationId);
      ...
      it('calls a regular user by uuid', () => handleErrorEvent(quark.spark.phone.dial(mccoy.id), (call) => Promise.all([
        expectConnectedEvent(call)
          .then(() => {
            assert.isDefined(call.correlationId);
            assert.equal(call.locus.self.devices[0].correlationId, call.correlationId);
