      ...
      it(`calls a user by sip uri`, () => mccoy.spark.internal.user.get()
        .then((mu) => {
          ...
          return handleErrorEvent(spock.spark.phone.dial(mu.sipAddresses[0].value), (call) => Promise.all([
            ...
            expectEvent(5000, `membership:connected`, call)
              .then(() => Promise.all([
                expectEvent(5000, `membership:disconnected`, call),
                expectEvent(5000, `disconnected`, call)
              ])),

            // McCoy's view of the call
            mccoy.spark.phone.when(`call:incoming`)
              .then(([mc]) => Promise.all([
                expectEvent(5000, `membership:connected`, mc),
                mc.answer()
              ])
              .then(() => mc.hangup()))
          ]));
