...
describe(`plugin-feature`, function() {
  ...
  describe(`#setBatchUserFeatures()`, () => {
    ...
    it(`sets a value for two user feature toggle`, () => {
      spark.feature.setFeature(`user`, `key1`, false);
      return spark.feature.setBundledFeatures(featureUpdateArray)
        .then((res) => {
          assert.isDefined(res);
          assert.equal(find(res.serialize().user, {key: `key1`}).value, `value1`);
          assert.equal(find(res.serialize().user, {key: `key2`}).value, `value2`);
