    ...
    describe('#initialize()', () => {
      describe('when there is a token in the url', () => {
        it('sets the token and sets ready', () => {
          const webex = makeSpark('http://example.com/#access_token=AT&expires_in=3600&token_type=Bearer');
        ...
        it('sets the token, refresh token and sets ready', () => {
          const webex = makeSpark('http://example.com/#access_token=AT&expires_in=3600&token_type=Bearer&refresh_token=RT&refresh_token_expires_in=36000');
        ...
        it('validates the csrf token', () => {
          ...
          assert.throws(() => {
            ...
            const webex = makeSpark(`http://example.com/#access_token=AT&expires_in=3600&token_type=Bearer&refresh_token=RT&refresh_token_expires_in=36000&state=${base64.encode(JSON.stringify({csrf_token: 'someothertoken'}))}`, csrfToken);
          }, /CSRF token someothertoken does not match stored token abcd/);

          assert.throws(() => {
            // eslint-disable-next-line no-unused-vars
            const webex = makeSpark(`http://example.com/#access_token=AT&expires_in=3600&token_type=Bearer&refresh_token=RT&refresh_token_expires_in=36000&state=${base64.encode(JSON.stringify({}))}`, csrfToken);
          }, /Expected CSRF token abcd, but not found in redirect hash/);

          assert.throws(() => {
            // eslint-disable-next-line no-unused-vars
            const webex = makeSpark('http://example.com/#access_token=AT&expires_in=3600&token_type=Bearer&refresh_token=RT&refresh_token_expires_in=36000', csrfToken);
          }, /Expected CSRF token abcd, but not found in redirect hash/);

          const webex = makeSpark(`http://example.com/#access_token=AT&expires_in=3600&token_type=Bearer&refresh_token=RT&refresh_token_expires_in=36000&state=${base64.encode(JSON.stringify({csrf_token: csrfToken}))}`, csrfToken);
        ...
        it('removes the oauth parameters from the url', () => {
          ...
          const webex = makeSpark(`http://example.com/#access_token=AT&expires_in=3600&token_type=Bearer&refresh_token=RT&refresh_token_expires_in=36000&state=${base64.encode(JSON.stringify({csrf_token: csrfToken, something: true}))}`, csrfToken);
