...
Why would we put an http server in front of our SDK? Encryption is hard and this
lets all of our client test suites (potentially written in languages for which
we do not have sdks) do encrypted things without a major time expenditure
...
> SDK: webex.internal.conversation.create({comment: 'first comment',
> displayName: 'title', participants: ['<userId1>', '<userId2>', '<userId3>']})
...
> SDK:
> `webex.inernal.conversation.post({url: '<conversation url>', {displayName: 'second comment'}})`
...
> SDK: `webex.internal.conversation.get({url: '<conversation url>'})` SDK:
> `webex.internal.conversation.get({url: '<conversation url>'})`
...
Clean up your session (If you don't do this, you'll have a bunch of long-running
