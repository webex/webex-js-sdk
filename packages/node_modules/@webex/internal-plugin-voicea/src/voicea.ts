...
import {EVENT_TRIGGERS, VOICEA_RELAY_TYPES} from '@webex/internal-plugin-voicea/src/constants';
import LLMChannel from '@webex/internal-plugin-llm/src/index';

import Trigger from '../../plugin-meetings/src/common/events/trigger-proxy.js';

// eslint-disable-next-line no-unused-vars
import {AnnouncementPayload, CaptionLanguageResponse, TranscriptionResponse} from './voicea.types';
...
export class VoiceaChannel extends LLMChannel {
   ...
   * @returns {void}
   */
  processTranscription = (voiceaPayload: TranscriptionResponse): void => {
    switch (voiceaPayload.type) {
      case 'transcript_interim_results':
      case 'transcript_final_result':
        ...
        EVENT_TRIGGERS.NEW_CAPTION, {
          isFinal: voiceaPayload.type === 'transcript_final_result',
      ...
      case 'highlight_created':
        ...
        EVENT_TRIGGERS.HIGHLIGHT_CREATED, {
          ...
          highlight_id: voiceaPayload.highlight.highlight_id,
      ...
      case 'eva_cancel':
        ...
        EVENT_TRIGGERS.EVA_COMMAND, {
          isListening: voiceaPayload.type === 'eva_wake'
        });
        break;
   ...
   * @returns {void}
   */
  processCaptionLanguageResponse = (voiceaPayload: CaptionLanguageResponse):void => {
 ...
 * @returns {void}
 */
  processAnnouncementMessage = (voiceaPayload: AnnouncementPayload):void => {
  ...
  sendAnnouncement = ():void => {
    ...
    this.socket.send({
      ...
      trackingId: `WEBCLIENT_${uuid.v4().toString()}`,
    });
    this.seqNum += 1;
  };
   ...
   * @returns {Promise}
   */
  turnOnCaptions = async (): undefined | Promise<void> => {
   ...
   * @returns {Promise}
   */
  toggleTranscribing = async (activate:boolean):undefined|Promise<void> => {
