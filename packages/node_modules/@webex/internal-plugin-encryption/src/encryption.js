...
const Encryption = WebexPlugin.extend({
  ...
  _callDownloadUrl(url, scr, result) {
    const _this2 = this;

    return this.request({
      method: 'GET',
      uri: url
    }).then(() => _this2._helperFunction(scr, result))
      .catch((err) => Promise.reject(err));
  },

  _helperFunction: function _helperFunction(scr, result) {
    const _this = this;
    const shunt = new EventEmitter();
    const options = {
      method: 'GET',
      uri: result.body.endpoints[scr.loc],
      responseType: 'buffer'
    };
    ...
    return ret.then((res) => _this.decryptBinary(scr, res.body));
  },
