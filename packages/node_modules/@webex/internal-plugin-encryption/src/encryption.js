...
const Encryption = WebexPlugin.extend({
  ...
  _fetchDownloadUrl(scr) {
    ...
    return this.request({
      ...
      var url = scr.loc;
      return _this2._callDownloadUrl(url, scr);
    });
  },

  // Call to /bytes API.
  _callDownloadUrl(url, scr){
    var _this2 = this;
    return this.request({
      method: 'GET',
      uri: url,
    }).then(function (res) {
      return _this2._helperFunction(url, scr);
    }).catch((err) => {
      if(err.statusCode == 302){
        return Promise.resolve(_this2._helperFunction(url, scr));
      } else {
