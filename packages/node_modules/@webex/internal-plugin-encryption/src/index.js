  ...
  payloadTransformer: {
    ...
    transforms: [{
      ...
      fn(ctx, object) {
        ...
        return ctx.spark.internal.encryption.kms.prepareRequest(object.kmsMessage)
          .then((req) => {
      ...
      name: 'decryptKmsMessage',
      fn(ctx, object) {
        return ctx.spark.internal.encryption.kms.decryptKmsMessage(object.kmsMessage)
          .then((kmsMessage) => {
      ...
      name: 'decryptErrorResponse',
      fn(ctx, reason) {
        const promises = reason.body.errors.map((error) => ctx.spark.internal.encryption.kms.decryptKmsMessage(error.description)
          .then((desc) => {
            error.description = desc;
          }));

        promises.push(ctx.spark.internal.encryption.kms.decryptKmsMessage(reason.body.message)
          .then((kmsMessage) => {
