...
const KMS = WebexPlugin.extend({
  ...
  */
  ...
  fetchPublicKey({assignedOrgId}) {
    ...
    console.warn(`---start to call fetchPublicKey in src ${assignedOrgId}`);
    ...
    return this.request({
      ...
      .then((res) => {
        ...
        console.warn(`successfully get public key ${JSON.stringify(res)}`);

        return Promise.all(res.publicKey.map(this.asKey));
      });
  },
  ...
  */
  ...
  uploadCustomerMasterKey({assignedOrgId, customerMasterKey}) {
    ...
    console.warn(`---Start to do uploadCustomerMasterKey in the source code, uuid is ${uuid.v4()}`);
    ...
    return this.request({
      ...
      console.warn(`successfully upload customer master key ${JSON.stringify(res)}`);
  ...
  listAllCustomerMasterKey({assignedOrgId}) {
    ...
    return this.request({
      ...
      console.warn(`successfully get all customer master keys ${JSON.stringify(res)}`);
  ...
  /**
   * delete one customer master key for one org.
   * @param {Object} options
   * @param {string} options.keyId the id of one customer master key, it should be a url
   * @param {UUID} options.assignedOrgId the orgId
   * @returns {Promise.<ActivateCmkResponse>} response of list CMKs api
   */
  deleteCustomerMasterKey({keyId, assignedOrgId}) {
    this.logger.info('kms: delete one customer master key for byok');
    ...
    return this.request({
      ...
      this.logger.info('kms: finish to delete one customer master key');
      console.warn(`successfully delete one customer master key ${JSON.stringify(res)}`);
  ...
  */
   ...
   */
  /**
   * activate one customer master key for one org.
   * @param {Object} options
   * @param {string} options.keyId the id of one customer master key, it should be a url
   * @param {UUID} options.assignedOrgId the orgId
   * @returns {Promise.<ActivateCmkResponse>} response of activate CMK api
   */
  activateCustomerMasterKey({keyId, assignedOrgId}) {
    this.logger.info('kms: activate one customer master key for byok');
    ...
    return this.request({
      ...
      this.logger.info('kms: finish to activate one customer master key');
      console.warn(`successfully activate one customer master key ${JSON.stringify(res)}`);
  ...
  prepareRequest(payload, onBehalfOf) {
    console.warn('---prepareRequest is called');
    const isECDHRequest = payload.method === 'create' && payload.uri.includes('/ecdhe');

    console.warn(`---prepareRequest the isECDHRequest is ${isECDHRequest}`);
    ...
    return Promise.resolve(isECDHRequest ? partialContexts.get(this) : this._getContext())
      .then((context) => {
        ...
        console.warn(`---run the real request, the request is ${JSON.stringify(req)}`);
  ...
  processKmsMessageEvent(event) {
    console.warn(`---inside kms, the processKmsMessageEvent, the event is ${event}`);
  ...
  request(payload, {timeout, onBehalfOf} = {}) {
    console.warn(`---start to call request method, the payload is ${JSON.stringify(payload)}`);
  ...
  _prepareContext() {
    ...
    return Promise.all([
      ...
      .then(([localECDHKey, cluster]) => {
        ...
        console.log('---use http call to call to tell kms the session key');
