  ...
  describe('KMS', () => {
    ...
    describe('#listAuthorizations()', () => {
      ...
      it('list authorizations', () => webex.internal.encryption.kms.listAuthorizations({kroUri: kro.uri})
        .then((authorizations) => {
          console.warn(`---the authorizations is ${JSON.stringify(authorizations)}`);
          ...
          console.warn('---the test list authorizations is ended');
        }));
    ...
    describe('upload customer master key 1', () => {
      let uploadedkeyId;

      it('upload customer master key', () => window.crypto.subtle.generateKey(
        {
          name: 'RSA-OAEP',
          modulusLength: 4096,
        ...
        .then((keyPair) => {
          console.warn('get the public key', keyPair.publicKey);
          const buf = window.crypto.getRandomValues(new Uint8Array(16));

          console.warn(`---the random master key is ${new TextDecoder('utf-8').decode(buf)}`);
          console.warn(`---the test user org id is ${spock.orgId}`);
        ...
        .then((encryptedData) => webex.internal.encryption.kms.uploadCustomerMasterKey({assignedOrgId: spock.orgId, customerMasterKey: new TextDecoder('utf-8').decode(encryptedData)}))
        .then((uploadRes) => {
          uploadedkeyId = uploadRes.uri;
        ...
        .then((uploadedCmk) => webex.internal.encryption.kms.activateCustomerMasterKey({keyId: uploadedCmk.uri, assignedOrgId: spock.orgId}))
        .then((activeRes) => {
          expect(activeRes.customerMasterKeys[0].usageState).to.have.string('');
        })
        .then(() => webex.internal.encryption.kms.deleteCustomerMasterKey({keyId: uploadedkeyId, assignedOrgId: spock.orgId})));
    });

    describe('#fetchPublicKey()', () => {
      it('requests public key from the KMS', () => webex.internal.encryption.kms.fetchPublicKey({assignedOrgId: spock.orgId})
        .then((publicKey) => {
          console.warn(`in test, the public key is ${JSON.stringify(publicKey)}`);
          assert.lengthOf(publicKey, 2);
        })
        .catch((error) => {
          console.warn(`---there is an error, the body is ${JSON.stringify(error.body)}`);
          assert.fail();
        }));
    });
