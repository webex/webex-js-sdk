...
> Helpers for rotating images and creating their thumbnails. Mostly
> browser-only.
...
Updates the image file with exif information, required to correctly rotate the
image activity. Can specify `options.shouldNotAddExifData` to not add exif
information. For example, clients may not need the exif data added if browsers
already auto orient the image
