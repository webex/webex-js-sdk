...
'socket activity for memberships:${event} event: ${e.message}`);
  ...
  listWithReadStatus(options) {
    ...
    return this.spark.internal.conversation.get(conversation,
      {
      ...
      .then((resp) => {
        try {
          ...
          const roomId = constructHydraId(hydraTypes.ROOM, roomUUID);
          const particpants = resp.participants.items;
          const lastReadInfo = {items: []};
          const roomType = resp.tags.includes(SDK_EVENT.INTERNAL.ACTIVITY_TAG.ONE_ON_ONE) ?
            SDK_EVENT.EXTERNAL.SPACE_TYPE.DIRECT :
            SDK_EVENT.EXTERNAL.SPACE_TYPE.GROUP;

          // isRoomHidden info is private to API caller only
          // Ideally we would have the callers ID even if they
          // didn't register a listener....
          const myId = this.eventEnvelope ? this.eventEnvelope.createdBy : '';
          ...
          for (let i = 0; i < particpants.length; i += 1) {
            ...
            const participantInfo = {
              id: constructHydraId(hydraTypes.MEMBERSHIP,
                `${participant.entryUUID}:${roomUUID}`),
              roomId,
              personId: constructHydraId(hydraTypes.PEOPLE,
                participant.entryUUID),
              personEmail: participant.entryEmailAddress ||
                participant.entryEmail,
              personDisplayName: participant.displayName,
              personOrgId: constructHydraId(hydraTypes.ORGANIZATION,
                participant.orgId),
              isModerator: participant.roomProperties.isModerator,
            ...
            if (participant.roomProperties.lastSeenActivityDate) {
              participantInfo.lastSeenId = constructHydraId(hydraTypes.MESSAGE,
  ...
  updateLastSeen(message) {
    ...
    return this.spark.internal.conversation.acknowledge(conversation, activity)
      .then((activity) => {
        ...
        return ({
          lastSeenId: constructHydraId(hydraTypes.MESSAGE, activity.object.id),
          id: constructHydraId(hydraTypes.MEMBERSHIP,
          ...
          personDisplayName: activity.actor.displayName,
          personOrgId: constructHydraId(hydraTypes.ORGANIZATION,
            activity.actor.orgId),
