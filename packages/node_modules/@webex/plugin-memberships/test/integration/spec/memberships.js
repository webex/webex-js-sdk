  ...
  describe('#memberships', () => {
    ...
    describe('#listWithReadStatus()', () => {
      ...
      it('retrieves memberships with read status for a room that has not been visited', () => {
        return spark.memberships.listWithReadStatus({roomId: room.id})
          .then((memberships) => {
            assert.isDefined(memberships);
            assert.isAbove(memberships.length, 0);
            for (const membership of memberships) {
    ...
    it('posts a message to the room to create activity for user', () => spark.messages.create(
      {
        roomId: room.id,
        text: 'Message to create activity'
      }));

    it('retrieves memberships with read status for a room that has been visited', () => {
      return spark.memberships.listWithReadStatus({roomId: room.id})
        .then((memberships) => {
          assert.isDefined(memberships);
          assert.isAbove(memberships.length, 0);
      ...
      it('marks a message as read and verifies membership:updated event', () => {
        const seenPromise = new Promise((resolve) => {
          spark.memberships.on('seen', (event) => {
            debug('membership:seen event handler called');
            resolve(event);
          });
        });
        ...
        return spark1.memberships.updateLastSeen(message)
          .then(async (m) => {
            ...
            validateMembershipEvent(event, m, actor1);
          });
      });
    });
