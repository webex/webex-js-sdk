...
describe('plugin-memberships', function () {
  ...
  describe('#memberships', () => {
    ...
    beforeEach(() => webex.rooms.create({title: 'Webex Teams Test Room'})
      .then((r) => {
    ...
    describe('#create()', () => {
      ...
      it('creates a space and memberships simultaniously', () => {
        ...
        // one user. The API also does not return an object with
        // a membership shape when performing a room creating request,
        // so validateMembership() cannot be used
        const created = new Promise((resolve) => {
          webex.memberships.on('created', (event) => {
        ...
        return webex.memberships.listen()
          // Create a space, this should trigger a membership event
          // for the creator
          .then(() => webex.rooms.create({title: 'Webex Teams Test Room'}))
          .then(async () => {
            // wait for event to trigger after creating the space
            const event = await created;

            // validate event data against actor
            validateMembershipEvent(event, event.data, actor);
          });
      });
    });
