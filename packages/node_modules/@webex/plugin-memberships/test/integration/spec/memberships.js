...
/*
      it('adds a user to the test space', () => spark.memberships.create({
        roomId: room.id,
        personId: user1.id
      })
        .then((m) => {
      ...
      it('posts a message in the test space ', () => spark.messages.create({
        roomId: room.id,
        text: 'This is a test message'
      })
        .then((m) => {
...
*/
      ...
      it('marks a message as read and verifies membership:updated event', () => {
        ...
        return spark2.memberships.updateLastSeen(message)
          .then(async (m) => {
            debug('membership updated');
            validateMembership(m);
            const event = await updatedPromise;

            debug(`...${user.displayName} got the membership:updated event.`);
            validateMembershipEvent(event, m, actor2);
          })
          .catch((e) => {
            console.log('Here\'s what happened: ' + e);
          })
      });
    });
    ...
    describe('#updateLastSeen()', () => {
      ...
      before('register with wdm', () => Promise.all([
        user1.spark.internal.device.register()
      ]));
