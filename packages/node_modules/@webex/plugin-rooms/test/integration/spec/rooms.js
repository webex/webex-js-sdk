...
describe('plugin-rooms', function () {
  ...
  describe('#rooms', () => {
    ...
    describe('#listWithReadStatus()', () => {
      // Reminder: we can't run the next two creates in parallel because some of
      // the tests rely on ordering.
      let room1, room2, spark2, actor2;

      before(() => testUsers.create({count: 1})
        .then(([user]) => {
          spark2 = new CiscoSpark({credentials: user.token});

          return spark2.people.get('me')
            .then((person) => {
              actor2 = person;
              debug('SDK User (Actor) for tests:');
              debug(`- name: ${actor.displayName}`);
      ...
      // Create a space with an unread message for user2
      ...
      // Create a space with an unread message for user1
      beforeEach(() => spark.rooms.create({title: 'Unread Message for User 1'})
        .then((room) => {
          rooms.push(room);
          room2 = room;
        })
      ...
      it('gets the read status for all rooms User 2 is in', () => spark2.rooms.listWithReadStatus()
        .then((roomList) => {
          for (const room of roomList) {
            assert.isRoom(room);
            if (room.id === room1.id) {
