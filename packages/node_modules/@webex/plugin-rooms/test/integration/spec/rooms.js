...
describe('plugin-rooms', function () {
  ...
  describe('#rooms', () => {
    ...
    afterEach(() => {
      ...
      spark.rooms.off('created');
      spark.rooms.off('updated');
    ...
    describe('#getWithReadStatus()', () => {
      ...
      before(() => testUsers.create({count: 1})
        .then(([user]) => {
          user2 = user;
          user2.spark = new CiscoSpark({credentials: user2.token});
        }));

      // Create a space with one user who is "caught up" and another "behind"
      beforeEach(() => spark.rooms.create({title: 'Space to get Read Status from'})
        .then((room) => {
          rooms.push(room);
          room1 = room;
      ...
      it('gets the read status for new room for user1', () => spark.rooms.getWithReadStatus(room1.id)
        .then((roomInfo) => {
      ...
      it('gets the read status for a new room for user2', () => user2.spark.rooms.getWithReadStatus(room1.id)
        .then((roomInfo) => {
          assert.equal(roomInfo.id, room1.id, 'Room 1\'s title matches');
          assert.equal(roomInfo.title, room1.title, 'Room 1\'s title matches');
          assert.isTrue(roomInfo.lastSeenActivityDate < roomInfo.lastActivityDate,
    ...
    describe('#listWithReadStatus()', () => {
      ...
      beforeEach(() => spark.rooms.create({title: 'Unread Message for User 2'})
        .then((room) => {
          rooms.push(room);
          room1 = room;
        ...
        .then(() => spark.rooms.create({title: 'Unread Message for User 1'}))
        .then((room) => {
          rooms.push(room);
          room2 = room;
        })
        ...
        .then(() => user2.spark.messages.create({
          roomId: room2.id,
          text: 'First message in room 2 from User 2'
        })));

      it('gets the read status for all rooms User 1 is in', () => spark.rooms.listWithReadStatus()
        .then((roomList) => {
      ...
      it('gets the read status for all rooms User 2 is in', () => user2.spark.rooms.listWithReadStatus()
        .then((roomList) => {
          assert.isArray(roomList.items,
            'Expect a list or rooms from listWithReadStatus()');
