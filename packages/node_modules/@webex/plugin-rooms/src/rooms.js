...
'socket activity for rooms:${event} event: ${e.message}`);
  ...
  listWithReadStatus() {
    return ensureMyIdIsAvailable(this.spark)
      ...
      .then(() => this.spark.internal.conversation.list(
        {
          activitiesLimit: 0,
          participantsLimit: 2,
          isActive: true
        ...
        .then((resp) => {
          try {
            ...
            const myDislayName = this.spark.internal.me.displayName;


            for (let i = 0; i < rooms.length; i += 1) {
              const room = rooms[i];
              const type = getHydraRoomType(room.tags);

              if ((!('displayName' in room)) && ('participants' in room) &&
              (room.participants.items.length === 2) &&
              (type === SDK_EVENT.EXTERNAL.SPACE_TYPE.DIRECT)) {
                // Derive 1-o-1 space title from other participant
                for (const person of room.participants.items) {
                  if (person.displayName !== myDislayName) {
                    room.displayName = person.displayName;
                    break;
                  }
                }
              }
              const roomInfo = {
                id: buildHydraRoomId(room.id),
                type,
              ...
              if (room.lastReadableActivityDate) {
                ...
                this.spark.logger.error(`rooms.listWithReadStatus: Cannot read last activity date for a space: ${roomInfo.roomId}.  Ignoring`);
