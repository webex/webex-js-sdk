...
'socket activity for rooms:${event} event: ${e.message}`);
   ...
   *  -- For users with thousands of spaces, this API can take literally minutes
   ...
   *     in the last week.  Reccomended value is 30.  Max supported is 100.
   ...
   *
   * Since this API can take literally minutes to return every space, it is
   ...
   *
   * For users in many spaces (ie: > 1000) this function can also be "lossy",
   * as we query multiple times for older spaces, new spaces or older spaces
   * with activity since the first query can be missed.  Custom clients
   * should be prepared to gracefully handle cases where an event occurs
   * in a space not returned by this call, by querying
   * rooms.getWithReadStatus() with the id of the room in question
   *
  ...
  async listWithReadStatus(maxRecent = 0) {
    ...
    if (maxRecent > 0) {
      ...
      options.sinceDate = now.setDate(now.getDate() - 7);
    }
    ...
    return ensureMyIdIsAvailable(this.spark)
      ...
      .then(() => this.spark.internal.conversation.list(options)
        .then((conversations) => {
          const myDisplayName = this.spark.internal.me.displayName;

          return buildRoomInfoList(conversations, myDisplayName);
          // let roomInfoList = buildRoomInfoList(conversations, myDisplayName);
          // return roomInfoList;
        }));
  },
    ...
    return ensureMyIdIsAvailable(this.spark)
      .then(() => ensureConvoAvailability(this.spark)
        .then(() => this.spark.internal.conversation.get(conversation,
          {
            participantsLimit: 2, // in case we need to derive 1-1 title
            activitiesLimit: 0 // don't send the whole history of activity
          })
          .then((room) => {
            const myDisplayName = this.spark.internal.me.displayName;

            return buildRoomInfo(room, myDisplayName);
          })));
  },
 ...
 * @returns {RoomInfoObject}
 */
async function buildRoomInfo(conversation, myDisplayName) {
  try {
    const type = getHydraRoomType(conversation.tags);

    if ((!('displayName' in conversation)) && ('participants' in conversation) &&
    (conversation.participants.items.length === 2) &&
    (type === SDK_EVENT.EXTERNAL.SPACE_TYPE.DIRECT)) {
      // Derive 1-o-1 space title from other participant
      for (const person of conversation.participants.items) {
        if (person.displayName !== myDisplayName) {
          conversation.displayName = person.displayName;
          break;
        }
      }
    }
    const roomInfo = {
      ...
      type,
      ...(conversation.displayName && {title: conversation.displayName}),
    ...
    if (conversation.lastReadableActivityDate) {
      roomInfo.lastActivityDate = conversation.lastReadableActivityDate;
    }
    else {
      roomInfo.lastActivityDate = conversation.lastRelevantActivityDate;
    }
 ...
 * @returns {RoomInfoObject}
 */
async function buildRoomInfoList(conversations, myDisplayName) {
  // Convert each Conversation into a roomInfo object
  const roomReadInfo = {items: []};
  const roomInfoPromises = [];

  for (const conversation of conversations) {
    roomInfoPromises.push(buildRoomInfo(conversation, myDisplayName));
  }
