...
Note: besides audioSender, from mediaProperties.peerConnection.audioTransceiver.sender there is... videoTransceiver with .sender/receiver and shareTransceiver
...
// operate with a Transform stream
    meeting.createStats({
      audioSender: {
        ...
        history: true,
        aggregator: true,
        filter: meeting.mediaProperties.peerConnection.audioTransceiver.sender,
        media: meeting.mediaProperties.peerConnection,
        events: true
      }
    });
  // read
      ...
      .getAudioSender()
      .onData((filtered) => {
        console.log('audioSenderData, ', filtered);
      });
// OR
...
// not for use with events, history, aggregator, interval, will result in undefined behavior
    ...
    meeting.getStats()
      .getAudioSender()
      .withId()
      .withStream(meeting.mediaProperties.peerConnection.audioTransceiver.sender, meeting.mediaProperties.peerConnection)
      .build()
      .getStream()
      .on('data', (raw) => {
        console.log('RAW->audioSenderData', raw);
      });
...
meeting.getStats({ // create the stats reference
  media: { // with screen option
    id: true, // with an id on the screen stats
    history: true, // with history
    interval: true, // with interval
    filter: meeting.mediaProperties.mediaPeerConnection.audioTransceiver.sender, // with filter
    media: meeting.mediaProperties.mediaPeerConnection
    events: true // with events
  }
});
meeting.getStats().getAudioSender().onData((filtered) => {
...
meeting.stats.media.aggregator...
```
...
meeting
  ...
  .getAudioSender() // get the media peer connection related stats, can also do getScreen()
...
meeting
  ...
  .getMedia()
  .withId()
  .withFilter(meeting.mediaProperties.peerConnection)
  .withEvents()
  .withHistory()
  .withInterval()
  .withAggregator()
