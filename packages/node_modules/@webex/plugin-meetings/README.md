...
meeting
  ...
  .withFilter(meeting.mediaProperties.peerConnection) // create the Transform stream filter on the media instance to convert the data to
...
meeting
  ...
  .build() // finish up building the getStats instance and get a reference back
  .getMedia() // get the media instance off of getStats
  .withStream(meeting.mediaProperties.peerConnection); // create the Readable stream on the media instance to be able to set up a data pipe
...
meeting.getStats({ // create the stats reference
  media: { // with screen option
    ...
    filter: meeting.mediaProperties.mediaPeerConnection, // with filter
    events: true // with events
  }
});
meeting.getStats().getMedia().onData((filtered) => {
  console.log(filtered);
});
