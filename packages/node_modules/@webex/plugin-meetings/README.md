...
#### Enable to receive transcription payload live

In order to receive the payload, Webex assistant must be enabled in the meeting.

```javascript
...
// join to receive transcription payload
meeting.join({enableTranscription: true}).then((res) => {
  meeting.on('meeting:started:transcription', (payload) => {
    console.log(payload);
  })
});
```
...
| `meeting:transcription:started` | Fired when transcription is enabled |
| `meeting:transcription:stopped` | Fired when transcription is disabled |
|---|---|
