...
meeting
  ...
  .withFilter(meeting.mediaPeerConnection) // create the Transform stream filter on the media instance to convert the data to
...
meeting
  ...
  .build() // finish up building the getStats instance and get a reference back
  .getMedia() // get the media instance off of getStats
  .withStream(meeting.mediaPeerConnection); // create the Readable stream on the media instance to be able to set up a data pipe
...
meeting.getStats({ // create the stats reference
  screen: { // with screen option
    ...
    filter: meeting.screenPeerConnection, // with filter
...
.on('stats:update', (stats) => { // listen to the stats update event that comes through here
...
.getScreen()
.withId()
.withStream(meeting.screenPeerConnection)
// do stuff
meeting
.getStats()
.getMedia()
