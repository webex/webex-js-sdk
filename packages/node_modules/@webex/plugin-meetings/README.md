...
To create supported meetings using Webex Teams, use [@webex/plugin-meetings](https://www.npmjs.com/package/@webex/plugin-meetings).
...
## API Docs and Kitchen Sink
API Docs: https://webex.github.io/webex-js-sdk/api/
Kitchen Sink Hosted App: https://js.samples.s4d.io/browser-plugin-meetings/


Examples

note: see `packages/node_modules/samples/browser-plugin-meetings/app.js` for the sample kitchen sink code vs the readme
...
meeting.join({hostPin: <WebexHostPin>, moderator: true}).then((res) => {
  // now you are in the meeting
});
```
...
...
// join as attendee
meeting.join({moderator: false}).then((res) => {
...
meeting.join().then((res) => {
  ...
  if (err.joinIntentRequired) {
    ...
    meeting.join({hostPin: <WebexHostPin>, moderator: true}).then(() => {
...
Select a different device than the default for media
...
Use this if you want to change the actual streams send and receive for audio or video component separately
updateAudio and updateVideo is for the developer to add and remove the streams completely
```js
//video
  if (media.sendVideo) {
...
for details on how to use the devices, please see @webex/plugin-devices

##### Leave a Meeting with Device
```js
let resourceId;
...
#### Scheduled Meetings
For scheduled meetings see @webex/internal-calendar
...
Files in the `src` folder will be compiled, bundled, and served as a static asset at `meetings.bundle.js` inside that directory.
