...
document.getElementById('reconnection-button').addEventListener('click', () => {
  meeting.reconnect();
});
...
document.getElementById('join').addEventListener('click', () => {
  ...
  meeting.on('meeting:reconnection_starting', () => {
    document.getElementById('reconnection-status').innerHTML = 'reconnecting in progress';
  });

  meeting.on('meeting:reconnection_success', () => {
  ...
  meeting.on('meeting:reconnection_failure', () => {
  ...
  const videoRenderInterval = setInterval(() => {
    ...
    if (currentFrameCount > 0 && currentFrameCount !== lastFrameCount) {
      ...
      if (callTime) {
        ...
        clearTimeout(saftyTimeout);
      }
    }
  }, 500);
  saftyTimeout = setTimeout(() => {
    clearInterval(videoRenderInterval);
  }, 15000);
  meeting.members.on('members:created', (members) => {
    document.getElementById('participants-collection').innerHTML = `MEMBERS INFO: 
    selfList-${getParticipantString(members.selfList)}-
    hostList-${getParticipantString(members.hostList)}-
    inLobbyList-${getParticipantString(members.inLobbyList)}-
    inMeetingList-${getParticipantString(members.inMeetingList)}-
    notInMeetingList-${getParticipantString(members.notInMeetingList)}-
    `;
  });
});
