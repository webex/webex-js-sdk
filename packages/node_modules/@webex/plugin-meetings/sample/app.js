...
audioOutputSelect.onchange = function () {
  attachSinkId(document.getElementById('localvideo'), audioOutputSelect.value);
};

document.getElementById('getDevices').addEventListener('click', () => {
  toggleDisplay('addMedia', true, true);
  toggleDisplay('getDevices', true, true);
...
document.getElementById('addDevicesMedia').addEventListener('click', () => {
  ...
  meeting.getMediaStreamsDevices(audio, video).then((stream) => {
    meeting.addMedia({localStream: stream, localShare: undefined, mediaSettings: media});
  });
});
