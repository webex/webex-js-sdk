...
let meeting, meetings, newMeeting, callTime, framesDecoded;
const mediaSettings = [];
...
function cleanUp() {
  meeting.media.stop(meeting.localStream);
...
function joinWithMedia(meeting, deviceId) {
  document.getElementsByName('mediadirection').forEach((options) => {
    mediaSettings[options.value] = options.checked;
  });
  meeting.join({
    ...
    .then(() => meeting.getMediaStreams(mediaSettings)
      .then(([localStream, localShare]) => meeting.addMedia({
        mediaSettings,
        localShare,
        localStream
...
joinWithMedia(meeting, deviceId);
