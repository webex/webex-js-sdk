...
document.getElementById('rechability').addEventListener('click', () => {
  if (meetings) {
    console.info(meetings);
  }
  spark.meetings._getRechability();
});
...
document.getElementById('dialer').addEventListener('submit', (event) => {
  ...
  event.preventDefault();
  const invitee = document.getElementById('invitee').value || document.getElementById('invitee-number').value || document.getElementById('invitee-convo').value;
  return spark.meetings.create(invitee).then((m) => {
    console.log(m);
    startMeeting(m);
  });
});
