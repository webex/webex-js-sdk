...
const RoapAdapater = {
  ...
  initialize() {
    ...
    spark
      .internal
      .mercury
      .on('event:locus.message.roap', (result) => {
  ...
  joinMeetingWithRoap(options) {
    ...
    config
      .console
      .info('Call->Locus: Join locus with roap');
    config
      .console
      .info('Call-local SDP', options.roapMessage);
    ...
    return spark.request({
      ...
      .then((res) => {
        ...
        config
          .console
          .info(`Call-Locus->App: Joined locus [${locus.id}][${locus.fullState.lastActive}]`);
        return locus;
      })
      .catch((err) => {
  ...
  sendRoap(options) {
    ...
    if (!options.mediaId) {
      config
        .console
        .info('Race Condition /call mediaID not present');
    }
    const mediaUrl = `loci/${locusId}/participant/${locusSelfId}/media`;
    config
      .console
      .info(`Call-sendRoap: ${mediaUrl} \n ${roapMessage.messageType} \n seq:${roapMessage.seq}`);
    const deviceUrl = spark.internal.device.url;
    return spark
      .request({
        api: 'locus',
      ...
      .then((res) => {
        ...
        config
          .console
          .info(`Call-sendRoap:response:${JSON.stringify(mediaConnection, null, 2)}'\n StatusCode:'${res.statusCode}`);
        const {locus} = res.body;
        locus.roapSeq = options.roapMessage.seq;
        return locus;
      })
      .catch((error) => {
        config
          .console
          .error(`Call-sendRoap Error:${JSON.stringify(error, null, 2)}`);
        config
          .console
          .error(`Call-sendRoap errorBody:${JSON.stringify(roapMessage, null, 2)} + '\\n mediaId:'${options.mediaId}`);
