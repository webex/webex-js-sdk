...
export default class RoapRequest extends StatelessSparkPlugin {
  ...
  joinMeetingWithRoap(options) {
    LoggerProxy.logger.info('Call->Locus: Join locus with roap');
    LoggerProxy.logger.info('Call-local SDP', options.roapMessage);
    ...
    return Promise.resolve().then(() => {
      ...
      return this.spark
        ...
        .then((res) => {
          ...
          LoggerProxy.logger.info(`Call-Locus->App: Joined locus [${locus.id}][${locus.fullState.lastActive}]`);

          return locus;
        })
        .catch((err) => {
          LoggerProxy.logger.error('MeetingAdapter#joinMeetingWithRoap failed', err);
          throw err;
        });
    });
  }
  ...
  sendRoap(options) {
    ...
    if (!mediaId) {
      LoggerProxy.logger.info('Race Condition /call mediaID not present');
    }

    const mediaUrl = `${LOCI}/${locusId}/${PARTICIPANT}/${locusSelfId}/${MEDIA}`;
    const deviceUrl = this.spark.internal.device.url;

    LoggerProxy.logger.info(`Call-sendRoap: ${mediaUrl} \n ${roapMessage.messageType} \n seq:${roapMessage.seq}`);
    ...
    return this.spark
      ...
      .catch((err) => {
        LoggerProxy.logger.error(`Call-sendRoap Error:${JSON.stringify(err, null, 2)}`);
