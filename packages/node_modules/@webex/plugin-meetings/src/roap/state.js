...
import {ROAP, ERROR, CONFLICT, IDLE, OFFER, REQUESTED, ANSWER, EVENTS} from '../constants';
import PeerConnectionManager from '../peer-connection-manager';

const config = window;

export default class RoapState extends StatelessSparkPlugin {
  sessions = {}; // 11/14/18 (holsted): Should we make this an external singleton collection?
  ...
  save(roap) {
    ...
    if (!this.sessions[correlationId][sequenceId].state) {
      this.sessions[correlationId][sequenceId].state = ROAP.ROAP_STATE.INIT;
    }
    ...
    switch (session.state) {
      case ROAP.ROAP_STATE.INIT:
        if (signal === ROAP.ROAP_SIGNAL.RX_OFFER) {
          session.state = ROAP.ROAP_STATE.WAIT_TX_ANSWER;
        }
        else if (signal === ROAP.ROAP_SIGNAL.TX_OFFER) {
          session.state = ROAP.ROAP_STATE.WAIT_RX_ANSWER;
        }
        break;
      ...
      case ROAP.ROAP_STATE.WAIT_RX_ANSWER:
        if (signal === ROAP.ROAP_SIGNAL.RX_ANSWER) {
          ...
          if (meeting.mediaId) {
            session.state = ROAP.ROAP_STATE.WAIT_TX_OK;
          }
          else {
          ...
          session.state = ROAP.ROAP_STATE.GLARE;
        }
        break;
      ...
      case ROAP.ROAP_STATE.WAIT_TX_ANSWER:
        if (signal === ROAP.ROAP_SIGNAL.TX_ANSWER) {
          session.state = ROAP.ROAP_STATE.WAIT_RX_OK;
        }
        break;

      case ROAP.ROAP_STATE.WAIT_TX_OK:
        if (signal === ROAP.ROAP_SIGNAL.TX_OK) {
          session.state = ROAP.ROAP_STATE.INIT;
        }
        break;
  ...
  performState(session, meeting) {
    ...
    const setRemoteDescription = () => {
      ...
      promise
        .then(() => {
          ...
          return Events.emit(EVENTS.ROAP_OK, {
    ...
    switch (session.state) {
      ...
      case ROAP.ROAP_STATE.ERROR:
        ...
        break;
      case session.state === ROAP.ROAP_STATE.GLARE:
        ...
        this.transition(ROAP.ROAP_SIGNAL.GLARE_RESOLVED, session, meeting);
        break;
      default:
        break;
    }
