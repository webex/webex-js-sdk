...
const RoapHandler = {
  ...
  save(roap) {
    ...
    const meeting = window.meetings.get(roap.correlationId);
    if (roap.msg.messageType === 'ERROR' && roap.msg.errorType === 'CONFLICT') {
      ...
      PeerConnectionManager.rollBackLocalDescription({mediaPeerConnection: meeting.mediaPeerConnection})
        .then(() => {
          config.console.info('Delete the sequence data ', roap.msg.seq);
        ...
        .catch((err) => {
          config.console.info('Cannot reset the peerconncetion', err);
        });
      return false;
    }
    ...
    if (this.sessions[correlationId][sequenceId].OFFER && roap.msg.messageType === 'OFFER') {
      ...
      config.console.info(
        `Save OFFER/ANSWER seq:${sequenceId} new mercury event ${roap.msg.messageType}local state: ${JSON.stringify(
    ...
    if (meeting && (!meeting.mediaPeerConnection && !meeting.screenPeerConnection) && roap.msg.messageType === 'ANSWER') {
      config.console.info(
  ...
  performState(session, meeting) {
    config.console.info('CALL-RoapHandler: PerfomeState ', session.state);
    ...
    const setRemoteDescription = () => {
      ...
      if (!(meeting && (meeting.mediaPeerConnection || meeting.screenPeerConnection))) {
        config.console.error('Call-RoapHandler: DANGER no meeting or peerconnection', meeting.correlationId);
        return;
      }
      let promise;
    ...
    switch (session.state) {
      ...
      case RoapConstants.RoapState.WAIT_TX_ANSWER:
        ...
        if (
          (meeting.mediaPeerConnection && meeting.mediaPeerConnection.signalingState === 'have-local-offer')
  ...
  submit(action) {
    ...
    const meeting = window.meetings.get(correlationId);
    let signal;
    let logging = true;
    ...
    switch (action.type) {
      ...
      case RoapConstants.SEND_ROAP_MSG_SUCCESS:
        ...
        if (!meeting) {
          ...
          config.console.error(
            'Call-RoapHandler#SEND_ROAP_MSG_SUCCESS: should always have meeting object on roap event'
          );
          break;
        }
      ...
      case 'RECEIVE_CALL_LEAVE':
        ...
        config.console.info('Call-RoapHandler#RECEIVE_CALL_LEAVE: cleaning up the RoapHandler', correlationId);
        break;
