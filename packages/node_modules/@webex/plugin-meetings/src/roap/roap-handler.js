...
import PeerConnectionHandler from '../media/peerconnection-handler';

import RoapConstants from './roap-constants';
import RoapInterface from './roap-interface';


const RoapHandler = assign({}, EventEmitter.prototype, {
  ...
  save(roap) {
    ...
    const meeting = MeetingStore.getMeetingByCorrelationId(roap.correlationId);
    ...
    if (meeting) {
      if (roap.msg.messageType === 'OFFER' && roap.remote && meeting.shareStatus === 'REQUESTED') {
        ...
        MeetingClientActionCreators.sendRoapError(RoapHandler.sessions[correlationId][sequenceId], MeetingStore.calls[correlationId], 'CONFLICT');
        return false;
      }
    }
  ...
  performState(session, meeting) {
    ...
    const setRemoteDescription = () => {
      ...
      if (meeting.peerConnection && meeting.screenPc) {
        promise = Promise.all([PeerConnectionHandler.setRemoteSessionDetails(meeting.peerConnection, 'answer', session.ANSWER.sdps[0], meeting.clientIdentifiers),
          PeerConnectionHandler.setRemoteSessionDetails(meeting.screenPc, 'answer', session.ANSWER.sdps[1], meeting.clientIdentifiers)
        ]);
      }
      else if (meeting.screenPc) {
        promise = PeerConnectionHandler.setRemoteSessionDetails(meeting.screenPc, 'answer', session.ANSWER.sdps[0], meeting.clientIdentifiers);
      }
      promise
        .then(() => {
          config
    ...
    switch (session.state) {
      ...
      // Server will send the mercury event comes back
        ...
        PeerConnectionHandler.updatePeerConnection({
  ...
  dispatcherIndex: Dispatcher.register(({action}) => {
    ...
    const meeting = MeetingStore.getMeetingByCorrelationId(correlationId);
    let signal;
    let logging = true;
    ...
    switch (action.type) {
      ...
      case Constants.RECEIVE_CALL_LEAVE:
