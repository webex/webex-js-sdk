...
RoapUtil.shouldHandleMedia = (meeting) => {
  ...
  const screenOffer
    = meeting.sharePeerConnection && meeting.sharePeerConnection.signalingState === SDP.HAVE_LOCAL_OFFER;
...
RoapUtil.updatePeerConnection = (meeting, session) => {
  ...
  if (!res) {
    return Promise.resolve([meeting.mediaPeerConnection.sdp, meeting.sharePeerConnection.sdp]);
  }
  return PeerConnectionManager.updatePeerConnection({
    offerSdp: session.OFFER.sdps,
    pc: meeting.mediaPeerConnection,
    sharePeerConnection: meeting.sharePeerConnection,
...
RoapUtil.setRemoteDescription = (meeting, session) => {
  ...
  if (!(meeting && (meeting.mediaPeerConnection || meeting.sharePeerConnection))) {
  ...
  let promise;
  if (meeting.mediaPeerConnection && meeting.sharePeerConnection) {
    promise = Promise.all([
  ...
  else if (meeting.sharePeerConnection) {
