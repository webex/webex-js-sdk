...
import {ROAP, EVENTS, _OFFER_} from '../constants';
...
export default class RoapHandler extends StatelessSparkPlugin {
  checkForAndHandleErrors(action, meeting, correlationId) {
      ...
      if (action.msg && action.msg.messageType && action.msg.errorType) {
        if (RoapUtil.findError(action.msg.messageType, action.msg.errorType, action.type)) {
          RoapUtil.handleError(meeting.mediaProperties.mediaPeerConnection)
            ...
            .catch((err) => {
              LoggerProxy.logger.error(`RoapHandler->checkForAndHandleErrors#Cannot reset the peer connection, failed with ${err}`);
            });
          return true;
        }
      }
  ...
  handleSessionStep(roap, session) {
  ...
  execute(signal, session, action, meeting, prefix) {
    if (session && session.state) {
      this.handleSessionStep(action, session);
  ...
  handleAction(session, action, meeting, correlationId) {
    ...
    switch (action.type) {
      ...
      case ROAP.SEND_ROAP_MSG_SUCCESS:
        ...
        if (RoapCollection.getSessionSequence(this, correlationId, action.seq).ANSWER) {
      ...
      case ROAP.RECEIVE_CALL_LEAVE:
        ...
        LoggerProxy.logger.log(`RoapHandler->handleAction#RECEIVE_CALL_LEAVE event captured, cleaning up the RoapHandler for correlationId: ${correlationId}`);
        break;
      case ROAP.RESET_ROAP_STATE:
        RoapCollection.deleteSessionSequence(correlationId, action.msg.seq);
        LoggerProxy.logger.log(`RoapHandler->handleAction#RESET_ROAP_STATE event captured, resetting the RoapHandler state based on sequenceId: ${action.msg.seq}`);
        break;
      default:
        return true;
    }
  ...
  submit(action) {
    ...
    const session = RoapCollection.getSessionSequence(this, correlationId, seq);
    const meeting = this.spark.meetings.meetingCollection.get(correlationId);
    if (this.checkForAndHandleErrors(action, meeting, correlationId)) {
      return true;
    }
