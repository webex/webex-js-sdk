...
const compareRemoteRoapOffer = (lastRoapMessage, currentRoapMessage) => currentRoapMessage.messageType === _OFFER_ && lastRoapMessage?.msg?.seq === currentRoapMessage.msg.seq;
...
export default class RoapHandler extends StatelessWebexPlugin {
  ...
  execute(signal, session, action, meeting, prefix) {
    if (session && session.state) {
      ...
      LoggerProxy.logger.log('Roap:handler#execute --> Executing state change to ', signal);
    ...
    switch (action.type) {
      case ROAP.RECEIVE_ROAP_MSG:
        if (compareRemoteRoapOffer(this.lastRoapMessage, action)) {
          ...
          LoggerProxy.logger.log('Roap:handler#handleAction --> Received a non duplicate roap message');
      ...
      case ROAP.SEND_ROAP_MSG:
        ...
        LoggerProxy.logger.log('Roap:handler#handleAction --> successfully send a ROAP message ');
