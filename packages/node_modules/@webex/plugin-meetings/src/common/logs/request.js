...
export default class LogsRequest {
  ...
  async uploadLogs(options = {}) {
    ...
    LoggerProxy.logger.info(`LogsRequest->uploadLogs# uploading user logs for feedbackId: ${id}`);

    try {
      await this.webex.internal.support.submitLogs({feedbackId: id, locusId, correlationId});
    }
    catch (error) {
      LoggerProxy.logger.error('LogsRequest->uploadLogs# uploading user logs failed', error);
    }
