...
StatsUtil.generateSingularOptions = (senderReceiver, config, meeting, name) => {
  const key = senderReceiver.correlate;
  const options = {};

  options[name] = {};
  options[name][key] = senderReceiver;
  StatsUtil.generateOptions(options, config, meeting);
};

StatsUtil.generateOptions = (options, config, meeting) => {
  if (!options || !config || !meeting) {
    throw new Error('stats/util->validateInitialization#options, config, and meeting must be provided to execute getStats');
  }
...
StatsUtil.configOptions = (meeting, config, options, name) => {
  if (options[name]) {
    forEach(options[name], (type) => {
      if (!type.correlate) {
        throw new Error(`stats/util->validateInitialization#each ${type} must be provided with a correlate as audio OR video OR share.`);
      }
