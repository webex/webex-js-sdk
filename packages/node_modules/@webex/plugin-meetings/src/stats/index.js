...
export default class WebRTCStats extends StatelessSparkPlugin {
   ...
   */
    ...
    addSender(sender, useConfig = false) {
      if (sender) {
        if (useConfig) {
          StatsUtil.generateSingularOptions(sender, this.config.stats, this.meetingRef, STATS.SENDERS);
        }
    ...
    addReceiver(receiver, useConfig = false) {
      if (receiver) {
        if (useConfig) {
          StatsUtil.generateSingularOptions(receiver, this.config.stats, this.meetingRef, STATS.RECEIVERS);
        }
