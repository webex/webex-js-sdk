...
export default class StatsAnalyzer extends EventsScope {
  ...
  analyze(data) {
    if (this.check >= this.interval) {
      ...
      this.increment();
      this.add(this.pull(this.access(data)));
    }
  }
  ...
  conduct() {
    ...
    LoggerProxy.logger.log(`stats/analyzer->conduct#Stats analysis completed: ${JSON.stringify(this.analysis)}`);
  }
  ...
  conductAnalysis() {
    ...
    this.relevance.forEach((key) => {
      ...
      if (!this.valid) {
        ...
        this.emit('meeting:stats:failure', {file: 'stats/analyzer', function: 'conductAnalysis'}, {property: key});
      }
      else {
