 ...
 */
const operateEvent = (operate, type, keys, stat, id, kind) => {
  const something = {};
  something.found = keys.some((key) => {
  ...
  receiveEventAudio(data) {
    if (data && data.data && data.data.getData()) {
      ...
      if (operate && !this.receivedFirstAudioBytes) {
        const somethingMatched = operateEvent(operate, EVENT_TYPES.REMOTE, DEFAULT_EVENT_AUDIO_RECEIVE_KEYS, EVENT_STATS_MAP.BYTES_RECEIVED, this.id, VIDEO);
  ...
  receiveEventVideo(data) {
    if (data && data.data && data.data.getData()) {
      ...
      if (operate && !this.receivedFirstVideoBytes) {
        const somethingMatched = operateEvent(operate, EVENT_TYPES.REMOTE, DEFAULT_EVENT_VIDEO_RECEIVE_KEYS, EVENT_STATS_MAP.BYTES_RECEIVED, this.id, VIDEO);
        if (somethingMatched && somethingMatched.found && somethingMatched.match) {
  ...
  sendEventVideo(data) {
    if (data && data.data && data.data.getData()) {
      ...
      if (operate && !this.sentFirstVideoBytes) {
        const somethingMatched = operateEvent(operate, EVENT_TYPES.LOCAL, DEFAULT_EVENT_VIDEO_SEND_KEYS, EVENT_STATS_MAP.BYTES_SENT, this.id, VIDEO);
        if (somethingMatched && somethingMatched.found && somethingMatched.match) {
  ...
  sendEventAudio(data) {
    if (data && data.data && data.data.getData()) {
      ...
      if (operate && !this.sentFirstAudioBytes) {
        const somethingMatched = operateEvent(operate, EVENT_TYPES.LOCAL, DEFAULT_EVENT_AUDIO_SEND_KEYS, EVENT_STATS_MAP.BYTES_SENT, this.id, AUDIO);
        if (somethingMatched && somethingMatched.found && somethingMatched.match) {
