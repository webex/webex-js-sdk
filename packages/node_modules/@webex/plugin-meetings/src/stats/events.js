...
export default class StatsEvents extends EventsScope {
  ...
  constructor(series = null, callback = () => {}) {
    super();
    // TODO: because it is based on transceiver, we can simplify the start events to be a unique events instance
    // to the sender or receiver (transceiver) data
    // TODO: client metrics bug if Alice and bob join space meeting K, bob leaves, then joins again, we need to send new media.rx.start events
    // this can be easily fixed when we refactor to do events based on transceiver
    ...
    // TODO: use the callback instead of emitting events
  }
  ...
  stop() {
    ...
    const sendAnalysis = StatsAnalyzer.analyze(fiveSecondsData, {analysisKeys: [{key: 'bytesSent', check: 'increasing'}]});
    const receiveAnalysis = StatsAnalyzer.analyze(fiveSecondsData, {analysisKeys: [{key: 'bytesReceived', check: 'increasing'}]});

    if (!sendAnalysis.valid && sendAnalysis.data.bytesSent.reports.length > 0) {
      if (!this.sendRemainStopped) {
        this.emit({file: 'stats/events', function: 'stop'}, EVENTS.STATS_UPDATE, {stop: true, stat: 'bytesSent'});
    ...
    if (!receiveAnalysis.valid && receiveAnalysis.data.bytesReceived.reports.length > 0) {
      if (!this.recvRemainStopped) {
        this.emit({file: 'stats/events', function: 'stop'}, EVENTS.STATS_UPDATE, {stop: true, stat: 'bytesReceived'});
   ...
   */
  event(data) {
    // TODO: base events on transceiver, and callbacks rather than pc model and event scope
    // return {
