...
export default class MeetingStats extends StatelessWebexPlugin {
   ...
   * @param {Boolean} optionalCreateOptions.metrics
  ...
  populate(optionalCreateOptions) {
    if (optionalCreateOptions) {
      ...
      if (optionalCreateOptions.metrics) {
        this.withMetrics();
      }
   ...
   */
  doMetrics(data) {
    if (this.metrics && data.data) {
      if (!data.data.isEmpty()) {
        this.metrics.add(data.data.omit());
      }
    }
  }
   ...
   */
  withMetrics() {
    const metrics = new StatsHistory(this.config.stats.mediaDiagnosticsInterval);
