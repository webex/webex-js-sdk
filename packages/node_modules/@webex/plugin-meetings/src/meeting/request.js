  ...
  joinMeeting(options) {
    ...
    return window.spark.request({
      ...
      .then((res) => {
        ...
        const mediaConnection = res.body.mediaConnections
                  && res.body.mediaConnections.length > 0 && res.body.mediaConnections[0];
  ...
  sendMeetingRequest(options) {
    ...
    if (localSdp || roapMessage) {
      try {
        ...
        config.console.error(`Exception in parsing reachability data : ${e}`);
      }
      localMedia.push({
    ...
    config
      .console
      .info(`Call->Locus: sending join request : ${url}`);

    // Metric
    CallDiagnosticUtil.sendCallDiagnosticEvent(
      CallDiagnosticConstants.eventType.CLIENT_LOCUS_JOIN_REQUEST, identifiers
    );
    ...
    return spark.request({
      ...
      .then((res) => {
        ...
        config
          .console
          .info(`Call-sendMeetingRequest response:${JSON.stringify(mediaConnection, null, 2)}'\n StatusCode:'${res.statusCode}`);
        locus.roapSeq = roapMessage && roapMessage.seq;
        identifiers = CallDiagnosticUtil.updateIdentifiersWithResponse(identifiers, res);
        CallDiagnosticUtil.sendCallDiagnosticEvent(
          CallDiagnosticConstants.eventType.CLIENT_LOCUS_JOIN_RESPONSE, identifiers,
