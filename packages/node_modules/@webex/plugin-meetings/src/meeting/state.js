    ...
    return new StateMachine({
      ...
      methods: {
        ...
        onRemote(transition, stop) {
          if (this.meeting) {
            Trigger.trigger(this.meeting, EVENT_TRIGGERS.MEETING_RINGING_STOP, {
              id: this.meeting.id,
              type: stop
            });
          }
        ...
        onRing(transition, type) {
          if (this.meeting) {
            Trigger.trigger(this, EVENT_TRIGGERS.MEETING_RINGING, {
              type,
              id: this.meeting.id
            });
          }
        ...
        onError(transition, error) {
          LoggerProxy.logger.error(`meeting.id: ${this.meeting.id} | Transition '${transition.transition}' : ${transition.from} -> ${transition.to} failed after last state transition, with error ${error}. Last states: ${this.history}`);
        },
        /**
        ...
        onAfterTransition(transition) {
          LoggerProxy.logger.log(`meeting.id: ${this.meeting.id} | Transition '${transition.transition}' : ${transition.from} -> ${transition.to} executed. Last states: ${this.history}`);
        }
      },
