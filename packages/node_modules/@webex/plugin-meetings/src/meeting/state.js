    ...
    return new StateMachine({
      ...
      methods: {
        ...
        onRing(transition, type) {
          if (this.meeting) {
            Trigger.trigger(this.meeting, EVENT_TRIGGERS.MEETING_RINGING, {
              type,
              id: this.meeting.id
            });
          }
        ...
        onError(transition, error) {
          LoggerProxy.logger.error(`meeting/state->onError#meeting.id: ${this.meeting.id} | Transition '${transition.transition}' : ${transition.from} -> ${transition.to} failed after last state transition, with error ${error}. Last states: ${this.history}`);
        },
        /**
