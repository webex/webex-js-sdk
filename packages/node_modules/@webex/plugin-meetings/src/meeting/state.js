...
const MeetingStateMachine = {
  ...
  create(meetingRef) {
    if (!meetingRef) {
      throw new Error('You must initialize the meeting state machine with a meeting reference.');
    }
    return new StateMachine({
      ...
      transitions: [
        ...
        // others will go straight to JOINED with this transition
        {
          ...
          from: [MEETING_STATE_MACHINE.STATES.IDLE, MEETING_STATE_MACHINE.STATES.RINGING],
        ...
        {
          name: MEETING_STATE_MACHINE.TRANSITIONS.RING_STOP,
          from: MEETING_STATE_MACHINE.STATES.JOINED,
          to(ringStop) {
            // other user answered the call
            if (ringStop.remoteAnswered) {
              return MEETING_STATE_MACHINE.STATES.ANSWERED;
            }
            // other user declined the call
            if (ringStop.remoteDeclined) {
        ...
        // when declining an incoming meeting it must be from the ringing state, and it moves to DECLINED state
        {
          ...
          from: MEETING_STATE_MACHINE.STATES.RINGING,
          to: MEETING_STATE_MACHINE.STATES.ENDED
        },
      ...
      methods: {
         ...
         * @returns {Boolean}
         */
        onRingStop(transition, stop) {
          if (this.meeting) {
            this.meeting.trigger(EVENT_TRIGGERS.MEETING_RINGING_STOP, {
              id: this.meeting.id,
              type: stop
            });
          }
        ...
        onRing(transition, type) {
          if (this.meeting) {
            this.meeting.trigger(EVENT_TRIGGERS.MEETING_RINGING, {
              type,
              id: this.meeting.id
            });
          }
        ...
        onError(transition, error) {
          this.meeting.logger.error(`meeting.id: ${this.meeting.id} | Transition '${transition.transition}' : ${transition.from} -> ${transition.to} failed after last state transition, with error ${error}. Last states: ${this.history}`);
        },
        /**
        ...
        onAfterTransition(transition) {
          this.meeting.logger.log(`meeting.id: ${this.meeting.id} | Transition '${transition.transition}' : ${transition.from} -> ${transition.to} executed. Last states: ${this.history}`);
        }
      },
