  ...
  MeetingUtil.remoteUpdateAudioVideo(audio, {mute: meeting.video.muted}, meeting)
    .then((resolution) => {
      try {
        ...
        MeetingUtil.remoteUpdateAudioVideo({mute: state.muted}, {mute: meeting.video.muted}, meeting).catch((error) => {
          console.log(
            `audio:state#onBeforeTogglek->${transition.event} fired! State failed to change with transition '${
              transition.transition
            }''. After local audio toggle failed, resetting remote also failed, meeting audio in bad state with error: ${error}.`
          );
        });
        triggerEvent(meeting, state, error, EVENT_TRIGGERS['MEDIA:AUDIO_CHANGED'], EVENT_TYPES.ERROR);
        console.log(
          `audio:state#onBeforeTogglek->${transition.event} fired! State failed to change with transition '${
            transition.transition
      ...
      triggerEvent(meeting, state, resolution, EVENT_TRIGGERS['MEDIA:AUDIO_CHANGED'], EVENT_TYPES.LOCAL);
      console.log(
