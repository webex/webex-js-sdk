      ...
      methods: {
        ...
        onAfterInit(transition, audio, meeting) {
          ...
          meeting.trigger(EVENT_TRIGGERS.MEDIA_AUDIO, {
            ...
            status: this.state,
            muted: this.muted,
            action: MEETING_AUDIO_STATE_MACHINE.TRANSITIONS.INIT
          });
          console.log(`audio:state#onAfterInit->${transition.event} fired! State changed from '${transition.from}' to '${transition.to}' with transition '${transition.transition}''.`);
        },
        onAfterHistoryBack(transition, meeting, error) {
          meeting.trigger(EVENT_TRIGGERS.MEDIA_AUDIO, {
            ...
            status: this.state,
            muted: this.muted,
            action: MEETING_AUDIO_STATE_MACHINE.TRANSITIONS.TOGGLE,
            error
          });
          console.log(`audio:state#onAfterHistoryBack->${transition.event} fired! State failed to change with transition '${transition.transition}''. State has been reset to ${this.history[this.history.length - 1]} and toggling has been reversed.`);
        },
