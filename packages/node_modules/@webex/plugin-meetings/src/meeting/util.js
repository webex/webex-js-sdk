...
MeetingUtil.handleAudioLogging = (audioTrack) => {
  if (audioTrack) {
    ...
    LoggerProxy.logger.log('MeetingUtil#handleAudioLogging');
...
MeetingUtil.handleVideoLogging = (videoTrack) => {
  LoggerProxy.logger.log('MeetingUtil#handleVideoLogging');
  if (videoTrack) {
    const settings = this.videoTrack.getSettings();

    LoggerProxy.logger.log('settings', JSON.stringify(settings));
  }
};

MeetingUtil.handleDeviceLogging = (devices) => {
  LoggerProxy.logger.log('MeetingUtil#handleDeviceLogging');

  if (devices && devices.length) {
    devices.forEach((d) => {
      LoggerProxy.logger.info(`device - ${d.label}`);
      LoggerProxy.logger.log('settings', JSON.stringify(d));
    });
  }
};
