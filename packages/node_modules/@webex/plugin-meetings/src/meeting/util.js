...
import {USER, INTENT_TO_JOIN, MODERATOR_TRUE, MODERATOR_FALSE, IDLE, JOINED} from '../constants';
...
MeetingUtil.isLocusGuestUnadmitted = (check) => check && check.guest && check.state === IDLE;

MeetingUtil.isLocusGuestAdmitted = (check) => check && check.guest && check.state === JOINED;

MeetingUtil.isUnadmittedGuest = (old, changed) => {
  if (!changed) {
    throw new Error('changed is undefined');
  }
  if (!old) {
    if (changed) {
      return MeetingUtil.isLocusGuestUnadmitted(changed);
    }
  }
  return false;
};

MeetingUtil.isAdmittedGuest = (old, changed) => {
  if (!changed) {
    throw new Error('changed is undefined.');
  }
  if (!old) {
  ...
  return MeetingUtil.isLocusGuestUnadmitted(old) && MeetingUtil.isLocusGuestAdmitted(changed);
};
