...
MeetingUtil.joinMeeting = (meeting, options) => {
  ...
  return meeting.meetingRequest
    ...
    .then((res) => {
      Metrics.joinResponse({meeting, data: {trigger: MetricsConfig.trigger.LOCI_UPDATE}});
...
MeetingUtil.canUserLock = (moderator, joined) => moderator.LOCK_CONTROL_LOCK && joined.LOCK_STATUS_UNLOCKED;

MeetingUtil.canUserUnlock = (moderator, joined) => moderator.LOCK_CONTROL_UNLOCK && joined.LOCK_STATUS_LOCKED;
...
MeetingUtil.lockMeeting = (actions, request, locusUrl) => {
  ...
  return Promise.reject(new Error('Lock not allowed.'));
};

MeetingUtil.unlockMeeting = (actions, request, locusUrl) => {
  if (actions && actions.canLock) {
    return request.unlockMeeting({locusUrl, lock: true});
  }

  return Promise.reject(new Error('Unlock not allowed.'));
};

MeetingUtil.startInternalStats = (meeting) => {
  const audioEvents = (event) => {
    if (event === 'started') {
