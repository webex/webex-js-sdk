...
import {INTENT_TO_JOIN, MODERATOR_TRUE, MODERATOR_FALSE, EVENTS} from '../constants';
...
MeetingUtil.getSdps = (meeting) => {
  ...
  if (meeting.mediaProperties.peerConnection) {
    sdps.push(meeting.mediaProperties.peerConnection.sdp);
  }
...
MeetingUtil.leaveMeeting = (meeting) => meeting.meetingRequest
  ...
  .catch((err) => {
    meeting.logger.error(`An error occured while trying to leave meeting with an id of ${meeting.id}`, err);
...
MeetingUtil.joinMeetingOptions = (meeting, options) => {
  ...
  return MeetingUtil.joinMeeting(meeting, options)
    ...
    .catch((err) => {
      ...
      this.meeting.logger.log('Meeting:index#join --> Error joining the call on getLocalMedia, ', err);
...
MeetingUtil.validateOptions = (options) => {
  ...
  if (sendVideo && (!localStream || localStream.getVideoTracks().length === 0)) {
    return Promise.reject(new Error('please pass valid video streams'));
  }

  if (sendAudio && (!localStream || localStream.getAudioTracks().length === 0)) {
    return Promise.reject(new Error('please pass valid audio streams'));
  }

  if (sendShare && (!localStream || localShare.getVideoTracks().length === 0)) {
