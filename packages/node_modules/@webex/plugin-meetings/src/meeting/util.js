...
MeetingUtil.isGuest = (locus) => {
  const activeLocus = locus.locus || locus;
  if (!activeLocus) {
    return false;
  }
  return activeLocus && activeLocus.self && activeLocus.self.guest && activeLocus.self.state === IDLE;
};
...
MeetingUtil.isAdmittedGuest = (old, changed) => {
  ...
  return (old.guest && old.state === IDLE) && (changed.state === JOINED);
};
