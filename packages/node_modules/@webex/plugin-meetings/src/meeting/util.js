...
MeetingUtil.remoteUpdateAudioVideo = (audioMuted, videoMuted, meeting) => {
  if (!meeting) {
    return Promise.reject(new ParameterError(new Error('You need a meeting object.')));
  }
  const localMedias = Media.generateLocalMedias(meeting.mediaId, audioMuted, videoMuted);

  if (isEmpty(localMedias)) {
    return Promise.reject(new ParameterError(new Error('You need a media id on the meeting to change remote audio.')));
  }

  Metrics.sendMediaRequest({meeting});
  ...
  return meeting.meetingRequest.remoteAudioVideoToggle({
    ...
    Metrics.sendMediaResponse({meeting});
...
MeetingUtil.joinMeeting = (meeting, options) => {
  ...
  return meeting.meetingRequest
    ...
    .then((res) => {
      Metrics.joinResponse({
        meeting,
        data: {
          trigger: MetricsConfig.trigger.LOCI_UPDATE,
...
MeetingUtil.joinMeetingOptions = (meeting, options = {}) => {
  ...
  return MeetingUtil.joinMeeting(meeting, options)
    ...
    .catch((err) => {
      ...
      return Promise.reject(new JoinMeetingError(err));
    });
};
...
MeetingUtil.validateOptions = (options) => {
  ...
  if (sendVideo && !MeetingUtil.getTrack(localStream).videoTrack) {
    return Promise.reject(new ParameterError(new Error('please pass valid video streams')));
  }

  if (sendAudio && !MeetingUtil.getTrack(localStream).audioTrack) {
    return Promise.reject(new ParameterError(new Error('please pass valid audio streams')));
  }

  if (sendShare && !MeetingUtil.getTrack(localShare).videoTrack) {
    return Promise.reject(new ParameterError(new Error('please pass valid share streams')));
  }
...
MeetingUtil.startRecording = (request, locusUrl, locusInfo) => {
  ...
  return Promise.reject(new PermissionError(new Error('Start recording not allowed, due to moderator property.')));
};
...
MeetingUtil.pauseRecording = (request, locusUrl, locusInfo) => {
  ...
  return Promise.reject(new PermissionError(new Error('Pause recording not allowed, due to moderator property.')));
};
...
MeetingUtil.resumeRecording = (request, locusUrl, locusInfo) => {
  ...
  return Promise.reject(new PermissionError(new Error('Resume recording not allowed, due to moderator property.')));
};
...
MeetingUtil.stopRecording = (request, locusUrl, locusInfo) => {
  ...
  return Promise.reject(new PermissionError(new Error('Stop recording not allowed, due to moderator property.')));
};
...
MeetingUtil.lockMeeting = (actions, request, locusUrl) => {
  ...
  return Promise.reject(new PermissionError(new Error('Lock not allowed, due to joined property.')));
};
...
MeetingUtil.unlockMeeting = (actions, request, locusUrl) => {
  ...
  return Promise.reject(new PermissionError(new Error('Unlock not allowed, due to joined property.')));
};
...
MeetingUtil.startInternalStats = (meeting) => {
  if (!meeting.internalStats) {
    ...
    // pass the attrs and options for the spark instance
  ...
  peerConnection.oniceconnectionstatechange = (ev) => {
    ...
    switch (peerConnection.iceConnectionState) {
      case ICE_STATE.CHECKING:
        Metrics.iceStart({meeting});
        break;
      case ICE_STATE.COMPLETED:
        Metrics.iceStop({meeting});
        break;
          ...
          data: {
            ...
            errors: [
              Metrics.generateErrorPayload(
                2004, false, MetricsConfig.error.name.MEDIA_ENGINE
              )]
          }
        });
        break;
