...
MeetingUtil.remoteUpdateAudioVideo = (audioMuted, videoMuted, meeting) => {
  ...
  console.log('MUTE MEETING ', meeting.locusInfo.self);
...
MeetingUtil.joinMeetingOptions = (meeting, options) => {
  ...
  return MeetingUtil.joinMeeting(meeting, options)
    .then((response) => {
      console.log(`${meeting.name} JOIN-RESPONSE: LOCUS OBJECT`, JSON.stringify(response, null, '\t'));
      meeting.setLocus(response);

      return Promise.resolve();
    })
    .catch((err) => {
      console.log(`ERROR JOINING MEETING ${this.name}`, err);
...
MeetingUtil.startRecording = (meeting) => {
  if (meeting.locusInfo &&
    meeting.locusInfo.parsedLocus &&
    meeting.locusInfo.parsedLocus.info &&
    meeting.locusInfo.parsedLocus.info &&
    MeetingUtil.canUserRecord(meeting.locusInfo.parsedLocus.info.moderator)) {
    return meeting.meetingRequest.recordMeeting({locusUrl: meeting.locusUrl, recording: true, paused: false});
  }

  return Promise.reject(new Error('Start recording not allowed.'));
};

MeetingUtil.pauseRecording = (meeting) => {
  if (meeting.locusInfo &&
    meeting.locusInfo.parsedLocus &&
    meeting.locusInfo.parsedLocus.info &&
    meeting.locusInfo.parsedLocus.info &&
    MeetingUtil.canUserPause(meeting.locusInfo.parsedLocus.info.moderator)) {
    return meeting.meetingRequest.recordMeeting({locusUrl: meeting.locusUrl, recording: true, paused: true});
  }

  return Promise.reject(new Error('Pause recording not allowed.'));
};

MeetingUtil.resumeRecording = (meeting) => {
  if (meeting.locusInfo &&
    meeting.locusInfo.parsedLocus &&
    meeting.locusInfo.parsedLocus.info &&
    meeting.locusInfo.parsedLocus.info &&
    MeetingUtil.canUserResume(meeting.locusInfo.parsedLocus.info.moderator)) {
    return meeting.meetingRequest.recordMeeting({locusUrl: meeting.locusUrl, recording: true, paused: false});
  }
...
MeetingUtil.stopRecording = (meeting) => {
  if (meeting.locusInfo &&
    meeting.locusInfo.parsedLocus &&
    meeting.locusInfo.parsedLocus.info &&
    meeting.locusInfo.parsedLocus.info &&
    MeetingUtil.canUserStop(meeting.locusInfo.parsedLocus.info.moderator)) {
    return meeting.meetingRequest.recordMeeting({locusUrl: meeting.locusUrl, recording: false, paused: false});
  }
