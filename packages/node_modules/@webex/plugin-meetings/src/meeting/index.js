...
export default class Meeting extends StatelessSparkPlugin {
  ...
  setUpLocusInfoSelfListener() {
    this.locusInfo.on(LOCUSINFO.EVENTS.SELF_UNADMITTED_GUEST, (payload) => {
      if (payload) {
        Trigger.trigger(this, EVENT_TRIGGERS.MEETING_SELF_LOBBY_WAITING, {
          payload
        });
      }
    });
    this.locusInfo.on(LOCUSINFO.EVENTS.SELF_ADMITTED_GUEST, (payload) => {
      if (payload) {
        Trigger.trigger(this, EVENT_TRIGGERS.MEETING_SELF_GUEST_ADMITTED, {
          payload
        });
      }
    });
  ...
  setUpLocusInfoMeetingListener() {
    ...
    this.locusInfo.on(EVENTS.DESTROY_MEETING, (payload) => {
      ...
      if (payload.selfState && payload.selfState !== MEETING_STATE.STATES.LEFT) {
        // TODO: FIX THIS AS IT LEADS TO A LOOP FOR END MEETING
        // LEAVE=>LOCUSRESPONSE=>LOCUS_INFO=>DESTROY=>LEAVE
        this.leave();
      }
  ...
  updateMeetingObject(object) {
    ...
    LoggerProxy.logger.log(`Meeting->updateMeeting#Meeting updated with ${JSON.stringify(object)}`);
  ...
  getStats(options, override) {
    ...
    if (override) {
      if (this.stats) {
        console.warn('meeting/index->getStats#Overriding the previous stats object without destroying first can result in memory leaks.');
      }
  ...
  setRemoteStream(pc) {
    ...
    pc.ontrack = (event) => {
      ...
      Trigger.trigger(this, EVENT_TRIGGERS.MEDIA_READY, {
      ...
      Trigger.trigger(this, EVENT_TRIGGERS.MEDIA_READY, {
        ...
        stream: MediaUtil.createMediaStream([
          this.mediaProperties.remoteStream.getVideoTracks()[1]])
      });
    };
  }
  ...
  closeRemoteStream() {
    return Media.stopStream(this.mediaProperties.remoteStream).then(() => {
      Trigger.trigger(this, EVENT_TRIGGERS.MEDIA_STOPPED, {
  ...
  reconnect() {
    ...
    return this.reconnectionManager
      .reconnect(this)
      .then((reconnect) => {
        Trigger.trigger(this, EVENT_TRIGGERS.MEETING_RECONNECTION_SUCCESS, {
          reconnect
        });
      ...
      .catch((error) => {
        Trigger.trigger(this, EVENT_TRIGGERS.MEETING_RECONNECTION_FAILURE, {
  ...
  join(options = {}) {
    ...
    if (options.resourceId === this.destination) {
  ...
  updateMedia(options = {}) {
    ...
    return Media.updateMedia(this.mediaProperties, PeerConnectionManager, localStream, localShare,
      ...
      .catch((err) => {
        LoggerProxy.logger.log('Meeting:index#join --> Error joining the call on roap initialization, ', err);
        throw err;
      })
      .then(() =>
        this.roap
  ...
  reconnectMedia(reCreate) {
    let promise = null;

    if (reCreate) {
      promise = Media.attachMedia(this.mediaProperties)
        .then((peerConnection) => this.setRemoteStream(peerConnection));
    }
    else {
      promise = Media.reconnectMedia(this.mediaProperties.peerConnection);
    }
  ...
  share() {
    ...
    if (content) {
      return this.meetingRequest.changeMeetingFloor({
        ...
        .then(() => {
          this.trigger('STARTED_SHARING_LOCAL');
        });
    }
  ...
  stopShare() {
    ...
    if (content) {
      ...
      return this.meetingRequest.changeMeetingFloor({
        ...
        this.isSharing = false;
        this.trigger('STOPPED_SHARING_LOCAL'); // TODO: constantize and document
      });
    }
