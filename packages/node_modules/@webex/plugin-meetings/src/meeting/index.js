...
export default class Meeting extends StatelessWebexPlugin {
  ...
  */
  ...
  /**
   * Fetches meeting information for public use.
   * @public
   * @memberof Meeting
   * @returns {Promise}
   */
  fetchMeetingInfoForMeeting() {
    // when fetch meeting info is called directly by the client, we want to clear out the random timer for sdk to do it
    if (this.setTimeoutFetchMeetingInfoId) {
      clearTimeout(this.setTimeoutFetchMeetingInfoId);
    }
    // we do not call fetchMeetingInfo if this one has already been called before
    if (!this.fetchMeetingInfoCalled) {
      return this.fetchMeetingInfo({destination: this.sipUri, type: _SIP_URI_});
    }
  ...
  async fetchMeetingInfo({
    ...
    this.fetchMeetingInfoCalled = true;
    ...
    try {
      ...
      this.parseMeetingInfo(info, destination);
   ...
   * @param {Object} destination locus object with meeting data
   * @returns {undefined}
   * @private
   * @memberof Meeting
   */
  parseMeetingInfo(meetingInfo, destination = null) {
    ...
    const locusMeetingObject = destination;
    ...
    if (webexMeetingInfo && !(meetingInfo.errors && meetingInfo.errors.length > 0)) {
      ...
      this.setSipUri(this.config.experimental.enableUnifiedMeetings ? webexMeetingInfo.sipUrl : locusMeetingObject.info.sipUri || webexMeetingInfo.sipMeetingUri || this.sipUri);
