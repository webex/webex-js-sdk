...
export default class Meeting extends StatelessSparkPlugin {
  ...
  setUpCallMetrics() {
    // TODO: started rendering and stopped rendering should be here as devloper cannot send this specific event
    // Or we can leave it to the developer
    // Metrics.renderingStarted()
    // Metrics.renderingStopped()
    const options = {
      audioEvent: (event) => {
        if (event === 'started') {
          Metrics.startedRecvMedia({
            meeting: this,
            data: {
              mediaType: MetricsConfig.mediaType.AUDIO
            }
          });
        }
        else if (event === 'stopped') {
          Metrics.stoppedRecvMedia({
            meeting: this,
            data: {
              mediaType: MetricsConfig.mediaType.AUDIO
            }
      ...
      videoEvent: (event) => {
        if (event === 'started') {
          Metrics.startedRecvMedia({
            meeting: this,
            data: {
              mediaType: MetricsConfig.mediaType.VIDEO
            }
          });
        }
        else if (event === 'stopped') {
      ...
      shareEvent: (event) => {
        if (event === 'started') {
          Metrics.startedRecvMedia({
            meeting: this,
            data: {
              mediaType: MetricsConfig.mediaType.SHARE
            }
          });
        }
        else if (event === 'stopped') {
  ...
  setUpLocusInfoMeetingInfoListener() {
    ...
    this.locusInfo.on(LOCUSINFO.EVENTS.MEETING_INFO_UPDATED, (payload) => {
      if (payload) {
        const newCanLock = MeetingUtil.canUserLock(payload.moderator, payload.policy);
        const newCanUnlock = MeetingUtil.canUserUnlock(payload.moderator, payload.policy);
        ...
        if (newCanUnlock && !this.inMeetingActions.canUnlock) {
          Trigger.trigger(
            this,
            {
              file: 'meeting/index',
  ...
  addMedia(options = {}) {
    ...
    return MeetingUtil.validateOptions(options)
      .then(() => {
        MeetingUtil.setPeerConnectionEvents(this.mediaProperties.peerConnection);

        return this.preMedia(localStream, localShare, mediaSettings);
      })
      .then(() => Media.attachMedia(this.mediaProperties, this.meetingId)
        .then((peerConnection) => {
  ...
  updateMedia(options = {}) {
    ...
    return MeetingUtil.validateOptions(options)
      ...
      .then(() => Media.updateMedia(this.mediaProperties, this.meetingId)
        .then((peerConnection) => {
  ...
  updateTransceiver(options) {
    ...
    (receiveTrack !== undefined && receiveTrack !== previousMediaDirection.receiveTrack)) {
      return Media.updateTransceiver(this.meetingId, peerConnection, transceiver,
        {
          track,
          type,
          receiveTrack,
