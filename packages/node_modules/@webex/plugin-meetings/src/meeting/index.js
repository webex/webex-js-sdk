...
import {MEETINGS, EVENT_TRIGGERS, EVENT_TYPES, FLOOR_ACTION} from '../constants';
...
export default class Meeting extends StatelessSparkPlugin {
  ...
  parseLocus(locus) {
    // parse all the object and attach it
    this.locusUrl = locus.url;
    this.type = this.getLocusType(locus.info.locusTags);
    this.partner = locus.participants.find((p) => (locus.self && p.identity !== locus.self.identity)
      && (locus.participants.length <= 2 || (p.type === 'USER' && !p.removed))) || this.partner;

    if (this.partner) {
      this.sipUri = this.partner.person.sipUrl || this.partner.person.emailAddress || this.partner.person.email;
    }
    this.setLocusJoin(locus);
  }
  ...
  getLocusType(locusTags) {
    // Parse all the value and get the type of meeting
    if (locusTags.find((tag) => tag === 'ONE_ON_ONE')) {
      return 'ONE_ON_ONE';
    }
    return null;
  }
  ...
  join(options = {}) {
    ...
    return MeetingUtil.joinMeeting(this)
      .then((response) => {
        ...
        // this.state.connect();
        // TODO: different scenarios for meeting , claimed and unclaimed
      })
      .catch((err) => {
