...
export default class Meeting extends StatelessWebexPlugin {
  ...
  */
  ...
  setLocalAudioTrack(audioTrack, emitEvent = true) {
    if (audioTrack) {
      // TODO - Add logic for more settings
  ...
  setLocalTracks(localStream) {
    if (localStream) {
      ...
      audioTrack.onmute = (e) => this.handleAudioLogging(e);
      audioTrack.onunmute = (e) => this.handleAudioLogging(e);
      videoTrack.onended = (e) => this.handleVideoLogging(e);
      videoTrack.onmute = (e) => this.handleVideoLogging(e);

      // TODO - did this trigger? any standard?
      // audioTrack.addEventListener('unmute', (e) => this.handleAudioLogging(e));
      // audioTrack.addEventListener('mute', (e) => this.handleAudioLogging(e));
      // videoTrack.addEventListener('ended', (e) => this.handleVideoLogging(e));
  ...
  muteVideo() {
    ...
    return logRequest(this.video.handleClientRequest(this, true)
      .then(() => {
        LoggerProxy.logger.info(LOG_HEADER, 'muted');
        Metrics.postEvent({
          event: eventType.MUTED,
          meeting: this,
  ...
  */
  ...
  /**
   *
   * Handle logging various things for meetings. Such as audio, video and device details.
   * @returns {Promise} resolves to an array of DeviceInfo
   * @memberof Meetings
   */
  handleMediaLogging = async () => {
    LoggerProxy.logger.log('Meeting:index#handleMediaLogging');
    await this.handleDeviceLogging();
  }

  /**
   * Handle audio player logging, dumps details such as autoGainControl, echoCancellation
   * volume and deviceId
   * @returns {Promise} resolves empty promise
   * @memberof Meetings
   */
  handleAudioLogging = async () => {
    if (this.audioTrack) {
      const {label} = this.audioTrack;
      const {
        autoGainControl, echoCancellation, noiseSuppression, deviceId
      } = this.audioTrack.getSettings();

      LoggerProxy.logger.log('Meeting:index#handleAudioLogging');
      LoggerProxy.logger.group('audio');
      LoggerProxy.logger.log('deviceId=', deviceId);
      LoggerProxy.logger.log('label=', label);
      LoggerProxy.logger.log('noiseSuppression=', noiseSuppression);
      LoggerProxy.logger.log('echoCancellation=', echoCancellation);
      LoggerProxy.logger.log('autoGainControl=', autoGainControl);
      LoggerProxy.logger.groupEnd('audio');
    }
  ...
  /**
   * Handle video player logging, dumps video details such as
   * frameRate, resolution and deviceId
   * @returns {Promise} resolves empty promise
   * @memberof Meetings
   */
  handleVideoLogging = () => {
    if (this.videoTrack) {
      const {
        deviceId, frameRate, aspectRatio, height, width
      } = this.videoTrack.getSettings();

      LoggerProxy.logger.log('Meeting:index#handleVideoLogging');
      LoggerProxy.logger.group('video');
      LoggerProxy.logger.log('deviceId=', deviceId);
      LoggerProxy.logger.log('aspectRatio=', aspectRatio);
      LoggerProxy.logger.log('frameRate=', frameRate);
      LoggerProxy.logger.log('width=', width);
      LoggerProxy.logger.log('height=', height);
      LoggerProxy.logger.groupEnd('video');
    }
  ...
  /**
   * Handle device logging, each device will be logged to console.
   * @returns {Promise} resolves empty promise
   * @memberof Meetings
   */
  handleDeviceLogging = async () => {
    LoggerProxy.logger.log('Meeting:index#handleDeviceLogging');
    LoggerProxy.logger.group('devices');
    const devices = await Media.getDevices();

    if (devices && devices.length) {
      devices.forEach((d) => {
        LoggerProxy.logger.group(`device - ${d.label}`);
        LoggerProxy.logger.info('deviceId=', d.deviceId);
        LoggerProxy.logger.info('groupId=', d.groupId);
        LoggerProxy.logger.info('kind=', d.kind);
        LoggerProxy.logger.groupEnd(`device - ${d.label}`);
      });
    }
    LoggerProxy.logger.groupEnd('devices');
