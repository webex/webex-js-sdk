...
export default class Meeting extends StatelessSparkPlugin {
  ...
  constructor(attrs, options) {
     ...
     * @memberof Meeting
     */
    this.roap = new Roap({}, {parent: this.spark});
    /**
     * created later
     * @instance
     ...
     * @memberof Meeting
     */
    this.locusInfo = new LocusInfo(this.updateMeetingObject.bind(this), attrs.locus ? attrs.locus : {}, this.spark, this.id);
    /**
     * Passing only info as we send basic info for meeting added event
     * @instance
  ...
  sendCallAnalyzerMetrics(options) {
    ...
    // config.console.info('[Call Diagnostic Event:', eventType, '] correlationId:', identifiers.correlationId);
    this.spark.internal.metrics.submitCallDiagnosticEvents(payload);
  }
  ...
  setUpLocusInfoSelfListener() {
    ...
    this.locusInfo.on(LOCUSINFO.EVENTS.SELF_UNADMITTED_GUEST, (payload) => {
      if (payload) {
        ...
        Metrics.enteredLobby({meeting: this});
      }
    });
    this.locusInfo.on(LOCUSINFO.EVENTS.SELF_ADMITTED_GUEST, (payload) => {
      if (payload) {
        ...
        Metrics.exitedLobby({meeting: this});
      }
    });
  }
  ...
  remove(removal) {
    return this.members.removeMember(removal);
  }
  ...
  muteAudio() {
    if (!this.audio || (this.audio && !this.audio.toggle)) {
      return Promise.reject(new ParameterError(new Error('no audio control associated to the meeting')));
    }
    ...
    return this.audio.toggle({
      ...
      .then(() => {
        ...
        Metrics.muted({
          meeting: this,
          data: {trigger: MetricsConfig.trigger.USER_INTERACTION, mediaType: MetricsConfig.mediaType.AUDIO}
        });
  ...
  muteVideo() {
    if (!this.video || (this.video && !this.video.toggle)) {
      return Promise.reject(new ParameterError(new Error('no video control associated to the meeting')));
    }
    ...
    return this.video.toggle({
      ...
      .then(() => {
        Metrics.muted({
          meeting: this,
          data: {trigger: MetricsConfig.trigger.USER_INTERACTION, mediaType: MetricsConfig.mediaType.VIDEO}
        });
      });
  }
  ...
  reconnect() {
    if (!this.reconnectionManager || !this.reconnectionManager.reconnect) {
      throw new ParameterError(new Error('Cannot reconnect, ReconnectionManager must first be defined.'));
    }
    Trigger.trigger(
      this,
      {
    ...
    Metrics.mediaReconnected({meeting: this});
    ...
    return this.reconnectionManager
      ...
      .then((reconnect) => {
        ...
        Metrics.mediaRecovered({meeting: this});
      ...
      .catch((error) => {
        ...
        Metrics.mediaReconnectionFailed({meeting: this});
  ...
  join(options = {}) {
    ...
    if (false) {
      ...
      Metrics.callInitiated({
        meeting: this,
        data: {trigger: MetricsConfig.trigger.USER_INTERACTION}
      });
    }
  ...
  moveTo(resourceId) {
    if (!resourceId) {
      throw new ParameterError(new Error('Cannot move call without a resourceId.'));
    }

    return MeetingUtil.joinMeetingOptions(this, {resourceId, moveToResource: true}).then((join) => {
      this.meetingFSM.join();
  ...
  moveFrom(resourceId) {
    ...
    return this.getMediaStreams({sendAudio: true, sendVideo: true, sendShare: false})
      .then(([localStream, localShare]) => this.preMedia(localStream, localShare, mediaSettings))
      .then(() => Media.updateMedia(this.mediaProperties).then((peerConnection) => {
        ...
        if (mediaSettings.receiveShare || mediaSettings.sendShare) {
          PeerConnectionManager.setContentSlides(peerConnection);
        }

        return this.roap.sendRoapCallRequest({
          meeting: this,
          sdp: peerConnection.sdp,
          roapSeq: this.roapSeq
        });
      }));
    // .then(() => MeetingUtil.leaveMeeting(this, {resourceId, correlationId: this.correlationId, moveMeeting: true}));
    // MeetingUtil.joinMeetingOptions(this).then((join) => this.getMediaStreams({sendAudio: true, sendVideo: true, sendShare: false})
  ...
  getMediaStreams = (mediaDirection, audioVideo = {}) => Media.getUserMedia(mediaDirection, audioVideo)
    .then((response) => {
      if (!response[0] && !response[1]) {
        return Promise.reject(new MediaError(new Error('Was not able to retrive media Stream')));
      }
  ...
  addMedia(options = {}) {
    if (MeetingUtil.isGuestUnjoined(this.guest, this.locusInfo) && !this.wirelessShare) {
      return Promise.reject(new MediaError(new Error('To add media, the guest user must be admitted. Wait to be admitted to call addMedia')));
    }
  ...
  updateTransceiver(options) {
    ...
    (receiveTrack !== undefined && receiveTrack !== previousMediaDirection.receiveTrack)) {
      return Media.updateTransceiver(this.id, peerConnection, transceiver,
        {
          track,
          type,
          receiveTrack,
    ...
    return Promise.reject(new ParameterError(new Error('update Failed: please pass valid parameter')));
  }
  ...
  updateAudio(options) {
    ...
    if (typeof sendAudio !== 'boolean' || typeof receiveAudio !== 'boolean') {
      return Promise.reject(new ParameterError(new Error('Pass sendVideo and receiveVideo parameter')));
    }
  ...
  updateShare(options) {
    ...
    if (typeof sendShare !== 'boolean' || typeof receiveShare !== 'boolean') {
      return Promise.reject(new ParameterError(new Error('Pass sendShare and receiveShare parameter')));
    }
    const previousSendShareStatus = this.mediaProperties.mediaDirection.sendShare;
    ...
    return MeetingUtil.validateOptions({sendShare, localShare: stream})
      ...
      .then((startShare) => this.updateTransceiver({
        type: 'video',
        sendTrack: sendShare,
  ...
  acknowledge(type) {
    if (!type) {
      return Promise.reject(new ParameterError(new Error('Type must be set to acknowledge the meeting.')));
    }
    if (type === _INCOMING_) {
      return this.meetingRequest
        ...
        .then((response) => {
          ...
          Metrics.alertDisplayed({meeting: this});
   ...
   */
  leave(options = {}) {
    Metrics.leaveMeeting({meeting: this, data: {trigger: MetricsConfig.trigger.USER_INTERACTION, canProceed: false}});
  ...
  share() {
    ...
    if (content && !this.isSharing) {
      Metrics.shareInitiated({meeting: this});
      ...
      return this.meetingRequest.changeMeetingFloor({
        ...
        .then(() => {
          ...
          Metrics.shareFloorGranted({meeting: this});
        })
        .catch((e) => {
    ...
    return Promise.reject(new ParameterError(new Error('Cannot share without content.')));
  }
  ...
  stopShare() {
    ...
    if (content && this.isSharing) {
      Metrics.shareStopped({meeting: this});
    ...
    return Promise.reject(new ParameterError(new Error('Cannot stop share without content')));
  }
