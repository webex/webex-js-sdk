  ...
  */
  constructor(attrs, options) {
    ...
    */
    ...
    /**
     * Whether a dial in device was is in a state that's ready to accept/already accepting a call.
     * @instance
     * @type {Boolean}
     * @private
     * @memberof Meeting
     */
    this.isDialInDeviceActive = false;
    /**
  ...
  pstnUpdate(payload) {
    if (this.locusInfo.self) {
      ...
      if (dialInPstnDevice) {
        this.isDialInDeviceActive = dialInPstnDevice.state === 'JOINED' || dialInPstnDevice.state === 'CONNECTED';
        Trigger.trigger(
          this,
          {
            file: 'meeting/index',
          ...
          EVENT_TRIGGERS.MEETING_SELF_PSTN_UPDATE,
          {
            dialIn: {
              status: dialInPstnDevice.dialingStatus ?? dialInPstnDevice.state,
  ...
  /**
   * Convenience function to tell whether a meeting has an audio connection
   * @returns {Boolean} if meeting audio is connected or not
   * @public
   * @memberof Meeting
   */
  isAudioConnected() {
  ...
  /**
   * Convenience function to tell whether a meeting has a video connection
   * @returns {Boolean} if meeting video connected or not
   * @public
   * @memberof Meeting
   */
  isVideoConnected() {
   ...
   */
  dialInPstn() {
    if (this.isDialInDeviceActive) return; // prevent multiple dial in devices from being provisioned

    const {correlationId, locusUrl} = this;

    if (!this.dialInUrl) this.dialInUrl = `dialin:///${uuid.v4()}`;

    this.meetingRequest.dialIn({
      correlationId,
      dialInUrl: this.dialInUrl,
      locusUrl,
      clientUrl: this.deviceUrl
    }).then((res) => {
      this.locusInfo.onDeltaLocus(res.body.locus);
    }).catch((error) => {
      Metrics.sendOperationalMetric(
