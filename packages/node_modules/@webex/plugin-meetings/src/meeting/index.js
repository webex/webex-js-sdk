...
export default class Meeting {
  ...
  constructor(options) {
    ...
    ON_GOING --> join:DIALING
    ON_GOING --> error:ERROR

    INCOMING --> join:DIALING
    INCOMING --> error:ERROR

    DIALING --> local:CONNECTING_MEDIA
    DIALING --> error:ERROR

    CONNECTING_MEDIA_LOCAL --> connect:CONNECTING_MEDIA_REMOTE
    CONNECTING_MEDIA_LOCAL --> error:ERROR

    CONNECTING_MEDIA_REMOTE --> establish:ESTABLISHED_MEDIA
    CONNECTING_MEDIA_REMOTE --> error:ERROR

    ESTABLISHED_MEDIA --> leave:TERMINATING
    ESTABLISHED_MEDIA --> error:ERROR

    TERMINATING --> end:ENDED
    ...
    ERROR: --> clean:IDLE
    // TODO, potentially, error can do a previous:<STATE>, rather than an entire cleanup
    */
    this.state = new StateMachine({
      ...
      methods: {
        ...
        onJoin: (transition) => {
          console.log(`Meeting:index#onJoin->${transition.event} fired! State changed from '${transition.from}' to '${transition.to}' with transition '${transition.transition}' on Meeting Object: '${this.id}'.`);
        },
        onLocal: (transition) => {
          console.log(`Meeting:index#onLocal->${transition.event} fired! State changed from '${transition.from}' to '${transition.to}' with transition '${transition.transition}' on Meeting Object: '${this.id}'.`);
        },
        onConnect: (transition) => {
          console.log(`Meeting:index#onConnect->${transition.event} fired! State changed from '${transition.from}' to '${transition.to}' with transition '${transition.transition}' on Meeting Object: '${this.id}'.`);
        },
        onEstablish: (transition) => {
          console.log(`Meeting:index#onEstablish->${transition.event} fired! State changed from '${transition.from}' to '${transition.to}' with transition '${transition.transition}' on Meeting Object: '${this.id}'.`);
        }
