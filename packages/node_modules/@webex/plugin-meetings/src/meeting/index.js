  ...
  getAnalyzerMetricsPrePayload(options) {
    if (options) {
      ...
      const identifiers = {
        ...
        orgId: null // TODO :find org Id if not found have a bummy org
        // locusUrl
      };
      ...
      .then(() => Media.attachMedia(this.mediaProperties, {
        ...
        .then((peerConnection) => {
          ...
          this.webrtcStats = new StatsAnalyzer({
            interval: 5000, mqaInterval: 60000, sendMqa: true, correlationId: this.correlationId
          });
          this.webrtcStats.on(EVENT_TRIGGERS.MEETING_HIGH_PACKETLOSS, (res) => {
            if (res.mediaType === VIDEO && this.mediaProperties.localQualityLevel !== QUALITY_LEVELS.MEDIUM && this.mediaProperties.localQualityLevel !== QUALITY_LEVELS.LOW) {
              this.setLocalVideoQuality(QUALITY_LEVELS.MEDIUM);
            }
            else if (res.mediaType === VIDEO && this.mediaProperties.localQualityLevel !== QUALITY_LEVELS.LOW) {
          ...
          this.webrtcStats.startAnalyzer(peerConnection);
