...
import {MEETINGS, EVENT_TRIGGERS, EVENT_TYPES, FLOOR_ACTION, BUSY, OUTGOING, MODERATOR_FALSE, MODERATOR_TRUE} from '../constants';
 ...
 */
  ...
  */
        ...
        if (MeetingUtil.isPinOrGuest(err)) {
          if (MeetingUtil.hasOwner(this.meetingInfo)) {
            return MeetingUtil.joinMeeting(this, {
              moderator: MeetingUtil.isOwnerSelf(this.owner, this.userId) ? MODERATOR_TRUE : MODERATOR_FALSE
            })
              .then((response) => {
                this.setLocus(response);
                // this.state.connect();
          ...
          // request host pin or non host for unclaimed PMR
          this.trigger(EVENT_TRIGGERS.LOBBY_HOST_GUEST, {
            error: err,
            message: 'The meeting is locked. This is expected behavior. Call #joinAsHost or #joinAsGuest',
  ...
  /**
   * @param {Number} hostPin a 6 digit webex hostpin for unclaimed PMR host pin
   * @param {Boolean} moderator optional
   * @returns {Promise}
   * @throws {Error} if hostPin is not passed
   * @memberof Meeting
   */
  joinLobbyAsHost(hostPin, moderator = MODERATOR_TRUE) {
    if (!hostPin) {
      throw new Error('Joining as host requires a hostPin.');
    }
    return this.join({hostPin, moderator});
  }
   ...
   * @memberof Meeting
   */
  joinLobbyAsGuest(moderator = MODERATOR_FALSE) {
    return this.join({moderator});
  }
   ...
   * @memberof Meeting
   */
  getMediaStreams(mediaDirection) {
    return Media.getUserMedia(mediaDirection);
  }
