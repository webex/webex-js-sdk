...
export default class Meeting extends StatelessWebexPlugin {
  ...
  */
  ...
  join(options = {}) {
    ...
    LoggerProxy.logger.group('JOIN MEETING');
    LoggerProxy.logger.log(`${Meeting.name}#${this.join.name} --> Joining a meeting`);
    ...
    return MeetingUtil.joinMeetingOptions(this, options).then((join) => {
      ...
      LoggerProxy.logger.log(`${Meeting.name}#${this.join.name} --> Success`);
      LoggerProxy.logger.groupEnd();

      return join;
    }).catch((error) => {
      this.meetingFiniteStateMachine.fail(error);

      LoggerProxy.logger.error(`${Meeting.name}#${this.join.name} --> Failed`, error);
      LoggerProxy.logger.groupEnd();
  ...
  addMedia(options = {}) {
    ...
    return MeetingUtil.validateOptions(options)
      ...
      .then(() => Media.attachMedia(this.mediaProperties, {
            ...
            this.statsAnalyzer.on(EVENT_TRIGGERS.MEETING_HIGH_PACKETLOSS, (res) => {
              if (res.mediaType === VIDEO) {
                if (this.config.stats.autoDowngradeEnabled && this.canUpdateMedia()) {
                  LoggerProxy.logger.log('Meeting:index#addMedia --> High Packet Loss Detected, Adjusting Local Video Quality');
        ...
        .catch((err) => {
          LoggerProxy.logger.error('Meeting:index#addMedia --> Error joining the call on roap initialization, ', err);
          throw err;
        })
        ...
        .then(() => this.roap
          ...
          .catch((err) => {
            LoggerProxy.logger.error('Meeting:index#addMedia --> Error joining the call on send roap media request, ', err);
            throw err;
          }))
  ...
  leave(options = {}) {
    ...
    LoggerProxy.logger.group('LEAVE MEETING');
    LoggerProxy.logger.log(`${Meeting.name}#${this.leave.name} --> Leaving a meeting`);
    ...
    return MeetingUtil.leaveMeeting(this, options)
      .then((leave) => {
        ...
        this.meetingFiniteStateMachine.fail(error);
        LoggerProxy.logger.error(`${Meeting.name}#${this.leave.name} --> Failed to leave `, error);
        LoggerProxy.logger.groupEnd();
