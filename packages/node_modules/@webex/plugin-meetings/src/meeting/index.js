...
import MuteState from '../meeting/muteState';
...
export default class Meeting extends StatelessWebexPlugin {
  ...
  */
  ...
  setUpLocusInfoSelfListener() {
    this.locusInfo.on(LOCUSINFO.EVENTS.LOCAL_UNMUTE_REQUIRED, () => {
      if (this.audio) {
    ...
    this.locusInfo.on(LOCUSINFO.EVENTS.SELF_REMOTE_MUTE_STATUS_UPDATED, (payload) => {
      if (payload) {
        if (this.audio) {
          ...
          // if being muted by others, always send the notification,
    ...
    this.locusInfo.on(LOCUSINFO.EVENTS.LOCAL_UNMUTE_REQUESTED, () => {
      Trigger.trigger(
        this,
        {
          file: 'meeting/index',
  ...
  /**
   * Creates a MuteState instance for audio or video if one doesn't exist
   * @param {String} type 'audio' or 'video'
   * @returns {undefined}
   * @private
   * @memberof Meeting
   */
  createMuteState(type) {
    const {remoteMuted, unmuteAllowed} = this;

    const sendLocalMuteToServer = (audioMuted, videoMuted) => MeetingUtil.remoteUpdateAudioVideo(audioMuted, videoMuted, this)
      .then((locus) => this.locusInfo.onFullLocus(locus));

    const sendRemoteMuteToServer = (remoteMute) => this.members.muteMember(this.members.selfId, remoteMute);
    ...
    if (type === AUDIO) {
      ...
      if (!this.mediaProperties.mediaDirection.sendAudio) {
        return;
      }

      LoggerProxy.logger.info(`Meeting:index#createMuteState --> AUDIO: creating MuteState for meeting ${this.id}`);
      this.audio = new MuteState(type, {remoteMuted, unmuteAllowed}, {
        enableLocalTrack: (enable) => Media.setLocalTrack(enable, this.mediaProperties.audioTrack),
        sendLocalMuteToServer,
        sendRemoteMuteToServer
      });
    }
    else if (type === VIDEO) {
      if (this.video) {
      ...
      LoggerProxy.logger.info(`Meeting:index#createMuteState --> VIDEO: creating MuteState for meeting ${this.id}`);
      this.video = new MuteState(type, {remoteMuted, unmuteAllowed}, {
        enableLocalTrack: (enable) => Media.setLocalTrack(enable, this.mediaProperties.videoTrack),
        sendLocalMuteToServer,
        sendRemoteMuteToServer
      });
    }
