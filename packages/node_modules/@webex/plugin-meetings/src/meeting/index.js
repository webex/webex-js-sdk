...
export default class Meeting extends StatelessSparkPlugin {
  ...
  addMedia(options = {}) {
    ...
    return Media.attachMedia(this.mediaProperties, localStream, localShare, {
  ...
  updateAudio(options) {
    ...
    if (sendAudio && (!stream || (stream && stream.getAudioTracks().length === 0))) {
      console.error('Pass valid audio Streams');
      return Promise.reject();
    }

    // lets keep this function as it is for now
    return Media.updateTransceiver(this.mediaProperties.peerConnection, this.mediaProperties.peerConnection.audioTransceiver,
      {
        stream: sendAudio ? stream : null,
        type: 'audio',
        receiveTrack: receiveAudio
      })
      .then(() => this.roap
        .sendRoapMediaRequest({
          sdp: this.mediaProperties.peerConnection.sdp,
      ...
      .catch((e) => {
        console.log('Error updating the audio streams', e);
      });
  }
  ...
  updateVideo(options) {
    ...
    if (sendVideo && (!stream || (stream && stream.getVideoTracks().length === 0))) {
      console.error('Pass valid video Streams');
      return Promise.reject();
    }

    // lets keep this function as it is for now
    return Media.updateTransceiver(this.mediaProperties.peerConnection, this.mediaProperties.peerConnection.videoTransceiver,
      {
        stream: sendVideo ? stream : null,
        type: 'video',
        receiveTrack: receiveVideo
      })
      .then(() => this.roap
        .sendRoapMediaRequest({
          sdp: this.mediaProperties.peerConnection.sdp,
      ...
      .catch((e) => {
        console.log('Error updating the video streams', e);
      });
  }
  ...
  updateShare(options) {
    ...
    // lets keep this function as it is for now
    return Media.updateTransceiver(this.mediaProperties.peerConnection, this.mediaProperties.peerConnection.shareTransceiver,
      {
