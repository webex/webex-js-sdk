...
import {MEETINGS, EVENT_TRIGGERS, EVENT_TYPES, EVENTS, FLOOR_ACTION, BUSY, INCOMING, ONLINE, CALL, CONTENT, ROAP_SEQ_PRE, LOCUSINFO, MEETING_STATE, MEETING_STATE_MACHINE, _JOIN_} from '../constants';
...
export default class Meeting extends StatelessSparkPlugin {
  ...
  constructor(attrs, options) {
     ...
     * @memberof Meeting
     */
    this.mtgState = MeetingStateMachine.create(this);
    /**
     * @instance
     * @type {MeetingStateMachine}
     * @readonly
     ...
     * @memberof Meeting
     */
    this.locusInfo = new LocusInfo(attrs.locus ? attrs.locus : {}, this.spark);
    /**
     * Passing only info as we send basic info for meeting added event
     * @instance
  ...
  setUpLocusInfoMeetingListener() {
    ...
    this.locusInfo.on(LOCUSINFO.EVENTS.MEETING_UPDATE, (object) => {
      this.updateMeetingObject(object);
    });
  }
  ...
  parseLocus(locus) {
    if (locus) {
      ...
      if (!this.sipUri && this.partner && this.type === CALL) {
   ...
   */
  join(options = {}) {
    if (this.mtgState.state === MEETING_STATE_MACHINE.STATES.ENDED) {
      this.mtgState.reset();
    }
    if (this.mtgState.state !== MEETING_STATE_MACHINE.STATES.RINGING) {
      this.mtgState.ring(_JOIN_);
    }
    return MeetingUtil.joinMeetingOptions(this, options).then((join) => {
      this.mtgState.join();
      return Promise.resolve(join);
    }).catch((error) => {
      this.mtgState.fail(error);
  ...
  addMedia(options = {}) {
    ...
    return Media.attachMedia(this.mediaProperties.mediaDirection, PeerConnectionManager, localStream, localShare, {
      ...
      .then(([mediaPeerConnection, sharePeerConnection]) => {
        this.postMedia(mediaPeerConnection, sharePeerConnection);
      })
      .catch((err) => {
  ...
  /**
   * do all the attach media post completion once the attach was successful
   * @param {RTCPeerConnection} mediaPeerConnection
   * @param {RTCPeerConnection} sharePeerConnection
   * @returns {undefined}
   * @private
   * @memberof Meeting
   */
  postMedia(mediaPeerConnection, sharePeerConnection) {
