...
import {MEETINGS, EVENT_TRIGGERS, EVENT_TYPES, EVENTS, FLOOR_ACTION, BUSY, INCOMING, STOP, ONLINE} from '../constants';
...
export default class Meeting extends StatelessSparkPlugin {
  ...
  constructor(attrs, options) {
    ...
    this.locusInfo = new LocusInfo(this.updateMeetingObject.bind(this));
    ...
    this.lastMediaDirection = null;
    this.policy = null;
    this.type = null;
    Events.on(EVENTS.SELF_CHANGE, (payload) => {
      this.triggerSelfChange(payload);
    });
  }


  /**
   * @param {Object} payload with {old, changed}
   * @returns {undefined}
   */
  triggerSelfChange(payload) {
    if (MeetingUtil.isUnadmittedGuest(payload.old, payload.changed)) {
      this.trigger(EVENT_TRIGGERS.MEETING_SELF_LOBBY_WAITING, {
        old: payload.old,
        self: payload.changed
      });
    }
    if (MeetingUtil.isAdmittedGuest(payload.old, payload.changed)) {
 ...
 */
  setLocalShare(localShare) {
    if (localShare) {
      ...
      const contentTracks = this.localShare.getVideoTracks();
  ...
  */
  ...
  setPeerConnections(mediaPeerConnection, sharePeerConnection) {
    ...
    if (this.config.reconnection.detection) {
      ...
      PeerConnectionManager.detectFailures(this.mediaPeerConnection, this.sharePeerConnection, this.reconnect);
   ...
   * @memberof Meeting
   */
  joinWithMedia(joinOptions, mediaOptions) {
    return this.join(joinOptions)
      .then((joinResponse) => this.addMedia(mediaOptions)
   ...
   */
  reconnect() {
    return this.reconnectionManager.reconnect(this).then((reconnect) => {
  ...
  addMedia(options = {}) {
    ...
    this.mediaDirection = Object.assign(this.config.mediaSettings, options.mediaSettings);
    // add a setup a function move the create and setup media in future
    // TODO: delete old audio and video if stale
    this.audio = this.audio || AudioStateMachine.create(this.mediaDirection, this);
    this.video = this.video || VideoStateMachine.create(this.mediaDirection, this);
    // this.state.local();
    this.setLocalStream(localStream);
    this.setLocalShare(localShare);
    return Media.attachMedia(this.mediaDirection, PeerConnectionManager, localStream, localShare, {mediaPeerConnection: this.mediaPeerConnection, sharePeerConnection: this.sharePeerConnection})
  ...
  decline(reason = BUSY) {
    return this.meetingRequest
      ...
      .then((response) => {
        this.ringing(STOP, false);
        return this.spark.meetings.destroy(this, response);
      });
  }
