    ...
    return MeetingUtil.joinMeetingOptions(this, options).then((join) => {
      ...
      .then(async (join) => {
        ...
        if (options.enableTranscription) {
          LoggerProxy.logger.info('Meeting:index#join --> Attempting to generate a web socket url.');

          const {body: {webSocketUrl}} = await this.request({
            method: HTTP_VERBS.POST,
            uri: this.locusInfo.info.datachannelUrl,
            body: {deviceUrl: this.deviceUrl}
          });

          LoggerProxy.logger.info('Meeting:index#join --> Generated web socket url succesfully.');
          ...
          LoggerProxy.logger.info('Meeting:index#join --> opened LLM web socket connection successfully.');

          this.transcription.getContent((payload) => {
            Trigger.trigger(
              this,
              {
                file: 'meeting/index',
              ...
              EVENT_TRIGGERS.MEETING_STARTED_TRANSCRIPTION,
              payload
            );
          });
        }
    ...
    return MeetingUtil.leaveMeeting(this, options)
      .then((leave) => {
        ...
        if (this.transcription) {
          ...
          LoggerProxy.logger.info('Meeting:index#leave --> closed transcription LLM web socket connection successfully.');
          ...
          Trigger.trigger(
            ...
            EVENT_TRIGGERS.MEETING_STOPPED_TRANSCRIPTION
          );
        }
