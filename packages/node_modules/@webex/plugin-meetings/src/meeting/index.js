...
import {MEETING_STATE_MACHINE, MEETING_CONTROLS_STATE_MACHINE, MEETINGS} from '../constants';
...
export default class Meeting extends StatelessSparkPlugin {
  ...
  constructor(attrs, options) {
    ...
    this.controls = new StateMachine({
      init: MEETING_CONTROLS_STATE_MACHINE.STATES.AUDIO_ON_VIDEO_ON,
      transitions: [
        {
          name: MEETING_CONTROLS_STATE_MACHINE.TRANSITIONS.TOGGLE,
          from: '*',
          to: function(audio, video, meeting) {
            let value = this.state;
            if (!audio && !video) {
              value = MEETING_CONTROLS_STATE_MACHINE.STATES.AUDIO_OFF_VIDEO_OFF
            }
            else if (!audio && video) {
              value = MEETING_CONTROLS_STATE_MACHINE.STATES.AUDIO_OFF_VIDEO_ON
            }
            else if (audio && !video) {
              value = MEETING_CONTROLS_STATE_MACHINE.STATES.AUDIO_ON_VIDEO_OFF
            }
            else if (audio && video) {
              value = MEETING_CONTROLS_STATE_MACHINE.STATES.AUDIO_ON_VIDEO_ON
            }
            return value;
          }
        }
      ...
      methods: {
        onInit() {
          this.controls = {};
          this.controls.audio = false;
          this.controls.video = false;
        },
        onAfterToggle(transition, audio, video, meeting) {
          if (transition.from !== transition.to) {
            Media.localTracks(audio, video, meeting.localMediaStream);
            MeetingUtil.remoteUpdateAudioVideo(audio, video, meeting)
            .then((resolution) => {
              this.controls.audio = !audio;
              this.controls.video = !video;
              meeting.trigger('media:controls', {
                type: 'local',
                status: this.state,
                resolution
              });
              ...
              Media.localTracks(this.controls.audio, this.controls.video, meeting.localMediaStream);
              meeting.trigger('media:controls', {
                type: 'error',
                status: this.state,
                error: err
  ...
  setLocalVideo(stream) {
    ...
    this.localMediaStream = stream[0];
    this.trigger('media:ready', {
      type: 'local',
  ...
  /**
   *
   * toggle audio on or off (mute)
   * @param {object} [options]
   * @returns {Promise}
   * @memberof Meeting
   */
  toggleAudioVideo(options) {
    this.controls.toggle(options.audio, options.video, this);
  }
