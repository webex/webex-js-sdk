...
export default class Meeting extends StatelessSparkPlugin {
  ...
  constructor(attrs, options) {
     ...
     * @memberof Meeting
     */
    this.locusInfo = new LocusInfo(this.updateMeetingObject, attrs.locus ? attrs.locus : {}, this.spark);
    /**
     * Passing only info as we send basic info for meeting added event
     * @instance
  ...
  setUpLocusInfoMeetingListener() {
    ...
    this.locusInfo.on(EVENTS.DESTROY_MEETING, (payload) => {
      ...
      if (payload.selfState && payload.selfState !== MEETING_STATE.STATES.LEFT) {
        this.leave();
      }
      Events.emit(EVENTS.DESTROY_MEETING, {
        response: payload,
        meeting: this
      });
    });
  ...
  updateMeetingObject(object) {
    ...
    console.log('UPDATES FROM ', object);
    ...
    pc.ontrack = (event) => {
      ...
      this.trigger(EVENT_TRIGGERS.MEDIA_READY, {
        ...
        stream: new MediaStream([
          this.mediaProperties.remoteStream.getAudioTracks()[0]])
      });

      this.trigger(EVENT_TRIGGERS.MEDIA_READY, {
        type: EVENT_TYPES.REMOTE_VIDEO,
        stream: new MediaStream([
          this.mediaProperties.remoteStream.getVideoTracks()[0]])
      });

      this.trigger(EVENT_TRIGGERS.MEDIA_READY, {
        type: EVENT_TYPES.REMOTE_SHARE,
        stream: new MediaStream([this.mediaProperties.remoteStream.getVideoTracks()[1]])
      });
    };
  }
   ...
   * @memberof Meeting
   */
  setPeerConnections(peerConnection, sharePeerConnection) {
    this.mediaProperties.setPeerConnections(peerConnection, sharePeerConnection);
  ...
  closePeerConnections() {
    return Promise.all([
      PeerConnectionManager.close(this.mediaProperties.peerConnection),
  ...
  addMedia(options = {}) {
    ...
    return MeetingUtil.validateOptions(options)
      .then(() => Media.attachMedia(this.mediaProperties, localStream, localShare, {
  ...
  updateTransceiver(options) {
    ...
    || (receiveTrack !== undefined && receiveTrack !== previousMediaDirection.receiveTrack)) {
      return Media.updateTransceiver(peerConnection, transceiver,
        ...
        .catch((e) => {
          console.log(`Error updating the ${type} streams`, e);
        });
    } if (stream) {
      let tracks = null;
      if (type === 'audio') { tracks = stream.getAudioTracks(); }
      if (type === 'video') { tracks = stream.getVideoTracks(); }

      if (tracks && tracks.length > 0) {
        transceiver.sender.replaceTrack(tracks);
  ...
  updateAudio(options) {
    ...
    const audioTransceiver = this.mediaProperties.peerConnection.audioTransceiver;
  ...
  updateVideo(options) {
    ...
    const videoTransceiver = this.mediaProperties.peerConnection.videoTransceiver;
  ...
  updateShare(options) {
    ...
    const shareTransceiver = this.mediaProperties.peerConnection.shareTransceiver;

    if (sendShare === undefined || receiveShare === undefined) {
      return Promise.reject(new Error('Pass sendVideo and receiveVideo parameter'));
    }

    if (sendShare && (!stream || (stream && stream.getVideoTracks().length === 0))) {
      console.error('Pass valid share Streams');
      return Promise.reject();
    }

    // If media direction have changed we need to do a renegotation
    if (sendShare !== this.mediaProperties.mediaDirection.sendShare
      || receiveShare !== this.mediaProperties.mediaDirection.receiveShare) {
      // lets keep this function as it is for now
      return Media.updateTransceiver(this.mediaProperties.peerConnection, shareTransceiver,
        {
          stream: sendShare ? stream : null,
          type: 'video',
        ...
        .catch((e) => {
          console.log('Error updating the share streams', e);
        });
    }

    if (stream && shareTransceiver && shareTransceiver.sender) {
      const tracks = stream.getVideoTracks();
      // Just replace the streams no need to negotation. We can replace the tracks with null as well
      shareTransceiver.sender.replaceTrack(tracks.length > 0 ? tracks[0] : null);
      return Promise.resolve();
    }
    return Promise.reject(new Error('error updating share'));
  }
   ...
   */
  leave() {
    return MeetingUtil.leaveMeeting(this).then((leave) => {
