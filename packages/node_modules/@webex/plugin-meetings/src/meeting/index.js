...
export default class Meeting extends StatelessSparkPlugin {
  ...
  constructor(attrs, options) {
    ...
    this.state = MeetingStateMachine.create();
    this.aggregatorMap = {}; // TODO: make a weakmap or re think how to link stats streams to aggregators
    this.historyMap = {}; // TODO: make a weakmap or re think how to link stats streams to histories
    this.intervalMap = {}; // TODO: make a weakmap or re think how to link stats streams to interval
  }
  ...
  /**
   * creates the aggregator at that name
   * @returns {StatsAggregator}
   * @param {String} name
   * @memberof Meeting
   */
  createStatsStreamAggregator(name) {
  ...
  /**
   * get the aggregator at that name, creates it if it doesn't exist
   * @returns {StatsAggregator}
   * @param {String} name
   * @memberof Meeting
   */
  getStatsStreamAggregator(name) {
    if (!this.aggregatorMap[name]) {
      return this.createStatsStreamAggregator(name);
    }
    return this.aggregatorMap[name];
  }

  /**
   * delete the aggregator at that name
   * @returns {undefined}
   * @param {String} name
   * @memberof Meeting
   */
  deleteStatsStreamAggregator(name) {
    delete this.aggregatorMap[name];
  }

  /**
   * creates the history at that name
   * @returns {StatsHistory}
   * @param {String} name
   * @memberof Meeting
   */
  createStatsStreamHistory(name) {
    const history = new StatsHistory();
    this.historyMap[name] = history;
    return history;
  }

  /**
   * get the history at that name, creates it if it doesn't exist
   * @returns {StatsHistory}
   * @param {String} name
   * @memberof Meeting
   */
  getStatsStreamHistory(name) {
    if (!this.historyMap[name]) {
      return this.createStatsStreamHistory(name);
    }
    return this.historyMap[name];
  }

  /**
   * delete the history at that name
   * @returns {undefined}
   * @param {String} name
   * @memberof Meeting
   */
  deleteStatsStreamHistory(name) {
    delete this.historyMap[name];
  }

  /**
   * creates the interval at that name
   * @returns {StatsInterval}
   * @param {String} name
   * @memberof Meeting
   */
  createStatsStreamInterval(name) {
    const interval = new StatsInterval();
    this.intervalMap[name] = interval;
    return interval;
  }

  /**
   * get the interval at that name, creates it if it doesn't exist
   * @returns {StatsInterval}
   * @param {String} name
   * @memberof Meeting
   */
  getStatsStreamInterval(name) {
    if (!this.intervalMap[name]) {
      return this.createStatsStreamInterval(name);
    }
    return this.intervalMap[name];
  }

  /**
   * delete the interval calculator at that name
   * @returns {undefined}
   * @param {String} name
   * @memberof Meeting
   */
  deleteStatsStreamInterval(name) {
   ...
   * @memberof Meeting
   */
  getRawMediaStatsStream() {
    return new StatsStream(this.mediaPeerConnection);
  }
   ...
   */
  getStatsMediaStream() {
    return this.getRawMediaStatsStream().pipe(new StatsFilter());
  }
   ...
   * @memberof Meeting
   */
  getRawScreenStatsStream() {
    return new StatsStream(this.screenPeerConnection);
  }
   ...
   * @memberof Meeting
   */
  getStatsScreenStream() {
    return this.getRawScreenStatsStream().pipe(new StatsFilter());
  }

  /**
   *
   * Get statistics of the current meeting
   * @returns {Array} - StatsStream, media first, screen second
   * @memberof Meeting
   */
  getRawStatsStreams() {
    return [this.getRawMediaStatsStream(), this.getRawScreenStatsStream()];
  }

  /**
   *
   * Get statistics of the current meeting
   * @returns {Array} - StatsFilter, media first, screen second
   * @memberof Meeting
   */
  getStatsStreams() {
    return [this.getStatsMediaStream(), this.getStatsScreenStream()];
  }
