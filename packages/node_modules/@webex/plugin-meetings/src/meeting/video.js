  ...
  MeetingUtil.remoteUpdateAudioVideo({mute: meeting.audio.muted}, video, meeting)
    .then((resolution) => {
      try {
        ...
        MeetingUtil.remoteUpdateAudioVideo({mute: meeting.audio.muted}, {mute: state.muted}, meeting)
          .catch((error) => {
            console.log(`video:state#onBeforeTogglek->${transition.event} fired! State failed to change with transition '${transition.transition}''. After local video toggle failed, resetting remote also failed, meeting video in bad state with error: ${error}.`);
          });
        triggerEvent(meeting, state, error, EVENT_TRIGGERS.MEDIA_VIDEO, EVENT_TYPES.ERROR);
      ...
      triggerEvent(meeting, state, resolution, EVENT_TRIGGERS.MEDIA_VIDEO, EVENT_TYPES.LOCAL);
