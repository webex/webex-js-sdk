...
*/
class MuteState {
  constructor(type, initialState, callbacks) {
    ...
    this.state = {
      ...
      server: {
        localMute: false,
        remoteMute: initialState.remoteMuted,
        unmuteAllowed: initialState.unmuteAllowed
      },
  ...
  applyClientStateLocally() {
    this.callbacks.enableLocalTrack(!this.state.client.localMute);
  }
  ...
  sendLocalMuteRequestToServer() {
    ...
    return this.callbacks.sendLocalMuteToServer(audioMuted, videoMuted)
      .then(() => {
  ...
  sendRemoteMuteRequestToServer() {
    ...
    if (this.type === AUDIO) {
      ...
      return this.callbacks.sendRemoteMuteToServer(remoteMute)
        .then(() => {
          LoggerProxy.logger.info(
