...
export default class PersonalMeetingRoomRequest extends StatelessWebexPlugin {
  ...
  claimPmr(options) {
    if (!options || !options.userId || !options.passcode || !options.meetingAddress || !options.preferred) {
      throw new ParameterError(new Error('Claiming a PMR should be done with userId, passcode, preferred, and meetingAddress in options.'));
    }
    const validPin = PersonalMeetingRoomUtil.getClaimPmrPin(options.passcode);

    if (!validPin) {
      return Promise.reject(new ParameterError(new Error('The host pin provided was of an invalid format')));
    }
    const validLink = PersonalMeetingRoomUtil.getClaimPmrLink(options.meetingAddress);

    if (!validLink) {
      return Promise.reject(new ParameterError(new Error('The PMR link provided was of an invalid format')));
    }
