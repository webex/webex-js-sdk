...
export default class MembersRequest extends StatelessWebexPlugin {
  ...
  addMembers(options) {
    if (!(!options || !options.invitee || (!options.invitee.emailAddress || !options.invitee.email) || !options.locusUrl)) {
      throw new ParameterError(new Error('invitee must be passed and the associated locus url for this meeting object must be defined.'));
    }
  ...
  admitMember(options) {
    if (!options || !options.locusUrl || !options.memberIds) {
      throw new ParameterError(new Error('memberIds must be an array passed and the associated locus url for this meeting object must be defined.'));
    }
  ...
  removeMember(options) {
    if (!options || !options.locusUrl || !options.memberId) {
      throw new ParameterError(new Error('memberId must be defined, and the associated locus url for this meeting object must be defined.'));
    }
  ...
  transferHostToMember(options) {
    if (!options || !options.locusUrl || !options.memberId || !options.moderator) {
      throw new ParameterError(new Error('memberId must be defined, the associated locus url, and the moderator for this meeting object must be defined.'));
    }
