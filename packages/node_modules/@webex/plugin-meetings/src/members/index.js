...
export default class Members extends StatelessSparkPlugin {
  ...
  setUpLocusMediaSharesListener() {
    Events.on('MEDIA_SHARES_CONTENT_UPDATE', (payload) => {
      const currentContent = payload.current;
      let whoSharing = null;
      let whoStopped = null;
      if (currentContent && currentContent.contentId) {
        if (currentContent.disposition === 'GRANTED') {
          whoSharing = currentContent.contentId;
        }
        else if (currentContent.disposition === 'RELEASED') {
      ...
      this.trigger('members:content:update', {
  ...
  setMediaShareContentId(locus, contentId) {
    if (contentId) {
      ...
      this.mediaShareContentId = locus && locus.mediaShares && locus.mediaShares.length && locus.mediaShares[0] && locus.mediaShares[0].name === 'content' && locus.mediaShares[0].floor && locus.mediaShares[0].floor.beneficiary ? locus.mediaShares[0].floor.beneficiary.id : null;
    }
    else {
      throw new Error('Setting hostid for the Members module should be done with a locus object or hostId');
    }
