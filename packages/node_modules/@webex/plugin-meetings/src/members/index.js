...
export default class Members extends StatelessWebexPlugin {
  ...
  sendDTMF(tones = '', memberId = '') {
    ...
    if (!memberId) {
      return Promise.reject(new ParameterError('a valid memberID must be passed in'));
    }
    ...
    if (locusUrl && url) {
      ...
      return this.membersRequest.sendDTMF(options);
    }

    return this.rejectWithErrorLog('Meeting:index#sendDTMFPartcipant --> cannot send DTMF, meeting does not have a connection to the "locus" call control service. Have you joined?');
  }
}
