...
export default class Members extends StatelessSparkPlugin {
  ...
  removeMember(memberId) {
    ...
    if (!memberId) {
      return Promise.reject(new Error('The member id must be defined to remove the member.'));
    }
  ...
  muteMember(memberId, mute) {
    if (!this.locusUrl) {
      return Promise.reject(new Error('The associated locus url for this meetings members object must be defined.'));
    }
    if (!memberId) {
      return Promise.reject(new Error('The member id must be defined to mute the member.'));
    }
  ...
  transferHostToMember(memberId, moderator) {
    ...
    if (!memberId) {
      return Promise.reject(new Error('The member id must be defined to transfer host to the member.'));
    }
