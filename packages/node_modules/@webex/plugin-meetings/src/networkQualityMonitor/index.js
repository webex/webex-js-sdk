 ...
 */
  ...
  determineUplinkNetworkQuality(configObj) {
    const {mediaType, remoteRtpResults, statsAnalyzerCurrentStats} = configObj;
    const convertRoundTripTimeToMilliseconds =
    remoteRtpResults.roundTripTime * 1000;
    const convertJitterToMilliseconds = remoteRtpResults.jitter * 1000;
   ...
   */
    const determineLatency = () => {
      if (convertRoundTripTimeToMilliseconds > this.config.rttThreshold) {
        this.networkQualityScore = 0;
   ...
   */
    const deterMineJitter = () => {
      if (convertJitterToMilliseconds > this.config.jitterThreshold) {
        this.networkQualityScore = 0;
  ...
  /**
   * Get the current status of network quaility object
   * @returns {Object}
   */
