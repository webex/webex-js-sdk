...
export default class ReconnectionManager {
  ...
  reconnectMedia(meeting, reCreate = false) {
    ...
    if (reCreate) {
      ...
      promise = Media.reconnectMedia(meeting.mediaProperties.peerConnection);
    }
  ...
  media(meeting) {
    ...
    return this.reconnectMedia(meeting)
      ...
      .catch((err) => {
        LoggerProxy.logger.warn(`Reconnection->media#Media could not re user peer connection, failed with ${err}`);
        ...
        return meeting.reconnectMedia(true)
          .catch((error) => LoggerProxy.logger.error(`ReconnectionManager->media#Error reconnecting , error: ${error}`));
      });
  }
