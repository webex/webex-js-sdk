...
MemberUtil.extractStatus = (participant) => {
  ...
  if (device) {
    if (device.state === _JOINED_ && (!device.intent || device.intent.type === NONE)) {
      return _IN_MEETING_;
    }
    if (device.state === _IDLE_ && device.intent && (device.intent.type === WAIT || device.intent.type === OBSERVE)) {
      return _IN_LOBBY_;
    }
    if (device.state === _LEFT_ || (device.state === _IDLE_ && (!device.intent || device.intent.type === NONE))) {
      return NOT_IN_MEETING;
    }
  }
  return NOT_IN_MEETING;
