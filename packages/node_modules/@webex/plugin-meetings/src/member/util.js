...
MemberUtil.extractStatus = (participant) => {
  ...
  // TODO: devices[0] is incorrect
  const device = participant && participant.devices ? participant.devices[0] : null;

  if (device) {
    if (device.state === _JOINED_ && (!device.intent || device.intent.type === _NONE_)) {
      return _IN_MEETING_;
    }
    if (device.state === _IDLE_ && device.intent && (device.intent.type === _WAIT_ || device.intent.type === _OBSERVE_)) {
      return _IN_LOBBY_;
    }
    if (device.state === _LEFT_ || (device.state === _IDLE_ && (!device.intent || device.intent.type === _NONE_))) {
      return _NOT_IN_MEETING_;
    }
  }
