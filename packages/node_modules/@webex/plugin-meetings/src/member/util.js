...
MemberUtil.isNotAdmitted = (participant) => participant
...
MemberUtil.isAudioMuted = (participant) => {
  ...
  if (participant.controls && participant.controls.audio) {
    return participant.controls.audio.muted;
  }
  return MemberUtil.isMuted(participant.status, AUDIOSTATUS);
};
...
MemberUtil.isVideoMuted = (participant) => {
  ...
  return MemberUtil.isMuted(participant.status, VIDEOSTATUS);
};

MemberUtil.isMuted = (status, accessor) => {
  if (status) {
    if (status[accessor] === RECVONLY) {
      return true;
    }
    if (status[accessor] === SENDRECV) {
      return false;
    }
  }
  return null;
