...
export default class StatsAnalyzer extends EventsScope {
  constructor(config) {
    ...
    this.lastMqaDataSent = {video: {send: {}, recv: {}}, audio: {send: {}, recv: {}}, share: {send: {}, recv: {}}};
  }
  ...
  getAudioReceiverMqa(audioReceiver) {
    ...
    audioReceiver.streams[0].common.receivedBitrate = (this.statsResults[mediaType].recv.totalBytesReceived - this.lastMqaDataSent.video.recv.totalBytesReceived) * 8 / 60 || 0;
    //
  }
  ...
  getVideoReceiverMqa(videoReceiver) {
    ...
    videoReceiver.streams[0].receivedHeight = this.statsResults.resolutions[mediaType][sendrecvType].height;
    videoReceiver.streams[0].receivedWidth = this.statsResults.resolutions[mediaType][sendrecvType].width;
    ...
    videoReceiver.streams[0].common.receivedBitrate = (this.statsResults[mediaType].recv.totalBytesReceived - this.lastMqaDataSent.video.recv.totalBytesReceived) * 8 / 60 || 0;
...
sendMqaData = () => {
  console.log(mqaConfig);
  const audioReciver = mqaConfig.intervals[0].audioReceive;
  const audioSender = mqaConfig.intervals[0].audioTransmit;
  ...
  // videoSender.common.remoteReceiveRate = this.statsResults[mediaType].send.reportsReceived;
  console.log('sent rtpPackets ,', videoSender.common.rtpPackets);

  // From candidate-pair
  videoSender.common.availableBitRate = this.statsResults[mediaType].send.availableOutgoingBitrate || 0;
  // Calculate based on how much packets lost of received compated to how to the client sent

  const totalpacketsLostForaMin = this.statsResults[mediaType].send.totalPacketsLostOnReceiver - this.lastMqaDataSent[mediaType][sendrecvType].totalPacketsLostOnReceiver;
  ...
  videoSender.common.roundTripTime = this.statsResults[mediaType].send.roundTripTime * 1000 || 0;
  ...
  const totalBytesSentInaMin = this.statsResults[mediaType][sendrecvType].totalBytesSent - this.lastMqaDataSent.video.send.totalBytesSent;
  ...
  // this.lastMqaDataSent.video.send.totalPacketsLostOnReceiver = this.statsResults[mediaType].send.totalPacketsLostOnReceiver;
  // this.lastMqaDataSent.video.send.totalBytesSent = this.statsResults[mediaType].send.totalBytesSent;

  // this.lastMqaDataSent.video.recv.totalBytesReceived = this.statsResults[mediaType].recv.totalBytesReceived;
  ...
  startStats = (pc) => {
    ...
    pc.audioTransceiver.sender.getStats().then((res) => {
      res.forEach((r) => {
        if (types.includes(r.type)) { this.parseResults(r, 'audio', true); }
      });
    });

    pc.audioTransceiver.receiver.getStats().then((res) => {
      res.forEach((r) => {
        if (types.includes(r.type)) { this.parseResults(r, 'audio', false); }
      });
    });
