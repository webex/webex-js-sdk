...
export default class StatsAnalyzer extends EventsScope {
  ...
  startAnalyzer = (peerconnection) => {
    ...
    this.peerconnectionpc.onconnectionstatechange = () => {
      if (this.peerconnectionpc.connectionState === 'closed') {
  ...
  startStats = (pc) => {
    ...
    pc.videoTransceiver.sender.getStats().then((res) => {
      res.forEach((r) => {
        if (types.includes(r.type)) { this.parseResults(r, 'video'); }
      });
    });
    ...
    pc.audioTransceiver.sender.getStats().then((res) => {
      res.forEach((r) => {
        if (types.includes(r.type)) { this.parseResults(r, 'audio'); }
      });
    });
    ...
    pc.shareTransceiver.sender.getStats().then((res) => {
      res.forEach((r) => {
        if (types.includes(r.type)) { this.parseResults(r, 'share'); }
      });
    });
