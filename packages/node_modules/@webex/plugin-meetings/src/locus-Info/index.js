...
import InfoUtils from '../locus-Info/utils/info';
import SelfUtils from '../locus-Info/utils/self';
...
export default class LocusInfo {
  ...
  emitChanges(type, old, changed) {
    Events.emit(type, {old, changed});
  ...
  updateParticipants(participants, deltaParticpantFlag) {
    ...
    if (deltaParticpantFlag) {
      ...
      // First time the participants are set
      ...
      if (this.participants && participants.length !== this.participants.length) {
  ...
  findChangedParticipants(newParticipants) {
    ...
    if (newParticipants.length < 25) {
      ...
      this.participants.forEach((participant, index) => {
        ...
        // find if anyof the participant  got missed
  ...
  updateMeetingInfo(info) {
    if (info && !isEqual(this.info, info)) {
      ...
      this.updateMeeting(InfoUtils.parse(info));
  ...
  updateSelf(self) {
    if (self && !isEqual(this.self, self)) {
      this.updateMeeting(SelfUtils.parse(self));
      this.emitChanges(EVENTS.SELF_CHANGE, this.self, self);
      this.self = self;
