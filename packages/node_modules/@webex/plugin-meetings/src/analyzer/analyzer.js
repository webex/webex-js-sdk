...
import {ANALYSIS_STATS} from '../constants';

const StatsAnalyzer = {};

// {valid: true/false, failed: { key: [number] }, data: { webRtcKeyToAnalyze: { valid: true/false, reports: [ { value: number, valid: true/false, difference: number } ] } } }
StatsAnalyzer.analyze = (series, options = {analysisKeys: ANALYSIS_STATS.DEFAULT_KEYS}) => {
  if (!isArray(series) || !series.length || !options || !isArray(options.analysisKeys) || !options.analysisKeys.length) {
    throw new Error('analyzer->analyze#series must be defined as a nonempty array of WebRTCData objects, and options.analysisKeys must be a nonempty array of strings, representing the properties to analyze.');
  }
  ...
  properties.forEach((config) => {
    ...
    for (let i = series.length - 1; i > 0; i -= 1) {
      ...
      forEach(series[i].data.getData(), (webrtcData) => { // eslint-disable-line
        ...
        if (config.check === 'increasing' && singular.difference > 0) {
          singular.valid = true;
        }
        else if (config.check === 'decreasing' && singular.difference < 0) {
          singular.valid = true;
        }
        else { // for when config.check === 'constant'
          singular.valid = true;
        }
        if (!singular.valid) {
