...
Media.getDisplayMedia = (options) => {
  ...
  if (bowser.name().toLowerCase() === 'firefox') {
    const mediaConfig = {
      ...
      video: options.sendShare ? {
        frameRate: {min: config.meetings.screenFrameRate.minFrameRate, max: config.meetings.screenFrameRate.maxFrameRate},
        width: {min: config.meetings.screenResolution.idealWidth, max: config.meetings.screenResolution.maxWidth},
        height: {min: config.meetings.screenResolution.idealHeight, max: config.meetings.screenResolution.maxHeight}
      } : false
    };
  ...
  return navigator.mediaDevices.getDisplayMedia({audio: options.sendAudio, video: !!options.sendShare});
};
