...
/**
 * calls callback when stream.onended event occurs for share stream
 * @param {MediaStream} stream -- the share stream
 * @param {Function} callback -- the callback
 * @returns {undefined}
 */
Media.shareStreamEnded = (stream, callback) => {
  if (stream && stream.getVideoTracks() && stream.getVideoTracks().length > 0) {
    stream.getVideoTracks()[0].onended = callback;
  }
};
 ...
 * @param {Function} mediaSetting.shareEnded shareEnded: {Functiion} share ended callback
...
Media.getUserMedia = (mediaSetting) => Media.getLocalMedia({
  ...
  .then((localStream) => Media.getLocalMedia({
    ...
    Media.shareStreamEnded(shareStream, mediaSetting.shareEnded);
