...
import {AUDIOINPUT, VIDEOINPUT} from '../constants';
/**
 *
 * @export
...
Media.getLocalMedia = async (options) => {
  const {sendAudio, sendVideo} = options;
  // we return undefined here as the second param until screen share is implemented
  return Promise.resolve().then(() => Media.getMedia(sendAudio, sendVideo).then((media) => [media, undefined]));
};
