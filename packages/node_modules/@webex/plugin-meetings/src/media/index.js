...
Media.getMedia = (audio, video, config) => {
  const defaultWidth = !video.width ? {ideal: config.resolution.idealWidth, max: config.resolution.maxWidth} : '';
  const defaultHeight = !video.height ? {ideal: config.resolution.idealHeight, max: config.resolution.maxHeight} : '';
  const mediaConfig = {
    audio,
