...
Media.reconnectMedia = (peerConnection, callFlowId) => {
  if (peerConnection.connectionState === PEER_CONNECTION_STATE.CLOSED ||
    ...
    return Promise.reject(new ReconnectionError(new Error('Reinitiate peerconnection')));
  }

  return PeerConnectionManager.createOffer(peerConnection, callFlowId);
};
 ...
 * @param {Object} callFlowId call flow id
 * @returns {Array} [peerConnection, ]
 */
Media.attachMedia = (mediaProperties, callFlowId) => {
  const {
...
Media.updateMedia = (mediaProperties, callFlowId) => {
  const {
    mediaDirection, audioTrack, videoTrack, shareTrack, peerConnection
  } = mediaProperties;
...
Media.updateTransceiver = (callFlowId, peerConnection, transceiver, options) => {
  Media.setTrackOnTransceiver(transceiver, options);
...
Media.getDevices = () => {
  ...
  return Promise.reject(new MediaError(new Error('enumerateDevices not supported.')));
};
