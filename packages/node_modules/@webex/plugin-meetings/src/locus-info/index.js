...
import {EVENTS, LOCUSEVENT, USER, ONE_ON_ONE} from '../constants';
 ...
 * @class LocusInfo
 */
export default class LocusInfo extends EventEmitter {
  ...
  isMeetingActive() {
    if (this.parsedLocus.info.type === ONE_ON_ONE) {
      ...
      if (partner.state === 'LEFT'
        ...
        && (this.parsedLocus.self.state === 'DECLINED'
        || this.parsedLocus.self.state === 'NOTIFIED')) {
        console.log('END the ONE ON ONE Call');
        this.emit(EVENTS.DESTROY_MEETING);
      }
      else
      if (this.parsedLocus.self
        && this.parsedLocus.self.state === 'LEFT'
      && (partner.state === 'LEFT'
      || partner.state === 'DECLINED'
      || partner.state === 'NOTIFIED'
      || partner.state === 'IDLE') // Happens when user just joins and adds no Media
      ) {
        console.log('END the ONE ON ONE Call');
        this.emit(EVENTS.DESTROY_MEETING);
      }
  ...
  updateSelf(self, participants) {
    if (self && !isEqual(this.self, self)) {
      ...
      this.updateMeeting(SelfUtils.getSipUrl(this.getLocusPartner(participants, self), this.parsedLocus.info));

      if (parsedSelves.updates.isUnadmittedGuest) {
        this.emit(EVENTS.SELF_UNADMITTED_GUEST, self);
      }
      if (parsedSelves.updates.isAdmittedGuest) {
        this.emit(EVENTS.SELF_ADMITTED_GUEST, self);
      }
