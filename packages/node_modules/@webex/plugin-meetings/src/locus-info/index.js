...
export default class LocusInfo extends EventsScope {
  ...
  emitGlobal(type, object) {
    Events.emit(type, object);
  }

  emitScoped(eventName, ...args) {
  ...
  handleOneOnOneEvent(eventType) {
    ...
    // for 1:1 bob calls alice and alice declines, notify the meeting state
      ...
      // trigger the event for stop ringing
        this.emitScoped(EVENTS.REMOTE_ANSWERED_DECLINED, {
          remoteDeclined: true,
  ...
  isMeetingActive() {
    if (this.parsedLocus.fullState.type === _CALL_) {
      ...
      this.emitScoped(LOCUSINFO.EVENTS.MEETING_UPDATE, {partner});
      ...
      if (partner.state === MEETING_STATE.STATES.LEFT
        ...
        this.emitScoped(EVENTS.DESTROY_MEETING, {
    ...
    else if (this.parsedLocus.info.type === _WEBEX_MEETING_) {
      if (this.fullState && this.fullState.state === LOCUS.STATE.INACTIVE) {
        // TODO: update the meeting state
        this.updateMeeting({state: LOCUS.STATE.ENDED});
        this.emitScoped(EVENTS.DESTROY_MEETING, {
          selfState: this.parsedLocus.self.state
        });
      }
      ...
      LoggerProxy.logger.logger.warn('LocusInfo#isMeetingActive-> Meeting Type is unknown.');
    }
  }
  ...
  updateParticipants(participants) {
    this.emitGlobal(super.spark, EVENTS.LOCUS_INFO_UPDATE_PARTICIPANTS, {
      participants,
  ...
  updateConversationUrl(conversationUrl) {
    if (conversationUrl && !isEqual(this.conversationUrl, conversationUrl)) {
      ...
      this.emitGlobal(EVENTS.CONVERSATION_URL_UPDATE, conversationUrl);
    }
  }
  ...
  updateHostInfo(host) {
    if (host && !isEqual(this.host, host)) {
      ...
      this.emitScoped(LOCUSINFO.EVENTS.MEETING_UPDATE, parsedHosts.current);
      this.parsedLocus.host = parsedHosts.current;
      if (parsedHosts.updates.isNewHost) {
        this.emitGlobal(EVENTS.LOCUS_INFO_UPDATE_HOST, {newHost: parsedHosts.current, oldHost: parsedHosts.previous});
      }
      this.host = host;
    }
  }
  ...
  updateMeetingInfo(info) {
    if (info && !isEqual(this.info, info)) {
      ...
      this.emitScoped(LOCUSINFO.EVENTS.MEETING_UPDATE, parsedInfo);
      this.emitGlobal('infoUpdated');
    }
  }
  ...
  updateMediaShares(mediaShares) {
    if (mediaShares && !isEqual(this.mediaShares, mediaShares)) {
      ...
      this.emitScoped(LOCUSINFO.EVENTS.MEETING_UPDATE, parsedMediaShares.current);
      this.emitGlobal(EVENTS.LOCUS_INFO_UPDATE_MEDIA_SHARES, {current: parsedMediaShares.current, previous: parsedMediaShares.previous});
  ...
  updateSelf(self, participants) {
    if (self && !isEqual(this.self, self)) {
      const parsedSelves = SelfUtils.getSelves(this.self, self, super.spark);
      this.emitScoped(LOCUSINFO.EVENTS.MEETING_UPDATE, parsedSelves.current);
      ...
      if (parsedSelves.updates.isUnadmittedGuest) {
        this.emitScoped(LOCUSINFO.EVENTS.SELF_UNADMITTED_GUEST, self);
      }
      if (parsedSelves.updates.isAdmittedGuest) {
        this.emitScoped(LOCUSINFO.EVENTS.SELF_ADMITTED_GUEST, self);
      }
      this.emitGlobal(EVENTS.LOCUS_INFO_UPDATE_SELF, {oldSelf: parsedSelves.previous, newSelf: parsedSelves.current});
      this.parsedLocus.self = parsedSelves.current;
      this.self = self;
    }
  }
  ...
  updateLocusUrl(url) {
    if (url && this.url !== url) {
      this.url = url;
      this.emitGlobal(EVENTS.LOCUS_INFO_UPDATE_URL, url);
    }
  }
