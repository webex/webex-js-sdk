...
import {EVENTS, LOCUSEVENT, USER, ONE_ON_ONE, MEETING_STATE, WEBEX_MEETING, LOCUSINFO} from '../constants';
...
export default class LocusInfo extends EventEmitter {
  ...
  updateSelf(self, participants) {
    if (self && !isEqual(this.self, self)) {
      ...
      this.emit(LOCUSINFO.EVENTS.MEETING_UPDATE, SelfUtils.getSipUrl(this.getLocusPartner(participants, self), this.parsedLocus.info));

      if (parsedSelves.updates.isUnadmittedGuest) {
        this.emit(LOCUSINFO.EVENTS.NTS.SELF_UNADMITTED_GUEST, self);
      }
