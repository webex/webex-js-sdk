...
export default class LocusInfo extends EventsScope {
  ...
  updateLocusInfo(locus) {
    ...
    this.updateFullState(locus.fullState);
    this.updateHostInfo(locus.host);
    this.updateMeetingInfo(locus.info, locus.self, this.parsedLocus.self.state);
    this.updateMediaShares(locus.mediaShares);
   ...
   * @memberof LocusInfo
   */
  updateMeetingInfo(info, self, meetingState) {
    if (info && (!isEqual(this.info, info))) {
      const roles = self ? SelfUtils.getRoles(self) : this.parsedLocus.self?.roles || [];

      const parsedInfo = InfoUtils.getInfos(this.parsedLocus.info, info, roles, meetingState === MEETING_STATE.STATES.JOINED);
