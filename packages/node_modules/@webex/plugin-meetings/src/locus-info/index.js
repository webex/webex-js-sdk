...
export default class LocusInfo {
  ...
  updateHostInfo(host) {
    if (host && !isEqual(this.host, host)) {
      const parsedHosts = HostUtils.getHosts(this.parsedLocus.host, host);
  ...
  updateMediaShares(mediaShares) {
    if (mediaShares && !isEqual(this.parsedLocus.mediaShares, mediaShares)) {
  ...
  updateSelf(self, participants) {
    if (self && !isEqual(this.self, self)) {
      const parsedSelves = SelfUtils.getSelves(this.parsedLocus.self, self);
