...
export default class LocusInfo extends EventsScope {
  ...
  updateParticipants(participants) {
    this.emitScoped(EVENTS.LOCUS_INFO_UPDATE_PARTICIPANTS, {
      participants,
  ...
  updateHostInfo(host) {
    if (host && !isEqual(this.host, host)) {
      ...
      if (parsedHosts.updates.isNewHost) {
        this.emitScoped(EVENTS.LOCUS_INFO_UPDATE_HOST, {newHost: parsedHosts.current, oldHost: parsedHosts.previous});
      }
      this.host = host;
    }
  }
  ...
  updateMediaShares(mediaShares) {
    if (mediaShares && !isEqual(this.mediaShares, mediaShares)) {
      ...
      this.emitScoped(EVENTS.LOCUS_INFO_UPDATE_MEDIA_SHARES, {current: parsedMediaShares.current, previous: parsedMediaShares.previous});
  ...
  updateSelf(self, participants) {
    if (self && !isEqual(this.self, self)) {
      const parsedSelves = SelfUtils.getSelves(this.self, self, this.spark.internal.device.url);
      ...
      this.emitScoped(EVENTS.LOCUS_INFO_UPDATE_SELF, {oldSelf: parsedSelves.previous, newSelf: parsedSelves.current});
      this.parsedLocus.self = parsedSelves.current;
      this.self = self;
    }
  }
  ...
  updateLocusUrl(url) {
    if (url && this.url !== url) {
      ...
      this.emitScoped(EVENTS.LOCUS_INFO_UPDATE_URL, url);
    }
  }
