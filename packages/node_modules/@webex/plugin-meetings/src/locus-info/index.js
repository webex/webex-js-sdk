...
export default class LocusInfo {
  ...
  updateParticipants(participants, deltaParticpantFlag) {
    this.emitChanges(
      deltaParticpantFlag ? EVENTS.LOCUS_INFO_DELTA_PARTICIPANTS : EVENTS.LOCUS_INFO_FULL_PARTICIPANTS,
      {
        participants,
  ...
  updateHostInfo(host) {
    if (host && !isEqual(this.host, host)) {
      ...
      if (parsedHosts.updates.isNewHost) {
        this.emitChanges(EVENTS.LOCUS_INFO_UPDATE_HOST, {host});
      }
      this.host = host;
  ...
  updateMediaShares(mediaShares) {
    if (mediaShares && !isEqual(this.mediaShares, mediaShares)) {
      ...
      this.emitChanges(EVENTS.LOCUS_INFO_MEDIA_SHARES_UPDATE, {current: parsedMediaShares.current, previous: parsedMediaShares.previous});
  ...
  updateSelf(self, participants) {
    if (self && !isEqual(this.self, self)) {
      const parsedSelves = SelfUtils.getSelves(this.self, self);
  ...
  updateLocusUrl(url) {
    if (url && this.url !== url) {
      this.url = url;
      this.emitChanges(EVENTS.LOCUS_URL_UPDATE, url);
