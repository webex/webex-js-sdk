...
selfUtils.isUnadmittedGuest = (self) => {
  if (!self) {
    throw new ParameterError(new Error('self must be defined to determine if self is unadmitted as guest.'));
  }
...
selfUtils.moderatorChanged = (oldSelf, changedSelf) => {
  ...
  if (!changedSelf) {
    throw new ParameterError(new Error('New self must be defined to determine if self transitioned moderator status.'));
  }
...
selfUtils.isAdmittedGuest = (oldSelf, changedSelf) => {
  ...
  if (!changedSelf) {
    throw new ParameterError(new Error('New self must be defined to determine if self transitioned to admitted as guest.'));
  }
...
selfUtils.mutedByOthers = (oldSelf, changedSelf) => {
  if (!changedSelf) {
    throw new ParameterError(new Error('New self must be defined to determine if self was muted by others.'));
  }
