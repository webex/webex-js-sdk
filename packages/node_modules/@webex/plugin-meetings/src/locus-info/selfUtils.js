...
selfUtils.moderatorChanged = (oldSelf, changedSelf) => {
  ...
  if (!changedSelf) {
    throw new TypeError('New self must be defined to determine if self transitioned moderator status.');
  }
...
selfUtils.mutedByOthers = (oldSelf, changedSelf) => {
  if (!changedSelf) {
    throw new TypeError('New self must be defined to determine if self was muted by others.');
  }
