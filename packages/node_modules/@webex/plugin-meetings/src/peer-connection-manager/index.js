...
const enqueueSdpOffer = (peerConnection, offerType, payload) => new Promise((resolve, reject) => {
  const queueItem = {
    pendingPromiseResolve: resolve, pendingPromiseReject: reject, offerType, payload
  };

  if (!peerConnection.queuedOfferRequests) {
    peerConnection.queuedOfferRequests = [];
  }
  LoggerProxy.logger.info(`Peer-connection-manager:index#enqueueSdpOffer --> enqueuing SDP offer: offerType=${offerType}`);
  peerConnection.queuedOfferRequests.push(queueItem);
});

// todo: remove canUpdateMedia

const processNextQueuedSdpOffer = (peerConnection) => {
  if (peerConnection.queuedOfferRequests?.length > 0) {
    const {
      pendingPromiseResolve, pendingPromiseReject, offerType, payload
    } = peerConnection.queuedOfferRequests.shift();

    if (offerType === SDP_OFFER_TYPE.LOCAL) {
      const {meetingId, remoteQualityLevel, enableRtx} = payload;

      LoggerProxy.logger.info(`Peer-connection-manager:index#processNextQueuedSdpOffer --> creating delayed SDP local offer for meeting ${meetingId}`);
      pc.createOffer(peerConnection, {meetingId, remoteQualityLevel, enableRtx})
        .then((peerConn) => pendingPromiseResolve(peerConn))
        .catch((e) => pendingPromiseReject(e));
    }
    else if (offerType === SDP_OFFER_TYPE.REMOTE) {
      const {meetingId, remoteSdp} = payload;

      LoggerProxy.logger.info(`Peer-connection-manager:index#processNextQueuedSdpOffer --> setting delayed SDP remote offer for meeting ${meetingId}`);
      pc.setRemoteSessionDetails(peerConnection, SDP.OFFER, remoteSdp, meetingId)
        .then(() => pendingPromiseResolve())
        .catch((e) => pendingPromiseReject(e));
    }
    else {
      LoggerProxy.logger.error(`Peer-connection-manager:index#processNextQueuedSdpOffer --> unsupported SDP offer type ${offerType} found in the queue`);
    }
  }
};
...
pc.setRemoteSessionDetails = (peerConnection, typeStr, remoteSdp, meetingId) => {
  ...
  if (typeStr === SDP.OFFER && peerConnection.signalingState !== SDP.STABLE) {
    return enqueueSdpOffer(peerConnection, SDP_OFFER_TYPE.REMOTE, {remoteSdp, meetingId});
  }

  if (typeStr === SDP.ANSWER && peerConnection.signalingState !== SDP.HAVE_LOCAL_OFFER) {
    LoggerProxy.logger.error(`Peer-connection-manager:index#setRemoteSessionDetails --> received remote SDP in wrong state: ${peerConnection.signalingState}`);

    return Promise.reject(new MediaError(`PeerConnection in wrong state: ${peerConnection.signalingState}`));
  }
  ...
  return peerConnection.setRemoteDescription(
    ...
    .then(() => {
      if (!peerConnection.remoteDescription) { // todo: why this condition is like this? shouldn't it be the opposite?
        Metrics.postEvent({
          event: eventType.REMOTE_SDP_RECEIVED,
          meetingId
        });
      }
      if (typeStr !== SDP.OFFER) {
        processNextQueuedSdpOffer(peerConnection);
      }
    })
    .catch((error) => {
...
pc.createOffer = (peerConnection, {meetingId, remoteQualityLevel, enableRtx}) => {
  ...
  if (peerConnection.signalingState !== SDP.STABLE) { // todo: handle "have-local-pranswer" and "have-remote-pranswer"
    return enqueueSdpOffer(peerConnection, SDP_OFFER_TYPE.LOCAL, {meetingId, remoteQualityLevel, enableRtx});
  }
