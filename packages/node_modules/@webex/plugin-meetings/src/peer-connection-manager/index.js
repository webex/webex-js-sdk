 ...
 * @returns {String}
 */
const insertBandwidthLimit = (sdpLines, index, tias) => {
  // eslint-disable-next-line no-warning-comments
  // TODO convert to sdp parser
  let limit;
  if (sdpLines[index].search(AUDIO) !== -1) {
    limit = tias?.audio || StaticConfig.meetings.bandwidth.audio;
  } else if(sdpLines[index].search(VIDEO) !== -1){
    limit = tias?.video || StaticConfig.meetings.bandwidth.video;
  }else {
    limit = tias?.share || StaticConfig.meetings.bandwidth.video;
  }
 ...
 * @returns {String}
 */
const linusServerhack = (sdp, config={},  level = QUALITY_LEVELS.HIGH, isAnswer) => {
  // TODO convert to sdp parser
  let offerSdp = sdp;
  let sdpLines = offerSdp.split(SDP.CARRIAGE_RETURN);
  let {audio, video , share} = config;
  ...
  // Appending the TIAS lines for the server, 
  // TODO: check if there more optimised way 
  sdpLines.splice(positions[0] + 1, 0, `${SDP.B_LINE}:${audio?.tias || StaticConfig.meetings.bandwidth.audio}`);
  sdpLines.splice(positions[1] + 2, 0, `${SDP.B_LINE}:${video?.tias || StaticConfig.meetings.bandwidth.video}`); 
  sdpLines.splice(positions[2] + 3, 0, `${SDP.B_LINE}:${share?.tias || StaticConfig.meetings.bandwidth.video}`); 

  offerSdp = sdpLines.join(SDP.CARRIAGE_RETURN);

if(video && share){
  // code to change the fmtp line
  const fmptLines = offerSdp.match(/(\na=fmtp:(\d+).*level-asymmetry-allowed=1.*)/g)

  const minMaxFsValue = parseInt(MAX_FRAMESIZES[level]) < parseInt(video?.maxFs) ? MAX_FRAMESIZES[level] : video?.maxFs;

  // change the video fmtp line same as what server is requesting 
  offerSdp = offerSdp.replace(fmptLines[0], fmptLines[0]+ `;max-mbps=${video?.maxMbps};max-fs=${minMaxFsValue};max-fps=${video?.maxFps};max-br=${video?.maxBr};max-dpb=${video?.maxDpb}`)

  // same changes for share line of the SDP
  offerSdp = offerSdp.replace(fmptLines[1], fmptLines[1]+ `;max-mbps=${share?.maxMbps};max-fs=${share?.maxFs};max-fps=${share?.maxFps};max-br=${share?.maxBr};max-dpb=${share?.maxDpb}`)
...
pc.iceCandidate = (peerConnection, {remoteQualityLevel}) =>
  new Promise((resolve) => {
    ...
    const timeout = setTimeout(() => {
      peerConnection.sdp = linusServerhack(peerConnection.localDescription.sdp, peerConnection.remoteConfig, remoteQualityLevel, false);
...
pc.setRemoteSessionDetails = (peerConnection, typeStr, remoteSdp, meetingId) => {
  ...
  // Save last TIAS line from the server OFFER
  // m=(?:audio)[\s\S]+?b=TIAS:(\d+)[\s\S]+?m=(?:video)[\s\S]+?b=TIAS:(\d+)[\s\S]+?max-mbps(.*)[\s\S]+?m=(?:video)[\s\S]+?b=TIAS:(\d+)[\s\S]+?max-mbps(.*)[\s\S]+?
  let regex =/m=(?:audio)[\s\S]+?b=TIAS:(\d+)[\s\S]+?m=(?:video)[\s\S]+?b=TIAS:(\d+)[\s\S]+?max-mbps=(\d+);max-fs=(\d+);max-fps=(\d+);max-br=(\d+);max-dpb=(\d+);(.*)[\s\S]+?m=(?:video)[\s\S]+?b=TIAS:(\d+)[\s\S]+?max-mbps=(\d+);max-fs=(\d+);max-fps=(\d+);max-br=(\d+);max-dpb=(\d+);(.*)[\s\S]+?/
  const parsedSdp = sdp.match(regex);
  peerConnection.remoteConfig = { 
    audio: {
    tias: parsedSdp[1]
  },
    video: {
...
pc.createAnswer = (params, {meetingId, remoteQualityLevel}) => {
  ...
  return pc.setRemoteSessionDetails(peerConnection, OFFER, params.offerSdp, meetingId)
    ...
    .then((answer) =>{
      // code to always send answer SDP values with equal bandwidth values
      //   if (edonusHack()) {
    ...
    .then(() => pc.iceCandidate(peerConnection, {remoteQualityLevel}))
    .then(() => {
      peerConnection.sdp = linusServerhack(peerConnection.localDescription.sdp, peerConnection.remoteConfig, remoteQualityLevel, true);
      // peerConnection.sdp = setMaxFs(peerConnection.sdp, remoteQualityLevel);
      if (!checkH264Support(peerConnection.sdp)) {
        throw new MediaError('openH264 is downloading please Wait. Upload logs if not working on second try');
      }
