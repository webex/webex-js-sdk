...
const insertVideoOrientation = (localSdp, remoteSdp) => {
  const regex = /a=extmap:[0-9]+ urn:3gpp:video-orientation/gm;
  const extmapValues = localSdp.match(regex);

  remoteSdp = remoteSdp.replace('a=mid:1\r\n', `a=mid:1\r\n${extmapValues[0]}\r\n`);
  remoteSdp = remoteSdp.replace('a=mid:2\r\n', `a=mid:2\r\n${extmapValues[1]}\r\n`);
...
pc.setRemoteSessionDetails = (peerConnection, typeStr, remoteSdp, meetingId) => {
  ...
  if (peerConnection.signalingState === SDP.HAVE_LOCAL_OFFER || (peerConnection.signalingState === SDP.STABLE && typeStr === SDP.OFFER)) {
    sdp = insertVideoOrientation(peerConnection.sdp, sdp);
