...
const setMaxFs = (sdp, level = QUALITY_LEVELS.HIGH) => {
  ...
  replaceSdp = replaceSdp.replace(/c=IN IP4 .*/gi, 'c=IN IP4 0.0.0.0');
...
const isSdpInvalid = (sdp) => {
  ...
  for (const mediaLine of parsedSdp.media) {
    ...
    // Ios version has ipv6 in the connection line and a bummy ipv4 in candidate in ice candidate
    const ipv4ConnectionLine = mediaLine.connection.version === 4 && mediaLine.connection.ip.match(IPV4_REGEX);

    if (hostCandidate.length === 0 && ipv4ConnectionLine.lenngth === 0) {
...
pc.iceCandidate = (peerConnection, {remoteQualityLevel}) =>
  new Promise((resolve, reject) => {
    ...
    const doneGatheringIceCandidate = () => {
      const seconds = parseInt(Math.abs((Date.now() - now / 1000) % 60), 2);
      ...
      LoggerProxy.logger.log(`PeerConnectionManager:index#iceCandidate --> Time to gather ice candidate${seconds}`);
