...
import {error} from '../metrics/config'
  ...
  pc.oniceconnectionstatechange = () => {
    switch (pc.iceConnectionState) {
      ...
      case ICE_STATE.DISCONNECTED:
        LoggerProxy.logger.warn(`PeerConnectionManager->oniceconnectionstatechange#${name} Interactive Connectivity Establishment(ICE)${pc.iceConnectionState} at ${new Date()}`);
        setTimeout(() => {
          switch (pc.iceConnectionState) {
            case ICE_STATE.FAILED:
            case ICE_STATE.DISCONNECTED:
...
PC.detectFailures = (pc, reconnectFn = () => {LoggerProxy.logger.error('PeerConnectionManager->reconnectFn#Ice failed, no function to reconnect with.')}) => {
 ...
 * @returns {undefined}
 */
PC.setRemoteSessionDetails = (pc, typeStr, remoteSdp, callFlowId) => {
  LoggerProxy.logger.log(`PeerConnectionManager->setRemoteSessionDetails#Setting the remote description type: ${typeStr}State: ${pc.signalingState}`);
  let sdp = remoteSdp;
  ...
  if(!sdp) {
    // Metrics.remoteSdpGeneration({meetingId, data: {
    //   canProceed: false,
    //   errors: [Metrics.generateErrorPayload(2001, true,
    //     MetricsConfig.error.name.MEDIA_ENGINE, 'missing remoteSdp')]
    // }})
  }
  if (pc.signalingState === SDP.HAVE_LOCAL_OFFER || (pc.signalingState === SDP.STABLE && typeStr === SDP.OFFER)) {
    return pc.setRemoteDescription(
      ...
      .then(() => Metrics.remoteSdpGeneration({meetingId: callFlowId}))
      .catch((error) => Metrics.remoteSdpGeneration({meetingId: callFlowId, data: {
        canProceed: false,
        errors: [Metrics.generateErrorPayload(2001, true,
    ...
    return Promise.reject(new MediaError(new Error('PeerConnection in wrong state')));
};
    ...
    return pc
      ...
      .then(() => {
        ...
        if (!checkH264Support(pc.sdp)) {
          throw new MediaError(new Error('openH264 is downloading please Wait. Upload logs if not working on second try'));
        }

        pc.sdp = pc.sdp.replace(/\na=extmap.*/g, '');
        PC.setContentSlides(pc);

        Metrics.localSdpGeneration({meetingId: callFlowId})
        return pc;
      })
      .catch((err) => {
        Metrics.localSdpGeneration({
          meetingId: callFlowId,
          data: {
            canProceed: false,
             errors: [
...
PC.updatePeerConnection = (params) => {
  LoggerProxy.logger.log(`PeerConnectionManager->updatePeerConnection#updating the pc with params: ${params}`);

  const {pc, mediaSettings} = params.meeting.mediaProperties;
...
PC.close = (pc) => {
  ...
  if(pc && pc.connectionState === PEER_CONNECTION_STATE.CLOSED) {
