...
class Metrics {
   ...
   */
  postEvent(options) {
    const {meetingId, data} = options;
    let {meeting} = options;

    if (!meeting && options.meetingId) {
  ...
  callInitiated(options) {
    options.event = MetricsConfig.eventType.CALL_INITIATED;
    this.postEvent(options);
  }

  moveMedia(options) {
    options.event = MetricsConfig.eventType.MOVE_MEDIA;
    this.postEvent(options);
  }

  localSdpGeneration(options) {
    options.event = MetricsConfig.eventType.CLIENT_MEDIA_ENGINE_LOCAL_SDP_GENERATED;
    this.postEvent(options);
  }

  remoteSdpGeneration(options) {
    options.event = MetricsConfig.eventType.CLIENT_MEDIA_ENGINE_REMOTE_SDP_RECEIVED;
    this.postEvent(options);
  }

  joinRequest(options) {
    options.event = MetricsConfig.eventType.CLIENT_LOCUS_JOIN_REQUEST;
    this.postEvent(options);
  }

  joinResponse(options) {
    options.event = MetricsConfig.eventType.CLIENT_LOCUS_JOIN_RESPONSE;
    this.postEvent(options);
  }

  notificationReceived(options) {
    options.event = MetricsConfig.eventType.CLIENT_NOTIFICATION_RECEIVED;
    this.postEvent(options);
  }

  alertDisplayed(options) {
    options.event = MetricsConfig.eventType.CLIENT_ALERT_DISPLAYED;
    this.postEvent(options);
  }

  iceStart(options) {
    options.event = MetricsConfig.eventType.CLIENT_ICE_START;
    this.postEvent(options);
  }

  iceStop(options) {
    options.event = MetricsConfig.eventType.CLIENT_ICE_END;
    this.postEvent(options);
  }

  startedRecvMedia(options) {
    options.event = MetricsConfig.eventType.CLIENT_MEDIA_RX_START;
    this.postEvent(options);
  }

  stoppedRecvMedia(options) {
    options.event = MetricsConfig.eventType.CLIENT_MEDIA_RX_STOP;
    this.postEvent(options);
  }

  startedSendingMedia(options) {
    options.event = MetricsConfig.eventType.CLIENT_MEDIA_TX_START;
    this.postEvent(options);
  }

  stoppedSendingMedia(options) {
    options.event = MetricsConfig.eventType.CLIENT_MEDIA_TX_STOP;
    this.postEvent(options);
  }

  renderingStarted(options) {
    options.event = MetricsConfig.eventType.CLIENT_MEDIA_RENDER_START;
    this.postEvent(options);
  }

  renderingStopped(options) {
    options.event = MetricsConfig.eventType.CLIENT_MEDIA_RENDER_STOP;
    this.postEvent(options);
  }

  // ?? Whats this one for
  mediaCapabilities(options) {
    options.event = MetricsConfig.eventType.CLIENT_MEDIA_CAPABILITIES;
    this.postEvent(options);
  }

  mediaReconnected(options) {
    options.event = MetricsConfig.eventType.CLIENT_MEDIA_RECONNECTING;
    this.postEvent(options);
  }

  mediaRecovered(options) {
    options.event = MetricsConfig.eventType.CLIENT_MEDIA_RECOVERED;
    this.postEvent(options);
  }

  mediaReconnectionFailed(options) {
    options.event = MetricsConfig.eventType.CALL_RECONNECTION_FAILED;
    this.postEvent(options);
  }

  pinPrompted(options) {
    options.event = MetricsConfig.eventType.CLIENT_PIN_PROMPT;
    this.postEvent(options);
  }

  pinCollected(options) {
    options.event = MetricsConfig.eventType.CLIENT_PIN_COLLECTED;
    this.postEvent(options);
  }

  enteredLobby(options) {
    options.event = MetricsConfig.eventType.CLIENT_LOBBY_ENTERED;
    this.postEvent(options);
  }

  exitedLobby(options) {
    options.event = MetricsConfig.eventType.CLIENT_LOBBY_EXITED;
    this.postEvent(options);
  }

  shareInitiated(options) {
    options.event = MetricsConfig.eventType.CLIENT_SHARE_INITIATED;
    this.postEvent(options);
  }

  shareStopped(options) {
    options.event = MetricsConfig.eventType.CLIENT_SHARE_STOPPED;
    this.postEvent(options);
  }

  shareFloorGranted(options) {
    options.event = MetricsConfig.eventType.CLIENT_SHARE_FLOOR_GRANTED_LOCAL;
    this.postEvent(options);
  }

  shareLayout(options) {
    options.event = MetricsConfig.eventType.CLIENT_SHARE_LAYOUT_DISPLAYED;
    this.postEvent(options);
  }

  muted(options) {
    options.event = MetricsConfig.eventType.CLIENT_MUTED;
    this.postEvent(options);
  }

  unmuted(options) {
    options.event = MetricsConfig.eventType.CLIENT_UNMUTED;
    this.postEvent(options);
  }

  leaveMeeting(options) {
    options.event = MetricsConfig.eventType.CLIENT_CALL_LEAVE;
    this.postEvent(options);
  }

  remoteEnded(options) {
    options.event = MetricsConfig.eventType.CLIENT_CALL_REMOTE_ENDED;
    this.postEvent(options);
  }

  // ???? check if its correct
  receivedIncoming(options) {
    options.event = MetricsConfig.eventType.CLIENT_CALL_REMOTE_STARTED;
    this.postEvent(options);
  }

  sendMediaRequest(options) {
    options.event = MetricsConfig.eventType.CLIENT_LOCUS_MEDIA_REQUEST;
    this.postEvent(options);
  }

  gotMediaResponse(options) {
    options.event = MetricsConfig.eventType.CLIENT_LOCUS_MEDIA_RESPONSE;
    this.postEvent(options);
  }

  startPstn(options) {
    options.event = MetricsConfig.eventType.CLIENT_PSTN_AUDIO_ATTEMPT_START;
    this.postEvent(options);
  }

  finishPstn(options) {
    options.event = MetricsConfig.eventType.CLIENT_PSTN_AUDIO_ATTEMPT_FINISH;
    this.postEvent(options);
  }

  skipPstn(options) {
    options.event = MetricsConfig.eventType.CLIENT_PSTN_AUDIO_ATTEMPT_SKIP;
    this.postEvent(options);
  }
  ...
  _initPayload(eventType, identifiers, options) {
  ...
  parseLocusError(err, showToUser) {
    ...
    return this.createErrorPayloadWithErrorCode(
      errorCode, showToUser, MetricsConfig.error.name.LOCUS_RESPONSE, err
    );
  }


  generateErrorPayload(errorCode, shownToUser, name, err) {
    if (MetricsConfig.error.errors[errorCode]) {
      const errorPayload = {
        shownToUser: shownToUser || false,
        category: MetricsConfig.error.errors[errorCode][2],
        errorDescription: MetricsConfig.error.errors[errorCode][0],
        errorCode,
        fatal: !includes(MetricsConfig.error.notFatalErrorList, errorCode),
        name: name || MetricsConfig.error.name.OTHER
      };
  ...
  getUserAgentInfo() {
    ...
    if (userAgentOption) {
      ...
      return util.format('SparkWeb/%s %s', config.version, userAgentOption);
    }

    return util.format('SparkWeb/%s', config.version);
  }
