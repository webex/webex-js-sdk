...
import {ICESTATE, MEETINGS, RECHABILITY, _ANSWER_} from '../constants';
...
export default class Rechability extends StatelessSparkPlugin {
  namespace = MEETINGS;
  ...
  /**
   * fetches rechability data
   * @returns {object} rechability data
   */
  orpheusCall = () => {
    // Reset reachability result to empty to ensure no stale data
    try {
      window.localStorage.setItem(RECHABILITY.localStorage, JSON.stringify({}));
    }
    catch (error) {
      ...
      .then((clusters) => {
        ...
        return this.performReachabilityCheck(clusters)
          .then((response) => {
            ...
            window.localStorage.setItem(RECHABILITY.localStorage, JSON.stringify(response));
            return response;
          });
      })
      .catch((error) => {
   ...
   * @returns {object} rechability check results
   */
