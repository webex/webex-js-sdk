...
export default class Meetings extends SparkPlugin {
  ...
  handleLocusEvent(data) {
    ...
    this.getMeetingByType('correlationId', MeetingsUtil.checkForCorrelationId(this.spark.internal.device.url, data.locus));

    const debugging = [];

    if (data.locus) {
      data.locus.participants.forEach((participant) => {
        debugging.push(participant.devices);
      });

      console.log('new Event \n DEVICES CHECK ', debugging);
    }
    if (!meeting) {
      ...
      if (data.locus && data.locus.fullState && data.locus.fullState.state === 'INACTIVE') {
        // just ignore the event as its already ended and not active
        console.info('LOCUS EVENT AFTER MEETING ENDED');
   ...
   * @memberof Meetings
   */
  destroy(meetingId, response, type) {
