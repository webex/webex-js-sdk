  ...
  */
  ...
  /**
   * (Beta) Maintain a cache of meetings and sync with services.
   * @class
   */
  export default class Meetings extends SparkPlugin {
    ...
    handleLocusEvent(data) {
      console.log(`MEETINGs NAME ${this.name}`);
      console.log(`MEETINGs Locus ${data.locusUrl}`);
      console.log(`MEETINGs correlation ID`, Object.keys(this.meetingCollection.getAll()))
      console.log(`MEETINGs Locus correlationId`, MeetingsUtil.checkForCorrelationId(this.spark.internal.device.url, data.locus));
      console.log(`MEETINGs Locus full`, data.locus);
      ...
      this.getMeetingByType(CORRELATION_ID, MeetingsUtil.checkForCorrelationId(this.spark.internal.device.url, data.locus)) ||
      this.getMeetingByType(SIP_URI, data.locus.self.callbackInfo.callbackAddress);
      ...
      if (!meeting) {
        ...
        console.log(`MEETING NOT FOUND ON EVENT ${data.locusUrl}`);
        console.log('MEETING NOT FOUND ON EVENT', this.meetingCollection.getAll());
    ...
    listenInternal() {
      Events.on(EVENTS.DESTROY_MEETING, (payload) => {
        console.log('DESTROY:MEETINS-REGISTRATION ', payload.meeting);
    ...
    onReady() {
      this.spark.once(READY, () => {
        ...
        LoggerProxy.set(this.spark.logger);
        ...
        return this.createMeeting(destination, type)
          .then((meeting) => {
            meeting.on(EVENTS.DESTROY_MEETING, (payload) => {
              console.log('DESTROY:MEETINS-REGISTRATION ', payload.meetingId);
