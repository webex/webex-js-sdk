...
import {MEETINGS, EVENT_TRIGGERS, READY, LOCUSEVENT, LOCUS_URL, CORRELATION_ID, ROAP, LOCUS_ID} from '../constants';
...
export default class Meetings extends SparkPlugin {
  ...
  createMeeting(destination, type = null) {
    ...
    // Moved getMeetingInfo later as it takes more time for the network request
    // meanwhile if the client is flooded with mercury event it wont cause any issues
    this.meetingInfo
  ...
  */
  syncMeetings() {
    ...
    return this.request.getActiveMeetings()
      .then((locus) => {
        console.log(locus.loci);
        if (locus.loci && locus.loci.length > 0) {
          locus.loci.forEach((locus) => {
            meeting = this.create(locus, LOCUS_ID);
            meeting.locusInfo.intialSetup(locus);
          });
        }
      });
  }
