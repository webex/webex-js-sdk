...
import { MEETINGS, LOCUSEVENT, EVENT_TRIGGERS} from '../constants';
import MeetingInfo from '../meeting-info';
import Meeting from '../meeting';
import Events from '../events';
import LocusParser from '../locus-parser';
...
export default class Meetings extends SparkPlugin {
  ...
  constructor(...args) {
    ...
    // TODO: This not working for some reason with Events
    this.spark.internal.mercury.on('event:locus', (envelope) => {
      this._locusEvent(envelope.data);
    });
  }

  /**
   * Handles all the locus event sent by mercury
  ...
  _locusEvent(data) {
    ...
    const meeting = this.getMeetingByType('locusUrl', data.locusUrl);

    LocusParser.parse(meeting, data, this);
  }
  ...
  createMeetingByLocus(locus) {
    const temp = locus.url.split('/');

    const locusId = temp[temp.length - 1];
    let meeting, mtgInfo;
    this.meetingInfo
      .fetchMeetingInfo(locusId, 'LOCUS_ID')
      .then((info) => info)
      .catch((err) => err)
      .then((info) => {
      ...
      .then(() => {
        ...
        meeting.parseLocus(locus);
        // if (!meeting.sipUri) {
        //   meeting.setSipUri(destination);
        // }

        console.log('Create new meeting object', meeting);

        //TODO : dispatch event change this
        this.trigger(EVENT_TRIGGERS.MEETING_ADDED, meeting);
