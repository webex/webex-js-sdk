...
export default class Meetings extends WebexPlugin {
    ...
    generateOfferSdp() {
      this.programableVideo.peerConnection.ontrack = (event) => {
        // Should get the remote streams as part of this event to play
        // event.track
      };

      return this.programableVideo.media.getUserMedia({sendAudio: true, sendVideo: true}, {
        video: {
          width: {
            max: 1280,
            ideal: 1280
      ...
      }, null,
      {
        ...
        .then(([localStream, localShare]) => {
          this.programableVideo.media.attachMedia({
            mediaDirection: {
              sendAudio: true,
              sendVideo: true,
              sendShare: false,
            ...
            shareTrack: null,
            peerConnection: this.programableVideo.peerConnection
          }, {})
            .then((peerConnection) => {
              console.log('SDP returned back ', peerConnection.sdp);
            // GET THE LOCAL SDP FROM peerconnection.sdp
            });
        });
    }

    // Make sure the SDP is a string
    setRemoteAnswer(sdp) {
      return this.programableVideo.peerConnectionManager.setRemoteSessionDetails(this.programableVideo.peerConnection, 'answer', sdp);
    }
        ...
        if (Object.keys(this.meetingCollection.getAll()).length > 0) {
          ...
          forEach(this.meetingCollection.getAll(), (meeting) => {
            if (!activeLocusUrl.includes(meeting.locusUrl)) {
              if (this.config.autoUploadLogs) {
                this.uploadLogs({
                  callStart: meeting.locusInfo?.fullState?.lastActive,
                  correlationId: meeting.correlationId,
                  feedbackId: meeting.correlationId,
                  locusId: meeting.locusId,
                  meetingId: meeting.locusInfo?.info?.webExMeetingId
                }).then(() => this.destroy(meeting, MEETING_REMOVED_REASON.NO_MEETINGS_TO_SYNC));
              }
              else {
