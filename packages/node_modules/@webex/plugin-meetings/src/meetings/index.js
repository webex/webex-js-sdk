...
import {MEETINGS, EVENT_TRIGGERS, READY, LOCUSEVENT, LOCUS_URL, CORRELATION_ID, ROAP} from '../constants';
import MeetingInfo from '../meeting-info';
import Meeting from '../meeting';
import PersonalMeetingRoom from '../personal-meeting-room';
import LocusParser from '../locus-events/parser';
...
export default class Meetings extends SparkPlugin {
  ...
  constructor(...args) {
    ...
    this.parser = new LocusParser({}, {parent: this.spark});
    this.onReady();
  }
  ...
  listenForEvents() {
    this.spark.internal.mercury.on(LOCUSEVENT.LOCUS_MERCURY, (envelope) => {
      ...
      if (eventType !== LOCUSEVENT.MESSAGE_ROAP) {
        const meeting = this.getMeetingByType(LOCUS_URL, data.locusUrl);
  ...
  createMeeting(info, destination, type = null) {
    ...
    if (meetingAddedType === 'INCOMING') { // TODO:
      meeting.acknowledge();
      this.notifyRinging(meeting, meetingAddedType, true);
    }
   ...
   * @param {Boolean} ringing
   * @returns {undefined}
   * TODO: move this to meeting and deal with sequence/synchronization
   */
  notifyRinging(meeting, type, ringing = true) {
    if (!ringing) {
      type = null; // coerce param to null
    }
