...
export default class Meetings extends SparkPlugin {
  ...
  constructor(...args) {
    ...
    this.scheduledMeetings = new ScheduledMeetings({}, {parent: this.spark});
  }
  ...
  /**
   * gets the scheduled meetings instance
   * @returns {ScheduledMeetings}
   */
  ...
  /**
   * @param {String} type a string representing the type of destination, such as SIP_URI,MEETING_ID,LOCUS_ID,PERSONAL_ROOM,MEETING_LINK,ONE_ON_ONE,MEDIA_SIP_URI,CONVERSATION_URL,TEMP_SIP_URI
   * @param {String} destination the actual mapped string for the type, i.e., we expect that if type is SIP_URI, the destination string WILL be a valid sip uri
   * @returns {Promise} a new meeting
   * See create() for description
   */
  createWithType(type, destination) {
    return this.meetingInfo
      .fetchMeetingInfo(destination, type)
      .then((info) => info)
      .catch((err) => err)
      .then((info) => this.createMeeting(info, destination))
      .catch((err) => err);
  ...
  createMeeting(info, destination) {
    ...
    meeting.setMeetingInfo(mtgInfo);
