...
export default class Meetings extends WebexPlugin {
     ...
     * @param {String} source the name of the function that calls handleLocusEvent
     * @param {String} data.eventType
     * @returns {undefined}
     * @private
     * @memberof Meetings
     */
    handleLocusEvent(data, source = null) {
      let meeting = null;
      ...
      if (!meeting) {
        ...
        this.create(data.locus, _LOCUS_ID_, source).then((newMeeting) => {
          meeting = newMeeting;
    ...
    */
     ...
     * @param {string} source - the function that calls create()
     * @returns {Promise<Meeting>} A new Meeting.
     * @public
     * @memberof Meetings
     */
    create(destination, type = null, source = null) {
      ...
      return this.meetingInfo.fetchInfoOptions(destination, type)
        ...
        .then((options = {}) => {
          ...
          if (!meeting) {
            ...
            return this.createMeeting(targetDest, type, source)
              .then((createdMeeting) => {
     ...
     * @param {String} source the source function that calls createMeeting
     * @returns {Promise} a new meeting instance complete with meeting info and destination
     * @private
     * @memberof Meetings
     */
    async createMeeting(destination, type = null, source = null) {
      const meeting = new Meeting(
        {
          ...
          setTimeoutFetchMeetingInfoId: undefined,
          fetchMeetingInfoCalled: false
        },
        {
          parent: this.webex
        }
      ...
      const syncMeetingsIsSource = (source === 'syncMeetings');

      try {
        if (!isMeetingActive && !syncMeetingsIsSource) {
          // if no participant has joined the scheduled meeting (meaning meeting is not active) and we still get a locusEvent,
          ...
          const maxWaitingTime = Math.max(Math.min(timeToStart, 3 * 60 * 1000), 0);
          const waitingTime = Math.round(Math.random() * maxWaitingTime);

          meeting.setTimeoutFetchMeetingInfoId = setTimeout(() => meeting.fetchMeetingInfo({destination, type}), waitingTime);
        }
        else {
