...
export default class Meetings extends SparkPlugin {
  ...
  constructor(...args) {
    ...
    window.MeetingCollection = MeetingCollection;
  }
  ...
  handleLocusEvent(data) {
    ...
    if (!meeting) {
      ...
      this.create(data.locus, LOCUS_ID)
        .then((meeting) => {
          meeting.locusInfo.parse(meeting, data);
        });
      // Remove this after we determine when to show incoming notification
      // if (data.eventType === LOCUSEVENT.PARTICIPANT_JOIN) {
  ...
  listenForEvents() {
    ...
    this.spark.internal.device('ready', () => {
      console.log('READY for netwrok call');
      this.request.getActiveMeetings().then((res) => {
        console.log(res);
      });
    });
  }
   ...
   */
  onReady() {
    this.spark.internal.once(READY, () => {
