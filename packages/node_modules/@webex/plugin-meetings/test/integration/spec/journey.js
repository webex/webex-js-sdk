...
skipInNode(describe)('plugin-meeting', () => {
  before(() => sparkTestUsers.generateTestUsers({count: 3})
    .then((users) => {
      ...
      chris.spark.meetings.name = 'chris';
    })
  ...
  describe('Successful 1:1 meeting (including Guest)', function () {
    ...
    it('No previous Call', () => {
      ...
      return alice.spark.internal.conversation.create({participants: [alice, bob]})
        .then((conversation) => alice.spark.internal.conversation.post(conversation, 'hello world how are you '));
    });
...
>>>>>>> fix: added automation fix
      .then(() => {
        assert.notEqual(alice.meeting.deviceUrl, bob.meeting.deviceUrl, 'deviceUrl are not same 2');
      })
...
=======
    it('alice stops sharing ', () =>
      testUtils.waitUntil(8000)
        .then(() => Promise.all([
...
=======
>>>>>>> fix: journey spec for automation
...
>>>>>>> fix: added automation fix
      ...
      //   assert.equal(bob.meeting.getStats().getSender('audioSend').getAnalyzer().getValidity(), true);
      //   assert.equal(alice.meeting.getStats().getSender('audioSend').getAnalyzer().getValidity(), true);
      //   assert.equal(bob.meeting.getStats().getReceiver('audioRecv').getAnalyzer().getValidity(), true);
      //   assert.equal(alice.meeting.getStats().getReceiver('audioRecv').getAnalyzer().getValidity(), true);
      // })
      .then(function aliceShare() {
        ...
        console.log('STARTED SHARING ALICE', alice.spark.internal.device.url);
        console.log('STARTED SHARING BOB', bob.spark.internal.device.url);
        // console.log('STARTED SHARING CHRIS', chris.spark.internal.device.url);
    ...
    //         const aliceParticipant = response[0].result.delta.updated.find((member) => alice.meeting.members.selfId === member.id);
      ...
      .then(function aliceUpdateVideo() {
...
<<<<<<< HEAD
    xit('alice adds chris as guest to 1:1 meeting', () => Promise.all([
=======
    it('alice shares the screen', () => alice.meeting.getMediaStreams({
      sendShare: true
    })
...
>>>>>>> fix: skipping unit test for karma and only run on node
