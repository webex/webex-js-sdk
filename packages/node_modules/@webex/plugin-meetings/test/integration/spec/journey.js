...
const {isBrowser, getBrowserName} = BrowserDetection();
...
skipInNode(describe)('plugin-meetings', () => {
  describe('journey', () => {
    ...
    describe('Successful 1:1 meeting (including Guest)', function () {
      ...
      // Firefox bug - https://bugzilla.mozilla.org/show_bug.cgi?id=1537986
      if (getBrowserName() !== 'Firefox') {
        const getRemoteHeight = (minHeight = 360, maxHeight = 1080) => new Promise((resolve, reject) => {
          testInterval = setInterval(() => {
            const remoteSettings = bob.meeting.mediaProperties.remoteVideoTrack.getSettings();
            const height = remoteSettings.height && +remoteSettings.height;

            console.log(`Resolution for ${minHeight} ${maxHeight} : `, count, height);
            ...
            if (count === INTERVAL_COUNT) {
              clearInterval(testInterval);
              if (!height) reject(new Error('Resolution undefined'));
              else resolve(height);
            }
        ...
        it('alice resolution is 1080p, check what bob receives', async () => {
          await testUtils.delayedPromise(alice.meeting.setLocalVideoQuality('1080p'));
          /**
           * getRemoteHeight(minHeight, maxHeight) tries to return value in the minHeight and maxHeight range
           * but if not able to in the given duration it returns whatever value is been achieved causing to have
           * assert with range not exactly as preferred in the getRemoteHeight().
           */
          const height = await getRemoteHeight(1080);

          assert.isAbove(height, 500);
          assert.isBelow(height, 1081);
        });

        /**
         * Will be added back when webrtc-media lib in SDK v3 gets used
         * Reference - https://sqbu-github.cisco.com/WebExSquared/webrtc-media-core/pull/139
         */
        it.skip('bob requests 480p, should get resolution around 480p', async () => {
          await testUtils.delayedPromise(bob.meeting.setRemoteQualityLevel('LOW'));
          const height = await getRemoteHeight(400, 480);

          assert.isAbove(height, 400);
          assert.isBelow(height, 481);
        });

        it('bob upgrades remote to 720p, should get resolution around 720', async () => {
          await testUtils.delayedPromise(bob.meeting.setRemoteQualityLevel('MEDIUM'));
          const height = await getRemoteHeight(720, 1079);

          assert.isAbove(height, 500);
          assert.isBelow(height, 721);
        });

        it('bob requests 1080p, should get 1080 or max supported resolution', async () => {
          await testUtils.delayedPromise(bob.meeting.setRemoteQualityLevel('HIGH'));
          const height = await getRemoteHeight(721);

          assert.isAbove(height, 719);
          assert.isBelow(height, 1081);
        });

        it('alice resolution is 720p, check what bob receives', async () => {
          await testUtils.delayedPromise(alice.meeting.setLocalVideoQuality('720p'));
          const height = await getRemoteHeight(600, 720);

          assert.isAbove(height, 500);
          assert.isBelow(height, 721);
        });

        it('alice downgrades to 480p, check what bob receives', async () => {
          await testUtils.delayedPromise(alice.meeting.setLocalVideoQuality('480p'));
          const height = await getRemoteHeight(400, 480);

          assert.isAbove(height, 360);
          assert.isBelow(height, 481);
        });
      }
