...
const waitForEvents = (scopeEvents) => {
  ...
  return Promise.race([
    new Promise((resolve, reject) => {
      timer = setTimeout(() => {
        reject(new Error(`Events not triggered ${JSON.stringify(events)}`));
      }, max);
    }),
    new Promise((resolve, reject) => {
      try {
...
const addMedia = (user) => {
  ...
  return user.meeting.getMediaStreams({
    ...
    .then(() => delayedTest(() => {
      assert.exists(mediaReadyEvent.local, 'local stream event not triggred');
      assert.exists(mediaReadyEvent.remoteAudio, 'remoteAudio event not triggred');
      assert.exists(mediaReadyEvent.remoteVideo, 'remoteVideo event not triggred');
