...
describe('plugin-meetings', () => {
  ...
  describe('members', () => {
    ...
    describe('#raiseHand', () => {
      it('should fire spies correctly when raiseLowerHand is called with valid params', async () => {
        const members = createMembers({url: url1});

        const {membersRequest} = members;

        const generateOptionsSpy = sandbox.spy(MembersUtil, 'generateRaiseHandMemberOptions');
        const raiseLowerHandMemberSpy = sandbox.spy(membersRequest, 'raiseLowerHandMember');

        await members.raiseLowerHand('test1', true);

        assert.calledOnce(generateOptionsSpy);
        assert.calledOnce(raiseLowerHandMemberSpy);
      });

      it('should throw a rejection if there is no locus url', async () => {
        const members = createMembers({url: false});

        assert.isRejected(members.raiseLowerHand('test1', true));
      });
    });
  });
});
