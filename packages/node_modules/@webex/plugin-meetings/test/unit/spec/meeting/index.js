    ...
    describe('Public Event Triggers', () => {
      describe('#reconnect', () => {
        ...
        describe('successful reconnect', () => {
          it('should reconnect and trigger reconnection and return a promise', async () => {
            ...
            assert.calledWith(TriggerProxy.trigger, sinon.match.instanceOf(Meeting), 'meeting:reconnectionSuccess', {reconnect: test1});
        ...
        describe('unsuccessful reconnect', () => {
          it('should trigger an unsuccessful reconnection and return a promise', async () => {
            ...
            await meeting.reconnect().catch(() => {
              assert.calledOnce(TriggerProxy.trigger);
              assert.calledWith(TriggerProxy.trigger, sinon.match.instanceOf(Meeting), 'meeting:reconnectionFailure', {error: sinon.match.any});
              assert.calledOnce(meeting.reconnectionManager.reset);
            });
          });
        });
      ...
      describe('#closeRemoteStream', () => {
        it('should stop the stream, and trigger a media:stopped event when the remote stream stops', async () => {
          ...
          assert.calledOnce(TriggerProxy.trigger);
          assert.calledWith(TriggerProxy.trigger, sinon.match.instanceOf(Meeting), 'media:stopped');
      ...
      describe('#closeLocalShare', () => {
        it('should stop the stream, and trigger a media:stopped event when the local share stream stops', async () => {
          ...
          assert.calledOnce(TriggerProxy.trigger);
          assert.calledWith(TriggerProxy.trigger, sinon.match.instanceOf(Meeting), 'media:stopped', {type: 'localShare'});
        });
      });
      describe('#closeLocalStream', () => {
        it('should stop the stream, and trigger a media:stopped event when the local stream stops', async () => {
          ...
          assert.calledOnce(TriggerProxy.trigger);
          assert.calledWith(TriggerProxy.trigger, sinon.match.instanceOf(Meeting), 'media:stopped', {type: 'local'});
        });
      });
      describe('#setLocalShareTrack', () => {
        it('should trigger a media:ready event with local share stream', () => {
          ...
          meeting.mediaProperties.shareTrack = {getVideoTracks};
          meeting.stopShare = sinon.stub().returns(true);
          meeting.setLocalShareTrack(test1);
          assert.calledOnce(TriggerProxy.trigger);
          assert.calledWith(TriggerProxy.trigger, sinon.match.instanceOf(Meeting), 'media:ready');
      ...
      describe('#setShareStream', () => {
        it('should trigger a media:ready event when share stream track ontrack is fired', () => {
          ...
          assert.calledOnce(TriggerProxy.trigger);
          assert.calledWith(TriggerProxy.trigger, sinon.match.instanceOf(Meeting), 'media:ready', {type: 'remoteShare', stream: test1});
      ...
      describe('#setUpLocusInfoSelfListener', () => {
        it('listens to the self unaddmitted guest event', (done) => {
          meeting.locusInfo.emit('SELF_UNADMITTED_GUEST', test1);
          assert.calledOnce(TriggerProxy.trigger);
          assert.calledWith(TriggerProxy.trigger, sinon.match.instanceOf(Meeting), 'meeting:self:lobbyWaiting', {payload: [test1]});
          done();
        });
        it('listens to the self addmitted guest event', (done) => {
          meeting.locusInfo.emit('SELF_ADMITTED_GUEST', test1);
          assert.calledOnce(TriggerProxy.trigger);
          assert.calledWith(TriggerProxy.trigger, sinon.match.instanceOf(Meeting), 'meeting:self:guestAdmitted', {payload: [test1]});
          done();
        });
      });
    });
