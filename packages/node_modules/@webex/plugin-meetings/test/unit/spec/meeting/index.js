    ...
    describe('Public Api Contract', () => {
        ...
        describe('failure', () => {
          ...
          it('should try to join the meeting and return promise reject', async () => {
            await meeting.join().catch(() => {
              ...
              assert.calledOnce(MeetingUtil.cleanOptions);
              assert.calledOnce(MeetingUtil.joinMeeting);
              assert.calledTwice(meeting.logger.log);
            });
          });
        });
      });
      describe('#addMedia', () => {
        ...
        beforeEach(() => {
          ...
          meeting.audio = true;
          meeting.video = true;
          meeting.setLocalVideoTrack = sinon.stub().returns(true);
          meeting.setLocalShareTrack = sinon.stub().returns(true);
        ...
        it('should attach the media and return promise', async () => {
          ...
          assert.calledOnce(meeting.setLocalVideoTrack);
          assert.calledOnce(meeting.setLocalShareTrack);
    ...
    describe('Public Event Triggers', () => {
      ...
      describe('#setLocalShareTrack', () => {
        it('should trigger a media:ready event with local share stream', () => {
          ...
          assert.calledWith(meeting.mediaProperties.setLocalShareTrack, test1);
          assert.equal(meeting.mediaProperties.localStream, undefined);
          track.onended();
          assert.calledOnce(meeting.stopShare);
        });
      });
      describe('#setLocalVideoTrack', () => {
        it('should trigger a media:ready event with local stream', () => {
          const spy = sinon.spy(meeting, 'trigger');

          meeting.mediaProperties.setLocalVideoTrack = sinon.stub().returns(true);
          meeting.setLocalVideoTrack(test1);
          assert.calledOnce(spy);
          assert.calledWith(spy, 'media:ready', {type: 'local', stream: undefined});
          assert.calledOnce(meeting.mediaProperties.setLocalVideoTrack);
          assert.calledWith(meeting.mediaProperties.setLocalVideoTrack, test1);
          assert.equal(meeting.mediaProperties.localStream, undefined);
        });
      });
