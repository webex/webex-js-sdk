    ...
    describe('Public Api Contract', () => {
      ...
      describe('#muteAudio', () => {
        ...
        describe('after audio is defined', () => {
          ...
          it('should call the participant mute', () => meeting.muteAudio()
            .then(() => {
              assert.calledOnce(participantMute);
              assert.calledWith(participantMute, uuid1, true);
            }));
        });
      });
    ...
    describe('Public Event Triggers', () => {
      ...
      describe('#closeRemoteStream', () => {
        it('should stop remote tracks, and trigger a media:stopped event when the remote tracks are stopped', async () => {
          meeting.mediaProperties.remoteAudioTrack = {readyState: 'ended'};
          meeting.mediaProperties.remoteVideoTrack = {readyState: 'ended'};
          meeting.mediaProperties.remoteShare = {readyState: 'ended'};
      ...
      describe('#closeLocalShare', () => {
        it('should stop the stream, and trigger a media:stopped event when the local share stream stops', async () => {
          meeting.mediaProperties.shareTrack = {readyState: 'ended'};
      ...
      describe('#closeLocalStream', () => {
        it('should stop the stream, and trigger a media:stopped event when the local stream stops', async () => {
          meeting.mediaProperties.audioTrack = {readyState: 'ended'};
          meeting.mediaProperties.videoTrack = {readyState: 'ended'};
      ...
      describe('#setRemoteStream', () => {
        ...
        it('should trigger a media:ready event when remote stream track ontrack is fired', () => {
          ...
          assert.equal(TriggerProxy.trigger.getCall(0).args[2], 'media:ready');
          assert.deepEqual(TriggerProxy.trigger.getCall(0).args[3], {type: 'remoteAudio', stream: true});

          pc.ontrack({track: 'track', transceiver: {mid: '1'}});
          assert.equal(TriggerProxy.trigger.getCall(1).args[2], 'media:ready');
          assert.deepEqual(TriggerProxy.trigger.getCall(1).args[3], {type: 'remoteVideo', stream: true});

          pc.ontrack({transceiver: {mid: '2'}, track: 'track'});
          assert.equal(TriggerProxy.trigger.getCall(2).args[2], 'media:ready');
          assert.deepEqual(TriggerProxy.trigger.getCall(2).args[3], {type: 'remoteShare', stream: true});
        });
      });
