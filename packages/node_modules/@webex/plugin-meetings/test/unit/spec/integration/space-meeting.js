...
describe('plugin-meetings-automation', () => {
  ...
  describe.only('plugin-meeting-webex-meeting', () => {
    before(() => sparkTestUsers.generateTestUsers({count: 3})
      .then((users) => {
        userSet = users;
        alice = userSet[0];
    ...
    after(() => {
      ...
      return Promise.all(promise)
        .then(() => sparkTestUsers.remove(userSet))
        .then(() => alice.cmr.release());
    });

    describe('Webex  meeting', () => {
      const aliceMeetingAdded = sinon.spy();
      const bobMeetingAdded = sinon.spy();
      const chrisMeetingAdded = sinon.spy();

      before(() => CMR.reserve(alice.spark)
        .then((cmr) => {
      ...
      after(() => CMR.release(alice.cmr.resourceUrl));
      ...
      it('bob and chris joins the meeting', () => bob.meeting.create(alice.cmr.sipAddress)
        .then((m) => { bob.meeting = m; return m.join(); })
        .then(() => chris.meeting.create(alice.cmr.sipAddress))
