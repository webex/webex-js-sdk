...
describe('plugin-meetings-automation', () => {
  ...
  describe('plugin-meeting-webex-meeting', () => {
    ...
    after(() => {
      ...
      return Promise.all(promise)
        .then(() => sparkTestUsers.remove(userSet))
        .then(() => CMR.release(alice.cmr.resourceUrl));
    });

    describe('Join meeting as attende', () => {
      it('bob and chris joins the meeting', () => bob.spark.meetings.create(alice.cmr.sipAddress)
        .then((m) => { bob.meeting = m; return m.join({moderator: false}); })
        .then(() => testUtils.waitForStateChange(bob.meeting, 'JOINED')));
    });

    xdescribe('Join meeting with host pin', () => {

    });

    xdescribe('Webex  meeting', () => {
      const aliceMeetingAdded = sinon.spy();
      ...
      it('bob and chris joins the meeting', () => bob.spark.meetings.create(alice.cmr.sipAddress)
        ...
        .then((m) => { chris.meeting = m; return m.join(); })
        .then(() => testUtils.waitForStateChange(bob.meeting, 'JOINED'))
        .then(() => testUtils.waitForStateChange(chris.meeting, 'JOINED')));
