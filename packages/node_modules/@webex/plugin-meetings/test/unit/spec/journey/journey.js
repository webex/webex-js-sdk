...
describe('plugin-meeting', () => {
  ...
  // Alice calls bob and bob rejects it
  describe.only('Successful 1:1 meeting (including Guest)', () => {
    it(' No previous Call', () => {
    ...
    it('alice dials bob and bob receives meeting added', () => Promise.all([
      ...
      .then(function bobJoinsMeeting() {
        ...
        return Promise.all([
          ...
          .then((response) => {
            ...
            // TODO: check with locus Team for the device array not found
      ...
      .then(function aliceMeetingProperties() {
        ...
        console.log(`Alices meetingId ${alice.meeting.id}, and her correlationId ${alice.meeting.correlationId}, sequence 1`);
        console.log(`Bobs meetingId ${bob.meeting.id}, and her correlationId ${bob.meeting.correlationId}, sequence 1`);
      }));
    ...
    //     assert.exists(updated[alice.memberId]);
    //     assert.equal(updated[alice.memberId].isInMeeting, false);
    //     assert.equal(alice.meeting.state, 'LEFT');

    //     console.log('UPDATED', updated);
    //     console.log('ALICE MEMBER', updated[alice.memberId]);
    ...
    // .then(() => console.log(`ALL MEETING ${bob.spark.meetings.getAllMeetings()}`))
      .then(() => testUtils.waitForCallEnded(bob, alice.emailAddress))
