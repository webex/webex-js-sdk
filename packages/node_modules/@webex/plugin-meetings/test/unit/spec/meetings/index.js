  ...
  describe('meetings index', () => {
    ...
    describe('Public API Contracts', () => {
      ...
      describe('#create', () => {
        ...
        beforeEach(() => {
          spark.meetings.createMeeting = sinon.stub().returns(Promise.resolve({on: () => true}));
        });
      ...
      describe('#handleLocusEvent', () => {
        ...
        describe('there was not a meeting', () => {
          ...
          it('should setup the meeting by difference event', async () => {
            await spark.meetings.handleLocusEvent({locus: {id: uuid1, self: {callBackInfo: {callbackAddress: uri1}}}, eventType: 'locus.difference', locusUrl: url1});
            assert.calledThrice(spark.meetings.meetingCollection.getByKey);
            assert.calledWith(spark.meetings.meetingCollection.getByKey, 'locusUrl', url1);
            assert.calledOnce(initialSetup);
          ...
          it('should setup the meeting by a not difference event', async () => {
            await spark.meetings.handleLocusEvent({locus: {id: uuid1, self: {callBackInfo: {callbackAddress: uri1}}}, eventType: test1, locusUrl: url1});
            assert.calledThrice(spark.meetings.meetingCollection.getByKey);
            assert.calledWith(spark.meetings.meetingCollection.getByKey, 'locusUrl', url1);
            assert.calledOnce(initialSetup);
            assert.calledWith(initialSetup, {id: uuid1, self: {callBackInfo: {callbackAddress: uri1}}});
      ...
      describe('#createMeeting', () => {
        ...
        describe('successful MeetingInfo.#fetchMeetingInfo', () => {
          ...
          it('creates the meeting from a successful meeting info fetch meeting resolve testing', async () => {
            ...
            assert.calledWith(MeetingsUtil.extractDestination, 'test', 'test');
            assert.calledWith(MeetingsUtil.getMeetingAddedType, 'test');
            assert.calledWith(TriggerProxy.trigger, sinon.match.instanceOf(Meetings), 'meeting:added', {meeting: sinon.match.instanceOf(Meeting), type: 'test'});
          });
        });
    ...
    describe('Public Event Triggers', () => {
      describe('#destroy', () => {
        ...
        beforeEach(() => {
          spark.meetings.meetingCollection.delete = sinon.stub().returns(true);
        });
        describe('correctly established meeting', () => {
          it('tests the destroy removal from the collection', () => {
            const response = spark.meetings.destroy(uuid1, test1);

            assert.calledOnce(spark.meetings.meetingCollection.delete);
            assert.calledWith(spark.meetings.meetingCollection.delete, uuid1);
            assert.equal(response, test1);
            assert.calledOnce(TriggerProxy.trigger);
            assert.calledWith(TriggerProxy.trigger, sinon.match.instanceOf(Meetings), 'meeting:removed', {meetingId: uuid1, response: test1});
          });
        });
      });
    });
