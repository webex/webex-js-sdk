...
const postInterceptors = ['HttpStatusInterceptor', 'NetworkTimingInterceptor', 'RequestLoggerInterceptor', 'RateLimitInterceptor'];
...
const WebexCore = AmpState.extend({
  ...
  applyNamedTransform(direction, ctx, name, ...rest) {
    ...
    const transforms = ctx.webex.config.payloadTransformer.transforms.filter((tx) => tx.name === name && (!tx.direction || tx.direction === direction));
  ...
  logout(options, ...rest) {
    ...
    return this.config.onBeforeLogout
      ...
      .reduce(
        (promise, {plugin, fn}) =>
          promise.then(() => (
            Promise.resolve(Reflect.apply(fn, this[plugin] || this.internal[plugin], [options, ...rest]))
            // eslint-disable-next-line max-nested-callbacks
              .catch((err) => {
  ...
  upload(options) {
    function s3UploadSupported() {
      return this.webex.internal && this.webex.internal.feature && this.webex.internal.feature.getFeature('web-s3-uploads');
    }
    if (!options.file) {
    ...
    defaults(
      options.phases.initialize,
      {
        ...
        headers: {},
        body: {
          uploadProtocol: s3UploadSupported ? 'content-length' : null
        }
      },
