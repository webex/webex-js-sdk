...
const Services = WebexPlugin.extend({
  ...
  /**
   * Get a service value from a provided clusterId. Currently, this method
   * will return either a service name, or a service url depending on the `value`
   * parameter. If the `value` parameter is set to `name`, it will return a
   * service name to be utilized within the Services plugin methods.
   * @param {object} params
   * @param {string} params.clusterId - clusterId of found service
   * @param {boolean} [params.priorityHost] - returns priority host url if true
   * @param {string} [params.serviceGroup] - specify service group
   * @param {string} [params.value] - select value of service to return
   * @returns {string} - URL of the Cluster ID provided
   */
  ...
  isServiceUrl(url) {
    ...
    return !!(catalog.findServiceFromUrl(url));
  },
