...
/**
 * Given a JWT acess token, it returns the user's orgId(realm)
 * @param {String} accessToken Access token in JWT (JSON Web Token) format
 * @returns {String} OrgId of the user
 * @throws {Error} if the access token is not in a JWT format
 */
const getOrgIdFromJWT = (accessToken = '') => {
  ...
  const info = JSON.parse(jose.util.base64url.decode(fields[1]));

  return info.realm;
};

/**
 * Given an old format of the access token, return the orgId
 * @param {String} accessToken Access Token in the old format
 * @returns {String} OrgId of the user
 * @throws {Error} if the access token is the old format
 */
const getOrgIdFromOldFormat = (accessToken = '') => {
  const fields = accessToken.split('_');

  if (fields.length !== 3) {
    throw new Error('This is not the old format of an access token');
  }
...
const Services = WebexPlugin.extend({
  ...
  updateServices({from, query, token} = {}) {
    ...
    if (serviceGroup === 'postauth' && !catalog.status.preauth.ready) {
      // Make sure preauth was called before doing a postauth call.
      const {supertoken} = this.webex.credentials;
      let orgId;

      try {
        orgId = getOrgIdFromJWT(supertoken.access_token);
      }
      catch (err) {
        // The token isn't a JWT token. Falling back to old format
        orgId = getOrgIdFromOldFormat(supertoken.access_token);
      }

      return this.updateServices({from: 'limited', query: {orgId}})
        .then(() => this.updateServices({from, query, token: supertoken}));
    }
    ...
    if (serviceGroup === 'preauth' || serviceGroup === 'signin') {
      ...
      if (queryKey === 'email') {
        formattedQuery.emailhash = sha256(query.email).toString();
      }
      else {
