...
const Services = WebexPlugin.extend({
   ...
   * @param {string} [serviceGroup=postauth] - The catalog group to validate against - defaults to postauth.
   * @returns {void}
   */
  hasRequiredServices(services, serviceGroup = 'postauth') {
    const missingServices = services.filter((service) => !this.get(service, undefined, serviceGroup)); // pass undefined since we don't want to assume the priorityHost value

    if (missingServices.length) {
      throw new Error(`${missingServices.join(', ')} services were not found`);
    }
  },
  ...
  updateServices({
    ...
    return this._fetchNewServiceHostmap({
      ...
      .then((serviceHostMap) => {
        ...
        this.hasRequiredServices(this.requiredServices[serviceGroup], serviceGroup);
      })
      .catch((error) => {
        catalog.status[serviceGroup].collecting = false;
  ...
  initConfig() {
    ...
    if (services) {
      ...
      if (services.requiredServices) {
        if (Array.isArray(services.requiredServices)) {
          this.requiredServices.postauth = [...services.requiredServices];
        }
        else {
          for (const [key, value] of Object.entries(services.requiredServices)) {
            this.requiredServices[key] = [...value];
          }
        }
      }
    ...
    return Promise.resolve()
      ...
      .then(() => {
        ...
        if (credentials.canAuthorize) {
          ...
          return this.updateServices()
            .catch((error) => this.logger.warn('services: cannot retrieve postauth catalog', error));
        }
