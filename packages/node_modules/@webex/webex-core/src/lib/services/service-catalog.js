...
const ServiceCatalog = AmpState.extend({
  ...
  findClusterId(url) {
    ...
    Object.keys(this.serviceGroups).forEach((key) => {
      this.serviceGroups[key].forEach((service) => {
  ...
  findServiceFromClusterId({
    ...
    if (identifiedServiceUrl) {
      switch (value) {
        case 'name': return identifiedServiceUrl.name;
        default: return identifiedServiceUrl.get(priorityHost);
      }
    }
