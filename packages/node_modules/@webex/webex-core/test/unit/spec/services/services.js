...
import {Services, ServiceRegistry, ServiceState, ServiceUrl} from '@webex/webex-core';
...
describe('webex-core', () => {
  describe('Services', () => {
    ...
    describe('#hasRequiredServices()', () => {
      describe('preauth catalog', () => {
        before(() => {
          const preAuthTemplate = {
            defaultUrl: 'https://www.preauth-service.com/api/v1',
            hosts: [],
            name: 'preauthValid'
          };

          const preAuthUrls = new ServiceUrl({...preAuthTemplate});

          catalog._loadServiceUrls('preauth', [preAuthUrls]);
        });

        it('should not throw an error if any of the required services are not found in preauth the catalog', () => {
          const servicesList = ['preauthValid'];

          assert.doesNotThrow(
            () => services.hasRequiredServices(servicesList, 'preauth'),
            Error
          );
        });

        it('should throw an error if any of the required services are not found in preauth the catalog', () => {
          const servicesList = ['invalid', 'preauth'];

          assert.throws(
            () => services.hasRequiredServices(servicesList, 'preauth'),
            Error,
            `${servicesList.join(', ')} services were not found`
          );
        });
      });

      describe('postauth catalog', () => {
        before(() => {
          const postAuthTemplate = {
            defaultUrl: 'https://www.postauth-service.com/api/v1',
            hosts: [],
            name: 'postAuthValid'
          };
          const postAuthUrls = new ServiceUrl({...postAuthTemplate});

          catalog._loadServiceUrls('postauth', [postAuthUrls]);
        });

        it('should not throw an error if any of the required services are not found in postauth the catalog', () => {
          const servicesList = ['postAuthValid'];

          assert.doesNotThrow(
            () => webex.internal.services.hasRequiredServices(servicesList),
            Error
          );
        });

        it('should throw an error if any of the required services are not found in postauth the catalog', () => {
          const servicesList = ['invalid', 'postauth'];
