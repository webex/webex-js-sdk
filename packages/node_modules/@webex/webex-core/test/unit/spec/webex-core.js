...
describe('Webex', () => {
  ...
  describe('#initialize()', () => {
    ...
    [
    ...
    ].reduce((acc, path) => acc.concat(['S_T_F', 'Bearer S_T_F'].map((str) => {
      const obj = {
      ...
      .forEach(({msg, data}) => {
        it(msg, () => {
          ...
          assert.equal(webex.credentials.supertoken.access_token, 'S_T_F');
  ...
  describe('initializes with Bearer Token', () => {
    [
      ['initializes with a correctly formatted token', 'Bearer 12_3_4'],
      ['initializes and removes extra space from a token that has an extra space after Bearer', 'Bearer  12_3_4'],
      ['initializes and adds a space after Bearer from a token that has no spaces after Bearer', 'Bearer12_3_4'],
      ['initializes and trims whitespace from a token that has spaces before Bearer and after token', ' Bearer 12_3_4 '],
      ['initializes and removes extra space and trims whitespace from a token that has spaces before and after Bearer and after token', ' Bearer  12_3_4 '],
      ['initializes and trims whitspace and adds a space after Bearer from a token that has spaces before Bearer and after token and no spaces after Bearer', ' Bearer12_3_4 ']
    ].forEach(([msg, token]) => {
      it(msg, () => {
        ...
        assert.isTrue(webex.credentials.canAuthorize);
        assert.equal(webex.credentials.supertoken.access_token, '12_3_4');
        assert.equal(webex.credentials.supertoken.token_type, 'Bearer');
        assert.isTrue(webex.canAuthorize);
      });
    });
