...
describe('webex-core', () => {
  describe('ServiceCatalog', () => {
    ...
    describe('#findServiceFromClusterId()', () => {
      ...
      it('returns a service name when specified', () => {
        assert.equal(
          catalog.findServiceFromClusterId({
            clusterId: testUrlTemplate.hosts[0].id,
            value: 'name'
    ...
    describe('#findServiceFromUrl()', () => {
      ...
      it('finds a service if it exists', () => {
        assert.equal(
          catalog.findServiceFromUrl(testUrlTemplate.defaultUrl),
          testUrl
        );
      });
      ...
      it('finds a service if its a priority host url', () => {
        assert.equal(
          catalog.findServiceFromUrl(testUrl.get(true)).name,
          testUrl.name
        );
      });

      it('returns undefined if the url doesn\'t exist', () => {
        assert.isUndefined(catalog.findServiceFromUrl('https://na.com/'));
      });

      it('returns undefined if the param is not a url', () => {
        assert.isUndefined(catalog.findServiceFromUrl('not a url'));
      });
    });
