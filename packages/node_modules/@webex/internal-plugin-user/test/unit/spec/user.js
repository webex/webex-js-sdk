...
describe('plugin-user', () => {
  describe('User', () => {
    ...
    describe('#validate()', () => {
      it('requires an `email` param', () => assert.isRejected(userService.validate(), /`options.email` is required/));
    });

    describe('#verify()', () => {
      it('calls validate()', () => {
        sinon.spy(userService, 'validate');

        return userService.verify()
          .catch(() => Promise.resolve())
          .then(() => {
            assert.called(userService.validate);
          });
      });
    });
  });
  ...
  describe('User-EU', () => {
    ...
    beforeEach(() => {
      ...
      webex.config.credentials.federation = true;
    });
