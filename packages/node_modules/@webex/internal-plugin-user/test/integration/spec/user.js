  ...
  describe(`plugin-user ${run.it}`, function () {
    ...
    before(() => testUsers.create(testUserParm)
      .then((users) => {
        ...
        user2 = users[1];
        user3 = users[2];
        webex = new WebexCore(merge({}, run.attrs, {
          credentials: {
    ...
    describe('#validate()', () => {
      ...
      it('registers a new user', () => unauthWebex.internal.user.validate({email: `Collabctg+webex-js-sdk-${uuid.v4()}@gmail.com`})
        .then((res) => {
      ...
      it('verifies an existing user', () => unauthWebex.internal.user.validate({email: user1.email})
        .then((res) => {
          assert.property(res, 'hasPassword');
          assert.property(res, 'verificationEmailTriggered');
          assert.property(res, 'sso');
          ...
          assert.equal(res.userEntities.idBrokerUrl, unauthWebex.config.credentials.idbroker.url);
          assert.equal(res.userEntities.identityUrl, unauthWebex.config.credentials.identity.url);
          if (run.EUUser) {
      ...
      it('verify doesn\'t reset CI on failure', () => {
        if (isFederationEnabled()) {
          unauthWebex.config.credentials.idbroker.url = 'https://idbrokerbts-eu.webex.com';
          unauthWebex.config.credentials.identity.url = 'https://identitybts-eu.webex.com';

          return unauthWebex.internal.user.validate({email: 'not an email address'})
            .catch((res) => {
              assert.statusCode(res, 400);
              assert.equal(unauthWebex.config.credentials.idbroker.url, 'https://idbrokerbts-eu.webex.com');
              assert.equal(unauthWebex.config.credentials.identity.url, 'https://identitybts-eu.webex.com');
            });
        }
      ...
      it('leaves email address validation up to Atlas', () => assert.isRejected(unauthWebex.internal.user.validate({email: 'not an email address'}))
        .then((res) => assert.statusCode(res, 400)));
    });

    describe('#setPassword()', () => {
      it('sets the user\'s password', () =>
        webex.internal.user.setPassword({email: (isFederationEnabled() ? user1.email : undefined), password: 'P@ssword123'})
          .then(() => webex.internal.user.validate({email: user1.email}))
          .then((res) => {
            assert.property(res, 'hasPassword');
            assert.property(res, 'verificationEmailTriggered');
            assert.property(res, 'sso');
            ...
            assert.equal(res.userEntities.idBrokerUrl, webex.config.credentials.idbroker.url);
            assert.equal(res.userEntities.identityUrl, webex.config.credentials.identity.url);
            if (run.EUUser) {
              assert.equal(res.userEntities.idBrokerUrl, 'https://idbrokerbts-eu.webex.com');
              assert.equal(res.userEntities.identityUrl, 'https://identitybts-eu.webex.com');
            }
    ...
    describe('#activate()', () => {
      ...
      it('retrieves a valid user token', () => {
        ...
        return unauthWebex.internal.user.validate({email})
          .then((res) => {
            assert.isTrue(res.verificationEmailTriggered);
      ...
      it('retrieves a valid user token and sets the password', () => {
        ...
        return unauthWebex.internal.user.validate({email})
          ...
          .then(() => unauthWebex.internal.user.validate({email}))
          .then((res) => {
            assert.property(res, 'hasPassword');
            assert.property(res, 'verificationEmailTriggered');
            assert.property(res, 'sso');
    ...
    describe('#asUUID()', () => {
      ...
      it('maps an email address to a uuid', () => webex.internal.user.asUUID(user2, {force: true})
        .then((result) => assert.equal(result, user2.id)));

      it('maps an email address for a non-existent user to a fake uuid', () => webex.internal.user.asUUID(email)
      ...
      describe('with {create: true}', () => {
        ...
        it('creates a new user', () => webex.internal.user.asUUID(email, {create: true})
          .then((result) => assert.match(result, patterns.uuid))
          .then(() => webex.internal.user.store.getByEmail(email))
          .then((u) => assert.isTrue(u.userExists, 'User exists')));

        it('does not use a cached value if the previous value was marked as non-existent', () => webex.internal.user.asUUID(email)
          .then((result) => assert.match(result, patterns.uuid))
          .then(() => webex.internal.user.store.getByEmail(email))
          .then((u) => assert.isFalse(u.userExists, 'User does not exist'))
        ...
        it('does not use a cached value if the previous value\'s existence is unknown', () => webex.internal.user.recordUUID({
          id: user3.id,
          ...
          .then(() => webex.internal.user.asUUID(user3.email, {create: true})
    ...
    describe('#update()', () => {
      it('updates a user\'s name', () => webex.internal.user.update({displayName: 'New Display Name'})
        .then((user) => {
          assert.equal(user.id, webex.internal.device.userId);
          assert.property(user, 'entitlements');
          assert.property(user, 'email');
