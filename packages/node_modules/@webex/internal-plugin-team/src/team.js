...
const Team = WebexPlugin.extend({
  ...
  get({url}, options = {}) {
    if (!url) {
      throw Error('`team.url` is required');
    }
...
[
  ...
  Team.prototype[verb] = function submitArchiveStateChangeActivity(target, activity) {
    ...
    const generalConvo = target.conversations.items.find(
      (convo) => convo.id === target.generalConversationUuid
    );

    if (!generalConvo) {
      return Promise.reject(
        new Error('Could not find the general conversation in the team')
      );
    }

    return this.webex.internal.conversation.prepare(activity, properties)
      .then(
        (a) => this.webex.internal.conversation.submit(a, generalConvo.url)
      );
  };
});
