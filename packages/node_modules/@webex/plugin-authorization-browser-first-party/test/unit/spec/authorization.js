  ...
  describe('Authorization', () => {
    ...
    describe('#initialize()', () => {
      describe('when there is a code in the url', () => {
        it('exchanges it for an access token and sets ready', () => {
          const webex = makeSpark('http://example.com/?code=5');
        ...
        it('validates the csrf token', () => {
          ...
          assert.throws(() => {
            ...
            const webex = makeSpark(`http://example.com/?code=5&state=${base64.encode(JSON.stringify({csrf_token: 'someothertoken'}))}`, csrfToken);
          }, /CSRF token someothertoken does not match stored token abcd/);

          assert.throws(() => {
            // eslint-disable-next-line no-unused-vars
            const webex = makeSpark(`http://example.com/?code=5&state=${base64.encode(JSON.stringify({}))}`, csrfToken);
          }, /Expected CSRF token abcd, but not found in redirect query/);

          assert.throws(() => {
            // eslint-disable-next-line no-unused-vars
            const webex = makeSpark('http://example.com/?code=5', csrfToken);
          }, /Expected CSRF token abcd, but not found in redirect query/);

          const webex = makeSpark(`http://example.com/?code=5&state=${base64.encode(JSON.stringify({csrf_token: csrfToken}))}`, csrfToken);
        ...
        it('removes the oauth parameters from the url', () => {
          ...
          const webex = makeSpark(`http://example.com/?code=5&state=${base64.encode(JSON.stringify({csrf_token: csrfToken, something: true}))}`, csrfToken);
      ...
      describe('when there is nothing in the url', () => {
        it('sets ready', () => {
          const webex = makeSpark('http://example.com');
      ...
      it('when the url state contains CI info', () => {
        const webex = makeSpark(`http://example.com/?code=5&state=${base64.encode(JSON.stringify({idbroker: 'newIdBroker', identity: 'newIdentity'}))}`);

        return webex.authorization.when('change:ready')
          .then(() => {
            assert.equal(webex.config.credentials.idbroker.url, 'newIdBroker');
            assert.equal(webex.config.credentials.identity.url, 'newIdentity');
          });
      });
    });
    ...
    describe('#initiateLogin()', () => {
      it('calls #initiateAuthorizationCodeGrant()', () => {
        const webex = makeSpark(undefined, undefined, {
          credentials: {
      ...
      it('puts CI info in state for #initiateAuthorizationCodeGrant() when non-default CI', () => {
        const webex = makeSpark(`http://example.com/?code=5&state=${base64.encode(JSON.stringify({csrf_token: 'csrf_token'}))}`, 'csrf_token');

        webex.config.credentials.idbroker.url = 'notDefaultIdBroker';
        webex.config.credentials.identity.url = 'notDefaultIdBroker';

        const spy = sinon.spy(webex.authorization, 'initiateAuthorizationCodeGrant');
