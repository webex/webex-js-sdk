    ...
    transforms: [
          ...
          return ctx.spark.internal.ediscovery.getContentSummaryBySpaceId(reportId, activity.targetId)
            .then((spaceSummary) => {
              ...
              if (spaceSummary.participants && spaceSummary.participants.length) {
                ...
                ctx.spark.logger.error(`Known error - detected space with no participant - unable to decrypt activity ${activity.activityId} in space ${activity.targetId}`);
              ...
              if (activity.verb !== 'add' && activity.objectDisplayName && activity.encryptionKeyUrl) {
                promises.push(requestWithRetries(ctx.spark.internal.encryption, ctx.spark.internal.encryption.decryptText,
                  ...
                  .catch((reason) => {
                    ctx.spark.logger.error(`Decrypt message error for activity ${activity.activityId} in space ${activity.targetId}: ${reason}`);
              ...
              for (let i = 0; i < shares.length; i += 1) {
                ...
                if (share.displayName && (!activity.whiteboards || !activity.whiteboards.includes(share)) && share.displayName !== 'NoName') {
                  promises.push(requestWithRetries(ctx.spark.internal.encryption, ctx.spark.internal.encryption.decryptText,
                    ...
                    .catch((reason) => {
                      ctx.spark.logger.error(`Decrypt DisplayName error for activity ${activity.activityId} in space ${activity.targetId} for share type: ${share.mimeType}, size: ${share.fileSize}, and url: ${share.url} due to error: ${reason}`);
                      // add error property to activity - this error will be recorded in the downloader and the activity omitted from the report
                      activity.error = reason;
                ...
                if (share.microsoftSharedLinkInfo) {
                  if (share.microsoftSharedLinkInfo.driveId) {
                    promises.push(requestWithRetries(ctx.spark.internal.encryption, ctx.spark.internal.encryption.decryptText,
                      ...
                      .catch((reason) => {
                        ctx.spark.logger.error(`Decrypt share.microsoftSharedLinkInfo.driveId error for activity ${activity.activityId} in space ${activity.targetId} for share type: ${share.mimeType}, size: ${share.fileSize}, and url: ${share.url} due to error: ${reason}`);
                        // add error property to activity - this error will be recorded in the downloader and the activity omitted from the report
                        activity.error = reason;
                  ...
                  if (share.microsoftSharedLinkInfo.itemId) {
                    promises.push(requestWithRetries(ctx.spark.internal.encryption, ctx.spark.internal.encryption.decryptText,
                      ...
                      .catch((reason) => {
                        ctx.spark.logger.error(`Decrypt share.microsoftSharedLinkInfo.itemId error for activity ${activity.activityId} in space ${activity.targetId} for share type: ${share.mimeType}, size: ${share.fileSize}, and url: ${share.url} due to error: ${reason}`);
                        // add error property to activity - this error will be recorded in the downloader and the activity omitted from the report
                        activity.error = reason;
                ...
                if (share.scr || share.sslr) {
                  promises.push(requestWithRetries(ctx.spark.internal.encryption, ctx.spark.internal.encryption.decryptScr,
                    ...
                    .catch((reason) => {
                      ctx.spark.logger.error(`Decrypt file scr or sslr error for activity ${activity.activityId} in space ${activity.targetId} for share type: ${share.mimeType}, size: ${share.fileSize}, and url: ${share.url} due to error: ${reason}`);
                      // add error property to activity - this error will be recorded in the downloader and the activity omitted from the report
                      activity.error = reason;
            ...
            .catch((reason) => {
              ctx.spark.logger.error(`Error retrieving space summary for: ${activity.activityId} in space ${activity.targetId}: ${reason}`);
              // add error property to activity - this error will be recorded in the downloader and the activity omitted from the report
              activity.error = reason;
      ...
      },
      {
        ...
        fn(ctx, object) {
          ...
          return requestWithRetries(ctx.spark.internal.encryption, ctx.spark.internal.encryption.decryptText,
