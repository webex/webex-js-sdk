    ...
    transforms: [
      ...
      },
      {
        ...
        fn(ctx, object) {
          ...
          return ctx.webex.internal.ediscovery.convertEmailsToUserIds(object.body.emails)
            .then((userIds) => {
              object.body.userIds = userIds;
            })
            .catch((reason) => {
              ctx.webex.logger.error(`Error while converting emails to uuids: ${reason}`);
          ...
          return ctx.webex.internal.ediscovery.getContentSummaryBySpaceId(reportId, activity.targetId)
            .then((spaceSummary) => {
              ...
              if (spaceSummary.participants && spaceSummary.participants.length) {
                ...
                ctx.webex.logger.error(`Known error - detected space with no participant - unable to decrypt activity ${activity.activityId} in space ${activity.targetId}`);
      ...
      },
      {
        ...
        fn(ctx, object) {
          ...
          if (!spaceSummary.participants || !spaceSummary.participants.length) {
            ctx.webex.logger.error(`Known error - detected space with no participant - unable to decrypt name for space ${spaceSummary.spaceId}`);
