          ...
          return ctx.spark.internal.ediscovery.getContentSummaryBySpaceId(reportId, activity.targetId)
            .then((spaceSummary) => {
              ...
              for (let i = 0; i < shares.length; i += 1) {
                ...
                if (share.displayName && share.displayName !== 'NoName') { // files/whiteboards are given this displayname by the ED service in error situations, if unchecked it causes a parsing error
                  promises.push(
                ...
                if (share.scr || share.sslr) {
                  promises.push(
                    ctx.spark.internal.encryption.decryptScr(activity.encryptionKeyUrl, share.scr || share.sslr, {onBehalfOf: spaceParticipantId})
                      .then((decryptedReference) => {
                        if (share.scr) {
                          share.scr = decryptedReference;
                        }
                        else {
                          share.sslr = decryptedReference;
                        }
                      })
                      .catch((reason) => {
