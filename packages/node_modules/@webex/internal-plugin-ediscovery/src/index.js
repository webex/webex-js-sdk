    ...
    transforms: [
      {
        ...
        fn(ctx, object) {
          ...
          const reportRequest = object.body.reportRequest;
            ...
            .then((keys) => {
              if (keys && keys.length > 0 && keys[0]) {
                ...
                return ctx.spark.internal.encryption.kms.createResource({userIds: [keys[0].userId], keys})
                  .then(() => {
                    const spaceNamePromises = Promise.all(reportRequest.spaceNames.map((spaceName) => ctx.spark.internal.encryption.encryptText(keys[0], spaceName)))
                      .then((encryptedSpaceNames) => {
                        reportRequest.spaceNames = encryptedSpaceNames;
                      });

                    const keywordPromises = Promise.all(reportRequest.keywords.map((keyword) => ctx.spark.internal.encryption.encryptText(keys[0], keyword)))
                      .then((encryptedKeywords) => {
                        reportRequest.keywords = encryptedKeywords;
                      });

                    const emailPromises = Promise.all(reportRequest.emails.map((email) => ctx.spark.internal.encryption.encryptText(keys[0], email)))
                      .then((encryptedEmails) => {
                        reportRequest.emails = encryptedEmails;
                      });

                    return Promise.all([
                      ctx.transform('encryptTextProp', 'name', keys[0], reportRequest),
                      ctx.transform('encryptTextProp', 'description', keys[0], reportRequest)
                    ].concat(spaceNamePromises, keywordPromises, emailPromises));
                  });
              }
        ...
        fn(ctx, object, reportId) {
          ...
          return ctx.spark.internal.ediscovery.getContentSummaryBySpaceId(reportId, activity.targetId)
            .then((spaceSummary) => {
              ...
              if (activity.verb === 'share' && activity.files && activity.files.length && activity.encryptionKeyUrl) {
                for (let i = 0; i < activity.files.length; i += 1) {
                  if (activity.files[i] && activity.files[i].displayName && activity.files[i].scr) {
                    if (activity.files[i].displayName !== 'NoName') { // shared whiteboards are given this displayname by the ED service, if unchecked it causes a parsing error
                      promises.push(ctx.spark.internal.encryption.decryptText(activity.encryptionKeyUrl, activity.files[i].displayName, {onBehalfOf: spaceParticipantId})
                        .then((decryptedFilename) => {
                          activity.files[i].displayName = decryptedFilename;
                        })
                        .catch((reason) => {
                          ctx.spark.logger.error(`Decrypt filename error for activity ${activity.activityId}: ${reason}`);
                          return object; // TODO - determine correct behaviour when an individual operation fails
                        }));
                    }
                    promises.push(ctx.spark.internal.encryption.decryptScr(activity.encryptionKeyUrl, activity.files[i].scr, {onBehalfOf: spaceParticipantId})
                      .then((decryptedScr) => {
                        activity.files[i].scr = decryptedScr;
                      })
                      .catch((reason) => {
                        ctx.spark.logger.error(`Decrypt file scr error for activity ${activity.activityId}: ${reason}`);
                        return object; // TODO - determine correct behaviour when an individual operation fails
                      }));
                  }
                }
              }
              else if (activity.verb === 'share' && activity.whiteboards && activity.whiteboards.length && activity.encryptionKeyUrl) {
                for (let i = 0; i < activity.whiteboards.length; i += 1) {
                  if (activity.whiteboards[i] && activity.whiteboards[i].scr) {
                    promises.push(ctx.spark.internal.encryption.decryptScr(activity.encryptionKeyUrl, activity.whiteboards[i].scr, {onBehalfOf: spaceParticipantId})
                      .then((decryptedScr) => {
                        activity.whiteboards[i].scr = decryptedScr;
                      })
                      .catch((reason) => {
                        ctx.spark.logger.error(`Decrypt whiteboard scr error for activity ${activity.activityId}: ${reason}`);
                        return object; // TODO - determine correct behaviour when an individual operation fails
                      }));
                  }
                }
