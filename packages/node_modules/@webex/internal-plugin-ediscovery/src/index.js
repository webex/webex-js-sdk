        ...
        fn(ctx, object) {
          ...
          reportRequest.emails.forEach((userEmail) => {
            promises.push(
              ...
              ctx.webex.people.list({email: userEmail})
                .then((res) => {
                  let uuid = '';

                  if (res.items.length <= 0) {
                    ctx.webex.logger.error(`Unable to find user with email ${userEmail}`);
            ...
            .then((keys) => {
              if (keys && keys.length > 0 && keys[0]) {
                ...
                return ctx.webex.internal.encryption.kms.createResource({userIds: [keys[0].userId], keys})
                  .then(() => {
                    const spaceNamePromises = Promise.all(reportRequest.spaceNames.map((spaceName) => ctx.webex.internal.encryption.encryptText(keys[0], spaceName)))
                      .then((encryptedSpaceNames) => {
                        reportRequest.spaceNames = encryptedSpaceNames;
                      });

                    const keywordPromises = Promise.all(reportRequest.keywords.map((keyword) => ctx.webex.internal.encryption.encryptText(keys[0], keyword)))
                      .then((encryptedKeywords) => {
                        reportRequest.keywords = encryptedKeywords;
                      });

                    const emailPromises = Promise.all(reportRequest.emails.map((email) => ctx.webex.internal.encryption.encryptText(keys[0], email)))
          ...
          return ctx.webex.internal.ediscovery.getContentSummaryBySpaceId(reportId, activity.targetId)
            .then((spaceSummary) => {
              ...
              if (spaceSummary.participants && spaceSummary.participants.length) {
                ...
                ctx.webex.logger.error(`Known error - detected space with no participant - unable to decrypt activity ${activity.activityId}`);
              ...
              if (activity.verb !== 'add' && activity.objectDisplayName && activity.encryptionKeyUrl) {
                promises.push(ctx.webex.internal.encryption.decryptText(activity.encryptionKeyUrl, activity.objectDisplayName, {onBehalfOf: spaceParticipantId})
                  ...
                  .catch((reason) => {
                    ctx.webex.logger.error(`Decrypt message error for activity ${activity.activityId}: ${reason}`);
                ...
                if (share.displayName && (!activity.whiteboards || !activity.whiteboards.includes(share)) && share.displayName !== 'NoName') {
                  promises.push(
                    ctx.webex.internal.encryption.decryptText(activity.encryptionKeyUrl, share.displayName, {onBehalfOf: spaceParticipantId})
                      ...
                      .catch((reason) => {
                        ctx.webex.logger.error(`Decrypt DisplayName error for activity ${activity.activityId} and share ${share}: ${reason}`);
                  ...
                  if (share.microsoftSharedLinkInfo.driveId) {
                    promises.push(
                      ctx.webex.internal.encryption.decryptText(activity.encryptionKeyUrl, share.microsoftSharedLinkInfo.driveId, {onBehalfOf: spaceParticipantId})
                        ...
                        .catch((reason) => {
                          ctx.webex.logger.error(`Decrypt share.microsoftSharedLinkInfo.driveId error for activity ${activity.activityId} and share ${share}: ${reason}`);
                  ...
                  if (share.microsoftSharedLinkInfo.itemId) {
                    promises.push(
                      ctx.webex.internal.encryption.decryptText(activity.encryptionKeyUrl, share.microsoftSharedLinkInfo.itemId, {onBehalfOf: spaceParticipantId})
                        ...
                        .catch((reason) => {
                          ctx.webex.logger.error(`Decrypt share.microsoftSharedLinkInfo.itemId error for activity ${activity.activityId} and share ${share}: ${reason}`);
                ...
                if (share.scr || share.sslr) {
                  promises.push(
                    ctx.webex.internal.encryption.decryptScr(activity.encryptionKeyUrl, share.scr || share.sslr, {onBehalfOf: spaceParticipantId})
                      ...
                      .catch((reason) => {
                        ctx.webex.logger.error(`Decrypt file scr or sslr error for activity ${activity.activityId} and share ${share}: ${reason}`);
            ...
            .catch((reason) => {
              ctx.webex.logger.error(`Error retrieving space summary for: ${object.activityId}: ${reason}`);
