  ...
  static decryptReportContent(ctx, object, reportId) {
    ...
    return ctx.webex.internal.ediscovery.getContentContainerByContainerId(reportId, activity.targetId)
      .then((res) => {
        ...
        if (!['post', 'share'].includes(activity.verb) && !activity.meeting && !activity.recording && !activity.extension) {
          return Promise.resolve(object);
        }
        ...
        // Decrypt content url, web url and display name if extension is present
        if(activity.extension){

          //Decrypt content url for extension custom app
          if(activity.extension.objectType =='extension' && activity.extension.extensionType =='customApp'){
            promises.push(requestWithRetries(ctx.webex.internal.encryption, ctx.webex.internal.encryption.decryptText,
          ...
          // Decrypt web url for extension custom app
          if(activity.extension.objectType =='extension' && activity.extension.extensionType =='customApp'){
            promises.push(requestWithRetries(ctx.webex.internal.encryption, ctx.webex.internal.encryption.decryptText,
           ...
           // Decrypt display Name for extension custom app
          if(activity.extension.objectType =='extension' && activity.extension.extensionType =='customApp'){
            promises.push(requestWithRetries(ctx.webex.internal.encryption, ctx.webex.internal.encryption.decryptText,
