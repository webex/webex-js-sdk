...
class Transforms {
  ...
  static decryptReportContent(ctx, object, reportId) {
    ...
    return ctx.webex.internal.ediscovery.getContentSummaryBySpaceId(reportId, activity.targetId)
      .then((space) => {
        ...
        // add error and warning properties to the activity - these will be recorded in the downloader
        if (space.error) {
          activity.error = space.error;
        }

        if (space.warn) {
          activity.warn = space.warn;
        }
        ...
        if (!activity.encryptionKeyUrl) {
          // The encryptionKeyUrl may not be necessary if the activity content is unencrypted, so we treat it as a warning, rather than an error
          const reason = `Activity ${activity.activityId} cannot be decrypted due to a missing encryption key url`;

          ctx.webex.logger.warn(reason);
          activity.warn = reason;
  ...
  static decryptReportContentSummary(ctx, object) {
    ...
    if (!space.encryptionKeyUrl) {
      // The encryptionKeyUrl may not be necessary if the space name is unencrypted, so we treat it as a warning, rather than an error
      const reason = `Space ${space.spaceId} cannot be decrypted due to a missing encryption key url`;
    ...
    return requestWithRetries(ctx.webex.internal.encryption, ctx.webex.internal.encryption.decryptText,
      ...
      .catch((reason) => {
        ...
        // this can fail if we attempt to decrypt an unencrypted spacename, so we report a warning rather than an error
