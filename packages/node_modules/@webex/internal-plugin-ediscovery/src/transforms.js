...
class Transforms {
  ...
  static decryptReportContent(ctx, object, reportId) {
    ...
    return ctx.webex.internal.ediscovery.getContentSummaryBySpaceId(reportId, activity.targetId)
      .then((spaceSummary) => {
        ...
        else if (spaceSummary.isOneOnOne && spaceSummary.participantDisplayNames && spaceSummary.participantDisplayNames.length === 2) {
  ...
  static decryptReportContentSummary(ctx, object) {
    ...
    if (!space.participants || !space.participants.length) {
      const reason = `Known error - detected space with no participants - unable to decrypt name for space ${space.spaceId}`;
    ...
    return requestWithRetries(ctx.webex.internal.encryption, ctx.webex.internal.encryption.decryptText,
      [space.encryptionKeyUrl, space.spaceName, {onBehalfOf: space.participants[0]}])
