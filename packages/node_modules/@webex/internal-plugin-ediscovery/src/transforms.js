...
class Transforms {
   ...
   * @param {Object} ctx - An object containg a webex instance and a transform
   * @param {Object} object - Generic object that you want to decrypt some property on based on the type
   * @param {String} reportId - Id of the report for which content is being retrieved. Needed for getContentSummaryBySpaceId
   * @returns {Promise} - Returns a transform promise
   */
  static decryptReportContent(ctx, object, reportId) {
    ...
    return ctx.webex.internal.ediscovery.getContentSummaryBySpaceId(reportId, activity.targetId)
      .then((res) => {
        ...
        if (!space) {
          const reason = `Space ${activity.targetId} not found - unable to decrypt activity ${activity.activityId}`;
        ...
        // add space warning properties to the activity - these will be recorded in the downloader
        if (space.warning) {
          activity.spaceWarning = space.warning;
        }
        ...
        if (!activity.encryptionKeyUrl) {
          // If the encryptionKeyUrl is empty we assume the space name is unencrypted
        ...
        if (activity.objectDisplayName) {
          promises.push(requestWithRetries(ctx.webex.internal.encryption, ctx.webex.internal.encryption.decryptText,
            [activity.encryptionKeyUrl, activity.objectDisplayName, {onBehalfOf: space.onBehalfOfUser}])
        ...
        for (let i = 0; i < shares.length; i += 1) {
          ...
          if (share.displayName && (!activity.whiteboards || !activity.whiteboards.includes(share))) {
            promises.push(requestWithRetries(ctx.webex.internal.encryption, ctx.webex.internal.encryption.decryptText,
              [activity.encryptionKeyUrl, share.displayName, {onBehalfOf: space.onBehalfOfUser}])
          ...
          if (share.microsoftSharedLinkInfo) {
            if (share.microsoftSharedLinkInfo.driveId) {
              promises.push(requestWithRetries(ctx.webex.internal.encryption, ctx.webex.internal.encryption.decryptText,
                [activity.encryptionKeyUrl, share.microsoftSharedLinkInfo.driveId, {onBehalfOf: space.onBehalfOfUser}])
            ...
            if (share.microsoftSharedLinkInfo.itemId) {
              promises.push(requestWithRetries(ctx.webex.internal.encryption, ctx.webex.internal.encryption.decryptText,
                [activity.encryptionKeyUrl, share.microsoftSharedLinkInfo.itemId, {onBehalfOf: space.onBehalfOfUser}])
          ...
          if (share.scr || share.sslr) {
            promises.push(requestWithRetries(ctx.webex.internal.encryption, ctx.webex.internal.encryption.decryptScr,
              [activity.encryptionKeyUrl, share.scr || share.sslr, {onBehalfOf: space.onBehalfOfUser}])
              .then((decryptedSCR) => {
                if (share.scr) {
  ...
  /**
   * This function is used to decrypt encrypted properties on the spaces that are returned from the eDiscovery Service getContentSummary API
   * @param {Object} ctx - An object containg a webex instance and a transform
   * @param {Object} object - Generic object that you want to decrypt some property on based on the type
   * @returns {Promise} - Returns a transform promise
   */
  static decryptReportContentSummary(ctx, object) {
    ...
    if (!space.encryptionKeyUrl) {
      // If the encryptionKeyUrl is empty we assume the space name is unencrypted
      ctx.webex.logger.info(`Space ${space.spaceId} cannot be decrypted due to a missing encryption key url`);
    ...
    return requestWithRetries(ctx.webex.internal.encryption, ctx.webex.internal.encryption.decryptText,
      [space.encryptionKeyUrl, space.spaceName, {onBehalfOf: space.onBehalfOfUser}])
      .then((decryptedSpaceName) => {
        space.spaceName = decryptedSpaceName;
      })
      .catch((reason) => {
        ctx.webex.logger.warn(`Decrypt space name error for space ${space.spaceId}: ${reason}`);
        // add warn property to space info - this warning will be recorded in the downloader
        space.warning = reason;
