...
const EDiscovery = WebexPlugin.extend({
   ...
   * @param {number} options.size - the number of records to retrieve
   * @param {number} options.timeoutMs - connection timeout in milliseconds, defaults to 30s
   * @param {String} options.url - full address to which the http request is being sent
  ...
  getContent(reportId, options) {
    ...
    // If a url option is set use that for the request, otherwise use the default ediscovery service
    if (options && options.url) {
      requestParams.url = options.url;
    }
    else {
      requestParams.service = 'ediscovery';
      requestParams.resource = `reports/${reportId}/contents`;
    }
  ...
  getContentSummary(reportId, options) {
    ...
    if (options && options.url) {
      ...
      requestParams.service = 'ediscovery';
      requestParams.resource = `reports/${reportId}/contents/summary`;
    }
  ...
  getContentSummaryBySpaceId(reportId, spaceId, options) {
    ...
    if (options && options.url) {
      ...
      requestParams.service = 'ediscovery';
      requestParams.resource = `reports/${reportId}/contents/summary/${spaceId}`;
    }
