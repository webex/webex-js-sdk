...
const EDiscovery = SparkPlugin.extend({
  ...
  getContentSummaryBySpaceId(reportId, spaceId) {
    ...
    if (this.contentSummaryCache.has(reportId) && this.contentSummaryCache.get(reportId).has(spaceId)) {
      return {body: this.contentSummaryCache.get(reportId).get(spaceId)};
    }
      ...
      return plugin.request({
        ...
        .then((res) => {
          ...
          while (activities.length) {
            ...
            const spaceInfoPromise = plugin.getContentSummaryBySpaceId(reportId, activity.targetId)
              .then((res) => {
                activity.spaceName = res.body.spaceName || res.body.participantDisplayNames.toString().replace(',', ' & ');
                const filteredParticipantDisplayNames = res.body.participantDisplayNames.filter((value) => value !== activity.actorDisplayName);
