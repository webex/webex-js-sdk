...
const EDiscovery = WebexPlugin.extend({
   ...
   * @param {int} timeoutMs - optional connection timeout in milliseconds, defaults to 10s
   * @returns {Promise<ResponseEntity>} Http response containing the new report record
   */
  createReport(reportRequest, timeoutMs = 10000) {
    if (!reportRequest) {
   ...
   */
  getReport(reportId, timeoutMs = 10000) {
    if (!reportId) {
      throw Error('Undefined parameter');
    }
   ...
   * @param {Object} {offset, size} - Start position from which to retrieve records and number of records to retrieve
   * @param {int} timeoutMs - optional connection timeout in milliseconds, defaults to 10s
   * @returns {Promise<ResponseEntity<Array<ReportRecord>>>} Http Response containing a list of report records
   */
  getReports({offset, size}, timeoutMs = 10000) {
    return this.request({
      method: 'GET',
      service: 'ediscovery',
      resource: 'reports',
   ...
   */
  deleteReport(reportId, timeoutMs = 10000) {
    if (!reportId) {
      throw Error('Undefined parameter');
    }
   ...
   */
  restartReport(reportId, timeoutMs = 10000) {
    if (!reportId) {
      throw Error('Undefined parameter');
    }
   ...
   * @param {Object} {offset, size} - Start position from which to retrieve content and number of activities to retrieve
   * @param {int} timeoutMs - optional connection timeout in milliseconds, defaults to 10s
   * @returns {Promise<ResponseEntity<[Activity]>>} Http response containing the activities
   */
  getContent(reportId, {offset, size}, timeoutMs = 10000) {
    if (!reportId) {
      throw Error('Undefined parameter');
    }
    const promises = [];
   ...
   */
  getContentSummary(reportId, timeoutMs = 10000) {
    if (!reportId) {
      throw Error('Undefined parameter');
    }
   ...
   */
  getContentSummaryBySpaceId(reportId, spaceId, timeoutMs = 10000) {
    if (!reportId || !spaceId) {
