...
const EDiscovery = SparkPlugin.extend({
   ...
   * @returns {UUID} spaceId = User ids for the email addresses being requested
   */
  convertEmailsToUserIds(emails) {
    ...
    return this.spark.people.get('me')
      .then((res) => {
        ...
        const decodedId = Buffer.from(res.orgId, 'base64').toString();

        orgId = decodedId.substring(decodedId.lastIndexOf('/') + 1, decodedId.length);
      })
      .then(() => {
        if (!orgId) {
        ...
        while (emailIndex < emails.length) {
          ...
          for (let i = emailIndex; i < emailIndex + maxFilterEmails && i < emails.length; i += 1) {
            filter += `userName eq "${emails[i]}" or `;
          }
          // remove trailing ' or '
          filter = filter.slice(0, -4);

          if (!filter) {
            throw Error('No filter created for CI User query');
          }
          ...
          const promise = this.request({
            ...
            .then((res) => {
              res.body.Resources.forEach((resource) => {
                userIds.push(resource.id);
                // compile a list of usernames that were successfully converted to uuids
                // we will cross check this with the original list of emails to ensure no errors occured
                userNames.push(resource.userName);
              });
            });

          promises.push(promise);
          emailIndex += maxFilterEmails;
        }
