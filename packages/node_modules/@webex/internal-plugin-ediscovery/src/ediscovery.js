...
const defaultOptions = {timeoutMs: 30000}; // 30s default timeout
...
const EDiscovery = WebexPlugin.extend({
   ...
   * @param {number} options.timeoutMs - connection timeout in milliseconds, defaults to 10s
   ...
   * @param {UUID} reportId - Id of the report being requested
   * @param {Object} options - optional parameters for this method
   * @param {number} options.timeoutMs - connection timeout in milliseconds, defaults to 90s
  ...
  getContentSummary(reportId, options) {
    ...
    const contentSummaryDefaultOptions = {timeoutMs: 90000};

    // use spread operator to set default options
    options = {...contentSummaryDefaultOptions, ...options};
    ...
    function getNextContentSummaryPage(plugin) {
      return plugin.request({
        ...
        .then((res) => {
          if (!plugin.contentSummaryBySpaceIdCache.has(reportId)) {
            plugin.contentSummaryBySpaceIdCache.set(reportId, new Map());
          }

          for (let i = 0; i < res.body.length; i += 1) {
            if (res.body[i] && res.body[i].spaceId) {
              try {
                // The results are written to the contentSummaryBySpaceIdCache since information for
                // a space is likely to be accessed very frequently when decrypting activities
                plugin.contentSummaryBySpaceIdCache.get(reportId).set(res.body[i].spaceId, res.body[i]);
              }
              catch (error) {
                plugin.logger.error(`Error adding ${res.body[i].spaceId} to contentSummaryBySpaceIdCache: ${error}`);
              }
            }
            else {
              plugin.logger.error('Error adding undefined space to contentSummaryBySpaceIdCache');
            }
          }
  ...
  @waitForValue('@')
  @oneFlight({keyFactory: (reportId, spaceId) => `${reportId}-${spaceId}`})
  /**
  ...
  getContentSummaryBySpaceId(reportId, spaceId, options) {
    ...
    if (this.contentSummaryBySpaceIdCache.has(reportId) && this.contentSummaryBySpaceIdCache.get(reportId).has(spaceId)) {
      return this.contentSummaryBySpaceIdCache.get(reportId).get(spaceId);
    }
    ...
    return this.request({
      ...
      .then((res) => {
        ...
        if (res.body && res.body.spaceId) {
          try {
            this.contentSummaryBySpaceIdCache.get(reportId).set(res.body.spaceId, res.body);
          }
          catch (error) {
            this.logger.error(`Error adding ${res.body.spaceId} to contentSummaryBySpaceIdCache: ${error}`);
          }
        }
        else {
