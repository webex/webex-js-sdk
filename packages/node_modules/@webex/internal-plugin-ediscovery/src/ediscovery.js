...
const defaultOptions = {offset: 0, size: 100, timeoutMs: 30000}; // 30s default timeout
...
const EDiscovery = WebexPlugin.extend({
  ...
  @waitForValue('@')
  @oneFlight({keyFactory: (reportId, options) => String(reportId + (options && options.offset && options.size ? options.offset + options.size : ''))})
  /**
  ...
  getContentSummary(reportId, options) {
    ...
    return this.request({
      ...
      .then((res) => {
        const cachePromise = this._writeSpacesToContentSummaryBySpaceIdCache(reportId, res.body);
