...
const EDiscovery = WebexPlugin.extend({
   ...
   * @param {Object} page - object with offset and size parameters indicating the start position from which to retrieve
   *                        content and the number of activities to retrieve
   * @param {int} timeoutMs - optional connection timeout in milliseconds, defaults to 10s
   * @returns {Promise<ResponseEntity<Array<ReportRecord>>>} Http Response containing a list of report records
   */
  getReports(page, timeoutMs = 10000) {
    return this.request({
      method: 'GET',
      service: 'ediscovery',
      resource: 'reports',
      qs: {offset: page.offset, size: page.size},
      timeout: timeoutMs
    });
  },
   ...
   */
  getContent(reportId, page, timeoutMs = 10000) {
    if (!reportId) {
      throw Error('Undefined parameter');
    }
    const promises = [];
