...
describe('EDiscovery Content API Tests', () => {
  ...
  describe('GetContentSummary Tests', () => {
    ...
    it('GetContentSummary makes one request when called multiple times with identical params', async () => {
      webex.internal.ediscovery.getContentSummary(uuid, {offset: 0, size: 1, timeoutMs: 5000});
      webex.internal.ediscovery.getContentSummary(uuid, {offset: 0, size: 1, timeoutMs: 5000});
      const result = webex.internal.ediscovery.getContentSummary(uuid, {offset: 0, size: 1, timeoutMs: 5000})
        .then(() => {
          sinon.assert.calledOnce(webex.request);
    ...
    it('GetContentSummary makes multiple requests when called multiple times with different params', async () => {
      webex.internal.ediscovery.getContentSummary(uuid, {offset: 0, size: 1, timeoutMs: 5000});
      webex.internal.ediscovery.getContentSummary(uuid, {offset: 1, size: 1, timeoutMs: 5000});
      const result = webex.internal.ediscovery.getContentSummary(uuid, {offset: 0, size: 2, timeoutMs: 5000})
        .then(() => {
          sinon.assert.calledThrice(webex.request);
        });
    ...
    it('GetContentSummary does not retrieve space from contentSummaryBySpaceIdCache if available', async () => {
  ...
  describe('GetContentSummary by url Tests', () => {
    ...
    it('GetContentSummary by url fails with no params', async () => {
      const result = expect(webex.internal.ediscovery.getContentSummary()).to.be.rejectedWith(Error, 'Undefined parameter');

      return result;
    });

    it('GetContentSummary by url fails with a url that does not contain a report id', async () => {
    ...
    it('GetContentSummary by url timeout defaults to 30s', async () => {
    ...
    it('GetContentSummary by url timeout defaults to 30s when other options are specified', async () => {
      const result = webex.internal.ediscovery.getContentSummary(url, {offset: 0, size: 1})
        .then(() => {
          sinon.assert.calledWith(webex.request, sinon.match.has('timeout', defaultTimeout));
        });
    ...
    it('GetContentSummary by url timeout can be overwritten to 5s with timeout as the only option', async () => {
    ...
    it('GetContentSummary by url timeout can be overwritten to 5s when other options are specified', async () => {
    ...
    it('GetContentSummary by url makes one request when called multiple times with identical params', async () => {
      webex.internal.ediscovery.getContentSummary(url, {offset: 0, size: 1, timeoutMs: 5000});
      webex.internal.ediscovery.getContentSummary(url, {offset: 0, size: 1, timeoutMs: 5000});
      const result = webex.internal.ediscovery.getContentSummary(url, {offset: 0, size: 1, timeoutMs: 5000})
        .then(() => {
          sinon.assert.calledOnce(webex.request);
    ...
    it('GetContentSummary by url makes multiple requests when called multiple times with different params', async () => {
      webex.internal.ediscovery.getContentSummary(url, {offset: 0, size: 1, timeoutMs: 5000});
      webex.internal.ediscovery.getContentSummary(url, {offset: 1, size: 1, timeoutMs: 5000});
      const result = webex.internal.ediscovery.getContentSummary(url, {offset: 0, size: 2, timeoutMs: 5000})
        .then(() => {
          sinon.assert.calledThrice(webex.request);
        });
    ...
    it('GetContentSummary by url sets report in contentSummaryBySpaceIdCache if it does not exist', async () => {
      const mockResponse = {body: [{spaceId: uuid}]};
    ...
    it('GetContentSummary by url does not retrieve space from contentSummaryBySpaceIdCache if available', async () => {
      const mockCachedSpace = {spaceId: uuid};
      const spaceIdMap = {
        get: sinon.stub().withArgs(uuid).returns(mockCachedSpace),
        set: sinon.stub(),
    ...
    it('GetContentSummary by url populates contentSummaryBySpaceIdCache with space when retrieved', async () => {
      const mockResponse = {body: [{spaceId: uuid}]};
  ...
  describe('GetContentSummaryBySpaceId Tests', () => {
    ...
    it('GetContentSummaryBySpaceId makes one request when called multiple times with identical params', async () => {
      webex.internal.ediscovery.getContentSummaryBySpaceId(uuid, uuid, {timeoutMs: 5000});
      webex.internal.ediscovery.getContentSummaryBySpaceId(uuid, uuid, {timeoutMs: 5000});
      const result = webex.internal.ediscovery.getContentSummaryBySpaceId(uuid, uuid, {timeoutMs: 5000})
        .then(() => {
          sinon.assert.calledOnce(webex.request);
    ...
    it('GetContentSummaryBySpaceId makes multiple requests when called multiple times with different params', async () => {
      webex.internal.ediscovery.getContentSummaryBySpaceId(uuid, '0d4084ce-8cef-43fd-9313-fb64579996ba', {timeoutMs: 5000});
      webex.internal.ediscovery.getContentSummaryBySpaceId(uuid, '8704cc4d-ed0a-4327-a0f6-25334f9c6a30', {timeoutMs: 5000});
      const result = webex.internal.ediscovery.getContentSummaryBySpaceId(uuid, uuid, {timeoutMs: 5000})
        .then(() => {
          sinon.assert.calledThrice(webex.request);
        });
  ...
  describe('GetContentSummaryBySpaceId by url Tests', () => {
    ...
    it('GetContentSummaryBySpaceId by url makes one request when called multiple times with identical params', async () => {
      webex.internal.ediscovery.getContentSummaryBySpaceId(url, uuid, {timeoutMs: 5000});
      webex.internal.ediscovery.getContentSummaryBySpaceId(url, uuid, {timeoutMs: 5000});
      const result = webex.internal.ediscovery.getContentSummaryBySpaceId(url, uuid, {timeoutMs: 5000})
        .then(() => {
          sinon.assert.calledOnce(webex.request);
    ...
    it('GetContentSummaryBySpaceId by url makes multiple requests when called multiple times with different params', async () => {
      webex.internal.ediscovery.getContentSummaryBySpaceId(url, '0d4084ce-8cef-43fd-9313-fb64579996ba', {timeoutMs: 5000});
      webex.internal.ediscovery.getContentSummaryBySpaceId(url, '8704cc4d-ed0a-4327-a0f6-25334f9c6a30', {timeoutMs: 5000});
      const result = webex.internal.ediscovery.getContentSummaryBySpaceId(url, uuid, {timeoutMs: 5000})
        .then(() => {
          sinon.assert.calledThrice(webex.request);
        });
