...
describe('EDiscovery Content API Tests', () => {
  ...
  describe('GetContentSummary Tests', () => {
    ...
    it('GetContentSummary submits an audit log request', () => {
      const result = webex.internal.ediscovery.postAuditLog(uuid, {timeoutMs: 1000})
        .then((res) => {
          expect(res.statusCode).equal(200);
    ...
    it('GetContentSummary submits a malformed audit request and gets rejected', () => {
      const noUuid = '';
      const result = expect(webex.internal.ediscovery.postAuditLog(noUuid, {timeoutMs: 1000})).to.be.rejectedWith(Error, 'Undefined parameter');

      return result;
    });

    it('GetContentSummary skips audit log on http error', () => {
