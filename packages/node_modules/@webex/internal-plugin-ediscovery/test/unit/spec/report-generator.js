...
describe('report-generator', () => {
  ...
  describe('Mapping', () => {
    it('mapPosts', async () => {
      ...
      for (let i = 0; i < postActivities.length; i += 1) {
        ...
        expect(eml).to.include(`Subject: ${activity.spaceName}`);
        expect(eml).to.include(`Date: ${new Date(activity.timestamp).toUTCString()}`);
    ...
    it('mapEvents', async () => {
      ...
      for (let i = 0; i < eventActivities.length; i += 1) {
        ...
        if (activity.objectDisplayName) {
          expect(eml).to.include(`${activity.verb} ${activity.objectDisplayName}`);
        }
        else {
    ...
    it('mapShares', async () => {
      ...
      for (let i = 0; i < shareActivities.length; i += 1) {
        ...
        const eml = generator.map(activity);
        if (activity.files && activity.actorDisplayName) {
  ...
  describe('Add Activities to Report Tests', () => {
    skipInNode(it)('addActivities', async () => {
      for (let i = 0; i < activities.length; i += 1) {
        ...
        assert.equal(emlFiles.length, i + 1);
  ...
  describe('Generate Tests', () => {
    skipInNode(it)('generate', async () => {
      generator.add(activities);
      generator.generate(reportId)
        .then(() => assert.isOk(true));
    });

    it('generateWithNoAdd', async () => {
      expect(() => generator.generate(reportId)).to.not.throw(Error);
    });
  });
});
