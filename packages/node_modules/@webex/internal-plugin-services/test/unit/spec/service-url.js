...
describe('plugin-services', () => {
  describe('ServiceUrl', () => {
    ...
    describe('#removeHost()', () => {
      ...
      it('removes a host', () => {
        serviceUrl.removeHost(hostUrl);

        serviceUrl.hosts.forEach((hostObj) => {
          assert.notEqual(hostObj.host, host);
        });
      });

      it('does not remove a host if the hostUrl is defaultUrl', () => {
        // Remove here as countermeasure to beforeEach
        serviceUrl.removeHost(hostUrl);

        const hostLength = serviceUrl.hosts.length;
        const foundHost = serviceUrl.removeHost(serviceUrl.defaultUrl);
      ...
      it('returns true if hostUrl was found', () => {
        const removedHostResult = serviceUrl.removeHost(hostUrl);

        assert.isTrue(removedHostResult);
      });

      it('returns false if hostUrl was not found', () => {
        const removedHostResult = serviceUrl.removeHost(
