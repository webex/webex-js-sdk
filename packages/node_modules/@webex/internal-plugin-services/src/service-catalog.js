...
const ServiceCatalog = AmpState.extend({
  ...
  _getUrl(name, serviceGroup) {
    const serviceUrls = (typeof serviceGroup === 'string') ?
      this._serviceGroups[serviceGroup] || [] :
      [
        ...this._serviceGroups.discovery,
        ...this._serviceGroups.preauth,
        ...this._serviceGroups.postauth
      ];
  ...
  _loadServiceUrls(serviceGroup, services) {
    ...
    services.forEach((service) => {
      ...
      if (!existingService) {
        this._serviceGroups[serviceGroup].push(service);
      }
    });
  ...
  _unloadServiceUrls(serviceGroup, services) {
    ...
    services.forEach((service) => {
      ...
      if (existingService) {
        this._serviceGroups[serviceGroup].splice(
          this._serviceGroups[serviceGroup].indexOf(existingService), 1
        );
      }
    });
  ...
  /**
   * Generates a status object relaying if the catalogs
   * have been updated and contain values
   * @returns {Record<string, string>}
   */
  status() {
    return {
      preauthorized: this._serviceGroups.preauth.length !== 0,
      postauthorized: this._serviceGroups.postauth.length !== 0
    };
  },
  ...
  updateServiceUrls(serviceGroup, serviceHostmap) {
    const currentServiceUrls = this._serviceGroups[serviceGroup];
    ...
    this.trigger(`${serviceGroup}orized`);
  ...
  waitForCatalog(serviceGroup) {
    return new Promise((resolve, reject) => {
      if (this.status()[`${serviceGroup}orized`]) {
        resolve();
      }

      this.once(`${serviceGroup}orized`, () => resolve());
