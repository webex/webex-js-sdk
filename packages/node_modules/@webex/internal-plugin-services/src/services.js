...
const Services = WebexPlugin.extend({
  ...
  services() {
    ...
    this._serviceUrls.forEach((serviceUrl) => {
      output[serviceUrl.name] = serviceUrl.get();
    });
  ...
  updateServices() {
    return this._fetchNewServiceHostmap()
      .then((serviceHostMap) => {
        this._updateServiceUrls(serviceHostMap);
   ...
   * @returns {Services}
   */
  _updateServiceUrls(serviceHostMap) {
    ...
    this._serviceUrls.forEach((serviceUrl) => {
      ...
      if (!service) { this._unloadServiceUrls(serviceUrl); }
    });

    serviceHostMap.forEach((serviceObj) => {
      const service = this._getUrl(serviceObj.name);
