...
export default class ServiceInterceptor extends Interceptor {
  ...
  onRequest(options) {
    ...
    if (!options.tags.serviceInterceptor) {
      ...
      // Check if the `services` plugin is currently ready.
      // If not, wait until it is and continue.
      services.waitUntilReady()
        .then(() => {
          this.validateOptions(options);

          service = services.get(options.service);
  ...
  generateUrl(service, resource) {
    return `${service.replace(pattern, '')}/${resource.replace(pattern, '')}`;
  }
