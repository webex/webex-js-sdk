...
const Conversation = WebexPlugin.extend({
  ...
  */
  ...
  get(conversation, options = {}) {
    ...
    try {
      const url = !user ? this.getConvoUrl(conversation) : '';
   ...
   *
   * @param {Object} convo convo object with either a url or id
   * @param {String} convo.url The location of the conversation
   * @param {UUID} convo.id If there is no url, fall back to id to lookup in cache
   ...
   */
  submit(activity) {
    const url = this.getConvoUrl(activity.target);
