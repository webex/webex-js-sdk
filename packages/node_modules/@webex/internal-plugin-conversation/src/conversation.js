...
const Conversation = WebexPlugin.extend({
   ...
   */
  getUrlFromCluserId({cluster, id}) {
    const clusterId =
      cluster === 'us' ? 'urn:TEAM:us-east-1_int13:identityLookup' : cluster;
  ...
  */
  bulkActivitiesFetch(activities, cluster) {
    ...
    if (cluster) {
      const uri = `${this.getUrlFromCluserId({cluster})}/${resource}`;
  ...
  getConvoUrl({
    ...
    if (id) {
      if (cluster) {
        return this.getUrlFromCluserId({cluster, id});
      }
      this.logger.warn(
