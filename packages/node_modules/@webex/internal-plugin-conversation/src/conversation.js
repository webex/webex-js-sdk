...
const Conversation = WebexPlugin.extend({
  ...
  */
  ...
  listActivitiesThreadOrdered(options) {
    let threadOrderer;
    const olderOptions = {...options, queryType: OLDER};
    ...
    const jumpToActivity = async (searchObject) => {
      ...
      const searchOptions = {
        ...options, queryType: MID, search: searchObject
      };
    ...
    const getOlder = async () => {
      if (!threadOrderer) {
        threadOrderer = this._listActivitiesThreadOrdered(options);
      }
    ...
    const getNewer = async () => {
      const newerOptions = {...options, queryType: NEWER};
  ...
  async* _listActivitiesThreadOrdered(options = {}) {
    ...
    const convoUrl = this.getConvoUrl({url: conversationUrl, id: conversationId});

    // keep track of generator state, like what our current oldest & newest activities are
    let oldestAct;
    let newestAct;

    const defaultBatchSize = Math.max(minBatchSize, Math.ceil(minActivities / 2));
    ...
    const getQuery = (type, activityToSearch = {}) => {
      switch (type) {
        case NEWER: {
          // TODO: should sinceDate be newestAct.published - 1?
          const sinceDate = getPublishedDate(newestAct);
        ...
        case OLDER: {
          // TODO: should maxDate be oldestAct.published - 1? + 1?
          const maxDate = getPublishedDate(oldestAct);
    ...
    while (true) {
      ...
      const sanitizeFinalActivity = (activity) => {
      ...
      const getRepliesByParentId = (replyParentId) => {
        ...
        sortedReplies.forEach((replyActivity) => {
          ...
          const sanitizedFullReply = sanitizeFinalActivity(fullReply);
      ...
      orderedRoots.forEach((rootActivity) => {
        ...
        const sanitizedFullRoot = sanitizeFinalActivity(fullRoot);
