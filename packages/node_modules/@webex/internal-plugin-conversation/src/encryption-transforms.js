...
export const transforms = toArray('outbound', {
    ...
    return ctx.webex.internal.conversation.get({url: conversationUrl})
      .then((conversation) => {
        ...
        if (!activity.target.defaultActivityEncryptionKeyUrl) {
          ctx.webex.logger.warn('plugin-conversation: downloaded conversation to determine its defaultActivityEncryptionKeyUrl; make sure to pass all encryption related properties when calling Spark.conversation methods.');
        }

        if (!activity.target.kmsResourceObjectUrl) {
          ctx.webex.logger.warn('plugin-conversation: downloaded conversation to determine its kmsResourceObjectUrl; make sure to pass all encryption related properties when calling Spark.conversation methods.');
        }
  ...
  encryptJsonProp(ctx, name, key, object) {
    ...
    return ctx.spark.internal.encryption.encryptText(key.uri || key, JSON.stringify(object[name]))
      .then((ciphertext) => {
