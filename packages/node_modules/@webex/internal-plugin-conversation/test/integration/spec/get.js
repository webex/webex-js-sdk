  ...
  describe('when fetching conversations', () => {
    ...
    describe('#listParentActivityIds', () => {
      ...
      it('retrieves parent IDs for thread parents()', () => webex.internal.conversation.post(conversation, {displayName: 'first thread reply'}, {
        ...
        assert.equal(reply.activityType, 'reply');
        assert.equal(reply.parentActivityId, parent.id);
    ...
    describe('#getReactionSummaryByParentId()', () => {
      ...
      before('create conversation with an activity and reactions', () => webex.internal.conversation.create({participants})
        .then((c) => {
          conversation = c;
          assert.lengthOf(conversation.participants.items, 3);
        ...
        .then((parentAct) => {
          ...
          return webex.internal.conversation.addReaction(conversation, reactionType, parent);
        }));

      const reactionFetch = () => webex.internal.conversation.getReactionSummaryByParentId(conversation.url, parent.id, {activityType: 'reactionSummary', includeChildren: true});
      const testReactionResponse = (reactionSummaries) => {
        assert.equal(reactionSummaries.length, 2);
        reactionSummaries.forEach((summary) => {
          assert.equal(summary.activity.object.reactions[0].displayName, reactionType);
        });
      };
      const handleReactionTest = (doneCb) => {
        reactionFetch().then((summary) => {
          if (summary.length) {
            testReactionResponse(summary);
            doneCb();
          }
          else {
            setTimeout(() => handleReactionTest(doneCb), 5000);
          }
        });
      };

      it('retrieves reactionSummarys and reactionSelfSummarys', (done) => {
        handleReactionTest(done);
      });
    });
  });
});
