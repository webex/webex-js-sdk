...
describe('plugin-conversation', function () {
  ...
  describe('when fetching conversations', () => {
    let kirk, mccoy, participants, scott, webex, spock;
    ...
    describe('#listActivities()', () => {
      ...
      it('retrieves activities for the specified conversation', () => webex.internal.conversation.listActivities({conversationId: conversation.id})
        .then((activities) => {
    ...
    describe('#bulk_activities_fetch()', () => {
      let jenny, maria, dan, convo_1, convo_2;
      let activityURLs = [];
      let webex3;
      ...
      it('add comments to convo_2, and check post requests successfully went through', () => 
        webex3.internal.conversation.post(convo_1, {displayName: 'BAGELS (O)'})
          .then((c1) => { 
            assert.equal(c1.object.displayName, 'BAGELS (O)');
            return webex3.internal.conversation.post(convo_1, {displayName: 'Cream Cheese'});
          })
          .then((c2) => { assert.equal(c2.object.displayName, 'Cream Cheese'); })
      );

      it('add comments to convo_2, and check post requests successfully went through', () => 
        webex3.internal.conversation.post(convo_2, {displayName: 'Want to head to lunch soon?'})
          .then((c1) => { 
            assert.equal(c1.object.displayName, 'Want to head to lunch soon?');
            return webex3.internal.conversation.post(convo_2, {displayName: 'Sure :)'});
          })
          .then((c2) => { 
            assert.equal(c2.object.displayName, 'Sure :)');
            return webex3.internal.conversation.post(convo_2, {displayName: 'where?'});
          })
          .then((c3) => {
            assert.equal(c3.object.displayName, 'where?');
            return webex3.internal.conversation.post(convo_2, {displayName: 'Meekong Bar!'});
          })
          .then((c4) => {assert.equal(c4.object.displayName, 'Meekong Bar!')})
      );
      
      it('retrieves activities from a single conversation', () => 
        webex3.internal.conversation.listActivities({conversationId: convo_1.id})
          .then((convo_activities) => {
            convo_activities.map((a) => {
              if (a.verb === 'post') { 
                activityURLs.push(a.url);
              }
            });
            webex3.internal.conversation.bulk_activities_fetch(activityURLs)
              .then((bulk_fetched_activities) => {
                assert.equal(bulk_fetched_activities, convo_activities);
              });
          })
      );
      ...
      it('retrieves activities from multiple conversations', () => 
        webex3.internal.conversation.listActivities({conversationId: convo_1.id})
          .then((convo_1_activities) => {
            // gets all post activty urls from convo_1
            convo_1_activities.map((a1) => {
              if (a1.verb === 'post') { 
                activityURLs.push(a1.url);
              }
            });
            // gets activity urls of only comment 3 and 4 from convo_2
            webex3.internal.conversation.listActivities({conversationId: convo_2.id})
              .then((convo_2_activities) => {
                [3,4].map((i) => activityURLs.push(convo_2_activities[i.url]))
              });
            webex3.internal.conversation.bulk_activities_fetch(activityURLs)
              .then((bulk_fetched_activities) => {
                let expectedActivities = convo_1_activities;
                expectedActivitie.push(onvo_1_activities[3]);
                expectedActivitie.push(onvo_1_activities[4]);
                assert.equal(bulk_fetched_activities, expectedActivities);
              });
          })
      );

      it('given a activity url that does not exist, should return []', () => {
        let mockURL = 'https://conversation-intb.ciscospark.com/conversation/api/v1/activities/6d8c7c90-a770-11e9-bcfb-6616ead99ac3';
        webex3.internal.conversation.bulk_activities_fetch([mockURL])
          .then((bulk_fetched_activities) => {
            assert.equal(bulk_fetched_activities, []);
          });
        }
      );
    });
