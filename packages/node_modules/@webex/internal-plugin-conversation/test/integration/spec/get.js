  ...
  describe('when fetching conversations', () => {
    ...
    describe('#getReactionSummaryByParentId()', () => {
      ...
      it('retrieves reactionSummarys and reactionSelfSummarys', (done) => {
        const count = 0;
        const maxCount = 10;
        const handleReactionTest = () => {
          if (count > maxCount) {
            done();

            return;
          }
          webex.internal.conversation.getReactionSummaryByParentId(conversation.url, parent.id, {activityType: 'reactionSummary', includeChildren: true}).then((summaries) => {
            if (summaries.length) {
              ...
              setTimeout(() => handleReactionTest(), 5000);
            }
          });
        };
