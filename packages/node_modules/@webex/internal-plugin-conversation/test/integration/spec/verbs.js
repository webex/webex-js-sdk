...
describe('plugin-conversation', function () {
  ...
  describe('verbs', () => {
    ...
    describe('#add()', () => {
      ...
      beforeEach(() => spark.internal.conversation.create({participants: [checkov]}, {forceGrouped: true})
        .then((c) => { conversation = c; }));

      it('adds the specified user to the specified conversation', () => spark.internal.conversation.add(conversation, mccoy)
        .then((activity) => {
          assert.isActivity(activity);
          assert.property(activity, 'kmsMessage');
        }));

      it('grants the specified user access to the conversation\'s key', () => spark.internal.conversation.post(conversation, {displayName: 'PROOF!'})
        .then(() => spark.internal.conversation.add(conversation, mccoy))
        .then(() => mccoy.spark.internal.conversation.get(conversation, {activitiesLimit: 10}))
        .then((c) => {
      ...
      it('sideboards a non-existent user', () => spark.internal.conversation.add(conversation, email)
        .then((activity) => {
          assert.isActivity(activity);

          return spark.internal.conversation.get(conversation, {includeParticipants: true});
        })
        .then((c) => {
          assert.isConversation(c);
    ...
    describe('#assign()', () => {
      before(() => spark.internal.conversation.create({participants})
        .then((c) => {
          conversation = c;
        }));
      ...
      it('assigns an avatar to a room', () => spark.internal.conversation.assign(conversation, sampleImageSmallOnePng)
    ...
    describe('#leave()', () => {
      ...
      it('removes the current user from the specified conversation', () => spark.internal.conversation.leave(conversation)
        .then((activity) => {
          assert.isActivity(activity);

          return assert.isRejected(spark.internal.conversation.get(conversation));
        })
        .then((reason) => {
          assert.statusCode(reason, 404);

          return assert.isRejected(spark.internal.encryption.kms.fetchKey({uri: conversation.defaultActivityEncryptionKeyUrl}));
        })
        .then((reason) => assert.equal(reason.status, 403)));

      it('removes the specified user from the specified conversation', () => spark.internal.conversation.leave(conversation, mccoy)
        .then((activity) => {
          assert.isActivity(activity);

          return assert.isRejected(mccoy.spark.internal.conversation.get(conversation));
        })
        .then((reason) => {
          assert.statusCode(reason, 404);

          return assert.isRejected(mccoy.spark.internal.encryption.kms.fetchKey({uri: conversation.defaultActivityEncryptionKeyUrl}));
        })
        .then((reason) => assert.equal(reason.status, 403)));
      ...
      describe('with deleted users', () => {
        ...
        beforeEach(() => testUsers.create({count: 1})
          .then(([rs]) => {
            ...
            return spark.internal.conversation.add(conversation, rs);
          }));

        it('removes the specified deleted user from the specified conversation', () => spark.internal.conversation.leave(conversation, redshirt)
          .then(() => spark.internal.conversation.get(conversation, {includeParticipants: true}))
          .then((c) => {
    ...
    describe('#post()', () => {
      ...
      it('posts a comment to the specified conversation', () => spark.internal.conversation.post(conversation, message)
        .then((activity) => {
          assert.isActivity(activity);
      ...
      it('updates the specified conversation\'s unread status', () => mccoy.spark.internal.conversation.get(conversation)
        .then((c) => {
          const {
          ...
          return spark.internal.conversation.post(conversation, message)
      ...
      it('posts rich content to the specified conversation', () => spark.internal.conversation.post(conversation, {
        displayName: message,
        content: richMessage
      })
        .then((activity) => {
      ...
      it('submits mentions to the specified conversation', () => spark.internal.conversation.post(conversation, {
        displayName: message,
        content: richMessage,
        mentions: {
          items: [{
      ...
      skipInNode(describe)('when there are html tags in rich messages', () => {
        ...
        [
          ...
          it(def.it, () => {
            ...
            return spark.internal.conversation.post(conversation, {
              displayName: message,
              ...
              .then((activity) => {
                ...
                return mccoy.spark.internal.conversation.get(conversation, {activitiesLimit: 1});
              })
              .then((convo) => {
    ...
    describe('#update()', () => {
      it('renames the specified conversation', () => spark.internal.conversation.update(conversation, {
        displayName: 'displayName2',
        objectType: 'conversation'
      })
        .then((c) => spark.internal.conversation.get({url: c.target.url}))
    ...
    describe('#unassign()', () => {
      ...
      beforeEach(() => spark.internal.conversation.assign(conversation, sampleImageSmallOnePng));

      it('unassigns an avatar from a room', () => spark.internal.conversation.unassign(conversation)
        .then(() => spark.internal.conversation.get(conversation)
          .then((c) => {
    ...
    describe('#updateKey()', () => {
      beforeEach(() => spark.internal.conversation.create({participants, comment: 'THIS IS A COMMENT'})
        .then((c) => { conversation = c; }));

      it('assigns an unused key to the specified conversation', () => spark.internal.conversation.updateKey(conversation)
        .then((activity) => {
          assert.isActivity(activity);

          return spark.internal.conversation.get(conversation);
        })
        .then((c) => {
      ...
      it('assigns the specified key to the specified conversation', () => spark.internal.encryption.kms.createUnboundKeys({count: 1})
        .then(([key]) => spark.internal.conversation.updateKey(conversation, key)
          .then((activity) => {
            assert.isActivity(activity);
      ...
      it('grants access to the key for all users in the conversation', () => spark.internal.conversation.updateKey(conversation)
        .then((activity) => {
          assert.isActivity(activity);

          return mccoy.spark.internal.conversation.get({
        ...
        .then((c) => {
          ...
          return mccoy.spark.internal.encryption.kms.fetchKey({uri: c.defaultActivityEncryptionKeyUrl});
        }));
    });
    ...
    describe('#updateTypingStatus()', () => {
      ...
      beforeEach(() => {
        ...
        mccoy.spark.internal.mercury.on('event:status.start_typing', () => {
          startTypingSpy();
          blockUntilMercuryStart.resolve();
        });
        mccoy.spark.internal.mercury.on('event:status.stop_typing', () => {
          stopTypingSpy();
          blockUntilMercuryStop.resolve();
        });

        return spark.internal.conversation.create({participants, comment: 'THIS IS A COMMENT'})
          .then((c) => {
            conversation = c;
          });
      });
      ...
      it('sets the typing indicator for the specified conversation', () => spark.internal.conversation.updateTypingStatus(conversation, {typing: true})
        .then(() => blockUntilMercuryStart.promise)
        .then(() => {
          assert.calledOnce(startTypingSpy);
        }));

      it('clears the typing indicator for the specified conversation', () => spark.internal.conversation.updateTypingStatus(conversation, {typing: false})
      ...
      it('fails if called with a bad conversation object', () => {
        ...
        return spark.internal.conversation.updateTypingStatus({}, {typing: false})
          .catch((reason) => {
            error = reason;
          })
          .then(() => {
      ...
      it('infers id from conversation url if missing', () => {
        ...
        return spark.internal.conversation.updateTypingStatus(conversation, {typing: true})
          .then(() => blockUntilMercuryStart.promise)
          .then(() => {
    ...
    describe('verbs that update conversation tags', () => {
      [
        ...
        describe(`#${verb}()`, () => {
          it(itString, () => spark.internal.conversation[verb](conversation)
            .then((activity) => {
              assert.isActivity(activity);
            })
            .then(() => spark.internal.conversation.get(conversation))
      ...
      [
        ...
        describe(`#${verb}()`, () => {
          beforeEach(() => spark.internal.conversation[setupVerb](conversation)
            .catch((reason) => {
      ...
      describe('#removeAllMuteTags()', () => {
        it('removes all mute tags on the convo', () => spark.internal.conversation.muteMessages(conversation)
          .then(() => spark.internal.conversation.muteMentions(conversation))
          .then(() => spark.internal.conversation.removeAllMuteTags(conversation))
          .then(() => spark.internal.conversation.get(conversation))
          .then((c) => {
        ...
        it('removes all unmute tags on the convo', () => spark.internal.conversation.unmuteMentions(conversation)
          .then(() => spark.internal.conversation.unmuteMessages(conversation))
          .then(() => spark.internal.conversation.removeAllMuteTags(conversation))
          .then(() => spark.internal.conversation.get(conversation))
          .then((c) => {
            assert.notInclude(c.tags, 'MESSAGE_NOTIFICATIONS_ON');
      ...
      describe('#muteMentions()', () => {
        it('mutes the specified conversation of Mentions only', () => spark.internal.conversation.muteMentions(conversation)
          .then(() => spark.internal.conversation.get(conversation))
          .then((c) => {
      ...
      describe('#unmuteMentions()', () => {
        before(() => spark.internal.conversation.muteMentions(conversation));

        it('unmutes the specified conversation of Mentions', () => spark.internal.conversation.unmuteMentions(conversation)
          .then(() => spark.internal.conversation.get(conversation))
          .then((c) => {
      ...
      describe('#muteMessages()', () => {
        it('mutes the specified conversation of Messages only', () => spark.internal.conversation.muteMessages(conversation)
          .then(() => spark.internal.conversation.get(conversation))
          .then((c) => {
      ...
      describe('#unmuteMessages()', () => {
        before(() => spark.internal.conversation.muteMessages(conversation));

        it('unmutes the specified conversation of Messages only', () => spark.internal.conversation.unmuteMessages(conversation)
          .then(() => spark.internal.conversation.get(conversation))
          .then((c) => {
    ...
    describe('verbs that update objects', () => {
      ...
      before(() => {
        if (!conversation) {
          return spark.internal.conversation.create({displayName: 'displayName', participants})
            .then((c) => {
              conversation = c;
            });
        }
      ...
      describe('#acknowledge()', () => {
        it('acknowledges the specified activity', () => spark.internal.conversation.post(conversation, {displayName: 'A comment to acknowledge'})
          .then((activity) => mccoy.spark.internal.conversation.acknowledge(conversation, activity))
          .then((ack) => spark.internal.conversation.get(conversation, {activitiesLimit: 1})
            .then((c) => assert.equal(c.activities.items[0].url, ack.object.url))));
      });

      describe('#assignModerator()', () => {
        it('assigns a moderator to a conversation', () => spark.internal.conversation.assignModerator(conversation, spock)
          .then(() => spark.internal.conversation.get(conversation, {
            activitiesLimit: 5,
      ...
      describe('#delete()', () => {
        ...
        it('deletes the current user\'s post', () => spark.internal.conversation.post(conversation, {displayName: 'Delete Me 1'})
          .then((a) => spark.internal.conversation.delete(conversation, a))
          .then(() => new Promise((resolve) => setTimeout(resolve, 2000)))
          .then(() => spark.internal.conversation.get(conversation, {activitiesLimit: 2}))
          .then((c) => {
        ...
        it('deletes the current user\'s share', () =>
          spark.internal.conversation.share(conversation, [sampleImageSmallOnePng])
            .then((a) => spark.internal.conversation.delete(conversation, a))
            .then(() => new Promise((resolve) => setTimeout(resolve, 2000)))
            .then(() => spark.internal.conversation.get(conversation, {activitiesLimit: 2}))
            .then((c) => {
        ...
        describe('when the current user is a moderator', () => {
          it('deletes any user\'s content', () => spark.internal.conversation.assignModerator(conversation)
            .catch(allowConflicts)
            .then(() => spark.internal.conversation.lock(conversation))
            .catch(allowConflicts)
            .then(() => mccoy.spark.internal.conversation.post(conversation, {displayName: 'Delete Me 2'}))
            .then((a) => spark.internal.conversation.delete(conversation, a)));
        });

        describe('when the current user is not a moderator', () => {
          it('fails to delete other users\' content', () => spark.internal.conversation.assignModerator(conversation)
            .catch(allowConflicts)
            .then(() => spark.internal.conversation.lock(conversation))
            .catch(allowConflicts)
            .then(() => spark.internal.conversation.post(conversation, {displayName: 'Delete Me 3'}))
            .then((a) => assert.isRejected(mccoy.spark.internal.conversation.delete(conversation, a)))
            .then((reason) => assert.instanceOf(reason, SparkHttpError.Forbidden)));
        });
      });

      describe('#unassignModerator()', () => {
        it('removes a moderator from a conversation', () => spark.internal.conversation.assignModerator(conversation, spock)
          .catch(allowConflicts)
          .then(() => spark.internal.conversation.unassignModerator(conversation, spock))
          .then(() => spark.internal.conversation.get(conversation, {
            activitiesLimit: 5,
            includeParticipants: true
          }))
