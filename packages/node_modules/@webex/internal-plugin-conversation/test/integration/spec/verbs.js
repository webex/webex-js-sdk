...
import {skipInNode, expectEvent} from '@webex/test-helper-mocha';
    ...
    describe('verbs that update objects', () => {
      ...
      describe('#addReaction', () => {
        it('adds a reaction', () => webex.internal.conversation.post(conversation, {displayName: 'React Me 1'})
          ...
          .then(() => Promise.resolve(
            expectEvent(10000, 'event:conversation.activity', webex.internal.mercury, 'should get the reaction add event')
          ))
          .then((event) => {
            // if reaction event comes before summary event
            if (event.data.activity.object.objectType === 'reaction2') {
              assert.equal(event.data.activity.object.displayName, 'smiley');

              return Promise.resolve(
                expectEvent(30000, 'event:conversation.activity', webex.internal.mercury, 'should get the reactionSummary event')
              ).then((summaryEvent) => {
                assert.equal(summaryEvent.data.activity.object.objectType, 'reaction2Summary');
                assert.equal(summaryEvent.data.activity.object.reactions[0].displayName, 'smiley');
                ...
                return webex.internal.conversation.get(conversation, {
                  ...
                  assert.equal(selfSummary.activity.object.reactions[0].displayName, 'smiley');
                });
              });
            }

            // else we got summary event first, so just check that and we know it's safe to GET convo now
            assert.equal(event.data.activity.object.objectType, 'reaction2Summary');
            assert.equal(event.data.activity.object.reactions[0].displayName, 'smiley');
      ...
      describe('#deleteReaction', () => {
        it('deletes a reaction', () => webex.internal.conversation.post(conversation, {displayName: 'React Me 1'})
          ...
          .then(() => Promise.resolve(
            expectEvent(10000, 'event:conversation.activity', webex.internal.mercury, 'should get the reaction delete event')
          ))
          .then((event) => {
            assert.equal(event.data.activity.verb, 'delete');
            assert.equal(event.data.activity.object.verb, 'tombstone');
            assert.equal(event.data.activity.object.parent.type, 'reaction');
