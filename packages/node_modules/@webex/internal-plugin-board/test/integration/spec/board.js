...
describe('plugin-board', () => {
  describe('service', () => {
    ...
    after('disconnect mercury', () => Promise.all(map(participants, (participant) => participant.spark.internal.mercury.disconnect())));

    describe('#getChannel', () => {
      it('gets the channel metadata', () => participants[0].spark.internal.board.getChannel(board)
        .then((channel) => {
    ...
    describe('#_uploadImage()', () => {
      ...
      it('uploads image to spark files', () => participants[0].spark.internal.board._uploadImage(board, fixture)
    ...
    describe('#ping()', () => {
      it('pings board service', () => participants[0].spark.internal.board.ping());
    });
    ...
    describe('#addImage()', () => {
      ...
      it('uploads image to spark files', () => participants[0].spark.internal.board.addImage(board, fixture, {displayName: fixture.name})
        .then((fileContent) => {
      ...
      it('adds to presistence', () => participants[0].spark.internal.board.getContents(board)
        .then((allContents) => {
      ...
      it('matches file file downloaded', () => participants[0].spark.internal.encryption.download(testScr)
        .then((downloadedFile) => fh.isMatchingFile(downloadedFile, fixture)
          .then((result) => assert.deepEqual(result, true))));

      it('allows others to download image', () => participants[2].spark.internal.encryption.download(testScr)
        .then((downloadedFile) => fh.isMatchingFile(downloadedFile, fixture)
          .then((result) => assert.deepEqual(result, true))));
      ...
      describe('when image content has no metadata', () => {
        ...
        it('decrypts no meta', () => {
          ...
          return participants[0].spark.internal.board.addImage(board, fixture)
            ...
            .then(() => participants[0].spark.internal.encryption.download(testScr)
              .then((downloadedFile) => fh.isMatchingFile(downloadedFile, fixture))
    ...
    describe('#getChannels()', () => {
      it('retrieves a newly created board for a specified conversation within a single page', () => participants[0].spark.internal.board.getChannels(conversation)
      ...
      it('retrieves all boards for a specified conversation across multiple pages', () => {
        ...
        return participants[0].spark.internal.conversation.create({
          ...
          // get boards, page 1
          .then(() => participants[0].spark.internal.board.getChannels(convo, {
    ...
    describe('#deletePartialContent()', () => {
      ...
      it('deletes some contents from the specified board', () => {
        ...
        return participants[0].spark.internal.board.addContent(channel, data)
          ...
          .then(() => participants[0].spark.internal.board.deletePartialContent(channel, contentsToKeep))
          .then(() => participants[0].spark.internal.board.getContents(channel))
          .then((page) => {
      ...
      it('deletes channel', () => {
        ...
        return participants[1].spark.internal.board.createChannel(conversation)
          .then((res) => {
            ...
            return participants[1].spark.internal.board.deleteChannel(conversation, newChannel);
          })
          .then(() => assert.isRejected(participants[1].spark.internal.board.getChannel(newChannel)));
      });
      ...
      describe('when preventDeleteActiveChannel is enabled', () => {
        it('does not delete when a channel is being used', () => {
          ...
          return participants[1].spark.internal.board.createChannel(conversation)
            ...
            .then(() => participants[1].spark.internal.board.getChannel(activeChannel));
        });
