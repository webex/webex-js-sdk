...
import Mercury, {Socket} from '@ciscospark/internal-plugin-mercury';
  ...
  describe('realtime', () => {
    ...
    describe('#publishEncrypted()', () => {
      ...
      it('rejects when socket not found', () => {
        ...
        return assert.isRejected(spark.internal.board.realtime.publishEncrypted({
          encryptedData: 'encryptedData',
          encryptedKeyUrl: 'fakeURL'
        }, 'STRING'));
      });
    ...
    describe('#connectByOpenNewMercuryConnection()', () => {
      it('opens new connections using the provided socket urls', () => spark.internal.board.realtime.connectByOpenNewMercuryConnection(channel)
        .then(() => {
    ...
    describe('#disconnectMercuryConnection', () => {
      it('disconnects the mercury connection', () => {
        ...
        return spark.internal.board.realtime.disconnectMercuryConnection(channel)
          .then(() => {
    ...
    describe('when trying to share mercury connection', () => {
      ...
      describe('#connectToSharedMercury', () => {
        it('registers and gets board binding', () => spark.internal.board.realtime.connectToSharedMercury(channel)
          .then((res) => {
      ...
      describe('#disconnectFromSharedMercury()', () => {
        it('requests to remove board bindings', () => spark.internal.board.realtime.connectToSharedMercury(channel)
          .then(() => spark.internal.board.realtime.disconnectFromSharedMercury(channel))
          .then((res) => {
    ...
    describe('#handleBoardActivityMessages', () => {
      it('finds the realtimeChannel from the envelope', () => {
        ...
        return spark.internal.board.realtime.createRealtimeChannel({channelId: '1'})
          .then((realtimeChannel) => {
            realtime1 = realtimeChannel;

            return spark.internal.board.realtime.createRealtimeChannel({channelId: '2'});
          })
          .then((realtimeChannel) => {
