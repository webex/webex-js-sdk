...
const RealtimeService = WebexPlugin.extend({
  ...
  createRealtimeChannel(channel) {
    ...
    if (this.config.useFederatedRegistration) {
      registrationPromise = this.webex.internal.board.registerToChannel(channel)
        .then(({webSocketUrl, binding}) => ({socketUrl: webSocketUrl, binding}));
    }
    else {
      const requestBindings = [this._boardChannelIdToMercuryBinding(channel.channelId)];

      registrationPromise = this.webex.internal.board.register({bindings: requestBindings})
        .then(({webSocketUrl, bindings}) => ({socketUrl: webSocketUrl, binding: bindings[0]}));
    }
