    ...
    describe('#listActiveCalls()', () => {
      ...
      describe('when a ongoing call exists', () => {
        ...
        it('lists the existing call', () => {
          // TODO: Look into why it's returning 2 items instead of 1
        ...
        it('does not add the existing call to the emittedCalls collection if it exists', () => {
          ...
          return spock.webex.phone.listActiveCalls()
            .then((result) => {
              ...
              assert.equal(spock.webex.phone.emittedCalls.length, 2, 'emitted calls collection should have one call');
              assert.isDefined(spock.webex.phone.emittedCalls.get(activeCall.internalCallId));
            });
        });
