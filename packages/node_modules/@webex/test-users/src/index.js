 ...
 * @param {string} [conversationServiceUrl] defaults to WEBEX_TEST_USERS_CONVERSATION_SERVICE_URL
...
export function createTestUser(options = {}) {
  ...
  const body = {
    ...
    password: options.password || `${generateRandomString(10)}zA1*`,
  ...
  return requestWithAuth({
    method: 'POST',
    uri: `${conversationServiceUrl}${BASE_PATH_SECURE}`,
    json: true,
    body
  })
...
export function removeTestUser(options = {}) {
  ...
  if (!options.token) {
    return loginTestUser(options)
      .then((token) => removeTestUser(Object.assign({token}, options)));
  }
