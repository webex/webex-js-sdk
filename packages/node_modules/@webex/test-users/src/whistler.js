...
export default function createTestUser(options = {}) {
  ...
  return getClientCredentials({
    ...
    .then((authorization) => {
      let testUserReservationUrl = `${whistlerServiceUrl}/reservations/testUser?isAccessTokenRequired=true`;

      if (reservationGroup) {
        testUserReservationUrl = `${testUserReservationUrl}&reservationGroup=${reservationGroup}`;
      }
      if (userScopes) {
        testUserReservationUrl = `${testUserReservationUrl}&userScopes=${userScopes}`;
      }

      return request({
        method: 'GET',
        uri: testUserReservationUrl,
        headers: {
          authorization,
        },
      });
