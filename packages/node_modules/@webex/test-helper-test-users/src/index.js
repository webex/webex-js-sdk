...
function _extractFromEnv(options) {
  ...
  const users = [{
    id: process.env.WEBEX_ID || process.env.WEBEX_ID_0,
    token: {
      access_token: process.env.WEBEX_ACCESS_TOKEN || process.env.WEBEX_ACCESS_TOKEN_0,
      refresh_token: process.env.WEBEX_REFRESH_TOKEN || process.env.WEBEX_REFRESH_TOKEN_0
    },
    email: process.env.WEBEX_EMAIL || process.env.WEBEX_EMAIL_0,
    name: process.env.WEBEX_NAME || process.env.WEBEX_NAME_0,
    password: process.env.WEBEX_PASSWORD || process.env.WEBEX_PASSWORD_0
  }];

  for (let i = 1; i < count; i += 1) {
    users.push({
      id: process.env[`WEBEX_ID_${i}`],
      token: {
        access_token: process.env[`WEBEX_ACCESS_TOKEN_${i}`],
        refresh_token: process.env[`WEBEX_REFRESH_TOKEN_${i}`]
      },
      email: process.env[`WEBEX_EMAIL_${i}`],
      name: process.env[`WEBEX_NAME_${i}`],
      password: process.env[`WEBEX_PASSWORD_${i}`]
    });
  }
